...
var txd=(tn1-tc)/trt;with(Math)
if(abs(txd)<1)txd=abs(txd)/txd;
...
function _tm(){
if(tdo.attachEvent){tdo.attachEvent('onmousemove',_tm2);
...
with(tmoveRec){
if(!tl1||tmo)return;
_tmv=tt[tmi];tom=_toi(_tmv.id+'div');
var tms=_tee(tvn),tyx=_tsx(tom),tpo1=tim?_tsc():[0,0];
ttx=tms[0]-tyx[0]+tpo1[0];tty=tms[1]-tyx[1]+tpo1[1];tmo=1;
};
};
function _tm2(event){
with(tmoveRec)
if(tmo&&tl1){
var tms=_tee(event),tpo1=tim?_tsc():[0,0],txn=tms[0]-ttx+tpo1[0]+txo,
tyn=tms[1]-tty+tpo1[1]+tyo;
with(tom.style){left=((txn>=0)?txn:0)+'px';top=((tyn>=0)?tyn:0)+'px';
};
};
return true;
};
...
with(tmoveRec){
if(!tl1||!tmo)return;
var tpo1=_tsc(),tyx=_tsx(tom);_tmv.left=tyx[0]-tpo1[0];
_tmv.top=tyx[1]-tpo1[1];tmo=0;
};
};
