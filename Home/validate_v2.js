...
function validate_NIC_Return_new(m_objval,m_gender,m_date_of_bitrh){ 
	
	if(m_objval != '') {  
		
		if(m_objval.length >= 12){
			//validateNIC_new(m_objval);
			
			var m_expression = new RegExp("[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]");
			var m_value = m_objval;
			if ((m_objval.length > 0) && ((m_objval.length != 12) || (m_expression.test(m_value) == false))) {
...
//===============================================Added by Kanchana to add new NIC formate======================
// Registration of persons department change the NIC number format
//New NIC formate validation
...
function validate_NIC(object){ 
	...
	if (object.value != '') { 
		if(m_length<11){ //To check old NIC formate
			//alert('You are entering old formatted NIC number');	
			if(val_nic1(object) && val_nic2(object)){	
				return true ; 
			} 		
			else {  	
				return false ; //comment by nuwan de silva for the entering old data 06-06-07 (to the requst) 	// released by udara 20-03-2018
				//return true ;  // commented by udara 20-03-2018
			} 	
		}else{//To check new NIC formate
			//alert('You are entering new formatted NIC number');
			if(validateNIC(object)&&val_nic2(object)){	
				return true ; 
			} 		
			else {  	
				return false ; //comment by nuwan de silva for the entering old data 06-06-07 (to the requst)  // released by udara 20-03-2018	
...
/* //Commented by Kanchana get both nic formates to validate.
// Modified By Samitha Kulatilaka On 2011-12-07
function validate_NIC(object){ 
...
{  
	...
	m_gender = document.Form1.TXT_GENDER.value;	
	m_objval = object.value;
	if(m_length<11){ //for old formate
		//alert('cc');
		m_year = m_objval.substring(0,2)
		m_days = parseFloat(m_objval.substring(2,5));
	}else{ //for new formate added by Kanchana on 2016-01-13
		//alert('dd');
	...
	if(document.Form1.TXT_DATE_OF_BIRTH_DD.value!='' && document.Form1.TXT_DATE_OF_BIRTH_MM.value!='' && document.Form1.TXT_DATE_OF_BIRTH_YY.value!=''  ) {
		...
		m_month = parseFloat(document.Form1.TXT_DATE_OF_BIRTH_MM.value); 		
		m_date = parseFloat(document.Form1.TXT_DATE_OF_BIRTH_DD.value); 			
		if(m_length<11){ //for old formate added by Kanchana on 2016-01-13
			//   alert('aa');
			m_yy = m_yyyy.substring(2,4)
		}else{//for new formate added by Kanchana on 2016-01-13
			//   alert('bb');
	...
	for(var i=0;i<size;i++){
		...
		if(document.Form1.elements[m_name].value!="" || document.Form1.elements[m_contact].value!=""   ) { //|| document.Form1.elements[m_appr].value!="" || document.Form1.elements[m_balance].value!=""  || document.Form1.elements[m_months].value!="" 
			...
			*/
			...
			if(type=='I'){
				// COMMENT  BY A/S ON 01-09-2021 FOR JB05012021-12810
...
//ADD BY A/S ON 01-09-2021 FOR JB05012021-12810
function header_credit_type_change(m_type){
	//alert('m_type='+document.Form1.elements["TXT_TYPE_OF_FACILITY"+m_type].value);
	if(document.Form1.elements["TXT_TYPE_OF_FACILITY"+m_type].value=="NOT APPLICABLE"){ 
		e_header_credit.innerHTML="";
...
function assign_data_values(){
	...
	//alert('adt   '+document.Form1.hid_client_type.value);
	// alert('adt   '+document.Form1.hid_app_screen.value);
	...
	if(document.Form1.hid_client_type.value=='I'){ 
		...
		//alert('ccc');
		//if(document.Form1.hid_app_screen.value=='APP_R' || document.Form1.hid_app_screen.value=='APP_N'){ 
		//alert('dfdfdfd');
		//makeRequestautoload();
		//}else{	
		makeRequest(document.Form1.TXT_CLIENT_CODE); 
	...
	//alert('****');
	document.Form1.TXT_CLIENT_CODE.disabled=true; //[Added milinda for field disable when after page loading]
	chk_disa();//[Added milinda for disable fields]
}
...
function disable_client_data_ind() {//added by nuwan de silva
	...
	if(mm_client_status>0){ // added by udara 20-10-2017
	...
	// document.Form1.edit.disabled =true;//[commented by milinda]
...
function enable_disable_client_data_cor(m_active_status,from_screen) {//added by nuwan de silva
	...
	if(from_screen=='client_verification'){
		...
		set_client_help_disable();                
	}  //Endded by Kanchana on 2016-09-09 
...
function active_contract_count(data_vec,m_client_type){ 			
	// mm_count=data_vec[0] ;
	//	alert(data_vec[0]);
	//alert(m_client_type);
	if(data_vec[0] > 0){
		...
		/*		document.Form1.BUT_HELP_MAIN.disabled=true;  
				...
				document.Form1.but_new2.disabled =true ;	*/	 
		//alert('bbbb'+from_screen+'aaaa'+data_vec[0]);
		//alert(m_client_type);		
		//alert(data_vec[0]);
		if(m_client_type=='I'){
			disable_client_data_ind();
...
function disable_temp(){
	
	document.Form1.temp.disabled =true ;
	document.Form1.temp_save.disabled=true;
}

//Added by Kanchana on 2016-09-08  
function set_client_help_disable(){
	alert(document.Form1.hid_client_count.value);
	alert(from_screen)
	if(from_screen != '')	{
		if(from_screen=='client_verification'){
			//alert('kkkk');	");
			//avoid selecting another lient from verify screens
			if(document.Form1.hid_client_count.value > 0){
				document.Form1.save.disabled=true;
			}	
			//document.Form1.save.disabled=true; // added by udara 30-08-2017 --[Commented by milinda]	
			document.Form1.save2.disabled=true; // added by udara 04-10-2017 
			document.Form1.temp_save2.disabled=true; // added by udara 04-10-2017 
			document.Form1.temp2.disabled=true; // added by udara 04-10-2017 
			document.Form1.cancel.disabled=true; // added by udara 04-10-2017
			document.Form1.cancel2.disabled=true; // added by udara 04-10-2017
			document.Form1.TXT_NIC_NO.disabled =true; // added by udara 20-10-2017
			...
			assign_help_status('H_active_cont'); 
			makeRequest(document.Form1.TXT_CLIENT_CODE); 
...
function set_client_help_disable_guarantor(){
	//alert(from_screen);
	if(from_screen !='main_page'){
		//alert('kkkk');	");
		//avoid selecting another lient from verify screens
		document.Form1.BUT_HELP_MAIN.disabled=true;   
		document.Form1.TXT_CLIENT_CODE.disabled =true ; 		
...
//End by Kanchana.
//[Added milinda for disable fields]
function chk_disa(){
	//alert("******"+document.Form1.hid_entity_type.value);
	//alert(document.Form1.hid_screen.value);
	if(document.Form1.hid_app_screen.value=="APP_R" || document.Form1.hid_app_screen.value=="APP_N" || document.Form1.hid_f_screen.value=="client_verification" ){ //|| document.Form1.hid_screen.value=="G"
		//alert('*********');
		if(document.Form1.hid_entity_type.value!="CORPORATE"){		
		document.Form1.TXT_CLIENT_CODE.disabled=false;
		document.Form1.TXT_CLIENT_CODE.focus();
		document.Form1.TXT_CLIENT_CODE.blur();
		document.Form1.TXT_CLIENT_CODE.disabled=true;
		 if(document.Form1.TXT_NIC_NO.value!=""){	
		document.Form1.TXT_NIC_NO.disabled =true;
			}
		}else{
		document.Form1.TXT_CLIENT_CODE.disabled=false;
