...
function getAllowedCharacterList(m_type) {
	...
	if ((m_type == 'DEFAULT') || (m_type == 'NAME') || (m_type == 'ADDRESS') || (m_type == 'COMMENT') || (m_type == 'NAME_2') || (m_type == 'FULL_NAME') || (m_type == 'FULL_NAME_DEBTOR') || (m_type == 'USERNAME') || (m_type == 'NO_WHITESPACE') || (m_type == 'INVOICE_NUMBER') || (m_type == 'PROFILE_ID')) {
	...
	/* Numerical Digits */
	if ((m_type == 'DEFAULT') || (m_type == 'ADDRESS') || (m_type == 'FULL_NAME') || (m_type == 'FULL_NAME_DEBTOR') || (m_type == 'ACCOUNT_NO') || (m_type == 'COMMENT') || (m_type == 'USERNAME') || (m_type == 'NO_WHITESPACE') || (m_type == 'NUMBER') || (m_type == 'INVOICE_NUMBER') || (m_type == 'PROFILE_ID')) {
	...
	if ((m_type == 'DEFAULT') || (m_type == 'NAME') || (m_type == 'ADDRESS') || (m_type == 'COMMENT') || (m_type == 'FULL_NAME') || (m_type == 'FULL_NAME_DEBTOR')) {
	...
	if ((m_type == 'FULL_NAME') || (m_type == 'FULL_NAME_DEBTOR') ) {
		allowed_character_list_04.splice(0, 0, ' ');            /* Add ' ' For Full Name */
		allowed_character_list_04.splice(0, 0, '&');            /* Add '&' For Full Name */ // thamali 2013.03.13
	}
	if (m_type == 'USERNAME') {
	...
	if (m_type == 'PROFILE_ID') {
		allowed_character_list_04[0]    = '_';
		allowed_character_list_04[1]    = '-';
	}
...
function checkMonthLength1(mObjectDay, mObjectMonth, mObjectYear, mObjectFullDate, mTriggerObject, mNullable, mSysdate, mDateRangeType) {
	...
	if (mSysdate != null) {
		...
		var mSysdateObject_Future = new Date(); /*added by ns on 15-09-2012*/
	...
	if ((mSysdate != null) && (mIsDateValidated == true)) {
		...
		/*added by ns on 15-09-2012*/
		else if (mDateRangeType!='no past' && mDateRangeType!='no future' && mDateRangeType!=null  ){
			var mSysdateDay_Future         = mDateRangeType.substring(0, 2);
			var mSysdateMonth_Future       = mDateRangeType.substring(3, 5);
			var mSysdateYear_Future    	   = mDateRangeType.substring(6);
			var mSysdateObject_Future      = new Date();
			mSysdateObject_Future.setFullYear(mSysdateYear_Future, mSysdateMonth_Future-1, mSysdateDay_Future);
			if (mDateObject < mSysdateObject) {
				mObjectFullDate.value = '';
			...
			if (mDateObject > mSysdateObject_Future) {
				mObjectFullDate.value = '';
...
/****************************************Added by pathum 28/12/2012*************************************************/
function checkMonthLength2(mObjectDay, mObjectMonth, mObjectYear, mObjectFullDate, mTriggerObject, mNullable, mSysdate, mDateRangeType) {
	...
	if ((mBoolIsDateObject1 == true) && (mBoolIsDateObject2 == true)) {
		if (mOperator == '<=') {
			...
			if (mDateObject1 <= mDateObject2) {
				...
				if (mTriggerObjectSet == 1) {
					mObjectFullDate1.value = ''; 
					mObjectDay1.value = ''; //pathum
					mObjectMonth1.value = ''; //pathum
					//mObjectYear1.value = ''; //pathum
				}
				else if (mTriggerObjectSet == 2) {
					mObjectFullDate2.value = '';
					mObjectDay2.value = ''; //pathum
					mObjectMonth2.value = ''; //pathum
					//mObjectYear2.value = ''; //pathum
				}
				
				mTriggerObject.value = '';  //com pathum
				
				//mTriggerObjectSet.value = ''; //pathum
...
/**********************************************Added by pathum 28/12/2012************************************/
function checkTwoDates2(mObjectDay1, mObjectMonth1, mObjectYear1, mObjectFullDate1, mObjectDay2, mObjectMonth2, mObjectYear2, mObjectFullDate2, mTriggerObjectSet, mTriggerObject, mTriggerObjectType, mOperator, mLabel_1, mLabel_2 ,mNumberOfDays) {
	...
	if ((mBoolIsDateObject1 == true) && (mBoolIsDateObject2 == true)) {
		if (mOperator == '<=') {
			...
			if (mDateObject1 <= mDateObject2) {
				...
				if (mTriggerObjectSet == 1) {
					mObjectFullDate1.value = '1';   //pathum
				}
				else if (mTriggerObjectSet == 2) {
					mObjectFullDate2.value = '2';  //pathum
				}
				...
				setTimeout('document.forms[0].elements[\'' + mTriggerObject.name + '\'].focus()', 10);
				/////
				//mObjectYear2.value = '88'; //pathum 12/31/2012
				return false;
			}
		}
...
function initFollowUpDialog(){
	$("#divId").dialog({
			...
			buttons: 
			{
				...
				Cancel: function() 
				{
					document.getElementById('modalIframeId').contentWindow.clearFields();
					$(this).dialog('close');
			...
			close: function() 
			{
				//document.getElementById('modalIframeId').contentWindow.childClose(); //added by madhawa 2012-05-29
				//$(this).dialog('close');
			},
			height: 500,
			width: 550
...
{
	/*if(typeof myTestFunction == 'function') //added by madhawa for settlement of return cheque screen
	{
	}	
	*/
...
function noWeekendsOrHolidays(date) {
	...
	var d = date.getDate();
	if (calendarHolidays[y] && calendarHolidays[y][m] && calendarHolidays[y][m][d]) {
		var s = calendarHolidays[y][m][d];
...
function hasWeekendsOrHolidays(date) {
	// var noWeekend = $.datepicker.noWeekends(date);
	// if (noWeekend[0]) {
	var y = date.getFullYear();
	var m = date.getMonth() + 1;
