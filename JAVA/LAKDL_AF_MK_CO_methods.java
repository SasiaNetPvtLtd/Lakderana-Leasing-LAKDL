...
public class LAKDL_AF_MK_CO_methods  
 {
 ...
 public String getCustomerCat(String Schema,String Active,String CatType) {
	 
		return " SELECT CAT_TYPE_CODE Category,DESCRIPTION \"Category Name\" "+
                           " FROM   "+Schema+".AF_MK_MAS_CUSTOMER_CATOGORY "+
													 " WHERE  ACTIVE_STATUS ='"+Active+"' AND "+
													 "        CAT_TYPE_CODE LIKE '%"+CatType+"%' "+
  ...
  public String getItemCat(String Schema,String Active,String CatCode) {
		...
		return " SELECT ITEM_CAT_CODE \"Item Category\",DESCRIPTION \"Category Name\" "+
                           " FROM   "+Schema+".AF_CO_MAS_ITEM_CATEGORY "+
													 ...
													 //" ORDER BY	DEFAULT_VALUE DESC"; // commented by udara 11-10-2018
														" ORDER BY	DESCRIPTION "; // added by udara 11-10-2018
		*/
		
		return " SELECT ITEM_CAT_CODE \"Item Category\",DESCRIPTION \"Category Name\" "+
			" FROM ("+
					" SELECT ITEM_CAT_CODE ITEM_CAT_CODE, INITCAP(DESCRIPTION) DESCRIPTION "+
                           " FROM   "+Schema+".AF_CO_MAS_ITEM_CATEGORY "+
													 " WHERE  ACTIVE_STATUS ='"+Active+"' AND "+
													 "        ITEM_CAT_CODE LIKE '%"+CatCode+"%' "+
														" ) "+
...
public String getScheduleType(String Schema,String Active) { //add by waruna 
		
		return " SELECT A.SHEDULE_REF, A.ITEM_CAT_CODE, A.ITEM_SUB_CAT "+
               " FROM "+Schema+".AF_CO_PRO_APP_SHEDULE A  " +
               " WHERE ACTIVE_STATUS='"+Active+"'"+
			   " ORDER BY A.ITEM_CAT_CODE, A.ITEM_SUB_CAT ";
 }		
...
public String getChargesApp_2(String Schema,String Active,String SubChargType,String SubItemCat,String Fual) {
		
		return " SELECT B.DESCRIPTION, A.SUB_TYPE_CODE,NVL(A.AMOUNT,'0'),NVL(A.PERCENTAGE*100,'0'),   "+
		       "        A.ITEM_SUB_CAT,A.FUAL_TYPE_CODE,A.FROM_DATE,A.TO_DATE,A.ACTIVE_STATUS,B.CHARGE_TYPE "+
 					 " FROM  "+Schema+".AF_CO_MAS_CHARGES_APPLICABLE A,"+Schema+".AF_CO_MAS_SUB_CHARGES B "+
					 " WHERE A.SUB_TYPE_CODE=B.SUB_TYPE_CODE AND A.ITEM_SUB_CAT LIKE '"+SubItemCat+"%' AND "+
					 "       A.SUB_TYPE_CODE LIKE '"+SubChargType+"%' AND A.SUB_TYPE_CODE LIKE '"+Fual+"%' AND "+
					 " FROM_DATE<=TO_DATE(TO_CHAR(SYSDATE,'DD-MON-YYYY'),'DD-MON-YYYY') AND "+
