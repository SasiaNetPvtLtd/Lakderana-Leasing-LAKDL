...
//SCREEN NAME:INVOICE FIRST REMINDER LETTER
...
public class LAKDL_FA_OP_Invoice_Final_Reminder_Letter extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if (m_chksql.trim().equals("main_page")) {
				...
				/*t.println("<tr><td width='*%' class='factoring-letter-body'>FACTORING DIVISION OF</td></tr>");
			  ...
			  out.println("<tr><td width='*%' class='factoring-letter-body'>COLOMBO 02.</td></tr>");*/
				...
				out.println("<table border='0' width='100%' class='table'> ");	
				out.println("<tr ><td width='*%' class='factoring-letter-body' style='{text-align:center;}'><b><u>FINAL REMINDER</u></b></td></tr>");
				...
				/*rs2 = stmt2.executeQuery ("SELECT SUM(B.BALANCE_AMOUNT)"+
					" FROM "+m_schema_name+".FA_CR_PRO_INVOICE_DETAIL B  "+
					" WHERE B.INVOICE_SEQ_NO='"+m_invoice_seq+"'");*/
	...
	rs2 = stmt2.executeQuery ("SELECT NVL(SUM(A.BALANCE_AMOUNT),0) "+
					...
					" AND TO_DATE(TO_CHAR(A.DUE_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')+30>=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
					" AND TO_DATE(TO_CHAR(A.DUE_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')+30<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
				    " GROUP BY A.DEBTOR_CODE ");
	
	
				double m_tot_balance=0;
				
				if(rs2.next()){
					m_tot_balance = rs2.getDouble(1);	
				}
				out.println(" We regret to inform you that you have failed to settle the overdue amount of Rs <b>"+nf.format(rs2.getDouble(1))+"</b>  ");
				out.println(" on your account as at date in spite several reminders sent to you, ");
				
				out.println(" we here by demand immediate payment of the said sum, and we also simultaneously instructing "+
					" our lawyers to institute legal proceedings for the recovery of the amount due. ");
				out.println("<br><br> ");		
				out.println(" The invoice details as follows. <br><br>");
				...
				" WHERE A.INVOICE_SEQ_NO='"+m_invoice_seq+"'"+
				" AND A.BALANCE_AMOUNT>0 "+
				" AND A.INVOICE_STATUS='CONF' ");
						*/
