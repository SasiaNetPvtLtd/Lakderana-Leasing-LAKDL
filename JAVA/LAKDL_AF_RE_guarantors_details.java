...
public class LAKDL_AF_RE_guarantors_details extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
      ...
      String m_username 				 = m_sn_methods.username;
			...
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_RE_Save_Temp_Receipt_Approval';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_RE_Temp_Settlement_Approval?chksql=main_page';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\" Collection - Temporary Receipts Approval - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
			...
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_guarantors_details?chksql=view&user=\"+document.Form1.TXT_USER.value+\" \";");
      ...
      out.println("}"); 
		  ...
		  out.println("  }");
			...
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Temp_Settlement_Approval?chksql=view&setle_mode=\"+document.Form1.SETT_MODE.value+\"&user=\"+document.Form1.TXT_USER.value+\"&sort_column=\"+m_sort_col+\"&order_by_type=\"+m_order_by_type;");	
		  out.println("load_interface(m_url,'NORM');");
			out.println("}");
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Collection - Temporary Receipts Approval</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<td width='20%' >Enter User</td>");
      ...
      out.println("</body>"); 
		...
		if(req.getParameter("sort_column")!=null && req.getParameter("order_by_type")!=null){
			...
			out.println("<td width=\"10%\" align=left  title='Click here to sort by - Receipt No  '     onclick=sort_data('A.TEMP_REC_NO')     STYLE='{cursor:hand}'>Receipt No</td>"); 
			out.println("<td width=\"10%\" align=left                                                                                                               >Finance No</td>"); 
			out.println("<td width=\"10%\" align=left  title='Click here to sort by - Client Name  '    onclick=sort_data('CLIENT_NAME')       STYLE='{cursor:hand}'>Client Name</td>"); 
			out.println("<td width=\"10%\" align=left  title='Click here to sort by - Settle Mode  '    onclick=sort_data('SETTLE_MODE')       STYLE='{cursor:hand}'>Settled Mode</td>"); 
			out.println("<td width=\"10%\" align=left  title='Click here to sort by - Receipt Date  '     onclick=sort_data('EFF_VALDATE')       STYLE='{cursor:hand}'>Receipt Date</td>"); 
			if(!m_set_mode.equals("CASH")){
			out.println("<td width=\"8%\"  align=left  title='Click here to sort by - Cheque No  '      onclick=sort_data('CHEQUE_NO')         STYLE='{cursor:hand}'>Cheque No</td>"); 
			out.println("<td width=\"8%\"  align=left  title='Click here to sort by - Cheque Date  '    onclick=sort_data('CHEQUE_DATE')       STYLE='{cursor:hand}'>Cheque Date</td>"); 
			out.println("<td width=\"8%\"  align=left  title='Click here to sort by - Account No '      onclick=sort_data('5')      STYLE='{cursor:hand}'>Account Code</td>"); 
			out.println("<td width=\"8%\"  align=left  title='Click here to sort by - Branch Code  '    onclick=sort_data('4') STYLE='{cursor:hand}'>Branch Code</td>"); 
			}
			out.println("<td width=\"10%\" align=right title='Click here to sort by - Receipt Amount  ' onclick=sort_data('REC_AMOUNT')        STYLE='{cursor:hand}'>Receipt Amount</td>"); 
     ...
     while(more){			
			...
			out.println("<td width=\"10%\" align=left >"+rs.getString(1)+"<input type='hidden' name=hid_rec_no_"+j+" value=\""+rs.getString(1)+"\"></td>"); 
			if(rs.getString(2).equals("-")){
			out.println("<td width=\"10%\" align=left style= 'WORD-BREAK:BREAK-ALL' >"+rs.getString(2)+"<input type='hidden' name=hid_finance_no_"+j+" value=\""+rs.getString(2)+"\"></td>"); 
			}else{
			out.println("<td width=\"10%\" align=left style= 'WORD-BREAK:BREAK-ALL;cursor:hand' onclick=\"contract_allo_details('"+rs.getString(1)+"')\" id=finance_no_"+j+" onmouseover=\"mouse_over('finance_no_"+j+"')\" onmouseout=\"mouse_out('finance_no_"+j+"')\">"+rs.getString(2)+"<input type='hidden' name=hid_finance_no_"+j+" value=\""+rs.getString(2)+"\"></td>"); 
			}
			out.println("<td width=\"10%\" align=left  style= 'cursor:hand' onclick=\"show_client('"+rs.getString(13)+"')\" id=client_"+j+" onmouseover=\"mouse_over('client_"+j+"')\" onmouseout=\"mouse_out('client_"+j+"')\" >"+rs.getString(6)+"</td>"); 
			...
			out.println("<td width=\"5%\" align=center><INPUT TYPE=\"checkbox\" NAME=CHK_RECEIPT"+j+"  VALUE=\"off\" onclick=\"change_val_receipt_status('"+j+"')\" ></td>");			
			out.println("</tr >"); 
      more  = rs.next();
			j=j+1;
