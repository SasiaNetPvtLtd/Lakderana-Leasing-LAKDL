...
/* Created by Chandana on 23/11/2007 */
public class LAKDL_AF_MISF_CB_accomadatons_granted_holding_Report extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			stmt2=conn.createStatement(); 
				...
				try{
				callstmt1 = conn.prepareCall("BEGIN "+m_schema_name+".AF_RE_SAVE_CB_BOR_LIMIT_RPT(:1,:2);END;");
			...
			out.println("var m_sav_msg='';");
				...
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_CB_accomadatons_granted_holding_Report?chksql=run_report&date=\"+m_to_date;"); 
				...
				*/
				...
				out.println("			m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_CB_accomadatons_granted_holding_Report?chksql=run_report&date=\"+m_date;"); 
				out.println("   	set_timer_actions();");
				out.println("			load_interface(m_url,'NORM');");
				out.println("		}");
				out.println(" } ");
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_CB_accomadatons_granted_holding_Report?chksql=print_report_new&from_date=\"+m_from_date+\"&to_date=\"+m_to_date;");	
        ...
        */
				...
				out.println("		if(document.Form1.VAL_DAY.value!=\"\" && document.Form1.VAL_MONTH.value!=\"\" && document.Form1.VAL_YEAR.value!=\"\"){");
				out.println("			m_date=document.Form1.VAL_DAY.value+'-'+document.Form1.VAL_MONTH.value+'-'+document.Form1.VAL_YEAR.value;");
				out.println("			m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_CB_accomadatons_granted_holding_Report?chksql=print_report_new&date=\"+m_date;");	
				out.println("			window.open(m_url);");
				out.println("		}");
				out.println(" } ");
				out.println("}");
      ...
      out.println("			else {");
			...
			out.println("		if(validate_date()) {");
			out.println("		 m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_CB_accomadatons_granted_holding_Report?chksql=invoice_analysis&m_from_date=\"+m_from_date+\"&m_to_date=\"+m_to_date+\"&order_by=ENT_DATE&sort_by=ASC\";");
			//out.println("		window.open(m_url);");
			...
			out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_sql_validations?chksql=get_month_date\";");
	  	...
	  	out.println("					}"); 
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'> Accounting - Accommodations Granted To Directors, Holding Company Report</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println(" <TD WIDTH=\"60%\"><input class=\"txt_input5\" type=\"text\" name=VAL_DAY maxlength=\"2\" size=\"2\" >");
			out.println("<input class=\"txt_input5\" type=\"text\" name=VAL_MONTH  maxlength=\"2\" size=\"2\" >");
			out.println("<input class=\"txt_input5\" type=\"text\" name=VAL_YEAR maxlength=\"4\" size=\"4\" >");
      ...
      rs1 = stmt1.executeQuery(Comp_Name);
			...
			boolean more1 = rs1.next();			
			  ...
			  out.println("<BODY class='body & txt-body' leftmargin='0' topmargin='0' marginwidth='0' > ");
				...
				out.println("<td align='center' width='*%' ><b>STATEMENT OF ACCOMMADATIONS GRANTED TO DIRECTORS, HOLDING COMPANY OR SUBSIDARY COMPANY </td>");
				out.println("</tr>");
				out.println("</table>");
				...
				out.println("<td align='left' width='*%' >SNBFI/FL/03/01</td>");
				out.println("</tr>");
				out.println("<tr >");
			  ...
			  out.println("<br>");
				...
				out.println("<td align='center' width='5%' ></td>");
				out.println("<td align='right' width='*%' >Amount in Rs '000</td>");
				out.println("</tr>");
				out.println("</table>");
				...
				out.println("<td width=\"10%\" align=\"center\">Contract No. </td>");
		    out.println("<td width=\"20%\" align=\"center\">Name of the Borrower </td>");
		    out.println("<td width=\"10%\" align=\"center\">Status of the Borrower </td>");
		    out.println("<td width=\"10%\" align=\"center\">Type of Facility </td>");
		    out.println("<td width=\"10%\" align=\"center\">Amount Granted </td>");
		    out.println("<td width=\"10%\" align=\"center\">Date granted & date of the settlement </td>");
		    out.println("<td width=\"15%\" align=\"center\">Terms of Repayment </td>");
		    out.println("<td width=\"5%\"  align=\"center\">Nominal rate of interest pa </td>");
		    out.println("<td width=\"10%\" align=\"center\">Total value of security offered under each category </td>");
		    out.println("</tr>");
				...
				" a.client_status, "+ //3
				" a.transaction_type, "+ //4
				" a.facility_amount, "+  //5
				" TO_CHAR(a.date_granted,'DD-MM-YYYY') ,"+  //6
				" TO_CHAR(a.date_settlement,'DD-MM-YYYY'), "+ //7
				" a.terms_of_payments,  "+ //8
				" a.irr  "+ //9
				" FROM "+m_schema_name+".af_re_tbd_cb_borrow_limit a "+
		    ...
		    out.println("<td align=\"left\"><P>"+rs.getString(6)+"<BR>"+rs.getString(7)+"</P>&nbsp;</td>");
		    out.println("<td align=\"left\">"+rs.getString(8)+"&nbsp;</td>");
		    out.println("<td align=\"right\">"+nf.format(rs.getDouble(9))+"%&nbsp;</td>");
		    out.println("<td align=\"left\">Leased Asset</td>");
		    out.println("</tr>");
				more =rs.next();
				}
				out.println("<tr>");
      ...
      rs1 = stmt1.executeQuery(Comp_Name);
		    ...
		    out.println("</tr>");
				...
				String m_client_stat="";
				
				rs = stmt.executeQuery( "SELECT MEMBER_ID , "+
				           " DECODE(CLIENT_STATUS,'HOLD_COMPANY','Holding Company','SUBS_COMPANY','Subsidiary Company','DIRECTOR','Derector','OTHER','Other') "+
									 " FROM "+m_schema_name+".AF_CO_MAS_CLIENT_GROUP "+
									 " WHERE GROUP_MASTER_ID ='OFSCL' "+
									 " AND MEMBER_ID !='OFSCL'");   
				...
				m_client_stat = rs.getString(2);
				
				
				rs1 = stmt1.executeQuery( " SELECT NVL(FINANCE_NO,'-'), "+
				      " "+m_schema_name+".af_co_get_client_name(CLIENT_CODE), "+
							" "+m_schema_name+".AF_CO_GET_TRANSACTION_DESC(TRANSACTION_TYPE), "+
							" "+m_schema_name+".AF_CO_GET_APP_FINANCE_AMT(APPLICATION_NO), "+
							" TO_CHAR(TO_DATE("+m_schema_name+".AF_CO_GET_APPROVED_DATE(APPLICATION_NO,'ACTIVATED'),'DD-MM-YYYY'),'DD-Mon-YY') ||'  '||TO_CHAR(TO_DATE("+m_schema_name+".AF_CO_GET_LAST_RENTAL_DATE(APPLICATION_NO),'DD-MM-YYYY'),'DD-Mon-YY'), "+
							" "+m_schema_name+".AF_CO_GET_NO_OF_INSTALMENT(APPLICATION_NO)||' x '||"+m_schema_name+".AF_CO_GET_INSTALMENT_AMT(APPLICATION_NO), "+
							" "+m_schema_name+".AF_CO_GET_CONTRCT_INTERST_RATE(FINANCE_NO), "+
							" "+m_schema_name+".AF_CO_GET_SECURITY_VALUE(APPLICATION_NO) "+
							" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS "+
							" WHERE CLIENT_CODE= '"+m_client_code+"' "+
							" AND APPLICATION_STATUS IN ('ACTIVATED','REPOSSESS','TERMINATE','TERMI','TERMINATED','COMPLETED','LEGAL') ");
							//" AND APPLICATION_STATUS NOT IN ('ENTERED','ENT','ENT-CON','ENT_CON','VERIFY','VERIFY1','VERIFY2','VERIFYL','V-APP','CANCEL','REJECT')");
		    ...
		    out.println("<td align=\"center\">"+m_client_stat+" &nbsp;</td>");
        ...
        m_tot_amnt  = rs.getDouble(6) + rs.getDouble(12) + rs.getDouble(18) + rs.getDouble(24);
			...
			if(m_State.equals("1")){
             ...
             contract_det = " SELECT X.FINANCE_NO, "+ //1
			                      " LAKDL.af_co_get_client_name(X.CLIENT_CODE), "+ //2
														" LAKDL.AF_CO_MAS_ASSET_DESC(X.APPLICATION_NO)||' / '||NVL(LAKDL.AF_CO_GET_ALL_REG_NUMBERS(X.FINANCE_NO),' '), "+ //3
														" LAKDL.AF_CO_GET_APP_FINANCE_AMT(X.APPLICATION_NO), "+ //4
														" NVL(LAKDL.AF_CO_GET_APPROVED_DATE(X.APPLICATION_NO,'ACTIVATED'),' ')||' to '||LAKDL.AF_CO_GET_LAST_RENTAL_DATE(X.APPLICATION_NO), "+ //5
														" LAKDL.AF_CO_GET_NO_OF_INSTALMENT(X.APPLICATION_NO)||' x '||LAKDL.AF_CO_GET_INSTALMENT_AMT(X.APPLICATION_NO), "+ //6
														" NVL(LAKDL.AF_CO_GET_MORE_3MON_RENTS_CNT(X.FINANCE_NO,TO_CHAR(SYSDATE,'DD-MM-YYYY')),0), "+ //7
														" LAKDL.AF_CO_GET_ARREAS_RENT_CHARGS(X.APPLICATION_NO,TO_CHAR(SYSDATE,'DD-MM-YYYY')), "+ //8 
														" '' WDV, "+ //9
														" LAKDL.AF_CO_GET_RENT_INCOME_SUSPENSE(X.APPLICATION_NO,TO_CHAR(SYSDATE,'DD-MM-YYYY')), "+ //10
														" '' DEPOSIT, "+ //11
														" NVL(LAKDL.AF_CO_GET_REPO_VALUATION_DATE(X.APPLICATION_NO),'-'), "+ // 12
														" NVL(LAKDL.AF_CO_GET_REPO_VALUATION_VAL(X.APPLICATION_NO),0), "+ //13
														" '-' ACTION "+  //14
														
														" FROM LAKDL.AF_CO_PRO_APPLICATION_DETAILS X "+
