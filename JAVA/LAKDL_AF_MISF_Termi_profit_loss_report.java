...
public class LAKDL_AF_MISF_Termi_profit_loss_report extends javax.servlet.http.HttpServlet { 
		...
		try { 
			...
			if(m_chksql.equals("run_report")){ 
				...
				try{
					...
					callstmt1   = conn.prepareCall("BEGIN "+m_schema_name+".AF_TERMI_PROF_LOSS_RPT_N3(:1,:2,:3,:4);END;"); // added by udara 23-03-2018
			...
			if(m_chksql.equals("main_page")){ 
				...
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Termi_profit_loss_report?chksql=run_report&to_date=\"+m_date+\"&location_id=\"+document.Form1.TXT_LOCATION_CODE.value+\"&from_date=\"+m_from_date;"); 
				...
				out.println("			alert('Recovery Report is generated. Use View Report button to get the view.');"); // added by udara 30-11-2018
				out.println("		}");
				out.println("		else{");
				...
				out.println("       m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Termi_profit_loss_report?chksql=print_report_new&location=\"+m_location+\"&date=\"+m_date+\"&from_date=\"+m_from_date+\"&status=\"+m_status;");	 // mod by udara on 18-06-2013
				out.println("			window.open(m_url);");
				out.println("	}else {");
				out.println("   DIV_TXT_LOCATION_CODE.style.color = 'red'; ");
				out.println("   alert(\"Please enter all required fields marked with a '*' on screen.\"); ");
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Termi_profit_loss_report?chksql=main_page&generate=page';"); 
				out.println("		}"); 
				out.println("}"); 
				...
				*/
				...
				// commented by udara 05-07-2018
				/*
				Sql_data="	SELECT CONTRACT_NO, "+//1
					" NAME, "+//2
					" NVL(VEHICLE_NO,'-'), "+//3
					
					//" DECODE(STATUS,'ERL_TER','Early Termination','NOR_TER','Normal Termination','PAR_TER','Partial Termination','RESCHEDULE','Reschedule','RENT_REVIS','Rental Revision','BALANCE_RE','Balance Receivable','ASSET_REPL','Asset Replacement','ENHA_DOWN','Enhancement And Downward Revision','BAL_TRANSF','Balance Transfer'), "+//4 OTHER TYPES ADDED BY KANCHANA ON 2016-05-31 // commented by udara 23-03-2018
					" DECODE(YARD_STATUS,'Y',DECODE(STATUS,'ERL_TER','Early Termination','NOR_TER','Normal Termination','PAR_TER','Partial Termination','RESCHEDULE','Reschedule','RENT_REVIS','Rental Revision','BALANCE_RE','Balance Receivable','ASSET_REPL','Asset Replacement','ENHA_DOWN','Enhancement And Downward Revision','BAL_TRANSF','Balance Transfer'),'Cease and Sold'),      "+ // added by udara 23-03-2018
					...
					" ENT_USER "+
					" FROM "+m_schema_name+".AF_TERMINATION_PROFIT_LOSS_RPT "+//  AF_NEW_PROFIT_LOSS_DET
				...
				*/
				...
				while(more){					
					...
					//out.println("<td STYLE='text-align:right;' > "+nf.format(rs.getDouble(21))+" </td>"); // Accounting Loss/Profit (G-(B+X)) // re-enabled by udara 02-04-2019
					out.println("<td STYLE='text-align:right;' > "+nf.format((rs.getDouble(17)-(rs.getDouble(11)+rs.getDouble(14))))+" </td>"); // Accounting Loss/Profit (G-(B+X)) // re-commented by udara 02-04-2019
