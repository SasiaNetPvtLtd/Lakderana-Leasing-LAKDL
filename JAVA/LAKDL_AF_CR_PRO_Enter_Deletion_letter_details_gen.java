...
public class LAKDL_AF_CR_PRO_Enter_Deletion_letter_details_gen extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_chksql.equals("main_page")){
					out.println("<HTML>"); 
					out.println("<HEAD>"); 
					out.println("<TITLE>Deletion Letter Approval </TITLE>"); 
					out.println("</HEAD>"); 
					...
					out.println("      m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Deletion_letter_details_gen?chksql=details&status=\"+document.Form1.hid_status.value+\"&APPLICATION_NO=\"+document.Form1.TXT_APP_NO.value;"); 
					...
					out.println("			window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Deletion_letter_details_gen?chksql=main_page';"); 
					out.println("		}"); 
					out.println("}"); 
					...
					out.println("   help_box.innerHTML=\" Deletion Letter Approval  - \"+m_val;"); 
					out.println("}"); 
					out.println(""); 

					out.println("function load_roll_out_value(){");
					out.println("help_box.innerHTML=\" Deletion Letter Approval  - \"+document.Form1.hid_status.value;"); 
					out.println("}"); 
				  ...
				  out.println("document.Form1.TXT_VALUE_DATE_YY.disabled=true;");
					...
					out.println("    m_sql = \"m_help_DELETION_LETTER_GEN\";");
				  ...
				  out.println("count=count+1;");
					...
					//out.println("		if(validate_data()){");	
					out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Deletion_letter_details_gen_save';");  
				...
				//out.println("  var app_no =  document.Form1.elements[\"HID_APP_NO_\"+row_num].value; ");
				//out.println("  var fin_no =  ''; ");
				//out.println("  var cli_no =  ''; ");
				
				
				out.println("  var app_no =  document.Form1.elements[\"HID_APP_NO_\"+row_num].value; ");
				out.println("  var fin_no =  document.Form1.elements[\"HID_FIN_NO_\"+row_num].value; ");
				out.println("  var cli_no =  document.Form1.elements[\"HID_CLI_NO_\"+row_num].value; ");
				
				out.println("  document.Form1.HID_APPLICATION_NO.value =  app_no; ");
				out.println("  document.Form1.HID_FINANCE_NO.value =  fin_no; ");
				out.println("  document.Form1.HID_SEIZER_NO.value =  cli_no; ");
				
				//out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Document_Printing_View?chksql=main_page&APP_NO\"+app_no+\"&FIN_NO=\"+fin_no+\"&CLIENT_NO=\"+cli_no;");
				//out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Ceasing_order_letter?chksql=main_page&application_no=\"+app_no+\"&client_code=\"+cli_no+\"&document_code=CEAS_ORDER\";");
				//out.println("  window.open(m_url); ");
				
				//out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Ceasing_order_letter?chksql=main_page&application_no=\"+app_no+\"&client_code=\"+cli_no+\"&document_code=CEAS_ORDER\";");
				
				//out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Ceasing_order_letter_for_seiz_app?chksql=main_page&application_no=\"+app_no+\"&client_code=\"+cli_no+\"&document_code=CEAS_ORDER&button_status=Y\";"); 
				
				out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Deletion_letter_new?chksql=main_page&application_no=\"+app_no+\"&client_code=\"+cli_no+\"&document_code=DELE_LETT&print=TRUE\";"); 
				...
				out.println("}");	
					...
					out.println("<INPUT TYPE='Hidden' NAME='HID_SEIZER_NO' VALUE=\"\">");
					...
					out.println("</tr>"); 
					out.println("<tr>"); 
					out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Deletion Letter Approval </td>"); 
					out.println("</tr>"); 
					out.println("<tr>"); 
					...
					out.println("<tr >"); 
					out.println("<td width='10%' ><DIV id='DIV_TXT_APP_NO' class=div_input>Application No</div></td>"); 
					out.println("<td width='40%' ><input class='txt_input' type='text' name='TXT_APP_NO' maxlength='15' size='15' >"); 
					...
					rs=stmt.executeQuery(" "+ 								
									" SELECT "+
										"A.APPLICATION_NO, "+ // 1
										"B.FINANCE_NO,"+ // " B.FINANCE_NO, "+     // 2
										"B.CLIENT_CODE,"+ //" B.CLIENT_CODE, "+    // 3
										"'-',"+ // " A.SEIZER_CODE, "+    // 4
										"'-',"+ // " "+m_schema_name+".AF_CO_GET_SEIZER_NAME(A.SEIZER_CODE), "+ // 5
										"'-',"+ // " (SELECT ADDRESS1 FROM AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) || ', ' || (SELECT ADDRESS2 FROM AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) ADDRESS, "+ // 6
										"'-',"+ // " (SELECT TEL_NO FROM AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) TEL_NO, "+      // 7
										"'-'"+ // " (SELECT MOBILE_NO FROM AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) MOBILE_NO "+ // 8
										  " FROM "+m_schema_name+".AF_CO_MAS_DELE_LET_DETAILS A, "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
				...
				out.println("<td  width='10%' > Application No </td>");
				/*
				out.println("<td  width='10%' > Finance No </td>");
				out.println("<td  width='10%' > Client Code </td>");
	      		out.println("<td  width='10%' > Seizer Code </td>");
				...
				*/
				...
				out.println("<td  width='10%' > <input type='hidden' name='HID_APP_NO_"+j+"'    id='HID_APP_NO_"+j+"'    value ='"+rs.getString(1)+"' > <input type='hidden' name='HID_FIN_NO_"+j+"'    id='HID_FIN_NO_"+j+"'    value ='"+rs.getString(2)+"' > <input type='hidden' name='HID_CLI_NO_"+j+"'    id='HID_CLI_NO_"+j+"'    value ='"+rs.getString(3)+"' > "+rs.getString(1)+" </td>"); 
				/*
				out.println("<td  width='10%' > <input type='hidden' name='HID_SEIZER_NO_"+j+"' id='HID_SEIZER_NO_"+j+"' value ='"+rs.getString(4)+"' > "+rs.getString(4)+" </td>"); 
				out.println("<td  width='20%' >  "+rs.getString(5)+" </td>"); 
                out.println("<td  width='20%' >  "+rs.getString(6)+" </td>");
				out.println("<td  width='20%' >  "+rs.getString(7)+" </td>");
				out.println("<td  width='20%' >  "+rs.getString(8)+" </td>");
				*/
				out.println("<td  width='5%' align=center><input type='button' class='but_input' name='CHK_"+j+"' value ='Print' onclick=\"check_status_app("+j+");\" ></td>");
				
				//out.println("<td  width='*%'  > &nbsp; <input type='hidden' name='HID_APP_NO_"+j+"'    id='HID_APP_NO_"+j+"'    value ='"+rs.getString(1)+"' > </td>");
