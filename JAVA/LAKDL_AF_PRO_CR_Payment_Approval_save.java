...
public class LAKDL_AF_PRO_CR_Payment_Approval_save extends HttpServlet {
    ...
    public synchronized void service(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
        ...
        try {
            ...
            m_process_status        = m_sn_methods.met_formdata(reqstr, "hid_process_status_post");
            m_approval_level        = m_sn_methods.met_formdata(reqstr, "hid_approval_level");
            
            j = 1;
            
            
            callableStatement = connection.prepareCall("BEGIN " + m_schema_name + ".AF_PRO_CR_SAVE_PAYMENT_APPR(:1, :2, :3, :4); END;");
            
            callableStatement.setString(2, null);   /* Only For Payment Reversal */
            ...
            for (int i = 1; i <= m_row_count; i++) {
                ...
                if (m_approve_status.equals("A")) {
                    ...
                    m_group_payment_no = m_sn_methods.met_formdata(reqstr, "HID_GROUP_PAYMENT_NO_" + i);
            ...
            out.println("               window.location.href = '" + m_class_url + "/" + m_fschema_name + "AF_PRO_CR_Payment_Approval_display?chksql=main_page&approval_level=" + m_approval_level + "';");
            out.println("           }");
            out.println("       </script>");
            out.println("   </head>");
