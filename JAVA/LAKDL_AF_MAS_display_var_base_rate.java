...
//Created By Sandun on 16-09-2008 For OFSCL-SYSTEM ADMINISTATION-PRICING
...
public class LAKDL_AF_MAS_display_var_base_rate extends javax.servlet.http.HttpServlet { 
    ...
    /* 5 Variables Added By Samitha Kulatilaka On 2010-02-01 */
    String apply_date_dd = null;
    String apply_date_mm = null;
    String apply_date_yy = null;
    String rate          = "";
    String status        = null;
  ...
  //********************************************************************************************
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>System Administration - Variable Base Rate</TITLE>"); 
			out.println("</HEAD>"); 
	  	...
	  	out.println("		}"); 
			out.println("		if(document.Form1.hid_help_type.value==\"88\"){"); 
			out.println("		help_value_assign_base_code();");  
	  	out.println("		}"); 
	  	...
	  	out.println("		}"); 
			...
			out.println("function help_value_assign_base_code() {"); 
			...
			//out.println("alert('MMM');");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MAS_display_var_base_rate?chksql=sub_charge_details&item_base_code=\"+document.Form1.TXT_BASE_CODE.value+\"\";");
		...
		//***************************************************	
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MAS_display_var_base_rate?chksql=main_page';");
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\" System Administration - Variable Base Rate - \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\" System Administration - Variable Base Rate - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
			...
			//out.println("alert(code.value+'-'+obj.value+'-'+day.value+'-'+month.value+'-'+year.value);");
		...
		out.println("function HelpView(Start,End,Hid_No,Max) {"); 
	  	...
	  	out.println("		}"); 
			...
			out.println("document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MAS_save_base_rate?number='+document.Form1.hid_count.value+'';");  
			...
			out.println("return false;");
			out.println("}else if(document.Form1.elements[\"txt_new_rate_\"+i].value==\"\" ){");
      ...
      out.println("  if(document.Form1.hid_cal_date.value=='2'){"); 
	    ...
	    out.println("}");
		  ...
		  out.println("function help_base_code() {");
			out.println(" sub_charge_details.innerHTML = ''; ");
			...
			out.println("<INPUT TYPE='Hidden' NAME='SCREEN_NAME' VALUE=\"NEW\">"); 			
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>System Administration - Variable Base Rate</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
      ...
      out.println("<br >"); 
			...
			out.println("<tr >"); 
			out.println("<td width='30%' ><DIV id='DIV_TXT_BASE_CODE'  class=div_input>Base Code *</DIV></td>"); 
			out.println("<td width='40%' ><input class='txt_input' type='text' name='TXT_BASE_CODE' maxlength='10' size='10'  onblur=\"assig('t5')\" >"); 
			out.println("<input class='but_input' type='button' name='BUT_HELP_MAIN' value=\"Help\" onClick=\"help_base_code()\"></td>"); 
			...
			out.println("<td width='25%' align='center'>Base Description</td>"); 
			out.println("<td width='10%' align='center'>Date</td>"); 
			out.println("<td width='10%' align='center'>Rate</td>"); 
			out.println("<td width='20%' align='center'>Apply Date</td>"); 
			out.println("<td width='10%' align='center'>Rate</td>"); 
	...
	rs = stmt.executeQuery( " SELECT "+
													" DISTINCT A.BASE_CODE, B.DESCRIPTION,TO_CHAR(A.APPLY_DATE,'DD-MM-YYYY'),A.RATE "+
													...
													" FROM "+m_schema_name+".AF_CO_PRO_INTEREST_BASE_RATE A "+
													" WHERE A.STATUS='Y' "+//ADDED BY LALANKA ON 25-01-2010
		...
		boolean more = rs.next();	
			...
			out.println("<td width='25%' align='left'><class=div_input>"+rs.getString(2)+"</td>"); 
			...
			out.println("</td>"); 
			out.println("<td width='10%' align='right'><class=div_input><input type='text' class=\"txt_input\" maxlength='6' onblur=\"format_number3(this,2)\" style=\"width: 45px\" name='txt_new_rate_"+j+"'></td>"); // Modified by Udara Viruwan on 12/10/2009 by calling rate validation method
			out.println("<td width='5%' align='center'><class=div_input><input type='checkbox' name='chk_update_"+j+"' onclick='makeRequest(document.Form1.chk_update_"+j+",document.Form1.base_rate_dd_"+j+",document.Form1.base_rate_mm_"+j+",document.Form1.base_rate_yy_"+j+",document.Form1.hid_base_code_"+j+")'></td>"); 
			out.println("</tr>"); 
			more = rs.next();
			j=j+1;
			}
			...
			String m_base_code= req.getParameter("item_base_code");
	...
	rs = stmt.executeQuery( " SELECT "+
													...
													" FROM "+m_schema_name+".AF_CO_PRO_INTEREST_BASE_RATE A "+
													" WHERE A.STATUS='Y'  "+//ADDED BY LALANKA ON 25-01-2010
													" GROUP BY A.BASE_CODE)	AND A.APPLY_DATE <= SYSDATE AND A.BASE_CODE='"+m_base_code+"' "+												
													" UNION ALL "+
													" SELECT "+
													" DISTINCT A.BASE_CODE, B.DESCRIPTION,TO_CHAR(A.APPLY_DATE,'DD-MM-YYYY'),A.RATE "+
													" FROM "+m_schema_name+".AF_CO_PRO_INTEREST_BASE_RATE A, "+
													...
													" SELECT MAX(A.ENT_DATE) "+
													" FROM "+m_schema_name+".AF_CO_PRO_INTEREST_BASE_RATE A "+
													" WHERE A.STATUS='Y' AND A.BASE_CODE='"+m_base_code+"' "+//ADDED BY LALANKA ON 25-01-2010
													" GROUP BY A.BASE_CODE)	AND A.APPLY_DATE=( SELECT MIN(VAL_DATE) "+
													" FROM "+m_schema_name+".AF_CO_MAS_INTEREST_BASE_DATES WHERE BASE_CODE='"+m_base_code+"' AND VAL_DATE > SYSDATE )		");
    ...
    /* Added By Samitha Kulatilaka On 2010-02-01 [Begin] */
    
    /* Get The Next Future Date For The 'Base Code' */
    rs1 = stmt.executeQuery("" + 
            " SELECT " + 
            "   NVL(TO_CHAR(VAL_DATE, 'DD'), '-'), " + 
            "   NVL(TO_CHAR(VAL_DATE, 'MM'), '-'), " + 
            "   NVL(TO_CHAR(VAL_DATE, 'YYYY'), '-') " + 
            " FROM " + m_schema_name + ".AF_CO_MAS_INTEREST_BASE_DATES " + 
            " WHERE BASE_CODE = '" + m_base_code + "' " + 
            " AND VAL_DATE > TO_DATE('" + rs.getString(3) + "', 'DD-MM-YYYY') " + 
            " ORDER BY VAL_DATE DESC ");
    /* Allocate The Last Row's Data To The Variables */
    while (rs1.next()) {
        apply_date_dd = rs1.getString(1);
        apply_date_mm = rs1.getString(2);
        apply_date_yy = rs1.getString(3);
    }
    rs1.close();
    
    
    /* Added By Samitha Kulatilaka On 2010-02-01 [End] */
            
            
			out.println("<input type='text' name='base_rate_dd_"+j+"' class=\"txt_input\" style=\"width: 25px\" maxlength='2' value = \"" + apply_date_dd + "\" onblur=\"chkstartdate(document.Form1.base_rate_dd_"+j+",document.Form1.base_rate_mm_"+j+",document.Form1.base_rate_yy_"+j+")\" > "); 
			out.println("<input type='text' name='base_rate_mm_"+j+"' class=\"txt_input\" style=\"width: 25px\" maxlength='2' value = \"" + apply_date_mm + "\" onblur=\"chkstartdate(document.Form1.base_rate_dd_"+j+",document.Form1.base_rate_mm_"+j+",document.Form1.base_rate_yy_"+j+")\" > "); 
			out.println("<input type='text' name='base_rate_yy_"+j+"' class=\"txt_input\" style=\"width: 45px\" maxlength='4' value = \"" + apply_date_yy + "\" onblur=\"chkstartdate(document.Form1.base_rate_dd_"+j+",document.Form1.base_rate_mm_"+j+",document.Form1.base_rate_yy_"+j+")\" > ");
			out.println("<span style= cursor:hand;cursor-color:blue;color:black onclick=load_calendar('2',"+j+")>Calender</span>"); 
			out.println("</td>"); 
    ...
    /* Get The Rate If It Is Unapproved */
    rs2 = stmt.executeQuery("" + 
            ...
            " WHERE BASE_CODE = '" + m_base_code + "' " + 
            " AND APPLY_DATE = TO_DATE('" + apply_date_dd + "-" + apply_date_mm + "-" + apply_date_yy + "', 'DD-MM-YYYY') ");
    while (rs2.next()) {
        status = rs2.getString(2);
        if(status.equals("N")) {
            rate = rs2.getString(1);
        }
    }
    rs2.close();
    ...
    /* Added By Samitha Kulatilaka On 2010-02-01 [End] */
			...
			out.println("<td width='10%' align='right'><class=div_input><input type='text' class=\"txt_input\" maxlength='6' value = \"" + rate + "\"  style=\"width: 45px\" name='txt_new_rate_"+j+"'></td>"); 
			out.println("<td width='5%' align='center'><class=div_input><input type='checkbox' name='chk_update_"+j+"' onclick='makeRequest(document.Form1.chk_update_"+j+",document.Form1.base_rate_dd_"+j+",document.Form1.base_rate_mm_"+j+",document.Form1.base_rate_yy_"+j+",document.Form1.hid_base_code_"+j+")'></td>"); 
			out.println("</tr>"); 
			more = rs.next();
			j=j+1;
