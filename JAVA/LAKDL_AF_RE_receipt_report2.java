/*
	* this is based on LAKDL_AF_RE_receipt_report
	* 2017-09-20
	* inesh 
	* 23772 Need a new auto allocation facility
*/
...
public class LAKDL_AF_RE_receipt_report2 extends javax.servlet.http.HttpServlet { 
		...
		try { 
			...
			if(m_chksql.equals("main_page")){
				...
				if(m_active_status!=null && !m_active_status.equals("A")){
					m_active_status_string = " AND "+m_schema_name+".AF_RE_IS_VEHICLE_IN_YARD2(C.FINANCE_NO,'"+m_to_date+"') = '"+m_active_status+"' ";
				}
				...
				rs1= stmt1.executeQuery(" SELECT "+
					...
					" NVL("+m_schema_name+".AF_CO_GET_LOCATION_DESC('"+m_branch_id+"'),'-')"+
					" FROM DUAL ");
				...
				if (m_location_code!=null && m_location_code.trim().equals("ALL")) {
					...
					String sql1 = " SELECT "+
						...
						//" ,DECODE(NVL(A.RENTAL_OTER_INVOICE,0),'0','Insurance','Rental') N_TYPE  "+ // 18 //  commented by udara 25-02-2014
						//" ,DECODE(NVL(CLOSING_FLAG,'N'),'Y','Closing',DECODE(NVL(RENTAL_OTER_INVOICE,0),'0','Insurance','Rental')) N_TYPE "+ // 18  added by udara 25-02-2014
						//" ,DECODE(NVL(CLOSING_FLAG,'N'),'Y','Closing','S','Stamp Duty','I','Documentation Charges',DECODE(NVL(RENTAL_OTER_INVOICE,0),'0','Insurance','Rental')) N_TYPE "+ // added by udara 14-07-2015
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code						
						...
						sql1 = sql1 + " AND TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						"  "+ m_user_query + //" AND "+m_schema_name+".AF_CO_GET_APP_LOCATION("+m_schema_name+".AF_CO_GET_APPLICATION_NO(NVL("+m_schema_name+".AF_CO_GET_REC_FIN_NO(A.REC_NO),'-'))) = '"+m_location_code+"' "+ m_user_query +
				...
				//out.println("<BODY   class='body & txt-body' LEFTMARGIN='0' TOPMARGIN='0'  STYLE='{ background-image: url(\""+m_html_client_url+"/vehicle_inspection/doc_1.jpg\"); background-repeat: no-repeat; background-attachment: fixed; background-position:center; background-size: 100%; }'   >"); // added by udara 09-05-2014 // background = \""+m_html_client_url+"/vehicle_inspection/doc_1.jpg\" // 
				//out.println("<BODY   class='body & txt-body' LEFTMARGIN='0' TOPMARGIN='0'  STYLE='{ background-image: url(\""+m_html_client_url+"/vehicle_inspection/doc_1.jpg\"); background-repeat: no-repeat;  background-size: 100%; }'   >");
				...
				if (m_location_code.trim().equals("ALL")) {
					...
					String sql2 = " SELECT "+
						...
						//" ,DECODE(NVL(RENTAL_OTER_INVOICE,0),'0','Insurance','Rental') N_TYPE "+ // 18 //  commented by udara 25-02-2014
						//" ,DECODE(NVL(CLOSING_FLAG,'N'),'Y','Closing',DECODE(NVL(RENTAL_OTER_INVOICE,0),'0','Insurance','Rental')) N_TYPE "+ // 18  added by udara 25-02-2014
						" ,DECODE(NVL(CLOSING_FLAG,'N'),'Y','Closing','S','Stamp Duty','I','Documentation Charges',DECODE(NVL(RENTAL_OTER_INVOICE,0),'0','Insurance','Rental')) N_TYPE "+ // added by udara 14-07-2015
						...
						" ,SUBSTR(SUB_REC_NO,1,INSTR(SUB_REC_NO,'/')-1)||LPAD(SUBSTR(SUB_REC_NO,INSTR(SUB_REC_NO, '/')+1), 5, '0') SUB_REC_SORT, "+ // added by udara 24-02-2015
						" A.BRANCH_CODE,"+
						" NVL("+m_schema_name+".AF_CO_GET_USER_REGION(A.BRANCH_CODE),'-') REGION "+ //-------Added by Samith Dilshan on 15-06-2015
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql2 = sql2 +"  AND  NVL("+m_schema_name+".AF_CO_GET_USER_REGION("+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN_3(A.ENT_USER,A.REC_NO)),'-') = '"+m_region+"' "; // added by udara 07-08-2015
					}
						...
						sql2 = sql2 + " "+ m_user_query + " "+ // added by udara on 25-07-2013
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql2 = sql2 + "  "+ m_user_query + //" AND "+m_schema_name+".AF_CO_GET_APP_LOCATION("+m_schema_name+".AF_CO_GET_APPLICATION_NO(NVL("+m_schema_name+".AF_CO_GET_REC_FIN_NO(A.REC_NO),'-'))) = '"+m_location_code+"' "+ m_user_query +
						//" AND A.STATUS <>'C'"+ //ADDED BY NUWAN ON 01-04-08 // commented by udara on 28-08-2013
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql2 = sql2 + " AND C.BRANCH_CODE = '"+m_location_code+"'  "+ m_user_query + //" AND "+m_schema_name+".AF_CO_GET_APP_LOCATION("+m_schema_name+".AF_CO_GET_APPLICATION_NO(NVL("+m_schema_name+".AF_CO_GET_REC_FIN_NO(A.REC_NO),'-'))) = '"+m_location_code+"' "+ m_user_query +
						//" AND A.STATUS <>'C'"+ //ADDED BY NUWAN ON 01-04-08 // commented by udara on 28-08-2013
						" AND A.STATUS  IN ('C','CAD') "+ // added by udara on 28-08-2013
						" "+ location_query_part + " "+ // " AND  "+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN(ENT_USER) LIKE '"+m_branch_id+"%'  "+ // added by udara on 27-02-2013 // mod by udara on 25-07-2013 for performance
				...
				if(mm_location_code.trim().equals("ALL")){  // added by udara on 07-12-2012
					...
					String sql3 = " SELECT TYPE,SUM(TOTAL),COUNT(COUNT),DUMMY FROM ( "+
						...
						sql3 = sql3 + " "+ location_query_part + " "+ //" AND  "+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN(ENT_USER) LIKE '"+m_branch_id+"%'  "+ // added by udara on 27-02-2013 // mod  by udara on 25-07-2013 for performance
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql3 = sql3 + " AND TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
					...
					*/
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						/*	" UNION ALL	"+		//WEB ACCOUNT LINK PART REMOVED BY KANISHKA ON 01-07-2014  [REQUESTED BY CLIENT]		 
				...
				if(mm_location_code.trim().equals("ALL")){  // added by udara on 07-12-2012
					...
					// commented by udara 13-08-2015
					/*
					String sql4 = " SELECT SETTLE_MODE ,SUM(REC_AMOUNT) ,COUNT(REC_AMOUNT)  "+
						" FROM "+m_schema_name+".AF_CO_PRO_SETTL_RECEIPT  A "+  // mod by udara 09-08-2013
						" WHERE  TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')>=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ 
						" AND TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						// " AND STATUS <>'C' "+ // commented by udara on 28-08-2013
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql4 = sql4 + " AND ACC_NO = 'TRANSFERAC' "+
						" "+ location_query_part + " "+ // " AND  "+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN(ENT_USER) LIKE '"+m_branch_id+"%'  "+ // added by udara on 27-02-2013 // mod by udara on 25-07-2013
				...
				if(mm_location_code.trim().equals("ALL")){  // added by udara on 07-12-2012
					...
					// commented by udara 13-08-2015
					/*
					String sql5= " SELECT ENT_USER,TYPE,SUM(TOTAL),COUNT(COUNT), DUMMY FROM( "+
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql5 = sql5 +"  AND  NVL("+m_schema_name+".AF_CO_GET_USER_REGION("+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN_3(A.ENT_USER,A.REC_NO)),'-') = '"+m_region+"' "; // added by udara 07-08-2015
					}
					
						sql5 = sql5 +  " AND TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql5 = sql5 +  " AND TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
				...
				*/
				...
				if(mm_location_code.trim().equals("ALL")){
					...
					// commented by udara 13-08-2015
					/*
					String sql6= " SELECT ENT_USER ,SETTLE_MODE ,SUM(REC_AMOUNT) ,COUNT(REC_AMOUNT)  "+
						" FROM "+m_schema_name+".AF_CO_PRO_SETTL_RECEIPT A "+ // mod by udara 09-08-2013
						" WHERE  TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')>=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
						" AND TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') ";
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql6 = sql6 +"  AND  NVL("+m_schema_name+".AF_CO_GET_USER_REGION("+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN_3(A.ENT_USER,A.REC_NO)),'-') = '"+m_region+"' "; // added by udara 07-08-2015
					}
					
						sql6 = sql6 +  " AND SETTLE_MODE = 'STD_ORD' "+
						" AND ACC_NO = 'TRANSFERAC' "+m_user_query +
						// " AND "+m_schema_name+".AF_CO_GET_APP_LOCATION("+m_schema_name+".AF_CO_GET_APPLICATION_NO(NVL("+m_schema_name+".AF_CO_GET_REC_FIN_NO(REC_NO),'-'))) LIKE '%"+m_location_code+"%' "+
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql6 = sql6 + " AND TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						//" AND A.STATUS <>'C' "+ // commented by udara on 28-08-2013
						//" AND A.STATUS NOT IN ('C','CAD') "+  // added by udara on 28-08-2013  // commented by Kanishka on 20-09-2013
						" AND A.SETTLE_MODE = 'STD_ORD' "+
				...
				if(mm_location_code.trim().equals("ALL")){  // added by udara on 07-12-2012
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql7 = sql7 +"  AND  NVL("+m_schema_name+".AF_CO_GET_USER_REGION("+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN_3(A.ENT_USER,A.REC_NO)),'-') = '"+m_region+"' "; // added by udara 07-08-2015
					}
					
						sql7 = sql7 + " AND TO_DATE(TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						//" AND STATUS <>'C' "+
					...
					if(!(m_region.equals("NOT_SELECT"))){    // Added By: Samith Dilshan on 2015-06-15 for Region Code
						...
						sql7 = sql7 + " TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')>=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ 
						" AND TO_DATE(TO_CHAR(A.EFF_VALDATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						//" AND STATUS <>'C' "+
			...
			rs1= stmt1.executeQuery(m_bbf_sql);
				...
				if(mm_location_code.trim().equals("ALL")){  
					...
					rs1= stmt1.executeQuery(" SELECT SUM(TOTAL),COUNT(COUNT) FROM ( "+
						...
						" AND  A.REC_CANCEL_DATE >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ // commented by udara 13-12-2013
						" AND  A.REC_CANCEL_DATE <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+ // commented by udara 13-12-2013
						" AND  A.REC_CANCEL_DATE < TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ // added by udara 13-12-2013 
						" AND A.STATUS IN ('C','CAD') "+ 
						"  "+m_user_query + 
						" "+location_query_part+"  "+
						*/
				...
				/*************Below query Added by Jithendra 23-01-2017 for Active Status*************/	
					rs1= stmt1.executeQuery(" "+
						...
						*/
						...
						//" AND A.REC_NO = B.RECEIPT_NO  "+//Commented by Jithendra 23-01-2017
						" AND D.STATUS = 'Y' "+ 
						" AND A.STATUS = 'B' "+m_user_query +
						" "+location_query_part+"  "+ 
				...
				if(!m_branch_id.equals("")){
					...
					*/
					...
					rs1= stmt1.executeQuery(" "+
						...
						//" AND "+m_schema_name+".AF_CO_GET_USER_LOCATION_RRN(B.ENT_USER) = '"+m_branch_id+"' "+ // added by udara on 09-08-2013 // commented by udara 02-12-2013
