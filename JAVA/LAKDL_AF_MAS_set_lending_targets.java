...
public class LAKDL_AF_MAS_set_lending_targets extends javax.servlet.http.HttpServlet {
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			*/
			...
			out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;    ><B>Employee Name</td>'+");
          ...
          out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand; title=\"Click here to sort by - Finance No  \"    onclick=sort_data(\"FINANCE_NO\") ><B>Finance No</td>'+");
			...
			out.println("'<td  width=\"18%\" align=\"left\" ><B>Branch Name</td>'+");
			out.println("'<td  width=\"20%\" align=\"left\" ><B>Employee Name</td>'+");
			...
			out.println("m_table.innerHTML+='<table align=\"center\" width=\"100%\" class=\"table\"><TR class=\"pdn_txtpos2\" align=\"center\">'+");
			out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;   onclick=sort_data(\"CLIENT_NAME\") ><B>Employee Name</td>'+");
			...
			//out.println("window.open(m_url)");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=m_prime_chk_LAKDL_AF_RE_users&user_location_set=\"+document.Form1.TXT_LOCATION_CODE.value+\"&user_year=\"+document.Form1.TXT_YEAR.value+\"&user_month=\"+document.Form1.TXT_MONTH.value;");  // added by udara on 13-09-2013
			//out.println("window.open(m_url);"); // commented by udara 12-04-2017
			out.println("load_interface(m_url,'XML');");
			out.println("}"); 
			...
			*/
			...
			out.println("var target_cases=0;");
			out.println("var target_values=0;");
			
			out.println("var target_cases_tmp=0;");
			out.println("var target_values_tmp=0;");
			...
			out.println("while(i<data_vec.length){");
			out.println("var n_tar_cases=data_vec[i+3];");
			out.println("var n_tar_vals=data_vec[i+4];");
			out.println("m_row_num='<TD WIDTH=\"2%\" align=\"left\"    ><u>'+count+'</u></TD>';");
			out.println("m_branch_code='<TD WIDTH=\"18%\" align=\"left\"    ><u>'+data_vec[i+5]+'</u></TD>';");
			...
			//out.println("m_btn='<td width=\"5%\" align=\"center\" ><input class=\"but_input\" type=\"button\" name=BUT_TXT_USER'+lineno+' value=\"Help\" onClick=\"help_button_user('+lineno+')\"></td>';"); 
			out.println("m_target_cases= '<td width=\"20%\"   align=\"center\" ><input class=\"txt_input\" align=\"right\" type=\"text\" value='+data_vec[i+3]+' onblur=\"val_total1('+lineno+')\" name=TXT_TARGET_CASES'+lineno+' style={width:120px;} maxlength=\"500\" size=\"15\"></TD>';") ;		// added by Sandun --- on 28/07/2008	
			out.println("m_target_value= '<td width=\"20%\"   align=\"center\" ><input class=\"txt_input\" align=\"right\" type=\"text\" value='+data_vec[i+4]+' onblur=\"val_total2('+lineno+')\" name=TXT_TARGET_VALUE'+lineno+' style={width:120px;} maxlength=\"500\" size=\"15\"></TD>';") ;
			out.println("m_hid_input      ='<INPUT TYPE=\"Hidden\" NAME=HID_USER_ID'+lineno+'	    VALUE=\"'+data_vec[i]+'\">';");
			out.println("m_hid_branch_code='<INPUT TYPE=\"Hidden\" NAME=TXT_BRANCH_CODE'+lineno+'	VALUE=\"'+data_vec[i+6]+'\">';");
			...
			//out.println("alert(\"data_vec[i+2]\"+ data_vec[i+2]);");
			//out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_TXT_APP_NO'+lineno+'	VALUE='+data_vec[i+1]+'>';");
			out.println("if(j>0 && j%2==1){");
			//       	out.println("<tr class=\"tr_input1\" >");
			out.println("m_writedata='<TR class=\"tr_input1\">'+m_row_num+m_branch_code+m_client_name+m_designation+m_target_cases+m_target_value+'</TR>'+m_hid_input+m_hid_branch_code+m_hid_client_name+m_hid_designatiom;"); 
			out.println("	}");
			out.println("	else{");
			//      	out.println("<tr class=\"tr_input\" >");
			out.println("m_writedata='<TR class=\"tr_input\">'+m_row_num+m_branch_code+m_client_name+m_designation+m_target_cases+m_target_value+'</TR>'+m_hid_input+m_hid_branch_code+m_hid_client_name+m_hid_designatiom;"); 
			out.println("	}");
			//   out.println("m_writedata='<TR>'+m_app_no+m_client_code+m_inq_no+m_fin_no+m_total_finance+m_cur_finance+m_officer+'</TR>'+m_hid_input;"); 
			...
			out.println("c_client=c_client+10;");
			out.println("target_cases=target_cases+parseInt(n_tar_cases);");
			out.println("target_values=target_values+parseInt(n_tar_vals);");
			out.println("target_cases_tmp=target_cases;");
			out.println("target_values_tmp=target_values;");
			out.println("lineno=lineno+1;");
			out.println("arr_size=arr_size+1;");		
			out.println("}"); //End while loop
			out.println("m_table.innerHTML+='<table align=\"center\" width=\"100%\" class=\"table\"><TR class=\"pdn_txtpos2\" align=\"center\">'+");
			out.println("'<td  width=\"60%\" align=\"left\" ><B>Total</td>'+");
			out.println("'<td  width=\"20%\" align=\"center\" name=TXT_TARGET_CASES_TOT ><B>'+target_cases+'</td>'+");
			out.println("'<td  width=\"20%\" align=\"center\" name=TXT_TARGET_VALS_TOT ><B>'+target_values+'</td>'+");		
			out.println("'</TR></table>';");
			out.println("m_row='<tr class=tr_input>'+");
			out.println("'<td width=\"5%\" align=\"right\"><input type=\"button\" name=\"end_b\"   value=\"Go to Top\" class=\"mainbut\" onclick=befor_end(\"GO_TOP\"); onMouseOver=load_roll_value(\"Top\"); onmouseout=load_roll_value(\"New\");></td>'+");
			out.println("'</tr>';");	
			...
			out.println("function val_total1(row_No){");
			out.println("target_cases_tmp = 0;");
			out.println("for(x=0;x<count-1;x++){");
			out.println("target_cases_tmp = target_cases_tmp + parseInt(document.Form1.elements[\"TXT_TARGET_CASES\"+x].value) ");
			out.println("}");
			out.println("m_table.innerHTML=\"\" ");
			...
			out.println("m_table.innerHTML+='<table align=\"center\" width=\"100%\" class=\"table\"><TR class=\"pdn_txtpos2\" align=\"center\">'+");
			out.println("'<td  width=\"60%\" align=\"left\" ><B>Total</td>'+");
			out.println("'<td  width=\"20%\" align=\"center\" name=TXT_TARGET_CASES_TOT ><B>'+target_cases_tmp+'</td>'+");
			out.println("'<td  width=\"20%\" align=\"center\" name=TXT_TARGET_VALS_TOT ><B>'+target_values_tmp+'</td>'+");		
			out.println("'</TR></table>';");
			
			
			out.println("}");
			
			out.println("function val_total2(row_No){");
			out.println("target_values_tmp = 0;");
			out.println("for(x=0;x<count-1;x++){");
			out.println("target_values_tmp = target_values_tmp + parseInt(document.Form1.elements[\"TXT_TARGET_VALUE\"+x].value) ");
			out.println("}");
			out.println("m_table.innerHTML=\"\" ");
			...
			//out.println(" alert(document.Form1.TXT_DESIGNATION0.value);");
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MAS_set_lending_targets';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\" Collection - Set Lending Targets - \"+m_val;"); 
			...
			out.println("help_box.innerHTML=\" Collection - Set Lending Targets - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
		...
		out.println("        '</tr>';"); */
			...
			out.println("        '<td width=\"30%\" ><input class=txt_input type=text name=TXT_LOCATION_CODE maxlength=25 size=15 onblur=show_data_by_finanace_no()>&nbsp;&nbsp;'+"); 
			...
			*/
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Collection - Set Lending Targets - New</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<td width='10%' >Year</td>"); 
			out.println("<td width='10%' ><select class='txt_input' type='text' name='TXT_YEAR' onChange=\"change_type()\" maxlength='1' size='1'>");  
