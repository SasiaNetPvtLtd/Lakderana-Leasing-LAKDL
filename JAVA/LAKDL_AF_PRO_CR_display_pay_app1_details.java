...
public class LAKDL_AF_PRO_CR_display_pay_app1_details extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
		  ...
		  nf.setMaximumFractionDigits(0);
			String m_client="";
			//out.println("m_status1"+m_status1);
			String m_sus_ref ="";
			String m_ref ="";
...
//------------------------------------------------------------------------------------------------------------------------------
		 ...
		 " NVL(TAX_INV_NO,'-'), NVL(TO_CHAR(TAX_INV_DATE,'DD-MM-YYYY'),'-') ,"+m_schema_name+".AF_CO_GET_BRANCH_NAME(LIC_BRANCH_CODE), NVL(TRIM(WHT),0),  NVL(TRIM(NET_AMOUNT),PAY_AMOUNT) "+	
      ...
      String m_tax_inv_date="";
			...
			out.println("<td width='8%' align=\"left\">Account No</td>");
			...
			out.println("<td width='8%' align=\"left\">Letter Date</td>");
     	...
     	out.println("<tr class=tr_input1 >");
		  ...
		  out.println("<tr >"); 
			...
			out.println("<HR>");	
			 ...
			 "nvl(decode(ENTRY_TYPE,'V','Vendor','S','Supplier','E','Seizer'),'-'),to_char(EFF_VALDATE,'dd-mm-yyyy'), "+
			 "NVL(LIC_ACC_NO,'-'),NVL(LIC_BRANCH_CODE,'-'),nvl(PAYEE_NAME,'-'),nvl(TO_CHAR(LETTER_DATE,'dd-mm-yyyy'),'-') "+ //modified by nuwan de silva handle the null values on 10-10-07
     ...
     // out.println("<table align='center' width='100%' class='table' border=\"0\">");  
      ...
      /*
        ...
        "                                 AND (FROM_SCREEN_NO <= (SELECT POSITION FROM "+m_schema_name+".CO_CO_MAS_USER_SCREEN  "+
				"	                                                    WHERE DIVISION_CODE='AF' AND SCREEN_NAME='AF_CR_PRO_PAYMENT_MAIN_APP_1' "+
        "                                                         )  "+
        ...
        "WHERE UPPER(APPLICATION_NO)=UPPER('"+m_application_no+"') "+
				...
				"AND STATUS!='Y' "+ // MODIFIED BY NUWAN DE SILVA ON 17-01-2008
...
//---------------
      ...
      out.println("<input type=\"hidden\" name=hid_asset_doc_no value="+j+">");
			...
			*/
			...
			"  SELECT DISTINCT C.DOCUMENT_TYPE ,A.DESCRIPTION D,NVL(C.REMARK,'-') REMARK, DECODE(C.STATUS,'Y','Yes','N','No','A','Applicable') STATUS,NEW_QUERY.FROM_SCREEN_NO,NEW_QUERY.TO_SCREEN_NO "+
