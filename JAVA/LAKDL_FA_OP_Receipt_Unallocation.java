...
public class LAKDL_FA_OP_Receipt_Unallocation extends javax.servlet.http.HttpServlet {
  ...
  public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
      ...
      out.println(" var m_amount_entered=0;");
       ...
       out.println("}");
        ...
        //out.println("   window.open(m_url);");
			  ...
			  out.println("   m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_OP_Receipt_Unallocation?chksql=get_return_receipt&m_receipt_no=\"+document.Form1.RECEIPT_NO.value+\"\";");
				out.println("  }");
        out.println("   makeRequest(m_url,'10','invoice_details');");
				...
				out.println("if(document.Form1.RECEIPT_NO.value!=\"\" ){");
			  out.println("   m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_OP_Receipt_Unallocation?chksql=get_deposite_details&m_receipt_no=\"+document.Form1.RECEIPT_NO.value+\"\";");
				out.println("  }");
        out.println("   makeRequest(m_url,'11','deposite_details');");
				...
				out.println("if(document.Form1.RECEIPT_NO.value!=\"\" ){");
			  out.println("   m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_OP_Receipt_Unallocation?chksql=get_retrealiz_details&m_receipt_no=\"+document.Form1.RECEIPT_NO.value+\"\";");
				out.println("  }");
        out.println("   makeRequest(m_url,'12','retandrealiz_details');");
			...
			out.println("		document.Form1.action='"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_OP_Receipt_Unallocation_Save?chksql=main_page';");  
			out.println("		document.Form1.submit();	"); 
			out.println("   }"); 
      out.println("}else{");
			out.println("		alert(\"Please enter the receipt number that you want to save\"); ");
			out.println("}");
			out.println("}");
      ...
      out.println("function befor_reset(){");
		  ...
		  out.println("window.location.href='"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_OP_Receipt_Unallocation?chksql=main_page'");
		  out.println(" }  ");
			out.println("}");
			...
			out.println("help_box.innerHTML=\" Collection - Receipts Unallocation - \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\" Collection - Receipts Unallocation - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
      ...
      out.println("}"); 
			...
			out.println("	"); 
			out.println("popupwin = window.showModalDialog('"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"FA_MAS_Help_Servlet?class_in="+m_client_name+"FA_OP_help_select&Sql_in='+Sql+'&Start_in='+Start+'&End_in='+End+'&Crit_In='+Crit+'&Hid_No='+Hid_No+'&Max_in='+Hid_No+'&Args=', oBj,\"dialogWidth:25em; dialogHeight:26em; center:yes; status:no\");");
			...
			out.println(""); 
			  ...
			  out.println("function receipt_help(){");
				...
				out.println("HelpBox('1','10','0',Crit,'m_help_DIV_TXT_RECEIPT_UNALLO_sql','2');");
				out.println("}");	
				...
				out.println(" get_invoice_settlement()"); 
				out.println("}");
      ...
      out.println("}");
	   ...
	   out.println(" function remv_inv_det(){ ");
	    ...
	    out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"FA_OP_Receipt_Unallocation?chksql=main_page';"); 
			out.println("}");
      ...
      out.println("}");
		  out.println("}");*/
			...
			//out.println("			if(document.Form1.TXT_ALLOCATION_MODE.value==\"INV_ALLO_ALL\"){");
			out.println("				m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_PRO_sql_invoice_settlement?chksql=LOAD_INVOICE_UNALLOCATION&RECEIPT_NO=\"+document.Form1.RECEIPT_NO.value;");	
		  ...
		  out.println("<BODY class='body & txt-body' leftmargin='0' topmargin='0' marginwidth='0' ONLOAD=\"load_roll_out_value(),get_sysdate()\">"); 
			...
			out.println("<input type='hidden' name='Hid_scr_name' value='FA_OP_Receipt_Unallocation' > ");
        ...
        out.println("<td>&nbsp;</td>");
				...
				out.println("<td width=\"30%\"><input name=\"RECEIPT_NO\" type=\"text\" maxlength=\"25\" class=\"txt_input\" onblur=\"makeRequest6(this.value)\"    > ");
				out.println("<input name=\"CLIENT_CODE\" type=\"hidden\"  value=\"\"  >");
				out.println("<input type=button name=rec_help value=Help class=\"but_input\" onclick=\"receipt_help()\"  ></td>");
				out.println("</td>");
				out.println("<td >&nbsp;</td>");
				out.println("<td>&nbsp; ");
				out.println("</td>");
        ...
        out.println("<td>&nbsp;</td>");
			...
			out.println("</html>"); 
				       	...
				       	rs1 = stmt1.executeQuery ("SELECT A.DIPOSIT_NO ,NVL(TO_CHAR(B.EFF_VALDATE,'DD-MM-YYYY'),'-') ,NVL(B.CHEQUE_NO,'-'),NVL(B.PAYER_ACC_NO,'-'),  "+
									                        "       NVL(LAKDL.AF_CO_GET_BANK_NAME(B.PAYER_BRANCH_CODE),'-') BANK_NAME, A.AMOUNT, NVL(C.BANK_CODE,'-') "+
							...
							String m_receipt_no    = req.getParameter("m_receipt_no");
  		 						...
  		 						rs1= stmt.executeQuery (" SELECT UPPER(REC_NO),"+m_schema_name+".AF_CO_GET_DEPOSIT_NO(REC_NO),TO_CHAR(BANK_DATE,'DD-MM-YYYY'),SETTLE_MODE,RECON_STATUS, "+
						                              " TO_CHAR(REALISED_DATE,'DD-MM-YYYY'),UPPER(NVL(CHEQUE_NO,'-')),REC_AMOUNT_CURR,UPPER(NVL(RETURN_CHARGE,'0')),UPPER(NVL(COMMENTS,' ')) "+
										                      ...
										                      " LAKDL.AF_CO_GET_DEPOSIT_NO(REC_NO) =DIPOSIT_NO (+) AND  "+
										                      " REC_NO IN "+
    									                    "(SELECT RECEIPT_NO "+
