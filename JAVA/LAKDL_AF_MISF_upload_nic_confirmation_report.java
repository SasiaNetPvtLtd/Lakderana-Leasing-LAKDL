...
public class LAKDL_AF_MISF_upload_nic_confirmation_report extends javax.servlet.http.HttpServlet { 
		...
		try { 
			...
			if(m_chksql.equals("main_page")){ 
				...
				try{
						...
						out.println(" Initial image count loading error "+ eee.toString());
					}
				...
				out.println("   } ");
				out.println("   else{ ");
				out.println("	  document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MISF_upload_nic_confirmation_report?chksql=save_document';");
				out.println("	  document.Form1.submit();");
				out.println("   }");
				out.println("}"); 	
			
			    out.println("function delete_window(){	"); 
				out.println("	  document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MISF_upload_nic_confirmation_report?chksql=delete_document';");
				out.println("	  document.Form1.submit();");
				out.println("}"); 	
				...
				out.println("<input  type='hidden' value='"+m_finance_no+"' name='TXT_FINANCE'> ");
				out.println("<input  type='hidden' value='"+m_nic+"' name='TXT_NIC'> ");
				out.println("<input  type='hidden' value='"+m_category+"' name='TXT_CATEGORY'> ");
				out.println("<input  type='hidden' value='"+m_category+"' name='TXT_DOCUMENT_NAME'> ");
				out.println("<input  type='hidden' value='"+m_serial_no+"' name='TXT_SERIAL_NO'> "); 
				...
				if(nic_count>0){
				    ...
				    out.println("<td width='10%'> <input type=\"button\" class='mainbut'   onClick='delete_window(\"HELP\")' value=\"Delete\"> </td>");
				}
				else{
					out.println("<td width='10%'> <input type=\"button\" class='mainbut'   onClick='save_window(\"HELP\")' value=\"Save\"> </td>");
				}
				...
				try {
					...
					while (it.hasNext()) {
						...
						if (isFormField) {
							...
							else if(fileItem.getFieldName().equals("TXT_NIC")){
								m_nic = fileItem.getString();
							}
							else if(fileItem.getFieldName().equals("TXT_CATEGORY")){
								m_category = fileItem.getString();
							}
							else if(fileItem.getFieldName().equals("TXT_SERIAL_NO")){
								m_serial_no = fileItem.getString();
							}
					...
					if(length<1048576){
								...
								callstmt = conn.prepareCall("BEGIN " + m_schema_name + ".AF_CONF_RPT_NIC_UPLOAD_SAVE(:1,:2,:3,:4,:5,:6,:7,:8,:9); END;");
								callstmt.setString(1,m_finance_no); 
								callstmt.setString(2,m_nic); 
								callstmt.setString(3,m_category);
								callstmt.setString(4,m_document_name);
								callstmt.setString(5,m_username);
								callstmt.setBlob(6,blob); 
								callstmt.setInt(7,Integer.parseInt(m_serial_no));
								callstmt.setString(8,extension); 
								callstmt.setString(9,m_new_file_name); // added by udara 20-10-2015
								callstmt.execute();
								...
								conn.commit();
											...
											Sql_data =	
													" SELECT A.DOCUMENT,A.SERIAL_NO, length(A.DOCUMENT), A.FILE_EXTENSION "+
													"  FROM	"+m_schema_name+".AF_CONFIRMATION_RPT_NIC_UPLOAD A "+
													"  WHERE  A.FINANCE_NO = '"+m_finance_no+"' "+
													"  AND    A.NIC = '"+m_nic+"' "+
													"  AND    A.SERIAL_NO = '"+m_serial_no+"' "+
											...
											if(rs.next()){
												...
												m_file_size = rs.getInt(3); // m_file_size = rs.getDouble(3);
								...
								out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MISF_upload_nic_confirmation_report?chksql=main_page';");
								out.println("window.close();");
								out.println("}</SCRIPT></HEAD>");
								out.println("<body onload='displaymsg();'></body>");
								out.println("</html>");
			...
			else if(m_chksql.equals("delete_document")){ 
				...
				try {
					...
					callstmt = conn.prepareCall("BEGIN " + m_schema_name + ".AF_CONF_RPT_NIC_UPLOAD_SAVE(:1,:2,:3,:4,:5,:6,:7); END;");
					callstmt.setString(1,m_finance_no); 
					callstmt.setString(2,m_nic); 
					callstmt.setString(3,m_category);
					callstmt.setString(4,m_document_name);
				...
				// added by udara 14-02-2018
				String Sql_data_n = "";
				int new_contract_count = 0;
				String document_code = "";
				
				
				// added by udara 19-02-2018
				...
				// end by udara 19-02-2018
				
				
				if (new_contract_status=="YES") {
					...
					if(m_category.equals("GUARANTER")){
						...
						Sql_data_n = " "+
							  " SELECT B.DOC_ID "+
				              " FROM "+m_schema_name+".AF_CO_CONFIRMATION_UPLOAD_MAP A, "+m_schema_name+".AF_MK_DOC_UPLOAD_LOG B "+
				              " WHERE A.DOC_ID = B.DOC_TYPE_ID "+
				              " AND B.FINANCE_NO = '"+m_finance_no+"' "+
				              " AND A.CONFIRMATION_ID = '"+m_category+"' "+
				              " AND B.DOC_NUM = '"+m_serial_no+"' ";
					...
					rs=stmt.executeQuery(Sql_data_n);
					
					if(rs.next()){
							document_code = rs.getString(1);
					}
					...
					out.println("window.location.href='"+m_class_url+"/LAKDL_AF_MK_Document_Upload_New_View?chksql=view_document&snap_view_status=Y&doc_id="+document_code+"';");
					
					out.println("}</SCRIPT></HEAD>");
					out.println("<body onload='load_screen();'></body>");
					out.println("</html>");
								...
								try{
										...
										if(nic_count==0){
											...
											//out.println("uv check 5");
												...
												out.println("m_url='"+m_class_url+"/"+m_fschema_name+"AF_MISF_upload_nic_confirmation_report?chksql=get_document&category='+m_category+'&finance_no='+m_finance_no+'&nic='+m_nic+'&serial_no='+m_serial_no;"); 
