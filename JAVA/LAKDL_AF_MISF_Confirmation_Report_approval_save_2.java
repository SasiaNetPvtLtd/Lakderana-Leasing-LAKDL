...
public class LAKDL_AF_MISF_Confirmation_Report_approval_save_2 extends HttpServlet {
	...
	public synchronized void  service(HttpServletRequest req, HttpServletResponse res)	throws IOException{
		...
		try {
			...
			String m_chk_type_2 = "off";
				callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_SAVE_CONFIRMATION_RPT_APP_3(:1,:2,:3,:4); END;");
				...
				for(int k=0;k<row_count;k++){
					...
					if(m_chk_type.equals("on")){
						...
						callstmt.setString(1,m_sn_methods.met_formdata(reqstr,"HID_APP_NO_"+(Integer.toString(k))));
						callstmt.setString(2,m_username);
						callstmt.setString(3,"DISAPPROVE2");
						callstmt.setString(4,m_sn_methods.met_formdata(reqstr,"APPROVE_COMMENT_"+(Integer.toString(k))));
						callstmt.execute();
			...
			out.println("window.location.href='"+m_url+"/LAKDL_AF_MISF_Confirmation_Report_approval_2?chksql=main_page';"); // reload // href
