...
public class LAKDL_AF_MIS_second_level_report extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			out.println("		 m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MIS_second_level_report?chksql=load_second_level&type=\"+document.Form1.TXT_SEARCH_TYPE.value+\"&from_date=\"+m_from_date+\"&to_date=\"+m_to_date+\"\";");
	  	...
	  	out.println("					}"); 
			...
			out.println("<tr >"); 
			out.println("<td width='10%' ><DIV id='DIV_TYPE'  class=div_input>Type</b></DIV></td>"); 
			out.println("<td width='20%' ><select name='TXT_SEARCH_TYPE' class='txt_input'>");
			out.println("<option value=\"INIT_TYPE\" SELECTED>Initiation type</option>");
			out.println("<option value=\"CUS_CAT\"           >Customer Category</option>");
			out.println("<option value=\"BROKER\"            >Broker</option>");
			out.println("<option value=\"MK_OFFICER\"        >MK Officer</option>");
			...
			out.println("<tr >"); 
			out.println("<td width='10%' ><DIV id='DIV_TXT_REAL_DATE'  class=div_input>From Date</b></DIV></td>"); 
			...
			out.println("</td> ");
			out.println("<td width='10%' ><DIV id='DIV_TXT_REAL_DATE'  class=div_input>To Date</b></DIV></td>"); 
			...
			out.println("<input class=\"txt_input5\" type=\"text\" name=TXT_TO_DATE_YY maxlength=\"4\" size=\"4\" ></td>");	
			out.println("<TD WIDTH=\"20%\"><input class='but_input' type='button' style='{width:150;}'  name='BUT_HELP_MAIN_1' value=\"Second Level Report\" onClick=\"makeRequest_detail()\"></td> ");
			out.println("<td width='*%' >&nbsp;</td>"); 
			out.println("</table>");  
        ...
        "  NVL((SELECT SUM(CAPITAL_AMOUNT+INTEREST_AMOUNT) FROM "+m_schema_name+".AF_MK_PRO_PRICING_INSTALLMENT WHERE PRICING_NO=B.PRICING_NO ),0),"+//4
			...
			"	A.MOD_USER='"+m_username+"' AND A.APPLICATION_STATUS='VERIFY1'  AND A.APPLICATION_NO=B.APPLICATION_NO ");
				  ...
				  if(more_app_very){
						...
						out.println("<td width='10%' ><DIV class=div_input align='right' ><b>R/T ratio - current </b></DIV></td>"); 
        ...
        "  NVL((SELECT SUM(CAPITAL_AMOUNT+INTEREST_AMOUNT) FROM "+m_schema_name+".AF_MK_PRO_PRICING_INSTALLMENT WHERE PRICING_NO=B.PRICING_NO ),0),"+//4
			...
			//----------------   Pending Purchase Orders  -------------------------------------------------------------------------------------------------
              ...
              "  UPPER(H.PURCHASE_ORDER_NO)=UPPER(X.PURCHASE_ORDER_NO)),'-'), "+ 
						  //"  NVL(TOTAL_VAT,0), "+//6
						  "  ACTIVE_STATUS "+//7
       ...
       "   PURCHASE_ORDER_NO NOT IN ( "+
				  ...
				  if(more_pur1){
						...
						out.println("<td width='20%' ><DIV class=div_input ><b>Vendor</b></DIV></td>"); 
						out.println("<td width='12%' align='right' colspan=2><DIV class=div_input ><b>Total Net Amount</b></DIV></td>"); 
						out.println("<td width='12%' colspan=2><DIV class=div_input ><b>Item Sub Cat.</b></DIV></td>"); 
					...
					while(more_pur1){
							...
							out.println("<td width='12%' class=div_input align='right' colspan=2 >"+nf.format(rs1.getDouble(5))+"</td>");
							out.println("<td width='12%' class=div_input colspan=2 >"+rs1.getString(6)+"</td>");
      ...
      " TO_DATE(TO_CHAR(A.DUE_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
			...
			"	(C.MK_OFFICER='"+m_username+"' OR B.COLLECTION_OFFICER='"+m_username+"') ");
				...
				else if(m_chksql.equals("load_second_level")){
