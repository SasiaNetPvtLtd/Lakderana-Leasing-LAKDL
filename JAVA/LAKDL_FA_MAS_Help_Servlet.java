...
public class LAKDL_FA_MAS_Help_Servlet extends HttpServlet {
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res) throws IOException{
		...
		try {
			...
			String bg_color1=m_sn_methods.bg_color1;
			String bg_color2=m_sn_methods.bg_color2;
			String fo_color1=m_sn_methods.fo_color1;
			String fo_color2=m_sn_methods.fo_color2;
			...
			out.println(" m_col1.style.background='" + bg_color2 + "';");
			out.println(" m_col1.style.color='" + fo_color2 + "';");
			out.println("}");
			...
			out.println(" m_col1.style.color='" + fo_color1 + "';");
			out.println(" m_col1.style.background='" + bg_color1 + "';");
			out.println("}");
			...
			if (more) {
				out.println("<TABLE WIDTH='100%' class='pdn_table2' border='0'>");
				...
				out.println(" NewArr = new Array(20);");
				out.println(" for(i=1;i<=20;i++){");
				...
				while (more) {
					...
					if (i<21) {
						...
						for(int j=1;j<=count-Hidden_Val;j++){	
							out.println("<TD onclick=\" Selected(NewArr,'"+i+"','0')\" >"+rs.getString(j)+"</TD>");
						}
						out.println("</TR>");
					}
					i=i+1;
				...
				rs.close();
				if(i<=10){
					out.println("<TABLE width='100%'  class=pdn_table2>");	
					for(int k = i; k<=10;k++) {
				...
				if(m_first_record){
					out.println("<INPUT TYPE ='Button' NAME ='Prev_Val' VALUE ='Prev 10' style='{cursor:hand; color: black; font: bold 8pt arial; border-color: silver; background: silver;}' DISABLED ></TD><TD>");	
				}
				else{
					out.println("<INPUT TYPE ='Button' NAME ='Prev_Val' VALUE ='Prev 10' style='{cursor:hand; color: black; font: bold 8pt arial; border-color: silver; background: silver;}' OnClick='Prev();'></TD><TD>");	
				}		
				
				if(i<=11){
					out.println("<INPUT TYPE ='Button' NAME ='Next_Val' VALUE ='Next 10' style='{cursor:hand; color: black; font: bold 8pt arial; border-color: silver; background: silver;}' DISABLED ></TD><TD>");
				}
				else{   
					out.println("<INPUT TYPE ='Button' NAME ='Next_Val' VALUE ='Next 10' style='{cursor:hand; color: black; font: bold 8pt arial; border-color: silver; background: silver;}'  OnClick='Next();' ></TD><TD>");
				}
				
				out.println("<INPUT TYPE ='Button' NAME ='Exit'  VALUE ='   Exit   ' style='{cursor:hand; color: black; font: bold 8pt arial; border-color: silver; background: silver;}' OnClick='Close();'></TD><TD align='right'></TD><TD width='20%'></TD></TR>");
				out.println("</TABLE>");		
			}
			else{
