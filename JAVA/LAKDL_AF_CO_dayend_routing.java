...
public class LAKDL_AF_CO_dayend_routing extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_chksql.equals("main_page")){
				...
				out.println("<HTML>"); 
				out.println("<HEAD>"); 
				out.println("<TITLE> Maintenance - Day End Routine </TITLE>");//Modified by Dineth on 29-04-2009 changed routing to routine 
				out.println("</HEAD>"); 
				...
				out.println("		}");
				out.println("}");	
				//Added By Kanishka Dilshan On 30-12-2014
				String m_check = "";
				rs = stmt.executeQuery( " SELECT COUNT(*) FROM "+m_schema_name+".AF_CO_MAS_SYS_PARAMETER A, "+m_schema_name+".AF_CO_PRO_INVOICE_DATE B "+
										" WHERE TRUNC((A.DAY_END_DATE+1),'DD') BETWEEN TRUNC(B.FROM_DATE,'DD') AND TRUNC(B.TO_DATE,'DD') ");
				if(rs.next()){
					m_check = rs.getString(1);
				}
				//End By Kanishka Dilshan On 30-12-2014
				
				out.println("function run_dayend() {");
				out.println("         	if(parseInt("+m_check+") > 0 ){");//Added By Kanishka Dilshan On 30-12-2014
				out.println(" 				document.Form1.TXT_PRO_DATE_DD.disabled=false; ");
				out.println(" 				document.Form1.TXT_PRO_DATE_MM.disabled=false;");
				out.println(" 				document.Form1.TXT_PRO_DATE_YY.disabled=false; ");
				out.println("				document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_CO_dayend_routing_save';");  
				out.println("				document.Form1.submit();	"); 
				out.println("			}else{ alert('Cannot Run Day End.. Please Run Invoice Generation for next month and process the Day End.'); }"); 
				out.println("		}"); 	
			
				
				out.println("function clear_window(){	"); 
				out.println("		if(confirm(\"Are you sure you want to clear the screen ?\")){ "); 
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CO_dayend_routing?chksql=main_page';"); 
				out.println("		}"); 
				out.println("}"); 			
				...
				out.println("	help_box.innerHTML=\" Maintenance - Day End Routine - \"+m_val;");//Modified by Dineth on 29-04-2009 changed routing to routine
				out.println("}"); 
				
				out.println("function load_roll_out_value(){");
				out.println("	help_box.innerHTML=\" Maintenance - Day End Routine \";");//Modified by Dineth on 29-04-2009 changed routing to routine
				out.println("}"); 
			...
			out.println("function MyDialog(){"); 
				...
				*/
				...
				rs = stmt.executeQuery("SELECT TO_CHAR(DAY_END_DATE,'DD-MM-YYYY'),TO_CHAR(DAY_END_DATE+1,'DD-MM-YYYY') FROM "+m_schema_name+".AF_CO_MAS_SYS_PARAMETER ");
				if(rs.next()){
					m_eff_date  = rs.getString(1).substring(0,2);
					m_eff_month = rs.getString(1).substring(3,5);
					m_eff_year  = rs.getString(1).substring(6,10);
					m_pro_date  = rs.getString(2).substring(0,2);
					m_pro_month = rs.getString(2).substring(3,5);
					m_pro_year  = rs.getString(2).substring(6,10);   
				}
				out.println("document.Form1.TXT_EFF_DATE_DD.value=\""+m_eff_date+"\";");
				out.println("document.Form1.TXT_EFF_DATE_MM.value=\""+m_eff_month+"\";");
				out.println("document.Form1.TXT_EFF_DATE_YY.value=\""+m_eff_year+"\";");
				out.println("document.Form1.TXT_PRO_DATE_DD.value=\""+m_pro_date+"\";");
				out.println("document.Form1.TXT_PRO_DATE_MM.value=\""+m_pro_month+"\";");
				out.println("document.Form1.TXT_PRO_DATE_YY.value=\""+m_pro_year+"\";");
				out.println("}");
				
				
				out.println("</script>"); 
				out.println("<BODY class='body & txt-body' leftmargin='0' topmargin='0' marginwidth='0' ONLOAD=\"get_system_date(),onLoad_eff_date();\">"); 
				...
				out.println("</tr>"); 
				out.println("<tr>"); 
				out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'> Maintenance - Day End Routine </td>"); //Modified by Dineth on 29-04-2009 changed routing to routine 
				out.println("</tr>"); 
				out.println("<tr>"); 
