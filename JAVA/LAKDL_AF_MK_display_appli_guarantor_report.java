//--
//SCREEN NAME	:Marketing - Application Guarantor report
...
public class LAKDL_AF_MK_display_appli_guarantor_report extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
		 ...
		 if(m_chksql.equals("main_page")){
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Marketing -  Application Guarantor report</TITLE>"); 
			out.println("</HEAD>"); 
			out.println("<link REL='STYLESHEET' HREF='"+m_html_client_url+"/css/Asset_Financing_System.css' TYPE=\"text/css\">"); 
			out.println("<SCRIPT language=\"JavaScript\">"); 

			out.println("function get_vector(data_vec) {");
			out.println("	if(data_vec.length==0   && document.Form1.TXT_FINANCE_NO.value!=\"\"  &&  document.Form1.hid_assig.value=='J2'){");
			...
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_display_appli_guarantor_report?chksql=lead_details&to_dd=\"+to_date+\"&from_dd=\"+from_date+\"&vehicle_no=&finance_no=\"+document.Form1.TXT_FINANCE_NO.value;");
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MK_display_appli_guarantor_report?chksql=main_page';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\"Marketing -   Application Guarantor report - \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\"Marketing -   Application Guarantor report - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
		  ...
		  out.println("	}");
			...
			out.println("    m_criteria = document.Form1.TXT_REG_NO.value+\"@\"+document.Form1.TXT_FINANCE_NO.value+\"@\"+\"Y@\";"); 
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Marketing -  Application Guarantor report</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<tr>"); 
			out.println("<td width='30%' ><DIV id='DIV_TXT_FINANCE_NO' class=div_input>Application No</DIV></td>"); 
			out.println("<td width='30%' ><input class='txt_input'  type='text' name='TXT_FINANCE_NO' value=\"\" maxlength='15' size='15' onblur=\"assig('J2'),check_finance(document.Form1.TXT_FINANCE_NO)\">"); 
	...
	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				...
				String m_reg = req.getParameter("vehicle_no");	
				String m_from_date = req.getParameter("from_dd");	
				String m_to_date = req.getParameter("to_dd");	


				rs2= stmt2.executeQuery ("SELECT count(*) "+
				"FROM "+m_schema_name+".AF_CO_PRO_APP_BANK_GUARANTEES "+	
				"WHERE UPPER(APPLICATION_NO) like UPPER('%"+m_finance+"%')  "+
				"or (TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
				"AND TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') ) "+
				"AND STATUS='Y' ");

				boolean more2=rs2.next();
				if(rs2.getInt(1)!=0){

				rs= stmt.executeQuery ("SELECT DISTINCT APPLICATION_NO "+
				"FROM "+m_schema_name+".AF_CO_PRO_APP_BANK_GUARANTEES "+	
				"WHERE UPPER(APPLICATION_NO) like UPPER('%"+m_finance+"%')  "+
				"or (TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
				"AND TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') ) "+
				...
				out.println("<html><head><font 12pt arial><title>Marketing -  Application Guarantor report</title></head>");
				...
				out.println("<div align='center' width='180%' class='rep-body' style='{font: bold;text-align:center;}'>Application Bank Guarantee Report</div>");
				out.println("</table>");			
		    ...
		    out.println("<HR width='100%'>");
				...
				rs1= stmt1.executeQuery("SELECT DISTINCT ISSUER_CODE,TO_CHAR(AMOUNT,'999,999,999,999,999.99'),TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),TO_CHAR(START_DATE,'DD-MM-YYYY'), "+
				"TO_CHAR(END_DATE,'DD-MM-YYYY'),STATUS "+
				"FROM "+m_schema_name+".AF_CO_PRO_APP_BANK_GUARANTEES "+	
				"WHERE UPPER(APPLICATION_NO) like UPPER('%"+rs.getString(1)+"%')  "+
				"or (TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
				"AND TO_DATE(TO_CHAR(ISSUED_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') ) "+
				"AND STATUS='Y' ");
				...
				out.println("<table align=\"center\" width=\"100%\" border=\"0\" class=\"table\">");
				out.println("<tr><td width='20%'><b>Application No</td>");
				out.println("<td width=\"20%\" ><b>:  "+rs.getString(1)+"</td>");
				out.println("<td width=\"*%\" >&nbsp</td></tr>");
				out.println("<tr>"); 
				out.println("</tr>");
				out.println("<tr>"); 
				out.println("</tr>");
			  ...
			  out.println("<tr class=tr_input1 >");
				...
				out.println("<td width=\"20%\" >"+rs1.getString(4)+"</td>");
				out.println("<td width=\"20%\" >"+rs1.getString(5)+"</td></tr>");
				...
				out.println("<tr><td><div align='center' width='180%' class='rep-body' style='{font: bold;text-align:center;}'>No Records</div></td></tr>");
				out.println("</table>");			
				out.println("<form name='form1'>");
				out.println("<table border='0' width='100%' bgcolor='white' style='{color: black; font: bold 10px;}'>");
				out.println("</table>");			
