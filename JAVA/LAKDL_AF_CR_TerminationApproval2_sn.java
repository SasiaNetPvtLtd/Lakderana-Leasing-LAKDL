...
public class LAKDL_AF_CR_TerminationApproval2_sn extends javax.servlet.http.HttpServlet 
{
	Connection conn;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("		document.Form1.action=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_TerminationSaveAppr2_sn?status1="+m_st+"\";");  
				...
				out.println("window.location.href='"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_TerminationApproval2_sn?chksql=main_page'");
				out.println(" }  ");
				out.println("}");
				...
				//out.println("m_url=\""+m_class_url+"/"+m_schema_name+"_AF_CR_TerminationCalculationReport?chksql=main_page&FINANCE_NO=\"+finance_no+\"&TERMINATION_TYPE=\"+termination_type+\"&TERMINATION_NO=\"+termination_no+\"\";");  //commented by kanishka dilshan on 10-12-2013
				out.println("m_url=\""+m_class_url+"/"+m_schema_name+"_AF_CR_TerminationCalculationReport_N?chksql=main_page&FINANCE_NO=\"+finance_no+\"&TERMINATION_TYPE=\"+termination_type+\"&TERMINATION_NO=\"+termination_no+\"\";");  //Added by kanishka dilshan on 10-12-2013
				...
				out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_TerminationApproval2_sn?chksql=main_page&status=DELETE&hstatus=Delete';"); 
				
				out.println("}"); 
				out.println("function load_new_page(){");
				out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_TerminationApproval2_sn?chksql=main_page';"); 
				out.println("}"); 
				...
				out.println("else{"); 
				out.println("  change(row_num); "); 
				out.println("}"); 
				// end by udara 11-02-2019
				...
				if(m_st.trim().equals("NEW")){
					rs = stmt.executeQuery (" SELECT A.TERMINATION_NO, A.FINANCE_NO, "+
						...
						" ,"+m_schema_name+".AF_CR_GET_IS_DATE_VALID(A.TERMINATION_NO) IS_VALID "+ // added by udara 16-10-2018
				...
				out.println("<td  width='8%' align=right>Net Bal Amount</td>");
				//if(m_st.trim().equals("NEW")){
	...
	out.println("<td align=right>"+nf.format(rs.getDouble(6))+"<input type=hidden name=\"BAL_AM_"+j+"\" value=\""+(rs.getString(6))+"\"></td>");*/
					...
					out.println("<td align=right>"+nf.format((rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(23)+rs.getDouble(12)-rs.getDouble(24)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC")))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>");
					...
					out.println("<td align=center><SELECT style=\"width:80px\" class=\"txt_input\" NAME=\"drp_appro_"+j+"\"><option value=\"APPROVE\" selected>Approve</option>");
					...
					//out.println("<td align=center><INPUT TYPE=\"checkbox\" NAME=\"chk_app_"+j+"\" value=\"N\" onClick=\"change("+j+")\"></td>"); // commented by udara 16-10-2018
