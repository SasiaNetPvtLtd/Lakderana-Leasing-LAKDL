...
public class LAKDL_AF_RE_rpt_age_analysis_detail extends javax.servlet.http.HttpServlet {
  ...
  public ResultSet rs1;
	...
	/*		if(m_date_category.equals("DUE_DATE")){
			...
			m_date = "A.TOLARENCE_END_DATE";
			}
			else if(m_date_category.equals("REFACT_DATE")){
			m_date = "A.REFACT_TOLARENCE_END_DATE";
			}
			else{
			m_date = "A.INVOICE_DATE";
			}	
			*/	
			...
			/*s1= stmt1.executeQuery("SELECT  A.DEBTOR_CODE,"+//1
    		...
    		"  NVL(A.BALANCE_AMOUNT,0)"+//9
 				...
 				"  WHERE A.BATCH_NO =B.BATCH_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.INVOICE_STATUS='CONF' "+
				"  AND ((TO_DATE("+m_date+") - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<-7 ) AND A.BALANCE_AMOUNT<>0 	");
				*/
						...
						rs1=stmt1.executeQuery(" SELECT "+ 
							...
							" A.INVOICE_NO, "+
							" TO_CHAR(B.DUE_DATE,'DD-MM-YYYY') DUE_DATE ,"+
							" A.INVOICED_AMOUNT, "+
							" A.SETTELED_AMOUNT, "+
							" A.BALANCE, "+
							" B.FINANCE_NO, "+
							...
							"  SUM(SETTELED_AMOUNT) SETTELED_AMOUNT , "+
							...
							" UPPER(B.FINANCE_NO)=UPPER('"+m_facility_no+"')  AND "+
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
						  
							" (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<-7  AND "+
							" A.BALANCE <> 0 " +
			 ...
			 if(m_date_range.equals("7m")) { 
					...
					rs1=stmt1.executeQuery(" SELECT "+ 
							...
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
							//" (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=-7  AND "+
							" ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<0 AND (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>=-7 ) AND "+
							" A.BALANCE <> 0 " +
							" ORDER BY B.CLIENT_CODE,B.FINANCE_NO ");
			 ...
			 if(m_date_range.equals("30")) { 
					...
					rs1=stmt1.executeQuery(" SELECT "+ 
						" B.CLIENT_CODE, "+
							...
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
			//				" ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>7 ) AND "+
						  " ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>7 AND (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=30 ) AND "+
			 ...
			 if(m_date_range.equals("60")) { 
					...
					rs1=stmt1.executeQuery(" SELECT "+ 
							...
							" UPPER(B.FINANCE_NO)=UPPER('"+m_facility_no+"')  AND "+
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
							//" ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>30 ) AND "+
						  " ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>30 AND (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=60 ) AND "+
  						" A.BALANCE <> 0 " +
							" ORDER BY B.CLIENT_CODE,B.FINANCE_NO ");
			 ...
			 if(m_date_range.equals("90")) { 
					...
					rs1=stmt1.executeQuery(" SELECT "+ 
							...
							" UPPER(B.FINANCE_NO)=UPPER('"+m_facility_no+"')  AND "+
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
						//	" ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>60 ) AND "+
						  " ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>60 AND (TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=90 ) AND "+
    					" A.BALANCE <> 0 " +
							" ORDER BY B.CLIENT_CODE,B.FINANCE_NO ");
			 ...
			 if(m_date_range.equals("99")) { 
						...
						rs1=stmt1.executeQuery(" SELECT "+ 
							...
							" UPPER(B.FINANCE_NO)=UPPER('"+m_facility_no+"')  AND "+
							" UPPER(B.CLIENT_CODE)=UPPER('"+m_client_code+"') AND "+
							" ((TO_DATE(B.DUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>90 ) AND "+
							" A.BALANCE <> 0 " +
							" ORDER BY B.CLIENT_CODE,B.FINANCE_NO ");
			 }		
         ...
         boolean mflag=true;							
			   boolean more = rs1.next();
				//	out.println("more"+more);
					 ...
					 out.println("<TR><TD align='Center' ><B> Invoice Details for Client Code - "+m_client_code+"  Client Name - "+m_client_name+" Finance No - "+m_facility_no+" </B></TD></TR>");
					 out.println("</TABLE>");
				  ...
				  if(more){
					...
					while(more){
							...
							out.println("<td width='10%' class=div_input onClick=\"show_invoice_drill('"+rs1.getString(3)+"')\" style='cursor:hand'><u>"+rs1.getString(3)+"</u></td>");
