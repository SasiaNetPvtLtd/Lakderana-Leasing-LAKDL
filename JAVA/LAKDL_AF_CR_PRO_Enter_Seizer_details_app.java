...
public class LAKDL_AF_CR_PRO_Enter_Seizer_details_app extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_chksql.equals("main_page")){
					...
					out.println("      m_url=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Seizer_details_app?chksql=details&status=\"+document.Form1.hid_status.value+\"&APPLICATION_NO=\"+document.Form1.TXT_APP_NO.value;"); 
					...
					out.println("			window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Seizer_details_app?chksql=main_page';"); 
					out.println("		}"); 
					out.println("}"); 
				  ...
				  out.println("count=count+1;");
					...
					//out.println("		if(validate_data()){");	
					out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Enter_Seizer_details_app_save';");  
				...
				out.println("}");	
					...
					rs=stmt.executeQuery(" "+ 								
									" SELECT "+
										" A.APPLICATION_NO, "+ // 1
										" A.SEIZER_CODE, "+    // 2
										" "+m_schema_name+".AF_CO_GET_SEIZER_NAME(A.SEIZER_CODE), "+ // 3
										" (SELECT ADDRESS1 FROM "+m_schema_name+".AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) || ', ' || (SELECT ADDRESS2 FROM "+m_schema_name+".AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) ADDRESS, "+ // 4
										" (SELECT TEL_NO FROM "+m_schema_name+".AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) TEL_NO, "+ // 5
										" (SELECT MOBILE_NO FROM "+m_schema_name+".AF_CO_MAS_SEIZER WHERE SEIZER_CODE = A.SEIZER_CODE) MOBILE_NO "+ // 6
										  " FROM "+m_schema_name+".AF_CO_MAS_SEIZER_DETAILS A "+
										  " WHERE (A.APPLICATION_NO LIKE '%"+m_app_no+"%' OR A.SEIZER_CODE LIKE '%"+m_app_no+"%' OR UPPER("+m_schema_name+".AF_CO_GET_SEIZER_NAME(A.SEIZER_CODE)) LIKE UPPER('%"+m_app_no+"%'))   "+
				...
				out.println("<td  width='10%' > <input type='hidden' name='HID_SEIZER_NO_"+j+"' id='HID_SEIZER_NO_"+j+"' value ='"+rs.getString(2)+"' > "+rs.getString(2)+" </td>"); 
