...
public class LAKDL_AF_CR_save_delete_guarantor extends HttpServlet {
	...
	synchronized(this){ 
      ...
      String m_scr_name=(String)m_sn_methods.met_formdata(reqstr,"Hid_scr_name"); 
			...
			m_num=Integer.parseInt(m_scr_num);
			 ...
			 for(int i=1;i<=m_num;i++){
				   ...
				   if(m_status.equals("Y")){
							...
							callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_SAVE_APPLI_GUARAN_DEL(:1,:2,:3,:4,:5); END;");						
							callstmt.setString(1,(m_sn_methods.met_formdata(reqstr,"TXT_APPLICATION_NO")).trim());
							callstmt.setString(2,m_sn_methods.met_formdata(reqstr,"TXT_GUARANTOR_CODE_"+i).trim());
							callstmt.setString(3,m_sn_methods.met_formdata(reqstr,"SCREEN_NAME"));
							callstmt.setString(4,m_username);			   
							callstmt.setInt(5,m_num);			   
							callstmt.execute();
	   	...
	   	out.println("<HTML><HEAD>");
			...
			out.println("window.location.href='"+m_url+"/"+m_client_name+"AF_CR_delete_guarantor?chksql=main_page';");
