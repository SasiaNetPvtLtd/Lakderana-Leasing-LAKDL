...
public class LAKDL_AF_CR_TerminationSchedule extends javax.servlet.http.HttpServlet {
	...
	public /*synchronized*/ void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("  }");
				*/
				out.println("		if(validate_data()){"); //Added by Kanchana on 2016-08-26
				out.println("      save_check(data_vec);"); // added by udara on 11-02-2014
				out.println(" } ");//Added by Kanchana on 2016-08-26
				out.println(" } ");
			  ...
			  out.println("	T33.innerHTML=format_noobject(parseFloat(unformat_noobject(document.Form1.ODI_NET.value))+parseFloat(unformat_noobject(document.Form1.T_S.value))+parseFloat(unformat_noobject(document.Form1.h_gtv.value))  + parseFloat(unformat_noobject(document.Form1.hid_sg_val.value))-parseFloat(unformat_noobject(document.Form1.Unallo_Rec.value)));");
				...
				*/
				...
				//out.println("         alert('calculated from assign div'); "); // added by udara 29-01-2019
        ...
        out.println("}");
				*/
				...
				out.println("   m_url=\"" + m_servlet_client_url + ":" + m_client_t3_port + "/" + m_client_name + "AF_CR_TerminationSchedule?chksql=getMoratoriumValues&Lease_no=\"+document.Form1.LEASE_NO.value;");
				...
				*/
				...
				//Added by Kanchana on 2016-05-31   
				...
				/*		out.println("function befor_submit(){ "); 
						...
						out.println("} "); 
						*/
				//modified by kanchana on 2016-08-26 validate_data removed from here and moved to upper step. to avoid anabling fields which are already disabled after error mesage displayed.
				out.println("function befor_submit(){ "); 
				out.println("for (var i=0; i < document.Form1.elements.length; i++ ) {");
				out.println("document.Form1.elements[i].disabled=false;");
				out.println("}");
				//out.println("		if(validate_data()){"); //commented by kanchana on 2016-08-26
				out.println(" if(confirm(\"Are you sure, Termination Type is correct?\")){  "); //Start -- Added By Sandun on 26-11-2008
				out.println("		if(confirm(\"Are you sure you want to save?\")){ "); 
				out.println("m_v_no='';");
				out.println("m_s_va='';");
				...
				out.println("		}"); 
				out.println("		}"); //End
				//out.println("		}");  //commented by kanchana on 2016-08-26
				out.println("} "); 
				//Endded by Kanchana on 2016-08-26
				...
				*/
				...
				out.println("	else{	"); 
				//out.println("	 alert('test'); "); 
				out.println("      document.Form1.LEASE_NO.value = ''; ");
				out.println("	}	"); 
				...
				out.println(" document.Form1.TER_RATE.focus();");
				...
				/*Added By Ns on 22-10-2013*/
				out.println(" function check_term_type() { ");
				...
				//out.println(" 			m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_Termination_calculation_rpt?chksql=CALCULATION_REPORT&termi_date=\"+document.Form1.TER_DAY.value+\"-\"+document.Form1.TER_MONTH.value+\"-\"+document.Form1.TER_YEAR.value+\"&finance_no=\"+val+\"&closing_rate=\"+m_closing_rate;");  
				out.println(" 			m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_Termination_calculation_rpt?chksql=CALCULATION_REPORT&termi_date=\"+document.Form1.TER_DAY.value+\"-\"+document.Form1.TER_MONTH.value+\"-\"+document.Form1.TER_YEAR.value+\"&finance_no=\"+val+\"&closing_rate=\"+m_closing_rate+\"&odi_net=\"+m_odi_net;");
				out.println(" 			window.open(m_url,'displayWindow4','left=50,top=60,width=800,height=500,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
				out.println("   	}");
				out.println("   	else{");
				out.println("   		alert('Please select a vehicle'); ");
				...
				String m_ter_type="<SELECT name=\"TERM_TYPE\" class=\"txt_input\" onChange=\"check_term_type()\" >";
				...
				out.println("<td><input name=\"TER_DAY\"   type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=\"check_sysdate(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);\"onchange=check_Date(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);check_lease();> ");
				out.println("    <input name=\"TER_MONTH\" type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=\"check_sysdate(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);\"onchange=check_Date(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);check_lease();> ");
				out.println("    <input name=\"TER_YEAR\"  type=\"text\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onblur=\"check_sysdate(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);\"onchange=check_Date(document.Form1.TER_DAY,document.Form1.TER_MONTH,document.Form1.TER_YEAR);check_lease();><a href style='{cursor:hand; }' onclick=load_calendar('2')>   Calendar</a> ");
				out.println("</td>");
				...
				out.println("<td><input name=\"ODI_ADJ\" type=\"text\" maxlength=\"25\" class=\"txt_input\" onblur=\"check_number_decimal(this,21)\" onchange=\"check_ODI_ADJ()\" STYLE=\"{text-align:right;}\" VALUE='0.00' > "); //disabled removed by Kanchana for the issue no #20149
				out.println("</td>");
				//out.println("<input name=\"LEAD_SOURCE_CATEGORY\" type=\"text\" maxlength=\"10\" id=\"txtAddress1\" class=\"txt_input\" style=\"width:150px;\" >");
				out.println("</tr>");
