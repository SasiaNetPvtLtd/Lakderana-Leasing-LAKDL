...
public class LAKDL_AF_MISF_Contract_Report extends javax.servlet.http.HttpServlet {
 ...
 // public ResultSet rs1,rs_doc_charge;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)	throws IOException	{
      ...
      stmt_invoice=conn.createStatement();
		...
		/*	String Sql_updated_query="SELECT  "+			
			"	  B.NET_AMOUNT,B.VAT_AMOUNT,B.GROSS_AMOUNT,C.START_DATE,C.END_DATE "+
			"		FROM "+
			"		(SELECT  "+
			...
			*/
			...
			"		(SUM(GRENTAL_AMOUNT) - SUM(NET_RENTAL_AMOUNT)) VAT_AMOUNT,   "+
			"		SUM(GRENTAL_AMOUNT) GROSS_AMOUNT  , "+
			"		NVL(TO_CHAR(A.ACTIVATED_DATE, 'DD-MM-YYYY'),'-')  START_DATE , "+
			"		NVL(TO_CHAR(MAX(RENTAL_DATE),'DD-MM-YYYY'),'-')                     "+
			"		FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A, "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT B   "+
			"		WHERE A.APPLICATION_NO=B.APPLICATION_NO AND   "+
			"		A.FINANCE_NO=UPPER('"+m_finance_no+"')  "+
			"		AND B.PRICING_NO=UPPER('"+m_tmp_pricing_no+"')  "+ //added by nuwan de silva on 01-11-07
			"		GROUP BY B.PRO_INVOICE_NO ,A.ACTIVATED_DATE ";
			...
			"		NVL(TO_CHAR(A.ACTIVATED_DATE, 'DD-MM-YYYY'),'-')  START_DATE , "+
			"		NVL(TO_CHAR(MAX(RENTAL_DATE),'DD-MM-YYYY'),'-')      ,               "+
			"   NVL("+m_schema_name+".AF_CO_GET_AMI_AMOUNT(A.APPLICATION_NO,A.PRICING_NO,B.PRO_INVOICE_NO),0) M_AMI "+
			"		FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A, "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT B   "+
			"		WHERE A.APPLICATION_NO=B.APPLICATION_NO  "+
			"   AND B.PRICING_NO=UPPER('"+m_tmp_pricing_no+"')  "+
			"   AND B.PRO_INVOICE_NO='"+m_tmp_invoice_no+"'   "+
			"		AND A.FINANCE_NO=UPPER('"+m_finance_no+"')  "+
			"		GROUP BY B.PRO_INVOICE_NO ,A.ACTIVATED_DATE ,"+m_schema_name+".AF_CO_GET_AMI_AMOUNT(A.APPLICATION_NO,A.PRICING_NO,B.PRO_INVOICE_NO) ";
		...
		*/
				...
				/*	String sql_ami="SELECT  "+
					...
					*/
					...
					String sql_ami="SELECT  "+
					" NVL("+m_schema_name+".AF_CO_GET_AMI_AMOUNT('"+m_application_no+"','"+m_tmp_pricing_no+"','"+m_tmp_invoice_no+"'),0) FROM DUAL ";
			    ...
			    out.println("<tr>");
					...
					//comment by nuwan de silva on 27-08-07-------------------------------------------
			    ...
			    "	A.FINANCE_NO=UPPER('"+m_finance_no+"') AND "+
				  ...
				  " GROUP BY B.PRICING_NO,B.SUB_CHAGE_CODE,C.DESCRIPTION ORDER BY  C.DESCRIPTION ");			
					*/
					...
					rs_charges =stmt_charges.executeQuery("SELECT "+					
					"				PRICING_NO,SUB_CHAGE_CODE,DESCRIPTION,(AMOUNT),PRO_INVOICE_NO,CHARGES   "+
					"				FROM( "+
					"				SELECT  "+
					...
					"				'MAINTAI' CHARGES "+
					"				FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A, "+m_schema_name+".AF_CO_PRO_APP_PRICING_MAINTEIN B ,  "+
					"				"+m_schema_name+".AF_CO_MAS_SUB_CHARGES C  "+
					"				WHERE A.APPLICATION_NO=B.APPLICATION_NO AND   "+
								    "	A.FINANCE_NO=UPPER('"+m_finance_no+"') AND "+
									  " B.PRICING_NO='"+m_tmp_pricing_no+"'  AND "+
				...
				/*	if(more_charges){
					...
					*/
					...
					if(rs_charges.getString(6).equals("CHARGES")){ //added by nuwan de silva on 27-08-07
					  if(b_charges==0){
						out.println("<table align='center' width='100%' class='table' >");						
						out.println("<tr >");
						out.println("<td width='1%'></td>"); 
						...
						b_charges=b_charges+1;

					}
					else if(rs_charges.getString(6).equals("MAINTAI")){ //added by nuwan de silva on 27-08-07
					  if(b_main==0){
						out.println("<table align='center' width='100%' class='table' >");						
						out.println("<tr >");
						out.println("<td width='1%'></td>"); 
						out.println("<td width='20%' ><b><u>Maintenance Charges</b></u></td>");
						out.println("<td width='2%'>&nbsp;</td>"); 
						out.println("<td width='35%' align='left'>&nbsp;</td>");
						out.println("<td width='*%'></td>"); 
						out.println("</tr>"); 
          ...
          more_charges=rs_charges.next();
            ...
            if(!more){
				...
				" SELECT "+ //added by nuwan de silva on 27-08-07
				" 'E' TOT, "+
				"  0, "+ 
				"  0, "+ 
				"  0, "+ 
				...
				"		GROUP BY A.PRO_INVOICE_NO ";
				    ...
				    rs.close();
						...
						else if(rs.getString(1).equals("E")){ //added by nuwan de silva on 27-08-07
						
						if(charge_count_main==0){
						out.println("<table align='center' width='100%' border='0' class='table' >");
						out.println("<br>");
						out.println("<tr>");
						out.println("<td width='1%'></td>"); 
						out.println("<td width='30%' class=div_input><b><u>Maintenance Charges Summary Details</u></b></td>");
						out.println("<td width='12%' class=div_input align='right' >&nbsp;</td>");
						out.println("<td width='20%' class=div_input><b>&nbsp;</b></td>");
						out.println("<td width='*%' class=div_input>&nbsp;</td>");
						out.println("</tr>");
						...
						out.println("</tr>");
						out.println("</table >");
						charge_count_main=charge_count_main+1;
						}
						more=rs.next();
						}
		   ...
		   //comment by nuwan de silva on 21-09-07			
				/*	" SELECT "+
					"    INVOICE_NO REF_NO, "+
					"    DUE_DATE  DUE_DATE, "+
  ...
  String Sql_future_rentals= "  SELECT "+
   ...
   "  SUM(NET_RENTAL_AMOUNT), "+
   //"  SUM(GRENTAL_AMOUNT-NET_RENTAL_AMOUNT), "+
	 "  SUM(CEIL(NET_RENTAL_AMOUNT*("+m_schema_name+".AF_CO_GET_VAT_ON_RENTAL_PRO(TRANSACTION_TYPE,A.RENTAL_DATE)/100))), "+	
   "  SUM(NET_RENTAL_AMOUNT+CEIL((NET_RENTAL_AMOUNT*("+m_schema_name+".AF_CO_GET_VAT_ON_RENTAL_PRO(TRANSACTION_TYPE,A.RENTAL_DATE)/100)))) "+ 
   //"  SUM(GRENTAL_AMOUNT) "+ 
   "  FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT A, "+
   "  "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
		     ...
		     rs.close();			
		  	  ...
		  	  rs.close();
					...
					//Added by Chandana on 04/10/2007
		  			...
		  			out.println("<tr>");
						out.println("<td width='1%'></td>"); 
						out.println("<td width='*%' align='left' class=div_input ><B><U>Client Special Comments</U></B></td>");
						out.println("</tr>");
						out.println("</table>");
						//out.println("<hr color='black'>");
						out.println("<br>");
						
				    rs3=stmt.executeQuery("SELECT TO_CHAR(ENT_DATE,'DD/MM/YYYY'),COMMENTS,ENT_USER "+
						                      " FROM "+m_schema_name+".AF_CO_MAS_CLIENT_COMMENT "+
																	" WHERE CLIENT_CODE ='"+m_client_code+"'"+
																	" AND APPLICATION_NO = UPPER('"+m_application_no+"')"); 
							boolean  more4 =rs3.next();			
						...
						out.println("<tr>");
						out.println("<td width='1%'></td>"); 
						out.println("<td width='13%' class=div_input valign='top'><B>Date: </B>"+rs.getString(1)+"&nbsp&nbsp&nbsp <B>User: </B>"+rs.getString(3)+"</td>");
		 				out.println("<td width='1%'></td>"); 
