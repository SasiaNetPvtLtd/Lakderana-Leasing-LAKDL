...
public class LAKDL_AF_CR_PRO_Cancellation_After_PO extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res) { 
      ...
      String m_username = m_sn_methods.username;
			...
			out.println("			}else if(data_vec.length>0 && document.Form1.TXT_FINANCE_NO.value!=\"\"   && document.Form1.hid_text.value==\"J5\"){");
			out.println("if (data_vec[0]!='0'){");
			out.println("alert('Please reverse the payment and continue.');");
			out.println("document.Form1.TXT_APPLICATION_NO.value=''");
			out.println("document.Form1.TXT_FINANCE_NO.value    =''");
			out.println("document.Form1.TXT_TOT_REC.value       =''");
			out.println("}");
			out.println("			}");
	...
	/*out.println("document.Form1.TXT_TOT_PENALTY.value    =data_vec[0]");
			...
			*/
			...
			out.println("		else if (document.Form1.hid_text.value==\"CHK_SUS\" ){");
			//out.println("		  alert(data_vec[0]); ");
			
			out.println("		  	if(data_vec[0]>0){");
			out.println("		  	   alert('Insurance balance initiated'); ");
			out.println("		  	}");
			out.println("		  	else{");
			...
			out.println("	}");
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Cancellation_After_PO?chksql=main_page';"); 
				out.println("		}"); 
				out.println("}"); 
				...
				out.println("help_box.innerHTML=\" Credit - Cancellation After PO - \"+m_val;"); 
				out.println("}"); 
		
				out.println("function load_roll_out_value(){");
				out.println("help_box.innerHTML=\" Credit - Cancellation After PO  \";"); 
				out.println("}"); 
			...
			out.println("    document.Form1.TXT_TOT_REC.value=format_noobject(oBj.valout[6]);");
			out.println("check_app_pay();");
			out.println("}");
			...
			// Purpose - Validate Pentaly Amount
			...
			// added by udara 11-06-2015
			out.println("function Chk_Amount_2() {");
			
			out.println("  if(document.Form1.TXT_TOT_PENALTY.value < 0){  ");
			out.println("     document.Form1.TXT_TOT_PENALTY.value =0; ");
			out.println("  }");
			
			out.println("  Chk_Amount(); ");
			...
			out.println("function check_app_pay() {");
			out.println("assig('J5');");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_app_pay_status&data_val1=\"+document.Form1.TXT_APPLICATION_NO.value+\"&ac_status=Y\";");
			//out.println("window.open(m_url)");
			out.println("load_interface(m_url,'XML');");
			out.println("}");
			...
			out.println("function validate_before_submit(){ "); 
			//out.println("   before_submit(); ");
			out.println("   assig('CHK_SUS'); ");
			out.println("   m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_sus_payment&data_val1=\"+document.Form1.TXT_FINANCE_NO.value+\"&ac_status=Y\";");
			out.println("   load_interface(m_url,'XML');");
			//out.println("   window.open(m_url); ");
			out.println("}");
		  ...
		  out.println("		if(validate_data()){"); 
			...
			out.println("document.Form1.btn_save.disabled    =true;"); // added by udara 08-01-2018
			
			out.println("		if(confirm(\"Are you sure you want to save?\")){ ");
			out.println("document.Form1.TXT_TOT_REC.disabled  =false;");
			out.println("document.Form1.TXT_TOT_REPAY.disabled    =false;");
			
			
			
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_Save_Cancel_After_PO';");  
			...
			out.println("} "); 
				...
				out.println("function load_sysdate(){	"); //Added By Sandun on 22-04-2009
			...
			out.println("function load_c_date(val) {");//Added By Sandun on 22-04-2009
      ...
      out.println(" document.Form1.hid_cal_date.value=num;"); 
			out.println("	popupwin = window.open(servlet_client_url+\":\"+client_t3_port+\"/\"+client_name+\"CO_Calendar_Window\", \"oBj\",\"left=400,top=200,width=320,height=230\");"); 
			out.println("}");
			...
			out.println("}");
					...
					out.println("</script>"); 
					out.println("<BODY class='body & txt-body' leftmargin='0' topmargin='0' marginwidth='0' ONLOAD=\"load_sysdate()\">"); //load_lock()
			    ...
			    out.println("<INPUT TYPE='Hidden' NAME='hid_close_sts' VALUE=\"N\">"); 
					...
					//out.println("<td width='10%' align='center'><input type=\"button\" class='mainbut' onMouseout='load_roll_out_value();' onMouseOver='load_roll_value(\"Save\");' onClick='load_screen_status(\"SAVE\"), before_submit()' value=\"Save\"></td>");   // validate_before_submit
					out.println("<td width='10%' align='center'><input type=\"button\" name=btn_save class='mainbut' onMouseout='load_roll_out_value();' onMouseOver='load_roll_value(\"Save\");' onClick='load_screen_status(\"SAVE\"), validate_before_submit()' value=\"Save\"></td>");   // added by udara 11-06-2015
	    		...
	    		out.println("<td width='10%' align='center'><input type=button name=back value=\"Close\" class=mainbut onclick=close_screen(); onMouseOver='load_roll_value(\"Close\");' onmouseout='load_roll_out_value();'></td>");
			   	...
			   	out.println("<td width='100%'>");	
					...
					out.println("<tr class=tr_input>");
					out.println("<td width='20%' ID=VDATE>Value Date</td>");
					out.println("<td width='35%' ><input name=\"VAL_DAY1\"   type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=check_date(document.Form1.VAL_DAY1,document.Form1.VAL_MONTH1,document.Form1.VAL_YEAR1)> ");
					...
					out.println("    <input name=\"VAL_YEAR1\"  type=\"text\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onchange=check_date(document.Form1.VAL_DAY1,document.Form1.VAL_MONTH1,document.Form1.VAL_YEAR1)><a href style='{cursor:hand; }' onclick=load_calendar('1')>   Calendar</a> ");
					out.println("</td>");
					out.println("</tr >");
					out.println("<tr >"); 
					...
					out.println("<input class='but_input' type='button' name='BUT_TXT_APPLICANT_CODE' value=\"...\" onClick=\"help_button_1()\"></td>"); 
					...
					out.println("<input class='but_input' type='button' name='BUT_HELP_MAIN' value=\"...\" onClick=\"help_button_2()\" ></td>"); //M_APPLICATION_PROCESS_APPLICATION_HELP m_help_TXT_APPLICATION_NO
					out.println("</tr>"); 
					out.println("<tr >"); 
					...
					out.println("<input class='but_input' type='button' name='BUT_TXT_APPLICATION_NO' value=\"...\" onClick=\"help_button_3()\" > </td>"); 
					out.println("</tr>"); 
					/////////////////////
					out.println("<tr >"); 
					out.println("<td >Receipt Amount</td>"); 
					out.println("<td ><input class='txt_input' type='text' name='TXT_TOT_REC' maxlength='20' size='10' disabled STYLE=\"{text-align:right;}\"></td>"); 
					out.println("<td ></td>");  
					out.println("<td ></td>"); 
					out.println("</tr>"); 
					...
					//out.println("<td ><input class='txt_input' type='text' name='TXT_TOT_PENALTY' maxlength='25' size='10' onblur=\"assig('J4'),check_number_decimal(this,21),Chk_Amount()\" STYLE=\"{text-align:right;}\"></td>"); // 
					out.println("<td ><input class='txt_input' type='text' name='TXT_TOT_PENALTY' maxlength='25' size='10' onblur=\"assig('J4'),check_number_decimal(this,21),Chk_Amount_2()\" STYLE=\"{text-align:right;}\"></td>");
					out.println("<td ></td>");  
					out.println("<td ></td>"); 
					out.println("</tr>"); 
					...
					out.println("<td ><input class='txt_input' type='text' name='TXT_TOT_REPAY' maxlength='20' size='10' disabled STYLE=\"{text-align:right;}\"></td>"); 
					out.println("<td ></td>");  
					out.println("<td ></td>"); 
					out.println("</tr>"); 
