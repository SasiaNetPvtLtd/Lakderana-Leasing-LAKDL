...
//--
//SCREEN NAME:DISPLAY FOLLOW UP REPORT - FILTERED BY ACTION DATE
//CREATED BY:UDARA
//DATE/TIME:14/10/2010
//NOTES:
...
public class LAKDL_AF_CO_REP_display_follow_up_report_view_as_action_date extends javax.servlet.http.HttpServlet { 
...
public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Display Follow Up Report - Filtered by Action Date</TITLE>"); 
			out.println("</HEAD>"); 
	  ...
	  //  out.println("	m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RPT_Advertisement_Offers_Report?chksql=MAIN&OFFER_NO=&from_date="+m_from_date+"&to_date="+m_to_date+"&sort_column=\"+m_sort_col+\"&order_by_type=\"+m_order_by_type;"); 
         ...
         out.println("window.open(m_url,'displayWindow3','left=50,top=60,width=850,height=500,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
			...
			out.println("<tr><td align='left' class='pdn_txtpos2' style='height: 18px'>Display Follow Up Report - Filtered by Action Date</td></tr>"); 
			out.println("</table>");  
			out.println("</table>");  
			out.println("</BR>");
			...
			//" A.ORG_FOLLOWUP_NO, NVL(A.FOLLOWUP_TIME,'-'), A.STATUS, A.ENT_REMARKS, "+m_schema_name+".AF_CO_GET_CLIENT_CODE("+m_schema_name+".AF_CO_GET_FIN_NO(A.ID_NO)), "+m_schema_name+".AF_CO_GET_FIN_NO(A.ID_NO), "+m_schema_name+".FA_CO_GET_LAST_FOLLOW_REMARK(A.ID_NO)   "+ // 14,15,16 values
			//" A.ORG_FOLLOWUP_NO, NVL(A.FOLLOWUP_TIME,'-'), A.STATUS, A.ENT_REMARKS, "+m_schema_name+".AF_CO_GET_CLIENT_CODE("+m_schema_name+".AF_CO_GET_FIN_NO(A.ID_NO)), NVL("+m_schema_name+".AF_CO_GET_FINANCE_NO ("+m_schema_name+".AF_CO_GET_FIN_NO(A.ID_NO) ),A.ID_NO), "+m_schema_name+".FA_CO_GET_LAST_FOLLOW_REMARK(A.ID_NO)   "+ // 14,15,16 values
			...
			while(more){
						...
						out.println("<TD style=\"cursor:hand\"   onclick=\"load_history_report(document.getElementById('ID_NO"+j+"').value, document.getElementById('CLI_NO"+j+"').value);\" class='txt_report_data' align='right'> "+rs.getString(2)+"</TD> <input type=hidden id=\"ID_NO"+j+"\" name=\"ID_NO"+j+"\" value=\""+rs.getString(15)+"\"> <input type=hidden id=\"CLI_NO"+j+"\" name=\"CLI_NO"+j+"\" value=\""+rs.getString(14)+"\"> ");
