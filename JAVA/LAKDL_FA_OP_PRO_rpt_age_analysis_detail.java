...
public class LAKDL_FA_OP_PRO_rpt_age_analysis_detail extends javax.servlet.http.HttpServlet {
		...
		try {
     	...
     	res.setDateHeader("Expires", 0);
  	  	...
  	  	"  TO_CHAR(A.DUE_DATE,'DD-MM-YYYY')"+//10
				...
				"  AND ((A.DUE_DATE - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') )<-7 ) AND A.BALANCE_AMOUNT >0 	");

			 }		
			 if(m_date_range.equals("7m")) { 
				rs1= stmt1.executeQuery("SELECT  A.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(A.DEBTOR_CODE), "+//2	
  			"  A.INVOICE_NO,"+//3
  	  	"  TO_CHAR(A.INVOICE_DATE,'DD-MM-YYYY'),"+//4
  	  	...
  	  	"  TO_CHAR(A.DUE_DATE,'DD-MM-YYYY')"+//10
 				"  FROM "+m_schema_name+".FA_CR_PRO_INVOICE_DETAIL A  "+
 				"  WHERE  A.CLIENT_CODE='"+m_client_code+"' AND A.FACILITY_NO='"+m_facility_no+"' AND A.INVOICE_STATUS='CONF' "+
				"  AND ((A.DUE_DATE - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') )<0 AND (A.DUE_DATE - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') )>=-7 ) AND A.BALANCE_AMOUNT >0 	");
				
			 }	
			 if(m_date_range.equals("7")) { 
  	  	...
  	  	"  TO_CHAR(A.DUE_DATE,'DD-MM-YYYY')"+//10
 				"  FROM "+m_schema_name+".FA_CR_PRO_INVOICE_DETAIL A "+
 				"  WHERE A.CLIENT_CODE='"+m_client_code+"' AND A.FACILITY_NO='"+m_facility_no+"' AND A.INVOICE_STATUS='CONF' "+
				"  AND ((A.DUE_DATE - TO_DATE('"+m_eff_date+"','DD-MM-YYYY')  )>7 AND (A.DUE_DATE - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') )<=30 ) AND A.BALANCE_AMOUNT >0 	");
			 }	
         ...
         boolean mflag=true;							
			...
			//Added by Dineth on 2009-02-17
			 if(m_date_range.equals("99m")) { 
				rs1= stmt1.executeQuery("SELECT  DISTINCT B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
  			"  B.POD_REF_NO,"+//3
				"	 NVL(A.CHEQUE_NO,'-'),	"+//4
  	  	"  TO_CHAR(A.PD_REALISE_DATE,'DD-MM-YYYY'),"+//5
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE),  "+//7
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND ((TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<-7 ) AND A.CHEQUE_AMOUNT<>0 	");
			 }	
				if(m_date_range.equals("7m")) { 
				rs1= stmt1.executeQuery("SELECT  DISTINCT B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND ((TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<0 ) AND A.CHEQUE_AMOUNT<>0 	");
			 }
				if(m_date_range.equals("7")) { 
				rs1= stmt1.executeQuery("SELECT  DISTINCT B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND ((TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>=0 AND ( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=7 ) AND A.CHEQUE_AMOUNT<>0 	");
			 }	
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND ((TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>7 AND ( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=30 ) AND A.CHEQUE_AMOUNT<>0 	");
			 }	
			 if(m_date_range.equals("60")) { 
				rs1= stmt1.executeQuery("SELECT  DISTINCT B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND (( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>30 AND ( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=60 ) AND A.CHEQUE_AMOUNT<>0 	");
			 }		
			 if(m_date_range.equals("90")) { 
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND (( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>60 AND ( TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=90 ) AND A.CHEQUE_AMOUNT<>0 	");
    		...
    		"  NVL(A.CHEQUE_AMOUNT,0),"+//6
				...
				"  TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY') "+
 				"  FROM "+m_schema_name+".FA_OP_PRO_POD_CHEQUES A,"+m_schema_name+".FA_OP_PRO_POD_CHEQUES_ALLO B "+
 				"  WHERE A.POD_REF_NO=B.POD_REF_NO AND A.POD_STATUS='N' AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' "+
				"  AND ((TO_DATE(A.PD_REALISE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>90 ) AND A.CHEQUE_AMOUNT<>0 	");
         ...
         boolean mflag=true;							
			   boolean more = rs1.next();
				  ...
				  if(more){
						...
						out.println("<td width='10%' ><DIV class=div_input><b>Pd Realize Date</b></DIV></td>"); 
						...
						out.println("<td width='10%' ><DIV class=div_input><b>No of Days</b></DIV></td>");
						//out.println("<td width='*%'></td>");
						out.println("</tr>"); 
					...
					while(more){
							...
							out.println("<td width='1%'></td>"); 
							out.println("<td width='15%' class=div_input onClick=\"show_client('"+rs1.getString(1)+"')\" style='cursor:hand' ><u>"+rs1.getString(2)+"</u></td>");
							out.println("<td width='10%' class=div_input onClick=\"show_pod_cheque_details('"+rs1.getString(3)+"')\" style='cursor:hand' ><u>"+rs1.getString(3)+"</u></td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(4)+"</td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(5)+"</td>");
							out.println("<td width='10%' class=div_input align='right'>"+nf.format(rs1.getDouble(6))+"</td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(7)+"</td>");
							out.println("<td width='10%' class=div_input align='right'>"+rs1.getInt(8)+"</td>");
							//out.println("<td width='*%'></td>");
							out.println("</tr>");
							more = rs1.next();
					}	
         ...
         boolean mflag=true;							
			...
			else if(m_chksql.equals("LOAD_RECEIPT_DETAILS")){
			...
			//Added by Dineth on 2009-02-17
			 if(m_date_range.equals("99m")) { 
				...
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
  			"  B.RECEIPT_NO,"+//3
				"	 DECODE(A.SETTLE_MODE,'CHEQUE','Cheque','CASH','Cash','BANKTR','Bank Transfer','-'),	"+//4
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				...
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N' "+
				"  AND ((TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<-7 ) AND A.REC_AMOUNT<>0 	");
			 }	
				if(m_date_range.equals("7m")) { 
				rs1= stmt1.executeQuery("SELECT  B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				"  FROM "+m_schema_name+".FA_OP_PRO_SETTL_RECEIPT A,"+m_schema_name+".FA_OP_PRO_RECEIPT_ALLO B "+
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N' "+
				"  AND ((TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<0  ) AND A.REC_AMOUNT<>0 	");
			 }
				if(m_date_range.equals("7")) { 
				rs1= stmt1.executeQuery("SELECT  B.DEBTOR_CODE,"+//1
				" "+m_schema_name+".FA_GET_CLIENT_FULL_NAME(B.DEBTOR_CODE), "+//2	
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				"  FROM "+m_schema_name+".FA_OP_PRO_SETTL_RECEIPT A,"+m_schema_name+".FA_OP_PRO_RECEIPT_ALLO B "+
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N' "+
				"  AND ((TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>=0 AND ( TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=30 ) AND A.REC_AMOUNT<>0 	");
			 }	
			//End by Dineth on 2009-02-17	
			 if(m_date_range.equals("30")) { 
				rs1= stmt1.executeQuery("SELECT  B.DEBTOR_CODE,"+//1
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				"  FROM "+m_schema_name+".FA_OP_PRO_SETTL_RECEIPT A,"+m_schema_name+".FA_OP_PRO_RECEIPT_ALLO B "+
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N'  "+
				"  AND (( TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>30 AND ( TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=60 ) AND A.REC_AMOUNT<>0 	");
			 }		
			 if(m_date_range.equals("90")) { 
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				"  FROM "+m_schema_name+".FA_OP_PRO_SETTL_RECEIPT A,"+m_schema_name+".FA_OP_PRO_RECEIPT_ALLO B "+
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N' "+
				"  AND (( TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>60 AND ( TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))<=90 ) AND A.REC_AMOUNT<>0 	");
    		...
    		"  NVL(A.REC_AMOUNT,0),"+//6
				" "+m_schema_name+".AF_CO_GET_BRANCH_NAME(A.PAYER_BRANCH_CODE)  "+//7
 				"  FROM "+m_schema_name+".FA_OP_PRO_SETTL_RECEIPT A,"+m_schema_name+".FA_OP_PRO_RECEIPT_ALLO B "+
 				"  WHERE A.RECEIPT_NO=B.RECEIPT_NO AND B.CLIENT_CODE='"+m_client_code+"' AND B.FACILITY_NO='"+m_facility_no+"' AND A.REC_STATUS='N'  "+
				"  AND ((TO_DATE(A.CHEQUE_DATE) - TO_DATE('"+m_eff_date+"','DD-MM-YYYY'))>90 ) AND A.REC_AMOUNT<>0 	");
         ...
         boolean mflag=true;							
			   boolean more = rs1.next();
					 ...
					 out.println("<TABLE  WIDTH='100%' class='pdn_txtpos2' STYLE='{ bgcolor='#8fb382' color: black; font: 20pt arial;}'>");
					 out.println("<TR><TD align='Center' ><B> Receipt Details for Client Code - "+m_client_code+"  Facility No - "+m_facility_no+" </B></TD></TR>");
					 out.println("</TABLE>");
				  ...
				  if(more){
					...
					while(more){
							...
							out.println("<td width='1%'></td>"); 
							out.println("<td width='15%' class=div_input onClick=\"show_client('"+rs1.getString(1)+"')\" style='cursor:hand' ><u>"+rs1.getString(2)+"</u></td>");
							out.println("<td width='10%' class=div_input onClick=\"show_receipt_details('"+rs1.getString(1)+"','"+rs1.getString(3)+"')\" style='cursor:hand'><u>"+rs1.getString(3)+"</u></td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(4)+"</td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(5)+"</td>");
							out.println("<td width='10%' class=div_input align='right'>"+nf.format(rs1.getDouble(6))+"</td>");
							out.println("<td width='10%' class=div_input >"+rs1.getString(7)+"</td>");
