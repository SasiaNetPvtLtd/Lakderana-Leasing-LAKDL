...
public class LAKDL_AF_CR_PRO_Application_Status_Report_appr1 extends javax.servlet.http.HttpServlet {
	
	Connection conn;
	Statement stmt,stmt2,stmt1,stmt3;
	java.text.NumberFormat nf,nf1;
	public ResultSet rs,rs2,rs1,rs3;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("}");
				out.println("}");
				out.println("if('"+m_pre_stage+"'=='V-RECOM'){");//V-APP
				out.println("if(m_val==''){");
				...
				out.println("}");
				out.println("else if('"+m_pre_stage+"'=='V-RECOM'){");//V-APP
			...
			*/
			...
			//  out.println(" window.location.href=m_url;"); 
				...
				*/
				...
				out.println("	m_url = \""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_PRO_Credit_Verifi_app_1?pre="+m_pre_stage+"&appro="+m_app_stage+"&qry="+m_pre_stage1+"&applicaton_no=\"+val+\"&insurance_done=\"+val2+\"&contract_no=\"+document.Form1.TXT_CONTRACT_NUMBER.value+\"&close=1\";"); 
				out.println(" window.location.href=m_url;"); 
				...
				out.println("<td  width='10%' style= cursor:hand; title='Click here to sort by - Contract No  '       onclick=sort_data('FACILITY_NO') >Contract No</td>");
				...
				if(m_pre_stage.equals("ENT_CON")){
					...
					//rs = stmt.executeQuery (" SELECT distinct "+ // commented by udara 07-03-2019
					rs = stmt.executeQuery (" SELECT  "+ // added by udara 07-03-2019
						" A.APPLICATION_NO,"+ //1
				...
				//*** CREDIT APPROVAL 1 ****
				else	if(m_pre_stage.equals("V-RECOM")){//V-APP
					...
					rs = stmt.executeQuery 
						//(" SELECT distinct "+ // commented by udara 07-03-2019
						(" SELECT  "+ // added by udara 07-03-2019
						" A.APPLICATION_NO,"+ //1
		...
		out.println("<td width=\"5%\" align=center><input type=\"checkbox\" name=chk_reject_"+j+" value=\"N\" unchecked onclick=\"change_reject('"+j+"')\" ></td>");
					...
					*/
					...
					if(((m_pre_stage.equals("V-RECOM") || m_pre_stage.equals("VERIFY-M")) && (m_limit < m_total))){
			...
			//added by nuwan de silva on 23-11-2007------
				...
				out.println("}");
				out.println("}");
				out.println("if('"+m_pre_stage+"'=='V-RECOM'){");
				out.println("if(m_val==''){");
				out.println("help_box.innerHTML=\"Credit  - Credit Approval 1 \";"); 
				out.println("}else{");
				out.println("help_box.innerHTML=\"Credit  - Credit Approval 1 - \"+m_val;"); 
				...
				out.println("}");
				out.println("else if('"+m_pre_stage+"'=='V-RECOM'){");
				out.println("var m_val=\"Approve\";");
				out.println("help_box.innerHTML=\"Credit  - Credit Approval 1 - \"+m_val;"); 
				...
				out.println("function verify_screen(val,val2,val3) {"); 
				//	out.println("alert('val3'+val3);");
				out.println("	m_url = \""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_PRO_Credit_Verifi_app_1?pre="+m_pre_stage+"&appro="+m_app_stage+"&qry="+m_pre_stage1+"&applicaton_no=\"+val+\"&insurance_done=\"+val2+\"&contract_no=\"+document.Form1.TXT_CONTRACT_NUMBER.value+\"&clientcode=\"+val3+\"&close=1\";"); 
				...
				//=======Added by Prabsh on 08-02-2012=============
				//----------------------------------------------------------------------		
				out.println("function help_button_2() {"); 
				out.println("    document.Form1.hid_help_type.value=\"4\";"); 
				out.println("    m_sql = \"m_help_TXT_CLIENT_NO_8\";"); 
				...
				//	out.println("m_url=\""+m_servlet_client_url+":"+m_client_t3_port+"/"+m_schema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_LAKDL_AF_MK_display_client&data_val=\"+obj.value+\"&ac_status=Y\";");
				//	out.println("	popupwin=window.showModalDialog('"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_schema_name+"_AF_MK_Help_Servlet?class_in="+m_schema_name+"_AF_PRO_CR_help_select&Sql_in='+Sql+'&Start_in='+Start+'&End_in='+End+'&Crit_In='+Crit+'&Hid_No='+Hid_No+'&Max_in='+Hid_No+'&Args=', oBj,\"dialogWidth:25em; dialogHeight:26em; center:yes; status:no\");");
				...
				out.println("	"); 
				out.println("	popupwin=window.showModalDialog('"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_schema_name+"_AF_MK_Help_Servlet?class_in="+m_schema_name+"_AF_PRO_CR_help_select&Sql_in='+m_sql+'&Start_in='+Start+'&End_in='+End+'&Crit_In='+m_criteria+'&Hid_No='+Hid_No+'&Max_in='+Hid_No+'&Args=', oBj,\"dialogWidth:25em; dialogHeight:26em; center:yes; status:no\");");
				...
				out.println("   document.Form1.TXT_CONTRACT_NUMBER.value=oBj.valout[5];");
				...
				//		out.println("    document.Form1.hid_help_type.value=\"4\";"); 
				//		out.println("    Crit =document.Form1.TXT_CLIENT_NAME.value+\"@Y@\";"); 
	...
	*/
				...
				out.println("   document.Form1.TXT_CLIENT_NAME.value=oBj.valout[1];"); 
				//	out.println("   document.Form1.TXT_CONTRACT_NUMBER.value=oBj.valout[3];"); 
				...
				if(m_pre_stage.equals("ENT_CON")){
					...
					out.println("<td width='10%' align='center'><input type=\"button\" class='mainbut'  name=Reverse onMouseout='load_roll_out_value(\"Reverse\");' onMouseOver='load_roll_value(\"Reverse\");'  onclick='load_screen_status(\"REVERSE\"),new_window(2)' value=\"Reverse\" disabled></td>");  
				...
				out.println("<tr class=tr_input>"); 
				out.println("<INPUT TYPE='Hidden' NAME='hid_help_type' VALUE=\"\">");  //prabash 
				...
				out.println("<input class='but_input' type='button' text-align='center' name='BUT_HELP_CLIENT' value=\" ... \" onClick=\"help_button_2()\">"); // prabash 0n 08-02-2012 -help_update()
			...
			//__________________________________________added by nuwan de silva on 23-11-2007 _____________________________________________
				...
				out.println("<td  width='5%'  style= cursor:hand; title='Click here to sort by - Currency  '          onclick=sort_data('CURRENCY_CODE') >Currency</td>");
				out.println("<td  width='10%'  >Rental / Net income Ratio</td>");
				if(m_pre_stage.equals("V-RECOM")){//[Added milinda for links only approval level 1]
					out.println("<td  width='10%'  >Links</td>");
				}
				if(m_pre_stage.equals("VERIFY-M")){
					out.println("<td  width='6%'  >User</td>");
				}
				...
				// added by udara 07-03-2019
				String user_location = " "; 
				String user_location_sql = " "; 
				...
				if(user_location.equals("HO")){
					user_location_sql = " ";
				}
				else{
					user_location_sql = " AND A.BRANCH_CODE = '"+user_location+"' ";
				}
				
				String m_client_sql = "";
				
				if(!_m_client_name.equals("")){
					m_client_sql = " AND  UPPER("+m_schema_name+".AF_CO_GET_CLIENT_NAME(A.CLIENT_CODE))  LIKE  UPPER('%"+_m_client_name+"%') ";
				}
				
				// end by udara 07-03-2019
				...
				if(m_pre_stage.equals("ENT_CON")){
					...
					if(_m_division_code.equals("ALL")){
						rs = stmt.executeQuery 
							...
							(" SELECT  "+ // added by udara 07-03-2019
							" A.APPLICATION_NO,"+ //1
							" NVL(A.FINANCE_NO,'-') FACILITY_NO , "+ //2
							"	TO_CHAR(A.ENT_DATE,'DD-MM-YYYY'), "+ //3
							" TO_DATE(TO_CHAR(SYSDATE,'DD-MM-YYYY'),'DD-MM-YYYY') - TO_DATE(TO_CHAR(A.ENT_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') PERIOD, "+ //4
							"	NVL("+m_schema_name+".AF_CO_GET_MK_OFFICER_NAME(A.INQUARY_NO),'-') 	MK_NAME, "+ //5
							"	"+m_schema_name+".AF_CO_GET_CLIENT_NAME(A.CLIENT_CODE) CLIENT, "+//6
							...
							"	FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A "+
							" WHERE APPLICATION_STATUS IN ('"+m_pre_stage1+"') "+
							//" AND  UPPER("+m_schema_name+".AF_CO_GET_CLIENT_NAME(A.CLIENT_CODE))  LIKE  UPPER('%"+_m_client_name+"%') "+ // commented by udara 08-03-2019
							" "+m_client_sql+" "+ // added by udara 08-03-2019
							//" AND DECODE("+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"'),'HO','-',"+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"')) LIKE  DECODE("+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"'),'HO','-',"+m_schema_name+".AF_CO_GET_USER_LOCATION(A.ENT_USER))||'%' "+ // added by udara on 18-06-2013//temp commented by milinda 2014-05-19
							//" AND DECODE("+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"'),'HO','-',"+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"')) LIKE  DECODE("+m_schema_name+".AF_CO_GET_USER_LOCATION('"+m_username+"'),'HO','-',A.BRANCH_CODE)||'%' "+ // added by udara on 18-06-2013//temp commented by milinda 2014-05-19 // commented by udara 07-03-2019
							" "+user_location_sql+" "+ // added by udara 07-03-2019
							" ORDER BY "+m_sort_column+" "+m_order_by_type+"");
					}
					else
					{
				...
				//*** CREDIT APPROVAL 1 ****
					...
					if(_m_division_code.equals("ALL")){
						...
						rs = stmt.executeQuery 
							...
							(" SELECT  "+ // added by udara 07-03-2019
							" A.APPLICATION_NO,"+ //1
							" NVL(A.FINANCE_NO,'-') as FACILITY_NO, "+ //2
							"	TO_CHAR(A.ENT_DATE,'DD-MM-YYYY'), "+ //3
							" TO_DATE(TO_CHAR(SYSDATE,'DD-MM-YYYY'),'DD-MM-YYYY') - TO_DATE(TO_CHAR(A.ENT_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') PERIOD, "+ //4
							"	NVL("+m_schema_name+".AF_CO_GET_MK_OFFICER_NAME(A.INQUARY_NO),'-') 	MK_NAME, "+ //5
							"	"+m_schema_name+".AF_CO_GET_CLIENT_NAME(A.CLIENT_CODE) CLIENT, "+//6
				...
				while(rs.next()){
					...
					//[Added by milinda for get comfirmation report count]
					String m_sql="SELECT A.REPORT_STATUS  "+
						" FROM "+m_schema_name+".AF_CONFIRMATION_REPORT A, "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
						" WHERE A.FINANCE_NO = "+m_schema_name+".AF_CO_GET_FINANCE_NO('"+rs.getString(1)+"')  "+
						" AND A.FINANCE_NO = B.FINANCE_NO  "+						
						//" AND A.REPORT_STATUS = 'GEN' "+ 
						" AND B.APPLICATION_STATUS NOT IN ('REJECT','CANCEL_PO') ";
					out.println("<!--"+m_sql+"-->");
					
					rs3=stmt3.executeQuery(m_sql);
					if(rs3.next()){
						m_count_confirm=rs3.getString(1);
					}
					//Modified by Mahela on 22-10-2007 -  rental count
					rs2 = stmt2.executeQuery (" SELECT "+
						" SUM(NET_RENTAL_AMOUNT), "+
					...
					out.println("<td width='10%' align='left' style= \"cursor:hand;cursor-color:blue\" onclick=show_transaction_history_by_contract('"+rs.getString(2)+"','"+rs.getString(20)+"') ><u>"+rs.getString(2) +"</u></td>");
					out.println("<td width='5%' align='left'>"+rs.getString(4) +"</td>");
					...
					*/
					...
					out.println("<td width='10%'  align='right'> - </td>"); // added by udara 29-10-2018
				...
				out.println("<td width=\"5%\" align=center><input type=\"checkbox\" name=chk_reject_"+j+" value=\"N\" unchecked onclick=\"change_reject('"+j+"')\"></td>");
					...
					*/
					...
					if(((m_pre_stage.equals("V-RECOM") || m_pre_stage.equals("VERIFY-M")) && (m_limit < m_total))){
						...
						if (m_opt.equals("1")){
							out.println("<td><input type=button name=\"linkdoc_"+j+"\" value=\"Upload\"   class=\"but_input\" onclick=view_documents_cr_approval('"+rs.getString(2)+"') >");//[Added milinda for add links for JB23092017-00731 on 12-10-2017]
							out.println("<input type=button name=\"linkcom_"+j+"\" value=\"Comfirmaion\"   class=\"but_input\" onClick=chk_confirmation_count('"+rs.getString(2)+"','"+j+"','"+m_count_confirm+"') ></td>");//[Added milinda for add links for JB23092017-00731 on 12-10-2017]
						...
						if (m_opt.equals("1")){
							...
							if(m_pre_stage.equals("VERIFY-M")){
								//out.println(m_pre_stage);
								//out.println("alert('12');");
								//out.println("<td><input type=button name=\"view_"+j+"\" value=\"View\"   class=\"but_input\" onclick=verify_screen(\""+rs.getString(1)+"\",\""+rs.getString(16)+"\") >");
								out.println("<td><input type=button name=\"view_"+j+"\" value=\"Approve\"   class=\"but_input\" onclick=verify_screen(\""+rs.getString(1)+"\",\""+rs.getString(16)+"\",\""+rs.getString(20)+"\") >");
							}
							...
							if(m_pre_stage.equals("V-RECOM")){
								...
								out.println("<td><input type=button name=\"linkdoc_"+j+"\" value=\"Upload\"   class=\"but_input\" onclick=view_documents_cr_approval('"+rs.getString(2)+"') >");//[Added milinda for add links for JB23092017-00731 on 12-10-2017]
								out.println("<input type=button name=\"linkcom_"+j+"\" value=\"Comfirmaion\"   class=\"but_input\" onClick=chk_confirmation_count('"+rs.getString(2)+"','"+j+"','"+m_count_confirm+"') ></td>");//[Added milinda for add links for JB23092017-00731 on 12-10-2017]
								out.println("<td><input type=button name=\"view_"+j+"\" value=\"Approve\"   class=\"but_input\" onclick=verify_screen(\""+rs.getString(1)+"\",\""+rs.getString(14)+"\",\""+rs.getString(20)+"\") >");
							}
							...
							if(m_pre_stage.equals("ENT_CON")){
								//out.println("alert('gggggg'+"+rs.getString(20)+");");
								out.println("<td><input type=button name=\"view_"+j+"\" value=\"Verify\"   class=\"but_input\" onclick=verify_screen(\""+rs.getString(1)+"\",\""+rs.getString(14)+"\",\""+rs.getString(15)+"\") >");
								out.println("<td><input type=button name=\"report_"+j+"\" value=\"Report\"   class=\"but_input\" onclick=load_report(\""+rs.getString(1)+"\",\""+rs.getString(2)+"\",\""+rs.getString(15)+"\") >");
							}
							//	}
