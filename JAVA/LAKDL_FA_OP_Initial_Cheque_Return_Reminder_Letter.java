...
public class LAKDL_FA_OP_Initial_Cheque_Return_Reminder_Letter extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
		  ...
		  if (m_chksql.trim().equals("main_page")) {
			...
			out.println("	 m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_save_initial_chq_return_rem_count?client_no="+m_client_no+"&debtor_code="+m_debtor_code+"&print="+m_print+"&receipt_no="+m_receipt_no+"&return_no="+m_return_no+"\";");  
    	...
    	out.println("m_writedata+'</table>';");
		  ...
		  out.println("</tr>"); 
			...
			rs = stmt.executeQuery ("SELECT TO_CHAR(SYSDATE+10,'DD-MM-YYYY') FROM DUAL ");								
			more = rs.next();
         ...
         out.println("<tr><td width='*%' class='rep-body' >"+m_city_desc+"</td></tr>");
			out.println("<tr><td width='*%'class='rep-body'>"+m_Letter_date+"</td></tr>"); // Added by Udara on 07-06-2011 for SUPPORT #2255 
			out.println("</TABLE>");
			...
			out.println("We regret to inform you that the Cheque bearing No <b>"+m_chq_no+"</b> "+
									...
									" payments and returned to us. The said cheque amounting to <b> Rs. "+nf.format(m_chq_amount)+"</b> "+
									" was in settlement of the invoices, detailed below. Kindly make "+
									" immediate settlement of the amount due and in any case not later than <b>"+m_deadline_date+"</b> failure to do"+
									" so will compel us to take further action for the recovery of the amount due. ");
