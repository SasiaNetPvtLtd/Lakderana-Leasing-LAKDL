...
public class LAKDL_FA_OP_Current_Account_Statement extends javax.servlet.http.HttpServlet {
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)	throws IOException	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("<html><head><font 10pt arial><title>Current Account Statement</title></head>");
			...
			else if (m_chksql.trim().equals("tax_invoice")){
				//------------------------------------------------------------------------------------
				...
				rs1 = stmt1.executeQuery (" SELECT TAX_INVOICE_NO,"+
					" FACILITY_NO,"+
					" CLIENT_CODE,"+
					" TO_CHAR(MONTH_START,'DD-MM-YYYY'),"+
					" TO_CHAR(MONTH_END,'DD-MM-YYYY'), "+
					" TO_CHAR(MONTH_END,'MONTH-YYYY') "+
					" FROM "+m_schema_name+".FA_CR_PRO_TAX_INVOICE "+
					" WHERE TAX_INVOICE_NO='"+m_invoice_no+"'");
				
				while(rs1.next()){
					m_client_no=rs1.getString(3);
					m_facility_no=rs1.getString(2);
					m_date_from=rs1.getString(4);
					m_date_to=rs1.getString(5);
					m_report_period=rs1.getString(6);
				}
				...
				if(rs1.next()){
					out.println("<blockquote>");
					out.println("<table border='0' width='100%' class=table >");
					out.println("<tr><td width='*%' class='factoring-letter-body'>"+m_date_to+"</td></tr>");
					//out.println("<tr><td width='*%' class='factoring-letter-body'>"+rs1.getString(6)+"</td></tr>");
					out.println("<tr><td width='*%' class='factoring-letter-body'>"+rs1.getString(7)+"</td></tr>");
					out.println("<tr><td width='*%' class='factoring-letter-body'>"+rs1.getString(2)+"</td></tr>");
					out.println("<tr><td width='*%' class='factoring-letter-body'>"+rs1.getString(3)+"</td></tr>");
					...
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>VAT NO :"+rs1.getString(8)+"</b></td></tr>");	
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>CLIENT CODE :"+m_client_no+"</b></td></tr>");
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>FACILITY NO :"+m_facility_no+"</b></td></tr>");	
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>REPORT PERIOD : "+m_report_period+"</b></td></tr>");	
					out.println("</table></blockquote>");
					out.println("<blockquote><table border='0' width='100%' class=table >");
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>INVOICE NO:"+m_invoice_no+"</b></td></tr>");	
					out.println("<tr><td width='*%' class='factoring-letter-body'><b>OFSCL VAT NO:134010762-7000</b></td></tr>");	
					out.println("</table></blockquote>");	
				}
				
				out.println("<table border='0' width='100%' class=table >");
				out.println("<tr><td width='*%' class='factoring-letter-body'><center><H5><b>TAX INVOICE</b></H5><center></td></tr>");
				out.println("</table>");		
				
				double m_tax_rate=0.00; //modified by nuwan de silva
				// commented by thamali 2011..09.13
				//rs = stmt.executeQuery (" SELECT TAX_AMOUNT FROM "+m_schema_name+".FA_OP_PRO_TAX  ");
				
				// added by thamali 2011.09.13
				rs = stmt.executeQuery(" SELECT  "+m_schema_name+".FA_CO_GET_VAT_ON_CHARGES('"+m_date_to+"') FROM DUAL  ");
				
				
				if(rs.next()){
					m_tax_rate=rs.getDouble(1);
				}
				
				
				rs = stmt.executeQuery (" SELECT 'CHER' CTYPE,"+
					" "+m_schema_name+".FA_GET_FEE_NAME(A.FEE_CODE) CDESC,"+
					" NVL(SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1)),0) CAMT,"+
					//" NVL((SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))*(15/100)),0) CAMTVAT, "+
					" NVL((SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))*("+m_tax_rate+"/100)),0) CAMTVAT, "+
					//" NVL((SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))+(SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))*(15/100))),0) CAMTALL "+
					" NVL((SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))+(SUM(DECODE(A.DRCR_STATUS,'DR',A.FEE_CHARGE_AMOUNT,A.FEE_CHARGE_AMOUNT*-1))*("+m_tax_rate+"/100))),0) CAMTALL "+
					" FROM "+m_schema_name+".FA_OP_PRO_CLIENT_CHARGES A,"+m_schema_name+".FA_CO_MAS_FEES B "+
					" WHERE A.FACILITY_NO='"+m_facility_no+"' "+
					" AND A.CLIENT_CODE='"+m_client_no+"' "+
					" AND TO_DATE(TO_CHAR(A.EFF_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')>=TO_DATE('"+m_date_from+"','DD-MM-YYYY')"+
					" AND TO_DATE(TO_CHAR(A.EFF_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')<=TO_DATE('"+m_date_to+"','DD-MM-YYYY')"+
					" AND A.FEE_CODE=B.FEE_CODE "+
					" AND B.TAX_APPLICABILITY='Y' "+
					" GROUP BY A.FEE_CODE ");
				...
				//out.println("<tr><td width='10%' class='factoring-letter-body' align='center'><b>Date</b></td>");
				out.println("<td width='20%' class='factoring-letter-body' align='center'><b>Description</b></td>");
				out.println("<td width='15%' class='factoring-letter-body' align='center'><b>Fee Amount (Rs.)</b></td>");
				out.println("<td width='10%' class='factoring-letter-body' align='center'><b>VAT (%)</b></td>");
				out.println("<td width='15%' class='factoring-letter-body' align='center'><b>VAT Amount (Rs.)</b></td>");
				out.println("<td width='15%' class='factoring-letter-body' align='center'><b>Gross Amount (Rs.)</b></td>");
				out.println("</tr>");
				
				while(rs.next()){
					out.println("<tr>");
					//out.println("<td width='10%' class='factoring-letter-body'>"+rs.getString(6)+"</td>");
					out.println("<td width='20%' class='factoring-letter-body'>"+rs.getString(2)+"</td>");
					out.println("<td width='15%' class='factoring-letter-body' align='right'>"+nf.format(rs.getDouble(3))+"</td>");
					//out.println("<td width='10%' class='factoring-letter-body' align='right'>15.0</td>");
					out.println("<td width='10%' class='factoring-letter-body' align='right'>"+m_tax_rate+"</td>"); //modified by nuwan de silva
					out.println("<td width='15%' class='factoring-letter-body' align='right'>"+nf.format(rs.getDouble(4))+"</td>");
					out.println("<td width='15%' class='factoring-letter-body' align='right'>"+nf.format(rs.getDouble(5))+"</td>");
					out.println("</tr>");
					m_bal1=m_bal1+rs.getDouble(3);
					m_bal2=m_bal2+rs.getDouble(4);
					m_bal3=m_bal3+rs.getDouble(5);
				}
				out.println("<tr>");
				//out.println("<td width='10%' class='factoring-letter-body'>-</td>");
				out.println("<td width='20%' class='factoring-letter-body'><b>Total</b></td>");
				out.println("<td width='15%' class='factoring-letter-body' align='right'><b>"+nf.format(m_bal1)+"</b></td>");
				out.println("<td width='10%' class='factoring-letter-body' align='right'>-</td>");
				out.println("<td width='15%' class='factoring-letter-body' align='right'><b>"+nf.format(m_bal2)+"</b></td>");
				out.println("<td width='15%' class='factoring-letter-body' align='right'><b>"+nf.format(m_bal3)+"</b></td>");
				out.println("</tr>");
				out.println("</table>");
				out.println("<br>");
				out.println("<br>");
			...
			else if (m_chksql.trim().equals("bulk_print_current_account")){
				...
				rs= stmt.executeQuery(" SELECT FACILITY_NO,CLIENT_CODE  "+
					" FROM "+m_schema_name+".FA_CR_PRO_CLIENT_FACILITY "+
					" WHERE FACILITY_STATUS='Y' ");
				
				while(rs.next()){
					int m_cknum=debtor_st_print.printing_interface_4(rs.getString(1),rs.getString(2),m_date_from,m_date_to);
					...
					out.println("<table border='0' width='100%' class=table >");
					if(m_cknum==1){
						out.println("<tr><td width='*%' class='factoring-letter-body'>Print Successfull "+rs.getString(1)+"</td></tr>");
					}
					else{
						out.println("<tr><td width='*%' class='factoring-letter-body'>Error when Printing "+rs.getString(1)+"</td></tr>");
					}
					out.println("</table>");
				}
			}
