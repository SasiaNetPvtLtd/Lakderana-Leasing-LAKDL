...
public class LAKDL_AF_RE_Collection_Asset_Insurnce_Detail_new extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_screen_type.trim().equals("main_page")){	
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Asset_Insurnce_Detail_new?chksql=main_page';"); 
				out.println("		}"); 
				out.println("}"); 
		...
		//		out.println(" load_c_date(document.Form1.hid_cal_date.value);");
				...
				out.println("m_status = document.Form1.TXT_STATUS.value;"); // added by udara 22-01-2015
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Asset_Insurnce_Detail_new?chksql=load_details&sr_name=\"+m_scr_name+\"&finance_no=\"+finance_no+\"&devision_type=\"+m_opt+\"&from_date=\"+m_from_date+\"&to_date=\"+m_to_date+\"&status=\"+m_status;");
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Asset_Insurnce_Detail_new?chksql=load_details_btt&sr_name=\"+m_scr_name+\"&invo_no=\"+invoice_no+\"&finance_no=\"+val1+\"&clint_name=\"+val2+\"&asst_deta=\"+description+\"&business_type=\"+m_business_type+\"&policy_no=\"+val4+\"&debit_note_no=\"+val5+\"&price_no=\"+m_price_no+\"&app_no=\"+m_app_no;");
				...
				//out.println("HelpBox('1','10','0',Crit,'m_help_TXT_FINANCE_NO_4','4');"); // commented by samith on 2015-05-27
				out.println("HelpBox('1','10','0',Crit,'m_help_TXT_FINANCENO_WITHOUT_TERMINATION','4');");
				out.println("}");
				
			//m_help_TXT_FINANCENO_WITHOUT_TERMINATION	
			...
			//	out.println("alert(app_no);");
			//	out.println("alert(div_type);");
			
				out.println("}");
				
				out.println("function set_pricing_no(m_app_no,m_devision_type,m_check_value,m_sel_type){"); 
			//	out.println("window.open(m_check_value);");
			...
			//	out.println("window.open(m_url);");
				...
				out.println("<tr>");  
				out.println("<td  width='5%' ><DIV id='DIV_TXT_STATUS'  class=div_input> Status </DIV></td>"); 
				out.println("<td ='40%' ><select name='TXT_STATUS' class='txt_input' style=\"width:100px;\" >");
				out.println("<option value=\"ALL\" SELECTED > All </option>");
				out.println("<option value=\"ACTIVATED\" > Active </option>");
				out.println("<option value=\"NON_ACTIVE\" >Non - Active</option>");				
				out.println("</select>");
				out.println("</td>");
				out.println("</tr>");
				...
				*/				
				...
				out.println("<tr style='{visibility:hidden;}' "); // Hidden By Kanishka On 01-07-2014
				...
				String m_status = req.getParameter("status"); // added by udara 22-01-2015
				String m_status_query = ""; // added by udara 22-01-2015
				
				if(m_status.equals("ALL"))
					m_status_query = " ";
				else if(m_status.equals("ACTIVATED"))
					m_status_query = " AND A.APPLICATION_STATUS = 'ACTIVATED' ";
				else if(m_status.equals("NON_ACTIVE"))
					m_status_query = " AND A.APPLICATION_STATUS <> 'ACTIVATED' ";
				...
				if(m_scr_name.equals("New")){
					...
					// commented by udara 22-01-2015
					/*
					if(m_finance_no.equals("")||m_finance_no.equals(null)||m_finance_no.equals(" ")){												
						...
						//out.println(" "+
							...
							//" WHERE C.BUSINESS_TYPE NOT IN('Rebewal','NEW')) "+ // commented by udara 21-01-2015 // BUSTYPE ADDED MILINDA 2014-06-09
							//" WHERE C.BUSINESS_TYPE NOT IN ('RENEWAL','NEW')) "+ // added by udara 21-01-2015
							" ) "+ // added by udara 21-01-2015
							// " AND A.INSURANCE_DONE_BY ='"+m_insuarane_type+"' "+ 
							
							//	" AND A.DIVISION_CODE  = '"+m_devision_type+"'  "+ //Added By Sanudn on 08-12-2008  //Commented By Kanishka On 01-07-2014							
							...
							// commented by udara 21-01-2015
							
							//" AND A.FINANCE_NO NOT IN(SELECT C.FINANCE_NO  "+
							//" FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA C where C.FINANCE_NO=A.FINANCE_NO)"+//ADDED MILINDA 2014-06-09
							
							
							//" AND A.APPLICATION_STATUS='ACTIVATED' "); // commented by udara 21-01-2015 //TEMP UNCOMMENTED BY MILINDA 2014-06-09 
						    " AND A.APPLICATION_STATUS NOT IN ('TERM','TERMINATED','NORM_TERMI') "); // released by udara 21-01-2015
					}
					else{
						rs=stmt.executeQuery(" SELECT DISTINCT A.FINANCE_NO, "+//1
							...
							" AND A.APPLICATION_STATUS NOT IN ('TERM','TERMINATED','NORM_TERMI') "); 
					}
					*/
					...
					if(m_finance_no.equals("")||m_finance_no.equals(null)||m_finance_no.equals(" ")){												
						rs=stmt.executeQuery("  "+
							...
							" A.CLIENT_CODE, "+//2
							" TO_CHAR(A.ACTIVATED_DATE,'DD-MM-YYYY'), "+//3
							" "+m_schema_name+".AF_CO_GET_APP_STATUS(A.APPLICATION_NO), "+ // mod by udara 22-01-2014" DECODE(A.APPLICATION_STATUS,'ACTIVATED','Activated'), "+//4`
							" "+m_schema_name+".AF_CO_GET_CLIENT_NAME(A.CLIENT_CODE), "+//5
							" A.INSURANCE_DONE_BY, "+//6
							" A.APPLICATION_NO "+//7
							" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A , "+
							"      "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS B "+                                                     
							" WHERE A.APPLICATION_NO = B.APPLICATION_NO "+
							//" AND (A.FINANCE_NO) NOT IN( "+ // commented by udara 10-07-2018
							" AND (A.FINANCE_NO,B.INVOICE_NO) NOT IN( "+ // added by udara 10-07-2018
										" SELECT C.FINANCE_NO, C.PRO_INVOICE_NO "+
										" FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA C "+
									" ) "+ 
							...
							" AND A.FINANCE_NO IS NOT NULL   "+ // added by udara 10-07-2018
							" AND TRUNC(A.ENT_DATE) <=TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+ 
							" AND TRUNC(A.ENT_DATE) >=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ 
						    " AND A.APPLICATION_STATUS NOT IN ('TERMI','TERMINATED','NORM_TERMI','CANCEL_PO','REJECT') "+
							" AND A.TRANSACTION_TYPE <> 'LOANS'  "+ // added by udara 10-10-2018
							" "+m_status_query+" "+
							" "); 
					}
					else{
					...
					while(more){
						...
						rs1=stmt1.executeQuery("SELECT B.MAKE_CODE || '-' || B.MODEL_CODE || '-' ||A.ENGINE_NO || '-'|| A.REG_NO, "+//A.REG_NO Added By SJ on 28-11-2008
							...
							//" AND D.DIVISION_CODE     = '"+m_devision_type+"'  "+ //Added By Sanudn on 08-12-2008 // commented by udara 25-07-2014
						...
						out.println("<td width='10%' align ='center'><input type='button' name='butt_detail' class='mainbut' value='Details' onclick=\"select_assets('"+rs.getString(7)+"','"+m_devision_type+"',document.Form1.TXT_ASSET_DETA_"+j+",'new'); setTimeout(function(){load_scr('"+rs.getString(1)+"','"+rs.getString(5)+"','"+j+"','','','"+rs.getString(7)+"')},3000);\"></td>"); // 2000
					...
					if(m_finance_no.equals("")){	
						rs=stmt.executeQuery(" SELECT A.FINANCE_NO, "+//1
							...
							//" AND B.APPLICATION_STATUS='ACTIVATED' "+	 // commented by udara 20-02-2014
							" AND B.APPLICATION_STATUS NOT IN ('TERMI','TERMINATED','NORM_TERMI','CANCEL_PO','REJECT') "+ // added by udara 20-02-2014
				...
				*/
				...
				out.println("		if(confirm(\"Are you sure you want to clear the screen?\")){ "); 
				out.println("          m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Asset_Insurnce_Detail_new?chksql=load_details_btt&sr_name=\"+m_sr_name+\"&invo_no=\"+m_invoice_no+\"&finance_no=\"+m_finance_no+\"&clint_name=\"+m_clint_name+\"&asst_deta=\"+m_asst_deta+\"&business_type=\"+m_business_type+\"&policy_no=\"+m_policy_no+\"&debit_note_no=\"+m_debit_note_no+\"&price_no=\"+m_price_no+\"&app_no=\"+m_app_no;");
				...
				out.println("	  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=start_date_check_new&hidden_fin_no=\"+document.Form2.hid_fi_no_val.value+\"&hidden_policy_no=\"+document.Form2.hid_policy_no.value+\"&hidden_debit_note_no=\"+document.Form2.hid_debit_note_no.value+\"&start_date=\"+v_date+\"-\"+v_month+\"-\"+val;");
				out.println("		load_interface(m_url,'XML');");
				out.println("function get_vector(data_vec) {");
				out.println("	if(data_vec.length>0  && document.Form2.hid_chk_status.value=='start_date_check' ){");				
				out.println("     document.Form2.end_dd.value=data_vec[1];");
				out.println("     document.Form2.end_mm.value=data_vec[2]");
				out.println("     document.Form2.end_yy.value=data_vec[3];");	
				out.println("}");
				//endded by kanchana.
				out.println("date1=v_date+'-'+v_month+'-'+val;");
				...
				out.println("document.Form2.txt_bas_pre.disabled=false;");
				out.println("document.Form2.txt_rcc_tc.disabled=false;");
				out.println("document.Form2.txt_bas_com.disabled=false;");
				out.println("document.Form2.txt_rcc_comm.disabled=false;");
				out.println("document.Form2.txt_tot_com.disabled=false;");
				out.println("document.Form2.txt_vat_total_com.disabled=false;");
				out.println("document.Form2.txt_tax_total.disabled=false;");
				...
				*/
				...
				// added by udara 20-06-2014
				out.println("function submit_data(){");
				
				//out.println(" alert('Validate policy no');  ");
				//out.println(" val_policy_no();  "); 
				
				out.println("if(validate_data()){");
				// commented by udara 11-11-2014
				/*
				out.println("    for (var i=0; i < document.Form2.elements.length; i++ ) {");
				out.println("     document.Form2.elements[i].disabled=false;");
				out.println("    }");
				*/
				...
				out.println("document.Form2.txt_policy_no.disabled=false;");
				//out.println("document.Form2.txt_bas_pre.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_rcc_tc.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_bas_com.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_rcc_comm.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_tot_com.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_vat_total_com.disabled=false;"); // commented by udara 12-11-2014
				//out.println("document.Form2.txt_tax_total.disabled=false;"); // commented by udara 12-11-2014
				
				out.println("    if(document.Form2.hid_ins_done_by.value=='LICENSEE'){    ");
				
				//out.println("       alert(document.Form2.txt_premium.value); ");
				
		       out.println("        if(unformat_noobject(document.Form2.txt_premium.value)>0){ "); // released by udara 03-09-2015	//--Comment by Prabash on 22-01-2015-15405
				//out.println("        if(document.Form2.txt_premium.value>0){ "); 
				out.println("			if(confirm(\"Are you sure, you want to save data?\")){ ");
				...
				out.println("        }");
				out.println("        else{"); // released by udara 03-09-2015
				out.println("        		alert('Total premium cannot be 0'); ");
				out.println("        }");
				...
				//Added by Kanchana.-------------------------------------------------------------------------------------------------------------------
				out.println("else if(document.Form2.hid_ins_done_by.value=='CLIENT'){  ");
				out.println("var m_start_date_diff_1=document.getElementById('hid_start_date_diff').value ");
				out.println("    if(m_start_date_diff_1<0){    ");
				out.println("   alert('Insurance Start Date can not be greater than the system date'); "); 		
				out.println("   } else{ "); 
				out.println("					if(confirm(\"Are you sure, you want to save data?\")){ ");
				...
				//Endded by Kanchana Karunarathna.
				out.println("    else{");
				
				out.println("     		if(unformat_noobject(document.Form2.txt_sum_in.value)>0){ ");
				...
				out.println("    		}");
				out.println("    		else{");
				out.println("        		alert('Sum Inssured cannot be 0'); ");
				out.println("    		}");
				...
				out.println("function val_policy_no(){");
				...
				out.println(" } ");
				out.println(" else if(document.Form2.txt_Debit_Note_no.value==''){ ");
				out.println(" 	alert('Debit note ref number cannot be blank');  ");
				out.println(" 	document.Form2.txt_policy_no.focus(); ");
				out.println(" } ");
				
				out.println(" else{ ");
				
				out.println("     if(vector_flag == ''){"); // added by udara 15-03-2018
				
				out.println("	     document.Form2.hid_chk_status.value='val_policy_no'; "); 
				out.println("	     m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=val_policy_no&policy_no=\"+document.Form2.txt_policy_no.value+\"&debit_note_no=\"+document.Form2.txt_Debit_Note_no.value;");
				out.println("	     load_interface(m_url,'XML');");
				
				out.println("     }	"); // added by udara 15-03-2018
				out.println("     else{	"); // added by udara 15-03-2018
				out.println("        alert('Wait until end date loads finish');	");
				out.println("     }	"); // added by udara 15-03-2018
				...
				/*
				// added by udara 13-02-2014
				out.println("function set_pricing_no(m_app_no,m_devision_type,m_check_value){"); 
				out.println("	document.Form2.hid_chk_status.value='set_pricing_no'; ");
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=set_pricing_no&app_no=\"+m_app_no+\"&devision_type=\"+m_devision_type+\"&check_value=\"+m_check_value;");
				out.println("		load_interface(m_url,'XML');");
				out.println("}"); 
				// end by udara 13-02-2014
				*/
				...
				out.println(" var vector_flag = '';   "); // added by udara 15-03-2018
				...
				out.println("	else if(data_vec.length>0  && document.Form2.hid_chk_status.value=='set_end_date' ){");
				...
				// added by udara 13-02-2014
				out.println("	else if(data_vec.length>0  && document.Form2.hid_chk_status.value=='set_pricing_no' ){");
				out.println("			alert(data_vec[0]);");
				out.println("	}");
				
				
				//Added by Kanchana.-----------------------------------------------------
				out.println("	else if(data_vec.length>0  && document.Form2.hid_chk_status.value=='check_date_range' && data_vec[0]>0  ){"); 
				//	out.println("			alert(data_vec[0]);");
				...
				out.println("	else if(data_vec.length>0  && document.Form2.hid_chk_status.value=='start_date_check' ){");
				// out.println("			alert('stat_date '+data_vec[0]+' '+data_vec[1]+' '+ data_vec[2]);");
			/*	out.println(" 			 if((Number(data_vec[0])>Number(0))&&(Number(data_vec[0]>Number(30))) ){ ");
				out.println("					alert('Insurance Start date is less than 30 days.');");
				out.println("	}");	
					
				out.println("			if(((parseInt(data_vec[0]))*(-1))>Number(60)) { ");
				out.println("					alert('Insurance Start date is grater than 60 days.');");
				//	out.println(" make_request_date(); ");
				out.println("								}");  */ //commented to remove start date validation.
				//Set end date
				out.println("			document.Form2.end_dd.value = data_vec[1];");
				out.println("			document.Form2.end_mm.value = data_vec[2];");
				out.println("			document.Form2.end_yy.value = data_vec[3];");
				out.println("	document.Form2.hid_start_date_diff.value=data_vec[0];");
				out.println(" make_request_date(); ");  
			//	out.println("					alert(document.Form2.hid_start_date_diff.value);");
				out.println("	 vector_flag =''; "); // added by udara 15-03-2018
			
				out.println("	}");
				//Ended by Kanchana.-----------------------------------------

				// added by udara 30-06-2014
				out.println("	else if(data_vec.length>0  && document.Form2.hid_chk_status.value=='val_policy_no' ){");
				//out.println("	  if(data_vec[0]=='1'){ "); // commented by udara 15-12-2014
				out.println("	  if(data_vec[0]>='1'){ "); // added by udara 15-12-2014
				out.println("	      alert('Existing policy number : ' + data_vec[2]);    ");
				out.println("         document.Form2.txt_policy_no.value = '';  ");
				out.println("         document.Form2.txt_policy_no.focus();  ");
				out.println("	  }");
				//out.println("	  else if(data_vec[1]=='1'){ "); // commented by udara 15-12-2014
				// commented by udara 31-12-2014
				/*
				out.println("	  else if(data_vec[1]>='1'){ "); // added by udara 15-12-2014
				out.println("	      alert('Existing debit note ref number : ' + data_vec[3]);    ");
				out.println("         document.Form2.txt_Debit_Note_no.value = '';  ");
				out.println("         document.Form2.txt_Debit_Note_no.focus();  ");
				out.println("	  }");
				*/
				...
				out.println("	else if( document.Form2.hid_chk_status.value=='get_basic_rcc_comm_rate' ){");
				...
				out.println("           get_flag_status(); "); // added by udara 19-07-2018
				...
				*/
				...
				// added by udara 21-07-2014
				out.println(" 	   var m_insurance_agent = document.Form2.txt_in_company_code.value; "); 
				out.println(" 	   var m_sum_insured     = unformat_noobject(document.Form2.txt_sum_in.value); "); 
				out.println(" 	   var m_sub_cat    = document.Form2.TXT_ITEM_SUB_CAT.value; "); 
				// end by udara 21-07-2014
				
				//out.println("      alert(m_insurance_agent + ' ' + m_sum_insured + ' ' + m_sub_cat); ");
				
				out.println("	   document.Form2.hid_chk_status.value='get_basic_rcc_comm_rate'; ");
				out.println("	   m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=get_basic_rcc_comm_rate&insurance_agent=\"+m_insurance_agent+\"&sum_insured=\"+m_sum_insured+\"&sub_cat=\"+m_sub_cat;");
				out.println("	   load_interface(m_url,'XML');");
				//out.println("      window.open(m_url);  ");
				...
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=check_date_range&hidden_fin_no=\"+document.Form2.hid_fi_no_val.value+\"&start_date=\"+document.Form2.start_dd.value+\"-\"+document.Form2.start_mm.value+\"-\"+document.Form2.start_yy.value;");
				...
				out.println("   vector_flag = 'start_date_check'; "); // added by udara 15-03-2018
				out.println("	document.Form2.hid_chk_status.value='start_date_check'; ");
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=start_date_check_new&hidden_fin_no=\"+document.Form2.hid_fi_no_val.value+\"&hidden_policy_no=\"+document.Form2.hid_policy_no.value+\"&hidden_debit_note_no=\"+document.Form2.hid_debit_note_no.value+\"&start_date=\"+document.Form2.start_dd.value+\"-\"+document.Form2.start_mm.value+\"-\"+document.Form2.start_yy.value;");
				out.println("		load_interface(m_url,'XML');");
				out.println("}");
				...
				//out.println(" var letterNumber = /^[0-9a-zA-Z_-]+$/;  ");
				out.println(" var letterNumber = /^[0-9\\/a-zA-Z_-]+$/;  ");
				...
				out.println("<INPUT TYPE='Hidden' NAME='hid_get_flag_status' VALUE=\"\" >"); // added by udara 19-07-2018
				...
				out.println("<tr>");			
				//out.println("<td width='6%'></td>");  
				//out.println("<td width='10%' align='center'><input type=\"button\" class='mainbut' onMouseout='load_roll_out_value();' onMouseOver='load_roll_value(\"Save\");' onClick='submit_data()' value=\"Save\"></td>");  // commented by udara 30-06-2014
				out.println("<td width='10%' align='center'><input type=\"button\" class='mainbut' onMouseout='load_roll_out_value();' onMouseOver='load_roll_value(\"Save\");' onClick='val_policy_no()' value=\"Save\"></td>"); // added by udara 30-06-2014
				out.println("<td width='10%' align='center'><input type=\"button\" class='mainbut' onMouseout='load_roll_out_value();' onMouseOver='load_roll_value(\"Cancel\");' onclick='clear_window()' value=\"Cancel\"></td>");  
				...
				out.println("<BR>");
				/*
					rs=stmt.executeQuery(" SELECT DECODE(A.DIVISION_CODE,'AF','Lease','BD','Bike','-') "+ 
											" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A "+		
											" WHERE A.FINANCE_NO = '"+m_finance_no+"'");*/
				
				
				//rs=stmt.executeQuery(" SELECT DECODE (A.DIVISION_CODE, 'AF', 'Lease', 'BD', 'Bike', '-'), "+ // commented by udara 11-02-2014
				...
				rs=stmt.executeQuery(" SELECT DECODE (A.DIVISION_CODE, 'AF', 'Hire Purchase', 'BD', 'Bike', '-'), "+ // added by udara 11-02-2014
					" TO_CHAR(A.ACTIVATED_DATE,'DD'),TO_CHAR(A.ACTIVATED_DATE,'MM'),TO_CHAR(A.ACTIVATED_DATE,'YYYY'), "+
					" TO_CHAR(ADD_MONTHS (A.ACTIVATED_DATE, 12),'DD'),TO_CHAR(ADD_MONTHS (A.ACTIVATED_DATE, 12),'MM'),TO_CHAR(ADD_MONTHS (A.ACTIVATED_DATE, 12),'YYYY'), "+
				...
				*/
				...
				out.println("<tr>");
				out.println("<td width='20%' align ='left'>Finance No</td>"); 
				out.println("<td width='30%' align ='left'><b>"+m_finance_no+"</b><input type='hidden'  name='hid_fi_no_val' value='"+m_finance_no+"' > </td>"); 
				...
				if(m_scr_name.equals("New")){
				 ...
				 //Modified by Kanchana for the issue number 18827 	
					out.println("<tr>");
					out.println("<td width='20%' align ='left'>Start Date</td>"); 
					out.println("<td width='30%' ><input name=\"start_dd\" value=\""+m_start_dd+"\"  type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=\"check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);\" onchange=\"start_date_check();\" > "); //onchange=\" set_end_date()\"  commented to hid  date range validation since client didn't requested.#18827
					out.println("    <input name=\"start_mm\" type=\"text\" value=\""+m_start_mm+"\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\"  onblur=\"check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);\"  onchange=\"start_date_check();\" > "); //onchange=\"set_end_date()\" 
					out.println("    <input name=\"start_yy\"  type=\"text\" value=\""+m_start_yy+"\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onblur=\" check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);\" onchange=\"start_date_check();\"  ><a href style='{cursor:hand; }' onclick=load_calendar('2')> Calendar</a> "); // check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy); //onchange=\"start_date_check()\"
					out.println("</td>");
					...
					out.println("<td width='30%' >");
					out.println("   <DIV id='DIV_TXT_ITEM_SUB_CAT_DESC_2'  class=div_input> </DIV> ");
					out.println("</td>"); 
					out.println("<td width='20%' align ='left' valign=top> &nbsp; </td>");
					out.println("<td width='*%'></td>"); 
					out.println("</tr>");
					...
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" style=\"width:150px;text-align:right\" name=\"txt_premium\" value=\"0.00\" maxlength=18 onblur=\"check_amt(this,18);\" disabled ></td>"); //mod by udara 03-11-2014 disabled	
					out.println("</tr>");
					...
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" style=\"width:150px;text-align:right\" name=\"txt_payable_premium\" value=\"0.00\" maxlength=18 onblur=\"check_amt(this,18)\"disabled>"); 			
					out.println("</td>");
					// end by udara 06-05-2014
					
					//out.println("<td width='*%'></td>");	// commented by udara 06-05-2014
					out.println("</tr>");
					...
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" style=\"width:150px;text-align:right\" name=\"txt_bas_pre\" value=\"0.00\" maxlength=18 onblur=\"check_amt(this,18);check_amt2()\"disabled></td>"); 			
					out.println("</tr>");
					...
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" name=\"txt_policy_no\" style=\"width:150px;\" maxlength=20 value=\""+m_policy_no+"\"></td>"); 			
					...
					*/
					...
					out.println("<tr>");
					out.println("<td width='20%' align ='left'>Start Date</td>"); 
					out.println("<td width='30%' ><input name=\"start_dd\" value=\""+m_start_date_dd+"\"  type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy)> ");
					out.println("    <input name=\"start_mm\" type=\"text\" value=\""+m_start_date_mm+"\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy)> ");
					out.println("    <input name=\"start_yy\"  type=\"text\" value=\""+m_start_date_yy+"\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onblur=\"set_end_date();\" ><a href style='{cursor:hand; }' onclick=load_calendar('2')> Calendar</a> "); // check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);
					out.println("</td>");
					...
					out.println("<tr>");
					out.println("<td width='20%' align ='left'>Sum Inssured</td>"); 
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\"style=\"width:150px;text-align:right\" name=\"txt_sum_in\"value=\""+nf.format(m_sum_insur)+"\" maxlength=20 onblur=\"set_rcc_value(this);check_amt(this,18); \"></td>"); 			
					
					out.println("<td width='20%' align ='left'>Total Premium</td>"); 
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" style=\"width:150px;text-align:right\" name=\"txt_premium\" value=\""+nf.format(m_premium)+"\" maxlength=18 onblur=\"check_amt(this,18);check_amt2()\" disabled ></td>"); // mod by udara 03-11-2014 disabled			
					out.println("</tr>");
					...
					out.println("<tr>");
					out.println("<td width='20%' align ='left'>Basic Permium</td>"); 
					out.println("<td width='30%' align ='left'><input type=\"text\" class=\"txt_input\" style=\"width:150px;text-align:right\" name=\"txt_bas_pre\" value=\""+nf.format(m_basic_premi)+"\" maxlength=18 onblur=\"check_amt(this,18);check_amt2()\"disabled></td>"); 			
					out.println("</tr>");
