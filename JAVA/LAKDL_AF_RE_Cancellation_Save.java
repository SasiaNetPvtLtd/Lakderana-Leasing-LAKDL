...
public class LAKDL_AF_RE_Cancellation_Save extends HttpServlet {
      ...
      m_username 						= m_sn_methods.username;
       ...
       if (m_scr_name.trim().equals("AF_RE_CANCELLATION")){
				...
				synchronized (this){
						...
						String m_client_code = m_sn_methods.met_formdata(reqstr,"CLIENT_CODE").toUpperCase();
						
						m_option_name=m_sn_methods.met_formdata(reqstr,"hid_status").toUpperCase();
						
					 	callstmt1 = conn.prepareCall( "BEGIN "+m_schema_name+"."+ 
					  "AF_RE_SAVE_RE_CANCELLATION(:1,:2,:3,:4,:5);END;");
						callstmt1.setString(1,m_rec_no);
						callstmt1.setString(2 ,m_username);
						callstmt1.setString(3,m_sn_methods.met_formdata(reqstr,"REMARK").toUpperCase());
						callstmt1.setString(4 ,m_client_code);
						//callstmt1.setString(5 ,m_scr_name);//value date
