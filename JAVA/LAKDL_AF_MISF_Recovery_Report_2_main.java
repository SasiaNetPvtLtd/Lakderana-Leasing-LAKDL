...
public class LAKDL_AF_MISF_Recovery_Report_2_main extends javax.servlet.http.HttpServlet { 
	...
	public  void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { // synchronized
		...
		try { 
			...
			if(m_chksql.equals("run_report")){ 
				...
				try{
					callstmt1 = conn.prepareCall("BEGIN "+m_schema_name+".AF_RE_SAVE_RECOVER_RPT_SUMRIZE(:1,:2,:3,:4,:5,:6,:7,:8,:9);END;"); // added by udara 13-08-2015
			...
			if(m_chksql.equals("main_page")){ 
				...
				out.println("<HTML>"); 
				out.println("<HEAD>"); 
				out.println("<TITLE>Collection - Recovery Report - New</TITLE>"); 
				out.println("</HEAD>"); 
				...
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Recovery_Report_2_main?chksql=run_report&finance_no=\"+finance_no+\"&date=\"+m_date+\"&location_id=\"+document.Form1.TXT_LOCATION_CODE.value+\"&user_id=\"+document.Form1.TXT_USER.value+\"&cr_officer=\"+m_cr_officer+\"&active_yard_status=\"+m_active_yard_status+\"&perform_stat=\"+m_perform_stat+\"&region=\"+m_region;"); // added by udara 13-08-2015 
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Recovery_Report_2_main?chksql=main_page&generate=page';"); 
				out.println("		}"); 
				out.println("}"); 
				...
				out.println("if(validate_data()){");
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Recovery_Report_2_main?chksql=main_page&generate=print_report&location=\"+m_location+\"&officer=\"+m_officer+\"&date=\"+date;");	
				...
				Sql_data="  "+
					...
					" FROM "+m_schema_name+".AF_MIS_RECOV_RPT_SUMRIZE_MAS A, "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
					...
					" AND B.ACTIVATED_DATE < TRUNC(TO_DATE('"+m_date+"','DD-MM-YYYY'),'MM') "+
				...
				Sql_data = Sql_data + "    "+m_active_status_string+"   "+
					" AND TRUNC(REPORT_DATE) = TO_DATE('"+m_date+"','DD-MM-YYYY') "+
				...
				if(count>0){
					...
					// added by udara 10-11-2015
					Sql_data=" "+
						" SELECT NVL(SUM(NEW_TEMP_FUTURE_RENT),0) FROM (  "+            
						
						" SELECT NVL(A.NEW_TEMP_FUTURE_RENT,0)  NEW_TEMP_FUTURE_RENT "+
				...
				Sql_data = "  "+
					...
					" AND A.TOTAL_PERIOD > 0  "+
					" AND B.ACTIVATED_DATE >= TRUNC(TO_DATE('"+m_date+"','DD-MM-YYYY'),'MM')  "+
					" AND B.ACTIVATED_DATE <= TRUNC(LAST_DAY(TO_DATE('"+m_date+"','DD-MM-YYYY')),'DD')  "+
					" AND A.PERFORM_STATUS LIKE '"+m_perform_status+"%'   "+
					" AND UPPER (NVL("+m_schema_name+".AF_CO_GET_CR_OFFICER (B.INQUARY_NO),' ')) LIKE UPPER('"+m_cr_officer+"%')  "+
				...
				Sql_data="  "+	
					...
					" FROM "+m_schema_name+".AF_RECOV_RPT_SUM_SUMRIZE_ALL A, "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
					" WHERE A.ENT_USER='"+m_username+"' "+
					//" WHERE ENT_USER='LAKDLALL' "+
				...
				Sql_data=" "+
					...
					" AND    A.FINANCE_NO = B.FINANCE_NO "+
					" AND    A.FINANCE_NO IN (SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOV_RPT_SUMRIZE_MAS WHERE ENT_USER = '"+m_username+"') "+ // added by udara 28-11-2013 // commented by udara 18-01-2015
					//" AND    A.FINANCE_NO IN (SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOV_RPT_SUMRIZE_MAS WHERE ENT_USER = 'LAKDLALL' AND REPORT_DATE = TO_DATE('"+m_date+"','DD-MM-YYYY')) "+ // added by udara 18-01-2016
					" AND    B.BRANCH_CODE LIKE '"+m_location+"%' "+
					" AND    A.INVOICE_NO IN "+
					" ( SELECT  INVOICE_NO "+
					" FROM     "+m_schema_name+".AF_CO_PRO_INVOICE "+
					//" WHERE  FINANCE_NO LIKE '%%' "+
					" WHERE  FINANCE_NO LIKE '%"+m_finance_no+"%' "+ // udara 18-01-2016
					" AND ACTIVE_STATUS = 'Y' "+
				...
				*/
				...
				// commented by udara 18-01-2016
				/*
				Sql_data=" "+
					" select  "+
					" NVL(GET_DIFF_CR_AMOUNTS('"+m_date+"','RENTAL','LAKDLALL'),0) CR_NOTES_RENTAL, "+
					" NVL(GET_DIFF_CR_AMOUNTS('"+m_date+"','ARREARS','LAKDLALL'),0) CR_NOTES_ARREARS, "+
					" NVL(GET_DIFF_CR_AMOUNTS('"+m_date+"','OTHER_CURR','LAKDLALL'),0) CR_NOTES_OTHER_CURR, "+
					" NVL(GET_DIFF_CR_AMOUNTS('"+m_date+"','OTHER_PREV','LAKDLALL'),0) CR_NOTES_OTHER_PREV "+
					" from DUAL "+
					" ";
				*/
				...
				Sql_data=" "+
					" select  "+
					" NVL(GET_DIFF_CR_AMOUNTS_RECOV_AUTO('"+m_date+"','RENTAL','LAKDLALL'),0) CR_NOTES_RENTAL, "+
					" NVL(GET_DIFF_CR_AMOUNTS_RECOV_AUTO('"+m_date+"','ARREARS','LAKDLALL'),0) CR_NOTES_ARREARS, "+
					" NVL(GET_DIFF_CR_AMOUNTS_RECOV_AUTO('"+m_date+"','OTHER_CURR','LAKDLALL'),0) CR_NOTES_OTHER_CURR, "+
					" NVL(GET_DIFF_CR_AMOUNTS_RECOV_AUTO('"+m_date+"','OTHER_PREV','LAKDLALL'),0) CR_NOTES_OTHER_PREV "+
					" from DUAL "+
					" ";
				*/
				...
				if(!m_finance_no.equals("")){	
						Sql_data=" "+
							" select  "+
							" NVL(GET_DIFF_CR_AMOU_RECOV_AUTO_2('"+m_date+"','RENTAL','LAKDLALL','"+m_finance_no+"'),0) CR_NOTES_RENTAL, "+
							" NVL(GET_DIFF_CR_AMOU_RECOV_AUTO_2('"+m_date+"','ARREARS','LAKDLALL','"+m_finance_no+"'),0) CR_NOTES_ARREARS, "+
							" NVL(GET_DIFF_CR_AMOU_RECOV_AUTO_2('"+m_date+"','OTHER_CURR','LAKDLALL','"+m_finance_no+"'),0) CR_NOTES_OTHER_CURR, "+
							" NVL(GET_DIFF_CR_AMOU_RECOV_AUTO_2('"+m_date+"','OTHER_PREV','LAKDLALL','"+m_finance_no+"'),0) CR_NOTES_OTHER_PREV "+
							" from DUAL "+
							" ";	
				}
				else{
