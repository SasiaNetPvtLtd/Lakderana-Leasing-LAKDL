...
public class LAKDL_AF_RPT_Temprery_Reciept_Details extends javax.servlet.http.HttpServlet { 
...
public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
      ...
      LAKDL_AF_CO_conn_methods con_method = new LAKDL_AF_CO_conn_methods();
	  	...
	  	out.println("		}"); 
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_RPT_Temprery_Reciept_Details?chksql=main_page1';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\"  Temporary Reciept Report - \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\"  Temporary Reciept Report \";"); 
			out.println("}"); 
			...
			// Added By Samitha Kulatilaka On 2009-10-20
			out.println("var validate_flag = true;");
			out.println("validate_flag = checkMonthLength(document.Form1.TXT_FROM_DATE_DD, document.Form1.TXT_FROM_DATE_MM, document.Form1.TXT_FROM_DATE_YY);");
			out.println("if(validate_flag == true) {");
			out.println("	validate_flag = checkMonthLength(document.Form1.TXT_TO_DATE_DD, document.Form1.TXT_TO_DATE_MM, document.Form1.TXT_TO_DATE_YY);");
			out.println("}");
			out.println("if(validate_flag == false) {");
			out.println("	return validate_flag");
			out.println("}");
			...
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td align=\"left\" class=\"pdn_txtpos2\" style=\"height: 18px\" id=help_box>Temporary Reciept Report </td>");
			out.println("</tr>");
			out.println("<tr>");
      ...
      String m_date_from=req.getParameter("date_from").trim();
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Temporary Reciept Report</TITLE>"); 
			out.println("</HEAD>"); 
     ...
     // out.println("	m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RPT_Advertisement_Offers_Report?chksql=MAIN&OFFER_NO=&from_date="+m_from_date+"&to_date="+m_to_date+"&sort_column=\"+m_sort_col+\"&order_by_type=\"+m_order_by_type;"); 
			...
			pstmt = conn.prepareStatement("SELECT  A.TEMP_REC_NO,A.FINANCE_NO,A.CLIENT_CODE, "+
			                              " B.FULL_NAME, TO_CHAR(A.TRN_DATE,'DD-MON-YYYY') TRN_DATE, "+
																		...
																		" AND TO_DATE(TO_CHAR(A.TRN_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_date_from+"','DD-MM-YYYY') AND "+
			...
			pstmt = conn.prepareStatement("SELECT  A.TEMP_REC_NO,A.FINANCE_NO,A.CLIENT_CODE, "+
			                              " B.FULL_NAME, TO_CHAR(A.TRN_DATE,'DD-MON-YYYY') TRN_DATE, "+
																		...
																		" AND TO_DATE(TO_CHAR(A.TRN_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') >= TO_DATE('"+m_date_from+"','DD-MM-YYYY') AND "+
																		" TO_DATE(TO_CHAR(A.TRN_DATE,'DD-MM-YYYY'),'DD-MM-YYYY') <= TO_DATE('"+m_date_to+"','DD-MM-YYYY') "+
																		" AND A.TEMP_REC_NO='"+m_reciept_no+"' "+
																		" ORDER BY "+m_sort_column+" "+m_order_by_type+" ");
