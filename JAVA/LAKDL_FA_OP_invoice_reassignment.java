...
public class LAKDL_FA_OP_invoice_reassignment extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Operation Process - Invoice Reassignment</TITLE>"); 
			out.println("</HEAD>"); 
			...
			out.println("				obj1=document.Form1.elements[\"TXT_INVOICE_ADJUST_AMOUNT_\"+k].value;");
			out.println("				obj2=document.Form1.elements[\"TXT_INVOICE_AMOUNT_\"+k].value;");
			out.println("				obj3=document.Form1.elements[\"TXT_INVOICE_ADJUST_OK_\"+k];");
			out.println("				if(obj3.checked){");
			out.println("					if(parseFloat(unformat_noobject(obj2))<=parseFloat(unformat_noobject(obj1))){");
			out.println("						alert(\"Invoice adjustment value should less than the original value\");");
			out.println("						m_flag=false;"); 
			out.println("					}");
			out.println("					else{"); 
			out.println("						m_flag=true;"); 
			...
			out.println("	if(parseInt(document.Form1.hid_invoice_count.value)>0){  "); 
			out.println("		for(k=1;k<=parseInt(document.Form1.hid_invoice_count.value);k++){");
			out.println("				obj1=document.Form1.elements[\"TXT_INVOICE_ADJUST_SOURCE_\"+k].value;");
			out.println("				obj3=document.Form1.elements[\"TXT_INVOICE_ADJUST_OK_\"+k];");
			out.println("				if(obj3.checked){");
			out.println("					if(obj1==\"\"){");
			...
			out.println("			}");
			out.println("			if(validate_data()){"); 
			out.println("				document.Form1.action='"+m_class_url+"/"+m_fschema_name+"FA_OP_Save_invoice_reassign';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"FA_OP_invoice_reassignment';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("    m_help_message = \"m_help_msg_LAKDL_FA_CR_INVOICE_REASSIGNMENT\";"); 
			...
			out.println("	help_box.innerHTML=\"  Operation Process - Invoice Reassignment - \"+m_val;"); 
			out.println("}"); 

			out.println("function load_roll_out_value(){");
			out.println("	help_box.innerHTML=\"  Operation Process - Invoice Reassignment - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
	  	...
	  	out.println("					}"); 
			...
			out.println("      if(document.Form1.TXT_INVOICE_BALANCE.value!=\"\"){ ");
			out.println("      invoice_balance_amount=parseFloat(unformat_noobject(document.Form1.TXT_INVOICE_BALANCE.value));"); 
			out.println("      }else ");
			out.println("      { ");
			out.println("      invoice_balance_amount=document.Form1.TXT_INVOICE_BALANCE.value; ");
			out.println("      } ");
			out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"FA_CR_PRO_sql_validations_normal?chksql=ADJUST_INVOICE_SEQUENCE_REASSIGN&batch_no=\"+document.Form1.TXT_INVOICE_BATCH_NO.value+\"&invoice_balance=\"+invoice_balance_amount;");
			out.println("		load_interface(m_url,'NO');");
			out.println("}");
			
			out.println("function get_vector_normal(m_data){");
			out.println("		invoice_detail_data.innerHTML=m_data;");
			out.println("		document.Form1.hid_invoice_count.value = document.Form1.NUM_CHKS.value;");
			out.println("}");
			...
			out.println(" 	m_sql = \"m_help_DIV_TXT_FACILITY_INVOICE_ADJUST_sql\";"); 
			...
			out.println(" 	m_sql = \"m_help_DIV_TXT_DEBTOR_INVOICE_ADJUEST_sql\";"); 
			...
			out.println("function check_balance_amount(){");
			out.println("   if(document.Form1.TXT_INVOICE_BALANCE.value!=\"\"){");
			out.println("    format_num(document.Form1.TXT_INVOICE_BALANCE,4); "); 
			out.println("   }");
			out.println("}");
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'> Operation Process - Invoice Reassignment </td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<td width=\"40%\"><input class=\"txt_input5\" type=\"text\" name=\"TXT_INVOICE_BATCH_DATE_DD\" maxlength=\"2\" size=\"2\" disabled>");
			out.println("<input class=\"txt_input5\" type=\"text\" name=\"TXT_INVOICE_BATCH_DATE_MM\" maxlength=\"2\" size=\"2\" disabled>");
			out.println("<input class=\"txt_input5\" type=\"text\" name=\"TXT_INVOICE_BATCH_DATE_YY\" maxlength=\"4\" size=\"4\" onblur=\"checkMonthLength(document.Form1.TXT_INVOICE_BATCH_DATE_DD,document.Form1.TXT_INVOICE_BATCH_DATE_MM,document.Form1.TXT_INVOICE_BATCH_DATE_YY)\" disabled>[DD-MM-YYYY]</td>");
			out.println("<td width='*%'></td>"); 
			out.println("</tr>");
			
			out.println("<tr >");
			out.println("<td width=\"15%\"><DIV id=\"DIV_TXT_INVOICE_BLANCE\" class=div_input>Invoice Balance </DIV></td>");
			out.println("<td width=\"40%\">");
			out.println("   <input class='txt_input' type='text' name='TXT_INVOICE_BALANCE' style=\"text-align:right\" onblur=\"check_balance_amount();\" maxlength='10' size='10'> ");
			out.println("   <input class='but_input' type='button' name='BUT_HELP_MAIN_3' value=\"Search\" onClick=\"makeRequest_update_invoice_detail();\">");
			out.println("</td>");
			out.println("<td width='*%'></td>"); 
			out.println("</tr>");
