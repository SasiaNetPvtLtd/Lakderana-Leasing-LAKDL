...
public class LAKDL_AF_MAS_display_Screen_Order extends javax.servlet.http.HttpServlet { 
  ...
  public ResultSet rs;
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		//	out.println("conn"+conn);
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>System Administration - Screen Order</TITLE>"); 
			out.println("</HEAD>"); 
			...
			out.println("var array_screen=new Array();");
	...
	//		out.println("'<TD WIDTH=\"30%\"     align=\"center\"><B>Screen Name</B></TD>'+");
		//	out.println("'<TD WIDTH=\"40%\"     align=\"center\"><B>Position</B></TD>'+");
		//	out.println("'</TR></table>';");
			
			out.println("m_writedata='<TR>' +");
	    out.println("'<TD><B>Row Number</B></TD><TD><B>Screen Name</B></TD><TD><B>Position</B></TD>' +");
	    out.println("'</TR>';");
      out.println("array_screen[lineno]=m_writedata;");		 
	    out.println("lineno=lineno+1;");
     	...
     	out.println("}");
			...
			out.println("function validate_number(size){");
			out.println("for(i=0;i<size;i++){");
			out.println("m_screen_name=\"TXT_SCREEN_NAME\"+i");
			out.println("document.Form1.elements[m_screen_name].disabled=true;");
			out.println("}");	
			out.println("}");	
			...
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MAS_sql_validations?chksql=m_prime_chk_LAKDL_AF_MAS_display_Screen_Order&data_val=\"+obj.value+\"&ac_status=Y\";");
	
		  //out.println("else");
		 // out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MAS_sql_validations?chksql=m_prime_chk_LAKDL_AF_MAS_display_Screen_Order_val&data_val=\"+obj.value+\"&ac_status=Y\";");
      ...
      out.println("alert('No Records To Save');");
			...
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MAS_Save_Screen_Position';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MAS_display_Screen_Order';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("    m_help_message = \"m_help_msg_LAKDL_AF_MAS_display_Screen_Order\";"); 
			...
			out.println("help_box.innerHTML=\" System Administration - Screen Order - \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\" System Administration - Screen Order - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
		...
		/*	out.println("function clear_data() {");
		  ...
		  out.println("j=1;");
			...
			out.println("m_row='<TD WIDTH=\"10%\">'+j+'</TD>';");		
		  out.println("m_scr_name='<TD WIDTH=\"30%\">'+data_vec[i]+'</TD>';");
			out.println("if(data_vec[i+1]==0){");
			out.println("m_position='<TD WIDTH=\"25%\"><INPUT TYPE=\"TEXT\" class=\"txt_input\" NAME=TXT_POSITION'+j+' VALUE=\"\" maxlength=\"2\" size=\"15\" onblur=\"val_num('+j+')\"></td>';");		
			out.println("}");
			out.println("else{");
			out.println("m_position='<TD WIDTH=\"25%\"><INPUT TYPE=\"TEXT\" class=\"txt_input\" NAME=TXT_POSITION'+j+' VALUE='+data_vec[i+1]+' maxlength=\"2\" size=\"15\" onblur=\"val_num('+j+')\"></td>';");			
			out.println("}");
			out.println("m_hid_input='<INPUT TYPE=\"Hidden\" NAME=hid_TXT_SCREEN_NAME'+j+'	VALUE='+data_vec[i]+'>';");
			 
	    
	    out.println("m_writedata='<TR>'+m_row+m_scr_name+m_position+'</TR>'+m_hid_input;");
     	out.println("array_screen[j]=m_writedata;");
    
		  out.println("m_table.innerHTML='<table align=\"center\" width=\"100%\" class=\"table\">'+");
	    out.println("array_screen.join(\" \")+'</table>';");
      out.println("j=j+1;");
			out.println("i=i+2;");
			...
			out.println("max=0");
		//	out.println("alert('Max Value'+m_max);");
			out.println("if(!isPosInteger(document.Form1.elements[m_position].value)){");
			out.println("alert('Please Enter a Number');");
			out.println("document.Form1.elements[m_position].value=\"\"; ");
			out.println("document.Form1.elements[m_position].focus();}");
			
						
			//out.println("if(document.Form1.elements[m_position].value!=\"\"){");
			
			out.println("for(i=1;i<arr_size;i++){");
			out.println("m_pos=\"TXT_POSITION\"+i;");
			
			out.println("if(i==rowNo || document.Form1.elements[m_pos].value ==\"\" ){");
			out.println("continue;}");
			
			out.println("if(document.Form1.elements[m_pos].value>max)");
			out.println("max=document.Form1.elements[m_pos].value");
			
			out.println("if(document.Form1.elements[m_position].value==document.Form1.elements[m_pos].value){");
			out.println("alert('Position Number can not be duplicated');");
			out.println("document.Form1.elements[m_position].value=\"\"; ");
			out.println("document.Form1.elements[m_position].focus();}");
			...
			//out.println("var max_next=0;");
		  out.println("max=parseInt(max)+1;");
		  	
		 
			out.println("if(document.Form1.elements[m_position].value>max){");
			out.println("alert('The Position Number Should Be Next Max Value');");
			out.println("document.Form1.elements[m_position].value=\"\"; ");
			out.println("document.Form1.elements[m_position].focus();}");			
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>System Administration - Screen Order</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<td width='40%' ><select name=\"TXT_DIVISION_CODE\" class=\"txt_input\" onChange=\"assignState('M1'),makeRequest(TXT_DIVISION_CODE)\" style='width:200'>");
							
			  rs = stmt.executeQuery(" SELECT DIVISION_CODE,DESCRIPTION  FROM LAKDL.CO_CO_MAS_DIVISION ORDER BY DESCRIPTION ");
