...
public class LAKDL_AF_MK_display_performa_invoice extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_prof_inv_no==null){
				m_prof_inv_no="";
			}
			//out.println(m_prof_inv_no);
			...
			//out.println(m_my_screen);	
			
			String m_refin_status = req.getParameter("refin_status"); // added by udara 09-05-2014
			String m_refin_no     = req.getParameter("refin_no"); // added by udara 09-05-2014
			
			String m_appNo = req.getParameter("APP_NO");
			//String m_app_name = req.getParameter("APP_NAME"); //modified by nuwan de silva 07-08-07
			...
			/*rs = stmt.executeQuery(" SELECT "+
                                ...
                                " WHERE UPPER(APPLICATION_NO)=UPPER('"+m_appNo+"'))");
    
			boolean more5 = rs.next();			
			
			String m_type=rs.getString(1);
			*/
			...
			rs = stmt.executeQuery(" SELECT "+
				...
				//" NVL(ADDRESS1,'-'),"+
				//" NVL(ADDRESS2,'-'),"+
				" NVL(DECODE(CLIENT_TYPE,'I',ADDRESS1,'C',REGISTERED_ADDRESS1),'-'), "+ //added by nuwan de silva on 30-06-2010
				" NVL(DECODE(CLIENT_TYPE,'I',ADDRESS2,'C',REGISTERED_ADDRESS2),'-'), "+ //added by nuwan de silva on 30-06-2010
			...
			//______ added by nuwan de silv aon 13-11-07 __________//
			else if(m_appNo.equals("view_invoices")){		
				int count=0;		
				
				String m_val = req.getParameter("val");	
				String m_status= req.getParameter("status");	
				String m_type= req.getParameter("type");	
				
				if (m_type.equals("CHASSIS")){
					...
					rs= stmt.executeQuery ("SELECT "+
						...
						" INVOICE_NO,"+
						" CHASSIS_NO,"+
						" NVL(DECODE(APPLICATION_STATUS,'ENTERED','Entered','ENT_CON','Entered','VERIFY1','Verification','V-APP','Score Approval','VERIFY-M','Approval 1','VERIFY2','Approval 2','VERIFYL','Entered Leasing No','ACTIVATED','Activated','CANCEL','Cancel','REPOSSESS','Repossess'),'-') "+	
						" FROM "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS A ,"+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
						" WHERE A.APPLICATION_NO=B.APPLICATION_NO "+
						" AND UPPER(CHASSIS_NO)=UPPER('"+m_val+"') "+
					...
					rs= stmt.executeQuery ("SELECT "+
						...
						" FROM "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS A ,"+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
						" WHERE A.APPLICATION_NO=B.APPLICATION_NO "+
						" AND UPPER(ENGINE_NO)=UPPER('"+m_val+"') "+
						" AND ACTIVE_STATUS='"+m_status+"' ");
				}
				
				out.println("<HTML><HEAD><TITLE>Engine No Chassiss No Details</TITLE></HEAD>");
				out.println("<link REL='STYLESHEET' HREF='"+m_html_client_url+"/css/Asset_Financing_System.css' TYPE=\"text/css\">");
				out.println("<BODY   class='body & txt-body' LEFTMARGIN='0' TOPMARGIN='0'>");
				out.println("<FORM NAME='Form1' method='post'>"); 
				out.println("<TABLE  WIDTH='100%' class='pdn_txtpos2' STYLE='{ bgcolor='#8fb382' color: black; font: 20pt arial;}'>");
				if (m_type.equals("CHASSIS")){
					out.println("<TR><TD><CENTER><B>Chassis No / Serial No  Details</B></TD></TR>");
				}
				else{
					out.println("<TR><TD><CENTER><B>Engine No Details</B></TD></TR>");
				}
				...
				if (more) {
					...
					out.println("<td width='1%'></td>"); 
					if (m_type.equals("CHASSIS")){
						out.println("<td width='*%' class=div_input><b><u>The Chassis No / Serial No that has been entered already exists </u></b></td>");
					}
					else{
						out.println("<td width='*%' class=div_input><b><u>The Engine Number that has been entered already exists </u></b></td>");
					}
					...
					out.println("<tr>");
					out.println("<td width='1%'></td>"); 
					out.println("<td width='15%' class=div_input><b>Application No</b></td>");
					out.println("<td width='12%' class=div_input><b>Client Code</b></td>");
					out.println("<td width='25%' class=div_input><b>Client Name</b></td>");
					out.println("<td width='15%' class=div_input><b>Application Status</b></td>");
					out.println("</tr>");
					out.println("</table>");
				}
				out.println("<table align='center' width='100%' class='table' >");
				while(more){
					out.println("<tr>");
					out.println("<td width='1%'></td>"); 
					out.println("<td width='15%' class=div_input style= cursor:hand; onclick=show_application_detail_drill('"+rs.getString(1)+"') ><u>"+rs.getString(1)+"</u></td>");
					out.println("<td width='12%' class=div_input style= cursor:hand; onclick=show_client('"+rs.getString(2)+"') ><u>"+rs.getString(2)+"</u></td>");
				...
				*/
				...
				//out.println("m_prof_inv_no"+m_prof_inv_no);
				...
				out.println("var m_row ='"+m_row+"';"); //added by nuwan de silva on 22-10-07
				
				out.println("  var m_refin_status = '"+m_refin_status+"';  "); // added by udara 09-05-2014
				out.println("  var m_refin_no = '"+m_refin_no+"';  ");  // added by udara 09-05-2014
				
				out.println("function get_vector(data_vec) {");
				
				//out.println("  alert(document.Form1.SCREEN_NAME.value+'  -  '+document.Form1.hid_chk_status.value);  ");
				
				out.println("			if(data_vec.length>0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M1'){");
				out.println("			alert('ccccccc');");
				out.println("				alert('Record already exists.');");
				out.println("				new_window();");
				out.println("			}");
				
				out.println("			if(data_vec.length==0 && (document.Form1.SCREEN_NAME.value==\"DEL\" || document.Form1.SCREEN_NAME.value==\"EDIT\") && document.Form1.hid_chk_status.value=='M1'  ){"); //&& document.Form1.TXT_INVOICE_NO.value!='' //comment by nwuan de silva on 06-09-07
				//out.println("			alert('aaaaaaa');");
				//out.println("				help_update('1','10','19','m_help_TXT_INVOICE_NO_sql','99');");
				out.println("				help_update('1','10','19','m_help_TXT_INVOICE_NO_NEW_sql','99');"); //added by nuwan de silva on 04-09-07
				...
				out.println("       document.getElementById(\"DIV_TXT_ENGINE_NO_CHK\").innerHTML = data_vec[3]; "); // added by udara 29-05-2015 document.getElementById(\"DIV_TXT_ENGINE_NO_CHK\").innerHTML
				out.println("    } "); 
				...
				out.println("    else{ "); 
				out.println("    document.Form1.TXT_CHASSIS_NO.value=data_vec[5];"); 
				out.println("       document.getElementById(\"DIV_TXT_CHASSIS_NO_CHK\").innerHTML = data_vec[5]; "); // added by udara 29-05-2015 document.getElementById(\"DIV_TXT_CHASSIS_NO_CHK\").innerHTML
				out.println("    } "); 
				
				out.println("   if(data_vec[6]!='null') { "); 
				out.println("      document.Form1.TXT_REG_NO.value=data_vec[6];"); 
				out.println("       document.getElementById(\"DIV_TXT_REG_NO_CHK\").innerHTML = data_vec[6]; "); // added by udara 29-05-2015 document.getElementById(\"DIV_TXT_REG_NO_CHK\").innerHTML
				out.println("		}");
				...
				out.println("    if(data_vec[20]=='  '){");
				out.println("    document.Form1.TXT_LOCATION.value='';"); 
				out.println("    } "); 
				out.println("    else{ "); 
				out.println("    document.Form1.TXT_LOCATION.value=data_vec[20];");
				out.println("    } "); 
				
				//out.println("     alert(data_vec[21]); "); // udara test
				
				
				out.println("    document.Form1.TXT_CITY_CODE.value=data_vec[21];"); 
				out.println("    document.Form1.TXT_CITY_NAME.value=data_vec[30];"); //added by nuwan de silva on 19-09-07
				...
				out.println("    asset_desc=data_vec[31]+' '+data_vec[27]+' '+data_vec[32];");
				out.println("    document.Form1.TXT_ASSET_DESC.value=asset_desc;"); 
				
				//out.println("    document.Form1.TXT_ASSET_DESC.value=data_vec[31]+' '+data_vec[26]+' '+data_vec[32];"); 
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_prime_chk_LAKDL_AF_MK_display_performa_sumins&data_val=\"+document.Form1.TXT_INVOICE_NO.value+\"&ac_status=Y\";");
				...
				out.println("	else if(data_vec.length>0 && (document.Form1.SCREEN_NAME.value==\"DEL\" || document.Form1.SCREEN_NAME.value==\"EDIT\") && document.Form1.hid_chk_status.value=='M12' ){"); //&& document.Form1.TXT_INVOICE_NO.value!='' //comment by nwuan de silva on 06-09-07
				
				//added by Prabash on 35-04-2012---**
				out.println("    document.Form1.TXT_SUM_INSURED.value=data_vec[0];"); 
				out.println("    document.Form1.TXT_AREA.value=data_vec[1];"); 
				out.println("    document.Form1.TXT_POLICE.value=data_vec[2];");
				out.println("    document.Form1.TXT_OWN_ADD.value=data_vec[3];");
				out.println("    document.Form1.TXT_COLLE_SEC.value=data_vec[4];"); 
				out.println("    document.Form1.TXT_LIC_AUTH.value=data_vec[5];"); 
				out.println("    document.Form1.VEHICAL_AGA.value=data_vec[6];"); 
				...
				out.println("       document.Form1.TXT_CITY_NAME.value=data_vec[1] "); 
				out.println("			}");
				//==========================================================================
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M7' && document.Form1.TXT_ENGINE_NO.value!=\"\"){");
				out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				
				// commented below 2 by udara 06-11-2013
				//out.println("  m_url='"+m_class_url+"/"+m_fschema_name+"AF_MK_display_performa_invoice?APP_NO=view_invoices&status=Y&type=ENGINE&val='+document.Form1.TXT_ENGINE_NO.value;");
				...
				out.println("       document.Form1.TXT_ENGINE_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_ENGINE_NO.focus();  ");  // added by udara 06-11-2013
				
				//out.println("       document.Form1.TXT_ENGINE_NO.value=\"\" ");  //comment by nuwan de silva on 13-11-07
				//out.println("       document.Form1.TXT_ENGINE_NO.focus();  ");   //comment by nuwan de silva on 13-11-07
				out.println("			}");
				//added by prabash on 18-05-2012----**
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M13' && document.Form1.TXT_REG_NO.value!=\"\"){");
				
				out.println("    alert('This Registration No is From a Blacklisted Vehicle - ' + data_vec[1]); "); 
				out.println("    document.Form1.TXT_REG_NO.value =\"\" "); 
				out.println("    document.Form1.TXT_REG_NO.focus(); "); 
				out.println("			}");
				
				// added by udara 27-10-2014
				
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M13' && document.Form1.TXT_REG_NO.value!=\"\"){");
				...
				// end by udara 27-10-2014
				
				// added by udara 13-03-2014
				//out.println("	else if(data_vec.length == 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M13' && document.Form1.TXT_REG_NO.value!=\"\"){");
				out.println("	else if(data_vec.length == 0  && document.Form1.hid_chk_status.value=='M13' && document.Form1.TXT_REG_NO.value!=\"\"){"); // mod by udara 14-03-2014
				out.println("        val_reg_no(); ");
				out.println("	}");
				// end by udara 13-03-2014
				
				
				
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M14' && document.Form1.TXT_ENGINE_NO.value!=\"\"){");
				out.println("       alert('This Engine No is From a Blacklisted Vehicle - ' + data_vec[1]); "); 
				out.println("    document.Form1.TXT_ENGINE_NO.value =\"\" "); 
				...
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M14' && document.Form1.TXT_ENGINE_NO.value!=\"\"){");
				out.println("       alert('This Engine No is From a Blacklisted Vehicle - ' + data_vec[1]); "); 
				out.println("    document.Form1.TXT_ENGINE_NO.value =\"\" "); 
				out.println("    document.Form1.TXT_ENGINE_NO.focus(); "); 
				out.println("			}");
				...
				//out.println("	else if(data_vec.length == 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M14' && document.Form1.TXT_ENGINE_NO.value!=\"\"){"); // commented by udara 14-03-2014
				out.println("	else if(data_vec.length == 0  && document.Form1.hid_chk_status.value=='M14' && document.Form1.TXT_ENGINE_NO.value!=\"\"){"); // added by udara 14-03-2014
				out.println("val_engine_no(); ");
				out.println("			}");
				
				// added by udara 14-03-2014
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M7' && document.Form1.TXT_ENGINE_NO.value!=\"\"){");
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M7' && document.Form1.TXT_ENGINE_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[3] ){"); // udara 13-06-2014
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M7' && document.Form1.TXT_ENGINE_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[2] ){"); // added by udara 24-10-2014
				out.println("       alert('Record already exists. ' + data_vec[3] + ' Application -  ' + data_vec[2]); "); // out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				out.println("       document.Form1.TXT_ENGINE_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_ENGINE_NO.focus();  ");  // added by udara 06-11-2013
				out.println("	}");
				...
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M15' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){");
				out.println("       alert('This Chassis No is From a Blacklisted Vehicle - ' + data_vec[1]); "); 
				out.println("    document.Form1.TXT_CHASSIS_NO.value =\"\" "); 
				...
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M15' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){");
				out.println("       alert('This Chassis No is From a Blacklisted Vehicle - ' + data_vec[1]); "); 
				out.println("    document.Form1.TXT_CHASSIS_NO.value =\"\" "); 
				out.println("    document.Form1.TXT_CHASSIS_NO.focus(); "); 
				out.println("			}");
				...
				//out.println("	else if(data_vec.length == 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M15' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){"); // commented by udara 14-03-2014
				out.println("	else if(data_vec.length == 0  && document.Form1.hid_chk_status.value=='M15' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){"); // added by udara 14-03-2014
				out.println("       val_chassis_no();");
				out.println("			}");
				
				//---------------------------------**
				
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M8' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){");
				out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				//out.println("       document.Form1.TXT_CHASSIS_NO.value=\"\" ");  //comment by nuwan de silva on 13-11-07
				//out.println("       document.Form1.TXT_CHASSIS_NO.focus();  ");   //comment by nuwan de silva on 13-11-07
				
				// commented by udara 06-11-2013
				//out.println("  m_url='"+m_class_url+"/"+m_fschema_name+"AF_MK_display_performa_invoice?APP_NO=view_invoices&status=Y&type=CHASSIS&val='+document.Form1.TXT_CHASSIS_NO.value;");
				//out.println("window.open(m_url,'displayWindow2','left=450,top=200,width=600,height=350,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
				
				out.println("       document.Form1.TXT_CHASSIS_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_CHASSIS_NO.focus();  ");  // added by udara 06-11-2013
				
				out.println("			}");
				
				
				// added by udara 13-03-2014
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"NEW\" && document.Form1.hid_chk_status.value=='M16' && document.Form1.TXT_REG_NO.value!=\"\"){");
				out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				out.println("       document.Form1.TXT_REG_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_REG_NO.focus();  ");  // added by udara 06-11-2013
				...
				// added by udara 14-03-2014
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M8' && document.Form1.TXT_CHASSIS_NO.value!=\"\"){");
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M8' && document.Form1.TXT_CHASSIS_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[3]){"); // udara 13-06-2014
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M8' && document.Form1.TXT_CHASSIS_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[2]){"); // udara 24-10-2014
				out.println("       alert('Record already exists. ' + data_vec[3] + ' Application -  ' + data_vec[2]); "); // out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				out.println("       document.Form1.TXT_CHASSIS_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_CHASSIS_NO.focus();  ");  // added by udara 06-11-2013
				...
				// added by udara 14-03-2014
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M16' && document.Form1.TXT_REG_NO.value!=\"\"){");
				//out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M16' && document.Form1.TXT_REG_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[3]){"); // udara 13-06-2014
				out.println("	else if(data_vec.length > 0 && document.Form1.SCREEN_NAME.value==\"EDIT\" && document.Form1.hid_chk_status.value=='M16' && document.Form1.TXT_REG_NO.value!=\"\" && document.Form1.TXT_APPLICATION_NO.value!=data_vec[2]){"); // udara 24-10-2014
				out.println("       alert('Record already exists. ' + data_vec[3] + ' Application -  ' + data_vec[2]); "); // out.println("       alert('Record already exists. ' + data_vec[3]); "); // mod by udara 06-11-2013
				out.println("       document.Form1.TXT_REG_NO.value=\"\" "); // added by udara 06-11-2013
				out.println("       document.Form1.TXT_REG_NO.focus();  ");  // added by udara 06-11-2013
				out.println("	}");
				...
				// commented by udara 10-02-2026
				/*
				out.println("	else if(data_vec.length == 0  &&  document.Form1.hid_chk_status.value=='M16' && document.Form1.TXT_REG_NO.value!=\"\" && m_refin_status == 'YES' ){");
				out.println("       val_refin_reg_no(); ");
				out.println("	}");
				
				out.println("	else if(data_vec.length == 0  && document.Form1.hid_chk_status.value=='M8'  && document.Form1.TXT_CHASSIS_NO.value!=\"\" && m_refin_status == 'YES'){");
				out.println("       val_refin_chasis_no(); "); 
				out.println("	}");
				
				out.println("	else if(data_vec.length == 0  && document.Form1.hid_chk_status.value=='M7'  && document.Form1.TXT_ENGINE_NO.value!=\"\" && m_refin_status == 'YES'){");
				out.println("       val_refin_engine_no(); ");
				out.println("	}");
				*/
				
				
				out.println("	else if(data_vec.length > 0  &&  document.Form1.hid_chk_status.value=='M20' && document.Form1.TXT_REG_NO.value!=\"\" && m_refin_status == 'YES' ){");
				out.println("       var m_val = data_vec[0];  ");
				out.println("       if(m_val != document.Form1.TXT_REG_NO.value){ ");
				out.println("           alert('Registration number of refinanced contract does not match');     ");
				out.println("	        document.Form1.TXT_REG_NO.value = ''; ");
				out.println("	        document.Form1.TXT_REG_NO.focus(); ");
				out.println("	    }");
				out.println("	}");
				...
				out.println("	else if(data_vec.length > 0  &&  document.Form1.hid_chk_status.value=='M21' && document.Form1.TXT_ENGINE_NO.value!=\"\" && m_refin_status == 'YES' ){");
				out.println("       if(data_vec[0] != document.Form1.TXT_ENGINE_NO.value){ ");
				out.println("           alert('Engine number of refinanced contract does not match');     ");
				out.println("	        document.Form1.TXT_ENGINE_NO.value = ''; ");
				out.println("	        document.Form1.TXT_ENGINE_NO.focus(); ");
				out.println("	    }");
				out.println("	}");
				
				out.println("	else if(data_vec.length > 0  &&  document.Form1.hid_chk_status.value=='M22' && document.Form1.TXT_CHASSIS_NO.value!=\"\" && m_refin_status == 'YES' ){");
				out.println("       if(data_vec[0] != document.Form1.TXT_CHASSIS_NO.value){ ");
				out.println("           alert('Chassis number of refinanced contract does not match');     ");
				out.println("	        document.Form1.TXT_CHASSIS_NO.value = ''; ");
				out.println("	        document.Form1.TXT_CHASSIS_NO.focus(); ");
				out.println("	    }");
				out.println("	}");
				...
				out.println("    document.Form1.TXT_TOTAL_AMOUNT.value=format_noobject( Number(unformat_noobject(data_vec[1])) + Number(unformat_noobject(data_vec[2])) );");//modified nuwan de silva 11/05/07
				...
				out.println("    help_vendor('1','10','0','m_help_vendor_proforma_invoice','77'); "); // m_help_TXT_VENDOR_CODE_sql
				...
				//	out.println("alert(document.Form1.hid_chk_status.value+obj.value)");
				
				out.println("if(document.Form1.hid_chk_status.value=='M1' && document.Form1.SCREEN_NAME.value!=\"RACT\")");
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_prime_chk_LAKDL_AF_MK_display_performa_invoice&data_val=\"+obj.value+\"&ac_status=Y\";");
				...
				//added by Prabash on 16-06-2012--* 
				out.println("else if(document.Form1.hid_chk_status.value=='M13')");
				out.println("    m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_LAKDL_AF_MK_display_performa_invoice_blacklist_reg_no&data_val=\"+obj.value;");
				
				out.println("else if(document.Form1.hid_chk_status.value=='M14')");
				out.println("    m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_LAKDL_AF_MK_display_performa_invoice_blacklist_engin_no&data_val=\"+obj.value;");
				
				out.println("else if(document.Form1.hid_chk_status.value=='M15')");
				out.println("    m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_LAKDL_AF_MK_display_performa_invoice_blacklist_chass_no&data_val=\"+obj.value;");
				...
				out.println("window.opener.document.Form1.elements[\"CHK_PIV_\"+m_row_no].checked=true; ");//added by nwuan de silva on 22-10-07
				out.println("window.opener.document.Form1.elements[\"CHK_PIV_\"+m_row_no].value='YES'; "); //added by nwuan de silva on 22-10-07
				...
				// added by udara 22-05-2014
				out.println("if(document.Form1.TXT_REG_NO.value==\"\"){  "); 
				...
				out.println("if(document.Form1.TXT_CHASSIS_NO.value==\"\"){  "); 
				out.println("DIV_TXT_CHASSIS_NO.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				
				out.println("if(document.Form1.TXT_ENGINE_NO.value==\"\"){  "); 
				out.println("DIV_TXT_ENGINE_NO.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				
				// end by udara 26-02-2014
				/* REmove by waruna 2014-06-2014
				out.println("if(document.Form1.TXT_SUM_INSURED.value==\"\"){  "); 
				out.println("DIV_TXT_SUM_INSURED.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				*/
				...
				out.println("return false;"); 
				out.println("}");
				if(m_trn_type==null ||(!m_trn_type.equals("LOANS"))){//added by SH for Loans ###L
					
					out.println("else if(document.Form1.TXT_PRICING_NO.value==\"\"){  "); 
					out.println("DIV_TXT_PRICING_NO.style.color='red';");
					out.println("return false;"); 
					out.println("}"); 
				}//added by SH for Loans ###L
				
				out.println("else if(document.Form1.TXT_SUB_MODEL_NO.value==\"\"){  "); 
				out.println("DIV_TXT_SUB_MODEL_NO.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				out.println("else if(document.Form1.TXT_YEAR_MANUFACTURE.value==\"\"){  ");//Added by Chandana on 30/11/2007 
				out.println("DIV_TXT_YEAR_MANUFACTURE.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				
				// added by udara 13-11-2014
				out.println("else if(document.Form1.TXT_EXTRAS.value==\"\"){  ");
				out.println("DIV_TXT_EXTRAS.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				// end by udara 13-11-2014
				...
				*/	
				...
				// added by Saviru 20-01-2021
				out.println("if(document.Form1.TXT_AREA.value==\"\"){  "); 
				out.println("DIV_TXT_AREA.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				
				out.println("if(document.Form1.TXT_POLICE.value==\"\"){  ");
				out.println("DIV_TXT_POLICE.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				
				out.println("if(document.Form1.TXT_LIC_AUTH.value==\"\"){  "); 
				out.println("DIV_LIC_AUTH.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				
				out.println("if(document.Form1.TXT_OWN_ADD.value==\"\"){  "); 
				out.println("DIV_TXT_OWN_ADD.style.color='red';");
				out.println("return false;"); 
				out.println("}"); 
				
				out.println("if(document.Form1.TXT_COLLE_SEC.value==\"\"){  "); 
				out.println("DIV_COLLE_SEC.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				
				out.println("if(document.Form1.VEHICAL_AGA.value==\"\"){  "); 
				out.println("DIV_VEHICAL_AGA.style.color='red';");
				out.println("return false;"); 
				out.println("}");
				//--------------------------------------------------------
				...
				//added by SH on 18-04-2008 for Loans ###L
				if(m_trn_type!=null && (m_trn_type.equals("LOANS"))){
					
					out.println("if(document.Form1.TXT_PRICING_NO.value==''){");
					out.println(" document.Form1.TXT_PRICING_NO.value=\"-\"; "); 
					out.println(" document.Form1.TXT_VAT.value=\"0\";");
					out.println("}");
				}
				// end of loans ###L
				...
				out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MK_save_performa_invoice?my_screen_name="+m_my_screen+"';");  
				...
				out.println("   window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MK_display_performa_invoice?APP_NO="+m_appNo+"';"); 
				...
				out.println("       document.Form1.elements[i].disabled=false;");
				out.println("     }");
				out.println("     document.Form1.TXT_NET_PRICE.disabled=true;");     //added by nuwan de silva on 29-10-07
				out.println("     document.Form1.TXT_VAT.disabled=true;");           //added by nuwan de silva on 29-10-07
				out.println("     document.Form1.TXT_TOTAL_AMOUNT.disabled=true;");  //added by nuwan de silva on 29-10-07
				out.println("			document.Form1.TXT_VALUE.disabled=true;");         //added by nuwan de silva on 29-10-07
				out.println("     document.Form1.TXT_VENDOR_NAME.disabled=true;"); 
				out.println("     document.Form1.TXT_ASSET_DESC.disabled=true;"); 
				...
				out.println("     document.Form1.TXT_MODEL_DESC.disabled=true;"); 
				out.println("     document.Form1.TXT_SUB_MODEL_DESC.disabled=true;"); 
				...
				out.println("document.Form1.TXT_ASSET_DESC.disabled=false;"); 
				...
				out.println("document.Form1.TXT_MODEL_DESC.disabled=true;"); 
				out.println("document.Form1.TXT_SUB_MODEL_DESC.disabled=true;");
				out.println("document.Form1.TXT_EXTRAS.disabled=true;"); 
				out.println("document.Form1.TXT_YEAR_MANUFACTURE.disabled=true;");  //added by nuwan de silva on 29-10-07
				out.println("document.Form1.TXT_FUEL_CONV_STS.disabled=true;");  //added by nuwan de silva on 29-10-07
				...
				//COMMENT BY NUWAN DE SILVA ON 15-11-07--------------------
				/*out.println("function help_button_asset_details(obj) {");
				out.println("show_asset_detail_drill(obj.value);"); 
				out.println("}"); 
				
				out.println("function help_button_pricing_details(obj) {");
				out.println("show_pricing_drill(obj.value);"); 
				out.println("}"); 
				*/
				...
				//  out.println("document.Form1.TXT_SUB_MODEL_NO.value=oBj.valout[4];"); //COMMENT BY Chandana ON 25/04/2007 FOR REFINMENTS.
				out.println("document.Form1.TXT_SUB_MODEL_NO.value=oBj.valout[5];");  //ADDED BY Chandana ON 25/04/2007 FOR REFINMENTS.
				...
				out.println("    document.Form1.TXT_MODEL_DESC.value=oBj.valout[4];");  ///ADDED BY NUWAN DE SILVA 21-05-07
				out.println("    document.Form1.TXT_SUB_MODEL_DESC.value=oBj.valout[6];");  ///ADDED BY NUWAN DE SILVA 21-05-07
				
				//out.println("    asset_desc=oBj.valout[4]+' '+oBj.valout[6];");
				//out.println("    document.Form1.TXT_ASSET_DESC.value=oBj.valout[4]+' '+oBj.valout[6];");  ///ADDED BY NUWAN DE SILVA 15-11-2007
				
				out.println("    asset_desc=oBj.valout[15]+' '+oBj.valout[6]+' '+oBj.valout[16];");
				out.println("    document.Form1.TXT_ASSET_DESC.value=asset_desc;");  ///ADDED BY NUWAN DE SILVA ON 21-12-07
				...
				out.println("    document.Form1.TXT_CURR_CODE.value=oBj.valout[30];"); 
				out.println("}"); 
				...
				out.println("    document.Form1.TXT_CITY_NAME.value=oBj.valout[3];"); //added by nuwan de silva on 19-09-07
				out.println("}"); 
				...
				out.println("else if(dval=='-'){");
				
				out.println("document.Form1.TXT_REG_DATE_DD.value=''");
				out.println("document.Form1.TXT_REG_DATE_MM.value=''");
				out.println("document.Form1.TXT_REG_DATE_YY.value=''");
				...
				//out.println("       document.Form1.DIV_TXT_ENGINE_NO_CHK.innerHTML = oBj.valout[6]; "); // added by udara 29-05-2015
				out.println("    } "); 
				...
				//out.println("       document.Form1.DIV_TXT_CHASSIS_NO_CHK.innerHTML = oBj.valout[7]; "); // added by udara 29-05-2015
				out.println("    } "); 
				
				out.println("   if(oBj.valout[8]!='null') { "); 
				out.println("    document.Form1.TXT_REG_NO.value=oBj.valout[8];"); 
				//out.println("       document.Form1.DIV_TXT_REG_NO_CHK.innerHTML = oBj.valout[8]; "); // added by udara 29-05-2015
				out.println("    } "); 
				out.println("    if(oBj.valout[9]!='null') {"); 
				out.println("     getDateValues(oBj.valout[9])");
				out.println("    } "); 
				out.println("    document.Form1.TXT_PRICING_NO.value=oBj.valout[10];"); 
				out.println("    document.Form1.TXT_SUB_MODEL_NO.value=oBj.valout[11];"); 
				
				out.println("    document.Form1.TXT_COLOUR.value=oBj.valout[12];"); 
				
				out.println("if(oBj.valout[12]=='-'){");
				out.println("    document.Form1.TXT_COLOUR.value='';"); 
				out.println("    } "); 
				out.println("    else{ "); 
				out.println("    document.Form1.TXT_COLOUR.value=oBj.valout[12];"); 
				...
				out.println("    } "); 
				out.println("    else{ "); 
				out.println("    document.Form1.TXT_SEATING_CAPACITY.value=oBj.valout[13];"); 
				out.println("    } "); 
				
				out.println("    document.Form1.TXT_NET_PRICE.value=format_noobject(Number(oBj.valout[14]) );"); 
				out.println("    document.Form1.TXT_VAT.value=format_noobject(Number(oBj.valout[15]) );"); 
				out.println("    document.Form1.TXT_TOTAL_AMOUNT.value=format_noobject(Number(oBj.valout[16]) );"); 
				out.println("    document.Form1.TXT_TO_BE_DELIVERD_TO.value=oBj.valout[17];"); 
				out.println("    document.Form1.TXT_VALUE.value=format_noobject(Number(oBj.valout[18]) );"); 
				out.println("    document.Form1.TXT_CURR_CODE.value=oBj.valout[19];"); 
				out.println("    document.Form1.TXT_MODEL_NO.value=oBj.valout[20];"); 
				out.println("    document.Form1.TXT_TRANSMISSION.value=oBj.valout[21];"); 
				out.println("    document.Form1.TXT_INVOICE_DOC_NO.value=oBj.valout[22];"); 
				out.println("    document.Form1.TXT_CITY_CODE.value=oBj.valout[23];"); 
				out.println("    document.Form1.TXT_CITY_NAME.value=oBj.valout[34];");  //added by nuwan de silva on 19-09-07
				out.println("    document.Form1.TXT_LOCATION.value=oBj.valout[24];"); 
				out.println("    document.Form1.TXT_VENDOR_CODE.value=oBj.valout[25];"); 
				out.println("    document.Form1.TXT_VENDOR_NAME.value=oBj.valout[26];"); 
				out.println("    document.Form1.TXT_LOCATION_CODE.value=oBj.valout[27];"); 
				out.println("    document.Form1.TXT_FUEL_CONV_STS.value=oBj.valout[28];"); 
				out.println("    if(oBj.valout[29]!='null') {"); 
				out.println("     getDateValues_Due(oBj.valout[29])");
				out.println("    } "); 
				
				out.println("    document.Form1.TXT_MODEL_DESC.value=oBj.valout[30];"); 
				out.println("    document.Form1.TXT_SUB_MODEL_DESC.value=oBj.valout[31];"); 
				
				//out.println("    document.Form1.TXT_ASSET_DESC.value=oBj.valout[35]+' '+oBj.valout[31]+' '+oBj.valout[36]"); //added by nuwan de silva on 15-11-2007
				//out.println("    document.Form1.TXT_ASSET_DESC.value=oBj.valout[31]+' '+oBj.valout[30]"); //added by nuwan de silva on 15-11-2007
				out.println("    document.Form1.TXT_ASSET_DESC.value=oBj.valout[35]+' '+oBj.valout[31]+' '+oBj.valout[36] "); //added by nuwan de silva on 15-11-2007
				
				
				
				out.println("    if(oBj.valout[32]!='null') {"); 
				out.println("    document.Form1.TXT_YEAR_MANUFACTURE.value=oBj.valout[32];"); 
				out.println("    } "); 
				out.println("    else {");
				out.println("    document.Form1.TXT_YEAR_MANUFACTURE.value='';"); 
				out.println("    } "); 
				
				out.println("    if(oBj.valout[33]!='null') {"); 
				out.println("    document.Form1.TXT_EXTRAS.value=oBj.valout[33];"); 
				out.println("    } "); 
				out.println("    else {");
				out.println("    document.Form1.TXT_EXTRAS.value='';"); 
				out.println("    } "); 
				...
				*/
				...
				out.println("    document.Form1.TXT_TO_BE_DELIVERD_TO.value=\""+m_full_name+"\";"); //nuwan de silva
				
				//out.println("    if('"+m_add1+"'=='-' && '"+m_add2+"'=='-' ){");//added by Mahela //comment by nuwan de silva on 04-09-07
				out.println("    if(\"'"+m_add1+"'\"=='-' && \"'"+m_add2+"'\"=='-' ){");//added by nuwan de silva on 04-09-07
				out.println("    document.Form1.TXT_LOCATION.value=\"\";"); 
				out.println("		 }");
				out.println("    else {");
				//out.println("    document.Form1.TXT_LOCATION.value=\""+m_add1+m_add2+"\";"); //nuwan de silva
				out.println("    document.Form1.TXT_LOCATION.value=\""+m_address+"\";"); //nuwan de silva
				
				out.println("		 }");
				out.println("    document.Form1.TXT_CITY_CODE.value=\""+m_city_code+"\";"); //nuwan de silva
				out.println("    document.Form1.TXT_CITY_NAME.value=\""+m_city_name+"\";"); //added by nuwan de silva on 19-09-07
				...
				out.println("if('"+m_prof_inv_no+"'!=\"\"){");
				out.println("    document.Form1.TXT_INVOICE_NO.value=\""+m_prof_inv_no+"\";"); 
				
				out.println("load_screen_status(\"EDIT\")"); //ADDED BY NUWAN DE SILVA
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_prime_chk_LAKDL_AF_MK_display_performa_invoice&data_val=\"+document.Form1.TXT_INVOICE_NO.value+\"&ac_status=Y\";");
				...
				out.println("    oBj.valout[3]  = \" \";"); 
				out.println("	"); 
				out.println("	popupwin = window.showModalDialog(servlet_client_url+\":\"+client_t3_port+\"/\"+client_name+\"AF_MK_View_Help_Servlet?class_in=\"+client_name+\"AF_MK_View_help_select\"+");  //AF_MAS_View_Help_Servlet AF_MK_View_Help_Servlet
				out.println("    \"&Sql_in=\"+m_sql+\"&Start_in=\"+Start+"); 
				out.println("    \"&End_in=\"+End+\"&Crit_In=\"+m_criteria+"); 
				...
				//out.println("    HelpBox_View('1',50,'0');"); 
				
				//out.println("function View_all(Start,End,Hid_No,Sql,IfCount){");	
				//out.println("    Sql = \"m_help_TXT_INVOICE_NO_sql\";");
				//out.println("    Crit = document.Form1.TXT_INVOICE_NO.value+\"@\"+\"Y@\";"); 
				//out.println("    HelpView('1',50,'0');"); 
				//	out.println("    HelpBox_View('0',50,'0',Crit,Sql,0);"); 
				//	out.println("    Crit = document.Form1.TXT_INVOICE_NO.value+\"@\"+\"Y@\";"); 
				//out.println("    HelpBox_View(Start,End,Hid_No,Crit,Sql,IfCount);"); 
				out.println("}"); 
				...
				/*	out.println("function HelpBox_View(Start,End,Hid_No,Crit,Sql,IfCount) {"); 
					...
					//out.println("popupwin = window.showModalDialog('"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_MK_View_Help_Servlet?class_in="+m_client_name+"AF_MK_View_help_select&Sql_in='+Sql+'&Start_in='+Start+'&End_in='+End+'&Crit_In='+Crit+'&Hid_No='+Hid_No+'&Max_in='+Hid_No+'&Args=', oBj,\"dialogWidth:25em; dialogHeight:26em; center:yes; status:no\");");
					out.println("	"); 
					out.println("	if(oBj.valout[1] ==\" \"){"); 
					out.println("    clear_fields(); ");
					out.println("		} else ");
				...
				out.println("function val_reg_no(){");
				out.println("assignState('M16')");
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_LAKDL_AF_MK_display_performa_invoice_reg_no&data_val=\"+document.Form1.TXT_REG_NO.value+\"&ac_status=Y\";");
				out.println("load_interface(m_url,'XML');");
				out.println("}");	
				...
				out.println("function val_refin_reg_no(){");
				out.println("  assignState('M20');");
				out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_check_refin_reg_no&refin_no=\"+\""+m_refin_no+"\";");
				out.println("  load_interface(m_url,'XML');");
				out.println("}");
				
				out.println("function val_refin_engine_no(){");
				out.println("  assignState('M21');");
				out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_check_refin_engine_no&refin_no=\"+\""+m_refin_no+"\";");
				out.println("  load_interface(m_url,'XML');");
				out.println("}");
				
				out.println("function val_refin_chasis_no(){");
				out.println("  assignState('M22');");
				out.println("  m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_sql_validations?chksql=m_check_refin_chasis_no&refin_no=\"+\""+m_refin_no+"\";");
				out.println("  load_interface(m_url,'XML');");
				out.println("}");
				...
				out.println("    document.Form1.TXT_CITY_CODE.value =''; ");
				out.println("    document.Form1.TXT_CITY_NAME.value =''; ");
				out.println("  }		"); 
				...
				out.println("function check_date(objDD,objMM,objYY){ "); //Modified Nuwan De Silva 15-05-07
				...
				//out.println("window.opener.document.Form1.elements[hidchk].checked=true"); //comment by nuwan de silva on 22-10-07
				...
				//out.println("window.opener.document.Form1.elements[hidchk].checked=false"); //comment by nuwan de silva on 22-10-07
				...
				//--added by Prabash on 16-03-2012-------**
				out.println("function help_Client(val){");
				//	out.println("alert('"+m_appNo+"');");
				//	out.println("alert(val);");
				out.println("    m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MK_display_App_Client_data?chksql=MAIN&App_no2=\"+val;");
				out.println("popupwin=window.open(m_url,'displayWindow1','left=10,top=40,width=600,height=400,toolbar=0,location=0,center:yes,direction=0,menuBar=1,status=0,scrollbars=1,resizable=1');");
				out.println("}");
				//--------------------------------------**	
				
				// commented by udara 26-02-2014
				/*
				// added by udara 17-12-2013
				out.println("function replace_strings(obj){");
				out.println(" 	if(obj.value!=''){ ");
				out.println("  		var str = obj.value; ");
				//out.println("  		var res = str.replace(/\s/g,''); ");
				out.println("  		var res = str.replace(/"+ "\\s" +"/g,''); ");
				
				//var str = "B I O S T A L L"  
				//str = str.replace(/\s/g,'');  
				
				out.println("  		obj.value = res; ");
				out.println("   }");
				out.println("}");
				...
				*/
				...
				out.println(" 		var letterNumber = /^[0-9a-zA-Z]+$/;  ");
				
				out.println("   	if(!obj.value.match(letterNumber)){    ");
				out.println("      		alert('Contains invalid characters - Inputs should be only the letters & numbers');   ");
				out.println("   	    obj.value = '';  ");
				out.println("   	    obj.focus(); ");
				out.println("   	}  ");
				...
				//added by nuwan de silva on 15-11-2007-----------------------------
				...
				out.println("<input class='but_input' type='button' name='BUT_HELP_MAIN' value=\"Help\" onClick=\"help_update('1','10','27','m_help_TXT_INVOICE_NO_NEW_sql','99')\" disabled></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_ASSET_ID' maxlength='15' size='15' onblur=\"assignState('M3'),makeRequest(document.Form1.TXT_ASSET_ID)\">"); 
				out.println("<input class='but_input' type='button' name='BUT_TXT_ASSET_ID' value=\"Help\" onClick=\"help_button_2('1','10','8','m_help_TXT_ASSET_ID_sql2','2')\"></td>"); 
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_VENDOR_CODE' maxlength='10' size='10' onblur=\"assignState('M10'),makeRequest(document.Form1.TXT_VENDOR_CODE)\">"); 
				out.println("<input class='but_input' type='button' name='BUT_HELP_VEN' value=\"Help\" onClick=\"help_vendor('1','10','0','m_help_vendor_proforma_invoice','77')\">"); // m_help_vendor_proforma_invoice //m_help_TXT_VENDOR_CODE_sql
				out.println("<input class='but_input' type='button' name='BUT_VEN' value=\"Client\" onClick=\"help_Client('"+m_appNo+"')\"></td>"); // Added by Prabash on 16-03-2012
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_LOCATION_CODE' maxlength='10' size='10' onblur=\"assignState('M11'),makeRequest(document.Form1.TXT_LOCATION_CODE)\" >"); 
				out.println("<input class='but_input' type='button' name='BUT_HELP_LOC' value=\"Help\" onClick=\"help_branch('1','10','7','m_help_TXT_MAS_VENDOR_LOCATION_sql','88')\"></td>"); 
				out.println("<td width='18%' ><DIV id='DIV_TXT_INVOICE_DOC_NO'  class=div_input>Vendor Document Reference No</DIV></td>"); 
				out.println("<td width='35%' ><input class='txt_input' type='text' name='TXT_INVOICE_DOC_NO' maxlength='30' size='20' ></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_PRICING_NO' maxlength='15' size='15' onBlur=\"assignState('M9'),makeRequest(document.Form1.TXT_PRICING_NO)\">"); 
				...
				out.println("<input class='but_input' type='button' name='BUT_TXT_PRICING_NO' value=\"Help\" onClick=\"help_button_5('1','10','32','PriceSql_invoice_new','5')\"></td>"); 
				...
				if((m_trn_type==null) || (!m_trn_type.equals("LOANS"))){//added by SH on 18-04-2008 for Loans ###L
					...
					out.println("<td width='20%' ><DIV id='DIV_TXT_NET_PRICE'  class=div_input>Net Price *</DIV></td>"); 
					out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_NET_PRICE' maxlength='25' size='25' onBlur=\"check_number(document.Form1.TXT_NET_PRICE,25),val_total_amt()\" STYLE='{text-align:right;}' ></td>"); 
					out.println("<td width='18%' ><DIV id='DIV_TXT_VAT'  class=div_input>VAT *</DIV></td>"); 
					out.println("<td width='35%' ><input class='txt_input' type='text' name='TXT_VAT' maxlength='25' size='25' onBlur=\"check_number(document.Form1.TXT_VAT,25),val_total_amt()\" STYLE='{text-align:right;}' disabled></td>"); 
				...
				*/
				...
				//out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_CHASSIS_NO' maxlength='50' size='20' onblur=\"assignState('M15'),makeRequest(document.Form1.TXT_CHASSIS_NO)\" ></td>"); // replace_strings(this),
				out.println("<td width='30%' >");
				out.println("   <input class='txt_input' type='text' name='TXT_CHASSIS_NO' maxlength='50' size='20' onblur=\"assignState('M15'),replace_strings(this),makeRequest(document.Form1.TXT_CHASSIS_NO)\" >");
				//out.println("   <DIV id='DIV_TXT_CHASSIS_NO_CHK'  class=div_input> div chasis </DIV>    "); // added by udara 29-05-2015
				out.println("</td>"); // added by udara 26-02-2014
				out.println("<td width='30%' >");
				//out.println("   <input class='txt_input' type='text' name='TXT_CHASSIS_NO' maxlength='50' size='20' onblur=\"assignState('M15'),replace_strings(this),makeRequest(document.Form1.TXT_CHASSIS_NO)\" >");
				out.println("   <DIV id='DIV_TXT_CHASSIS_NO_CHK'  class=div_input> div chasis </DIV>    "); // added by udara 29-05-2015
				out.println("</td>"); // added by udara 26-02-2014
				...
				//out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_ENGINE_NO' maxlength='50' size='20' onblur=\"assignState('M14'),makeRequest(document.Form1.TXT_ENGINE_NO)\" ></td>"); // replace_strings(this),
				out.println("<td width='30%' >");
				out.println("    <input class='txt_input' type='text' name='TXT_ENGINE_NO' maxlength='50' size='20' onblur=\"assignState('M14'),replace_strings(this),makeRequest(document.Form1.TXT_ENGINE_NO)\" >");
				//out.println("    <DIV id='DIV_TXT_ENGINE_NO_CHK'  class=div_input> div engine </DIV>    "); // added by udara 29-05-2015
				out.println("</td>"); // added by udara 26-02-2014
				...
				out.println("<td width='20%' ><DIV id='DIV_TXT_SEATING_CAPACITY'  class=div_input>Seating Capacity </DIV></td>"); 
				out.println("<td width='30%' >");
				out.println("    <input class='txt_input' type='text' STYLE=\"{text-align:left}\" name='TXT_SEATING_CAPACITY' maxlength='3' size='10' onBlur=\"val_seating_capacity()\">");
				out.println("</td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_COLOUR' maxlength='50' size='10'></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<tr>"); 
				//out.println("<td width='20%' >Registration No</td>"); // commented by udara 22-05-2014
				out.println("<td width='20%' > <DIV id='DIV_TXT_REG_NO'  class=div_input> Registration No *</td>");  // added by udara 22-05-2014
				out.println("<td width='30%' >");
				out.println("    <input class='txt_input' type='text' name='TXT_REG_NO' maxlength='20' size='20' onBlur=\" assignState('M13'),makeRequest(document.Form1.TXT_REG_NO)\">");
				//out.println("    <DIV id='DIV_TXT_REG_NO_CHK'  class=div_input> div reg no </DIV>    "); // added by udara 29-05-2015
				out.println("</td>"); //pra // replace_strings(this),
				//out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_REG_NO' maxlength='20' size='20' onBlur=\" assignState('M13'),replace_strings(this),makeRequest(document.Form1.TXT_REG_NO)\"></td>"); //pra // added by udara 26-02-2014 
				...
				out.println("<td width='30%' ><input class='txt_input5' type='text' name='TXT_REG_DATE_DD' maxlength='2' size='2' onBlur='check_date(document.Form1.TXT_REG_DATE_DD,document.Form1.TXT_REG_DATE_MM,document.Form1.TXT_REG_DATE_YY)'>");
				out.println("                 <input class='txt_input5' type='text' name='TXT_REG_DATE_MM' maxlength='2' size='2' onBlur='check_date(document.Form1.TXT_REG_DATE_DD,document.Form1.TXT_REG_DATE_MM,document.Form1.TXT_REG_DATE_YY)'>");
				...
				*/
				...
				//out.println("<td width='20%' >Extras Included </td>"); // commented by udara 13-11-2014
				out.println("<td width='20%' ><DIV id='DIV_TXT_EXTRAS'  class=div_input>CR Book No. *</td>");  // added by udara 13-11-2014
				out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_EXTRAS' style='width:350px' maxlength='100' size='100' value=\"\" ></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>");
				...
				out.println("<td width='30%' ><input class='txt_input' type='text' name='TXT_TO_BE_DELIVERD_TO' maxlength='100' style='width:350px' size='100'></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_CITY_CODE' maxlength='10' size='10' onblur=\"validate_city()\">"); 
				...
				out.println("<td width='30%' ><input class='txt_input5' type='text' name='TXT_DUE_DATE_DD' maxlength='2' size='2' onBlur='check_date(document.Form1.TXT_DUE_DATE_DD,document.Form1.TXT_DUE_DATE_MM,document.Form1.TXT_DUE_DATE_YY)'>");
				out.println("                 <input class='txt_input5' type='text' name='TXT_DUE_DATE_MM' maxlength='2' size='2' onBlur='check_date(document.Form1.TXT_DUE_DATE_DD,document.Form1.TXT_DUE_DATE_MM,document.Form1.TXT_DUE_DATE_YY)'>");
				out.println("                 <input class='txt_input5' type='text' name='TXT_DUE_DATE_YY' maxlength='4' size='4' onBlur='check_date(document.Form1.TXT_DUE_DATE_DD,document.Form1.TXT_DUE_DATE_MM,document.Form1.TXT_DUE_DATE_YY)'><a href style='{cursor:hand; }' onclick=load_calendar('2')>   Calendar</a></td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				//----added by prabash on 26-03-2012----**
				...
				out.println("<td width='30%'><input  class='txt_input' type='hidden' name='TXT_SUM_INSURED' maxlength='15' value='0.00' size='15' STYLE='{text-align:right;}' onBlur=\"check_number(document.Form1.TXT_SUM_INSURED,25) \" >"); 
				out.println("</td>"); 
				out.println("<td width='*%'></td>"); 
				out.println("</tr>"); 
				...
				out.println("<td width='20%' ><input class='txt_input' type='text' name='TXT_POLICE' maxlength='40' size='30'  >"); 
				out.println("</td>"); 
				
				out.println("<td width='18%' ><DIV id='DIV_COLLE_SEC'  class=div_input>Collectral Security *</DIV></td>"); 
				out.println("<td width='35%' ><input class='txt_input' type='text' name='TXT_COLLE_SEC' maxlength='10' size='20'  >"); 
				out.println("</td>");
				...
				out.println("<td width='25%' ><input class='txt_input' type='text' name='TXT_LIC_AUTH' maxlength='20' size='20'  >"); 
				out.println("</td>"); 
				
				out.println("<td width='18%' ><DIV id='DIV_VEHICAL_AGA'  class=div_input>Pradeshiya Sabaha *</DIV></td>"); 
				out.println("<td width='35%' ><input class='txt_input' type='text' name='VEHICAL_AGA' maxlength='30' size='20'  >"); 
				out.println("</td>");
				...
				if  (!m_my_screen.equals("")){
					//	out.println("<tr><td align=\"center\"><b>Acknowledge<input  type=\"checkbox\" name=CHK_ACK value=\"N\" unchecked onclick=\"check_change()\" ></td></tr>");
