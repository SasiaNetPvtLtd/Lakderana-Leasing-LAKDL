...
public class LAKDL_AF_MAS_Client_CRIB extends javax.servlet.http.HttpServlet {
  ...
  public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
      ...
      out.println("}");
			...
			out.println("}");
			  ...
			  out.println("var m_sort_column='CLIENT_CODE';");
        ...
        out.println("}");
      	...
      	out.println("}");
		      ...
		      out.println("m_table.innerHTML+='<table align=\"center\" width=\"100%\" class=\"table\"><TR class=\"pdn_txtpos2\" align=\"center\">'+");
					out.println("'<td  width=\"15%\" align=\"left\" style= cursor:hand;  title=\"Click here to sort by - Client Name  \"    onclick=sort_data(\"FULL_NAME\") ><B>Client Name</td>'+");
          out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;  title=\"Click here to sort by - NIC     \"    onclick=sort_data(\"NIC_NO\") ><B>NIC</td>'+");
				  out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;  title=\"Click here to sort by - Date of Birth     \"    onclick=sort_data(\"DATE_OF_BIRTH\") ><B>Date of Birth</td>'+");
          out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;  title=\"Click here to sort by - Client Category  \"        onclick=sort_data(\"CLIENT_CATEGORY\") ><B>Client Category</td>'+");
          out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand;  title=\"Click here to sort by - Address 1  \"    onclick=sort_data(\"ADDRESS1\") ><B>Address 1</td>'+");
					out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand; title=\"Click here to sort by - Address 2  \"    onclick=sort_data(\"ADDRESS2\") ><B>Address 2</td>'+");
					out.println("'<td  width=\"10%\" align=\"left\" style= cursor:hand; title=\"Click here to sort by - City  \"    onclick=sort_data(\"CITY_CODE\") ><B>City</td>'+");
					out.println("'<td  width=\"15%\" align=\"center\"><B>CRIB No</td>'+");
     	...
     	out.println("}");
			...
			*/
			...
			out.println("function CRIB_REquest(row_No) {"); 
			
			out.println("}"); 
			
			out.println("function val_CRIB(row_No) {"); 
			...
			out.println("}"); 
					...
					out.println("  function  display_data(data_vec){");
					 ...
					 out.println("while(i<data_vec.length){");
						...
						out.println("m_full_name='<TD WIDTH=\"15%\" align=\"left\">'+data_vec[i]+'</TD>';");
						out.println("m_nic='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+1]+'</TD>';");
						out.println("m_dob='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+2]+'</TD>';");
						out.println("m_category='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+3]+'</TD>';");
						out.println("m_add1='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+4]+'</TD>';");
						out.println("m_add2='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+5]+'</TD>';");		
						out.println("m_city='<TD WIDTH=\"10%\" align=\"left\">'+data_vec[i+6]+'</TD>';");		
						
						out.println("m_officer='<td width=\"15%\" align=\"center\" ><input class=\"txt_input\" type=\"text\" name=TXT_CRIB_NO'+lineno+' maxlength=\"15\" size=\"15\" onblur=\"val_CRIB('+lineno+')\"></TD>';") ;
			      out.println("m_btn='<td width=\"10%\" align=\"center\" ><input class=\"but_input\" type=\"button\" name=BUT_TXT_CRIB_NO'+lineno+' style=\"width:80px; \"  value=\"CRIB Request\" onClick=\"CRIB_REquest('+lineno+')\"></td>';"); 

						
			
						out.println("m_hid_input='<INPUT TYPE=\"Hidden\" NAME=hid_TXT_CLIENT_CODE'+lineno+'	VALUE='+data_vec[i+7]+'>';");
								
						
						out.println("if(j>0 && j%2==1){");
     
						out.println("m_writedata='<TR class=\"tr_input1\">'+m_full_name+m_nic+m_dob+m_category+m_add1+m_add2+m_city+m_officer+m_btn+'</TR>'+m_hid_input;"); 
					 out.println("	}");
					 out.println("	else{");
      
				   out.println("m_writedata='<TR class=\"tr_input\">'+m_full_name+m_nic+m_dob+m_category+m_add1+m_add2+m_city+m_officer+m_btn+'</TR>'+m_hid_input;"); 
					 out.println("	}");
		...
		//out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MAS_sql_validations1?chksql=m_prime_chk_LAKDL_AF_MAS_CRIB\";");
		out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MAS_sql_validations1?chksql=m_prime_chk_LAKDL_AF_MAS_CRIB&sort_column=\"+m_sort_column+\"&order_by_type=\"+m_order_by_type+\"&ac_status=Y\";");
		//	out.println("window.open(m_url);");
		out.println("load_interface(m_url,'XML');");
		out.println("}");	
			...
			out.println("}");
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_MAS_Save_Client_CRIB';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MAS_Client_CRIB';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("    m_help_message = \"m_help_msg_LAKDL_AF_MAS_Client_CRIB\";"); 
			...
			out.println("help_box.innerHTML=\" System Administration - CRIB No - \"+m_val;"); 
			...
			out.println("help_box.innerHTML=\" System Administration - CRIB No - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
      ...
      out.println("</Script>");
					...
					out.println("<BODY class='body & txt-body' leftmargin='0' topmargin='0' marginwidth='0' onLoad=\"get_Application_numbers('CLIENT_CODE','ASC')\">"); //load_lock()
			    ...
			    out.println("<INPUT TYPE='Hidden' NAME='hid_no_rec' VALUE=\"\">"); 
					out.println("<INPUT TYPE='Hidden' NAME='Hid_scr_name' VALUE=\"AF_AD_CLIENT_CRIB\">"); 
					...
					out.println("</tr>"); 
					out.println("<tr>"); 
					out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>System Administration - CRIB No </td>"); 
					out.println("</tr>"); 
					out.println("<tr>"); 
