...
public class LAKDL_AF_RE_Bulk_Print_Main extends javax.servlet.http.HttpServlet {
		...
		try {
     	...
     	//out.println(reqstr);
			...
			if(m_chksql.trim().equals("PRINT")) {
				...
				/*
				 rs = stmt.executeQuery (" SELECT DISTINCT GROUP_INV_NO, "+
					                       " FINANCE_NO, "+
																 " CLIENT_CODE, "+
																 " LAKDL.AF_CO_GET_CLIENT_NAME(CLIENT_CODE) CLIENT_NAME, "+
																 " TOTAL_AMOUNT , "+
																 " INVOICE_TYPE, "+
																 ...
																 " AND TO_DATE(ENT_DATE,'DD-MON-YY') <= TO_DATE('09-01-2007','DD-MM-YY') "+
																 " AND TO_DATE(ENT_DATE,'DD-MON-YY') > TO_DATE('09-01-2006','DD-MM-YY') ");
														*/
			...
			/*	rs = stmt.executeQuery (" SELECT GROUP_INV_NO, "+
				...
				rs = stmt.executeQuery (
				                        ...
				                        " A.CLIENT_CODE CLIENT_CODE, "+
															  ...
															  " FROM "+m_schema_name+".AF_CO_PRO_INVOICE A , "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
																...
																" AND B.collection_officer<>'L001'"+
																" AND A.invoice_type='INV_GENER' "+
																
										//added by ns on 11-07-2011 to enable the coparate customer in bulk printing						
										" UNION "+
										...
										" ORDER BY CLIENT_CODE,VALUE_DATE  "); 
				...
				*/
				//out.println(
				// commented by thamali 2012.02.22
				/*rs = stmt.executeQuery (
										" SELECT A.INVOICE_NO, "+
				                        " A.FINANCE_NO, "+
				                        " A.CLIENT_CODE CLIENT_CODE, "+
			  ...
			  boolean	more = rs.next();
				...
				int m_ginvoice_no=invoce_print.printing_interface_9(rs.getString(1),m_from_date,m_to_date);
				//Test_Bulk_Print_test test_bulk_print = new Test_Bulk_Print_test();
				//PrintService[]  aprintservice = test_bulk_print.Bulk_Printing_Main_Interface("TPRINT");
				 //for(int j = 0; j < aprintservice.length; j++){
         //out.println("hhhhhhhhhhhhhhhhhh  found--" + aprintservice[j].getName());}
				
				//int m_ginvoice_no=invoce_print.printing_interface_9("GI20080507-4875",m_from_date,m_to_date); ///
				//int m_ginvoice_no=invoce_print.printing_interface_9("GI20071207-0054"); 
			...
			//	for (int j=1;j<=count;j++){
 			...
 			//	} 
			 ...
			 else if(m_chksql.trim().equals("BULK_PRINT_RECEIPT")) {
			  ...
			  boolean	more = rs.next();
				...
				else if(m_chksql.trim().equals("BULK_PRINT_INSUARANCE")) {
				
				Bulk_Printing_Main ins_print =new Bulk_Printing_Main();
				//out.println("---------- m_from_date--------"+m_from_date);
				int count = 1;
								
				rs = stmt.executeQuery ("SELECT FINANCE_NO,PRO_INVOICE_NO ,POLICY_NO FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA "+
				" WHERE  TO_DATE(TO_CHAR(DUE_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')  >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
				" AND    TO_DATE(TO_CHAR(DUE_DATE,'DD-MM-YYYY'),'DD-MM-YYYY')  <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
				" ORDER BY  DUE_DATE ");
			  ...
			  boolean	more = rs.next();
				...
				while(more){
				if(i<1){
				out.println("---------- Policy No--"+i+"------"+rs.getString(3));
				int m_ginvoice_no=ins_print.printing_interface_insurance(rs.getString(1),rs.getString(2));
				//int m_ginvoice_no=pdc_print.printing_interface_pdc("SR0070802-00051");
				//int m_ginvoice_no=pdc_print.printing_interface_pdc("SR0080827-26902");
				out.println("---------- m_ginvoice_no--"+i+"------"+m_ginvoice_no);
				...
				//int m_ginvoice_no=ins_print.printing_interface_insurance("LP070387","PI2005-9874");
