//--
//SCREEN NAME:Credit Process - Payment Approval - NEW
//CREATED BY :delanjli
//DATE/TIME  :
//NOTES      :
...
public class LAKDL_AF_PRO_CR_display_main_screen_payment_details extends javax.servlet.http.HttpServlet { 
		Connection conn;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
		  ...
		  nf.setMaximumFractionDigits(0);
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Credit Process - Payment "+m_level+"</TITLE>"); 
			out.println("</HEAD>"); 
			...
			out.println("m_pay=''");
			out.println("}");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_LAKDL_AF_PRO_CR_display_main_screen_payment_details_sort&screen_type="+m_level+"&app_status=ACTIVATED&payment_status=\"+m_pay+\"&data_val=\"+nval+\"&ac_status=\"+st_des1+\"&sort_column=\"+m_sort_col+\"&order_by_type=\"+m_order_by_type;");
		//out.println("window.open(m_url)");
			...
			out.println("m_pay=''");
			out.println("}");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_LAKDL_AF_PRO_CR_display_main_screen_payment_details_sort&screen_type="+m_level+"&app_status=ACTIVATED&payment_status=\"+m_pay+\"&data_val=\"+nval+\"&ac_status=\"+st_des2+\"&sort_column=\"+m_sort_col+\"&order_by_type=\"+m_order_by_type;");
			out.println("load_interface(m_url,'XML');");
			out.println("}");
			out.println("}");
			...
			out.println("}");
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_save_main_screen_payment_details?number='+document.Form1.hid_no.value+'&actst1='+st_val+'&actst2='+st_val1+'';");   
		...
		//out.println("document.oncontextmenu=new Function(\"return false\");"); 
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_display_main_screen_payment_details?chksql='+st_val+'&chksql2='+st_val1+'';");
			out.println("		}"); 
			out.println("}"); 

			out.println("function new_window(){	"); 
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_display_main_screen_payment_details?chksql='+st_val+'&chksql2='+st_val1+'&st_c="+m_sort_column+"&oby="+m_order_by_type+"';");
			out.println("}"); 
			out.println(""); 
			out.println(""); 
			...
			out.println("help_box.innerHTML=\" Credit Process - Payment  "+m_level+"- \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\" Credit Process - Payment  "+m_level+"- \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
	  	...
	  	out.println("		}"); 
			...
			out.println("	m_url = '"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_display_payment_details_screen1?PO_NO='+document.Form1.elements[pur].value+'&REQ_NO='+document.Form1.elements[\"TXT_REQU_\"+row].value+'&CLSTATUS=A&SUS_REF_NO='+document.Form1.elements[\"TXT_SUS_REF_NO_\"+row].value+'&REF_NO='+document.Form1.elements[\"TXT_REF_NO_\"+row].value+'&st_val="+m_chksql+"&st_val1="+m_chksql1+"';"); 
			out.println(" window.location.href=m_url;"); 
			out.println("}"); 
			out.println("}"); 
			out.println("else if(st_val1=='A'){");
			out.println("		if(confirm(\"Are you sure you want to view Approval level 2?\")){ "); 
			out.println("	m_url = '"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_display_payment_details_1?PO_NO='+document.Form1.elements[pur].value+'&REQ_NO='+document.Form1.elements[\"TXT_REQU_\"+row].value+'&CLSTATUS=A&SUS_REF_NO='+document.Form1.elements[\"TXT_SUS_REF_NO_\"+row].value+'&REF_NO='+document.Form1.elements[\"TXT_REF_NO_\"+row].value+'&st_val="+m_chksql+"&st_val1="+m_chksql1+"';"); 
		  ...
		  out.println("var j=0;");
			...
			out.println("if(st_val!='B'){");//requ /approve1
			...
			out.println("if(st_val=='R' && (parseFloat(data_vec[i+18]) < parseFloat(data_vec[i+19]))){");//approve1
			out.println("m_row=m_row+'<td width=\"7%\" align=\"center\" ><input class=\"but_input\" type=\"button\" style=\"width: 50px\" name=BUT_VIEW_'+j+' value=\"Approve\" onClick=\"help_button_1('+j+')\" disabled><input class=\"txt_input2\" type=\"hidden\" name=TXT_VAT_'+j+' value=\"'+data_vec[i+6]+'\" maxlength=\"25\" size=\"25\"><input type=\"hidden\" name=TXT_REQU_'+j+' value=\"'+data_vec[i+14]+'\"></td></tr></table>';"); 
			out.println("}");
			out.println("if(st_val=='R' && (parseFloat(data_vec[i+18]) >= parseFloat(data_vec[i+19]))){");//approve1
			...
			out.println("if(st_val=='VERIFY'){");//approve1
			out.println("m_row=m_row+'<td width=\"7%\" align=\"center\" ><input class=\"but_input\" type=\"button\" style=\"width: 50px\" name=BUT_VIEW_'+j+' value=\"Approve\" onClick=\"help_button_1('+j+')\" ><input class=\"txt_input2\" type=\"hidden\" name=TXT_VAT_'+j+' value=\"'+data_vec[i+6]+'\" maxlength=\"25\" size=\"25\"><input type=\"hidden\" name=TXT_REQU_'+j+' value=\"'+data_vec[i+14]+'\"></td></tr></table>';"); 
			out.println("}");					
			...
			out.println("if(parseFloat(data_vec[i+18]) < parseFloat(data_vec[i+19])){");//approve2
		...
		//**********	
			...
			out.println("'<td width=\"6%\" align=\"left\" >'+data_vec[i+12]+'</td>';");
			//****
			out.println("if((st_val=='R' || st_val=='B') && (parseFloat(data_vec[i+18]) <parseFloat(data_vec[i+19]))){");//approve1
			out.println("m_row=m_row+'<td width=\"4%\" align=\"center\" ><input type=\"checkbox\" name=CHK_APP_'+j+' value=\"N\" unchecked  onclick=\"check_change('+j+')\" disabled><input type=\"hidden\" name=TXT_SUS_REF_NO_'+j+' VALUE=\"'+data_vec[i+16]+'\"><input type=\"hidden\" name=TXT_REF_NO_'+j+' VALUE=\"'+data_vec[i+17]+'\"><input class=\"but_input\" style=\"width: 25px\" type=\"hidden\" name=BUT_VIEW_'+j+' value=\"Approve\" onClick=\"help_button_1('+j+')\"><input type=\"hidden\" name=TXT_REQU_'+j+' value=\"'+data_vec[i+14]+'\"><input type=\"hidden\" name=TXT_SETTLE_'+j+' value=\"'+data_vec[i+15]+'\"></td></tr></table>';");
			out.println("}");
			out.println("else if((st_val=='R' || st_val=='B') && (parseFloat(data_vec[i+18]) >= parseFloat(data_vec[i+19]))){");//approve1
...
//-------------------------------------------------------------------------------------------------------
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Credit Process - Payment  "+m_level+"- New</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
