/************************CREATED BY MILINDA -FOR -OTHER PAYMENT REVERSAL 2015-09-24************************************/
/***********************                  SYSTEM: LAKDERANA                        ***********************************/
...
public class LAKDL_AF_PRO_CR_Other_Payment_Reversal_display extends HttpServlet {
	...
	public synchronized void service(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws IOException {
		...
		try {
			...
			m_screen_title = "Finance - Other Payment Reversal";
			...
			if(m_chksql.equals("main_page")){
				...
				out.println("                   document.Form1.action = '" + m_class_url + "/" + m_fschema_name + "AF_PRO_CR_Other_Payment_Reversal_save';");
				out.println("                   document.Form1.submit();");
				out.println("               }");
				out.println("           }");
				
				
				//-----------------------------------------MAIN SEARCH BUTTON GONE HERE ---------------------------------------------------------//
				out.println("           function load_payments() {");
				
				out.println("               g_get_data_type = 1;");
				out.println("               m_url = '" + m_class_url + "/" + m_fschema_name + "AF_PRO_CR_Other_Payment_Reversal_display?chksql=load_data&payee='+ document.getElementById('CLIENT_CODE').value+'&pamt_no='+ document.getElementById('TXT_PAYMENT_NO').value+'&process_status=' + document.getElementById('SEL_REVERSE_TYPE').value;");
				...
				out.println("               var m_url = servlet_client_url + ':' + client_t3_port + '/' + client_name + 'AF_RE_PRO_drill_downs_two?chksql=SHOW_PAYMENT_DRILL&payment_no=' + group_payment_no;");
				
				//LAKDL_AF_RE_PRO_drill_downs_two?chksql=SHOW_PAYMENT_DRILL&payment_no=SP20081231-1514
				// out.println("               load_interface(m_url, 'NORM');");
				...
				//out.println(" document.getElementById('hid_selected_checkbox_count').value='1'");
				...
				/*out.println("function check_approve(row_no){")	;
				out.println("m_chk_required=\"CHK_APPROVE_\"+row_no;");
				out.println("alert(m_chk_required);");
				
				out.println("if(document.Form1.elements[m_chk_required].checked==true){");
				out.println("document.Form1.elements[m_chk_required].value='A'");
				...
				out.println("           <input type=\"hidden\" name=\"hid_selected_checkbox_count\" id=\"hid_selected_checkbox_count\" value=\"1\" />");
				...
				out.println("                                                       <option value=\"APPRO2\">Other Payment Approval 2</option>");
				out.println("                                                       <option value=\"APPRO1\">Other Payment Approval 1</option>");
				out.println("                                                       <option value=\"Y\">Other Payment Account Selection</option>");
				out.println("                                                       <option value=\"ENTER\">Other Payment Authorizations </option>");
				out.println("                                                       <option value=\"AUTHO\">Other Payment Processing</option>");
			...
			//------------------------------------MAIN SEARCH QUERY RESULT SET------------------------------------------------------//
			else if (m_chksql.equals("load_data")) {
				
				process_status_get 	= httpServletRequest.getParameter("process_status");
				String m_payee   	= httpServletRequest.getParameter("payee");		
				String m_pamt_no   	= httpServletRequest.getParameter("pamt_no");	
				...
				if (process_status_get.equals("PRINT")) {
					...
					process_status_post = "Y";
				}
				else if (process_status_get.equals("Y")) {
					process_status_post = "ENTER";
				}
				else if (process_status_get.equals("ENTER")) {
					process_status_post = "AUTHO";
				}
				
				else if (process_status_get.equals("AUTHO")) {
					process_status_post = "AUTHO";
				}
				sql = " " +
					"   SELECT B.PAYMENT_NO, " +
					...
					"   AND    A.ENTRY_TYPE <> 'V' " +
					"   AND    B.PAYMENT_NO IS NOT NULL " +
					"   AND   UPPER( B.PAYMENT_NO) LIKE UPPER('%"+m_pamt_no+"%') "+ 
					"   AND   UPPER( B.PAYEE_NAME) LIKE UPPER('%"+m_payee+"%') "+ 
					"   GROUP BY B.PAYMENT_NO, " +
					"            B.PAYEE_NAME, " +
					"            B.SETTLE_MODE " +
					" ";
				...
				out.println("<td width=\"4%\" align=\"center\">Approved <input type=\"checkbox\" name=\"CHECK_ALL\" id=\"CHECK_ALL\" value=\"\" onclick=\"check_all_chkbox();\" /> </td>"); 
				...
				while (resultSet.next()) {
					...
					out.println("<td style=\"text-align: center;\"><div>" + resultSet.getString("PAYMENT_NO") + "</div><input type=\"hidden\" name=\"HID_GROUP_PAYMENT_NO_" + i + "\" id=\"HID_GROUP_PAYMENT_NO_" + i + "\" value=\"" + resultSet.getString("PAYMENT_NO") + "\" /></td>");
