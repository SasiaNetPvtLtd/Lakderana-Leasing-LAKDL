...
public class LAKDL_AF_MISF_CR_Notes_Report_auto extends javax.servlet.http.HttpServlet { 
				...
				*/
				...
				Sql_data=" "+		
					" SELECT "+
						...
						" WHERE  A.FINANCE_NO LIKE '%%' "+
						" AND    A.FINANCE_NO = B.FINANCE_NO "+
						" AND    A.FINANCE_NO IN (SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOVERY_REPORT_HIS WHERE ENT_USER = '"+m_username+"' AND TRUNC(REPORT_DATE) = TO_DATE('"+m_to_date+"','DD-MM-YYYY')  UNION SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOVERY_REPORT_DAILY WHERE ENT_USER = '"+m_username+"' AND TRUNC(REPORT_DATE) = TO_DATE('"+m_to_date+"','DD-MM-YYYY') ) "+
						" AND    B.BRANCH_CODE LIKE '"+m_location+"%' "+
						" AND    A.INVOICE_NO IN "+
						      " ( SELECT  INVOICE_NO "+
						              " FROM     "+m_schema_name+".AF_CO_PRO_INVOICE "+
					...
					" SELECT "+
						...
						" WHERE  A.FINANCE_NO LIKE '%%' "+
						" AND    A.FINANCE_NO = B.FINANCE_NO "+
						//" AND    A.FINANCE_NO IN (SELECT FINANCE_NO FROM "+m_schema_name+".AF_RE_TBD_RECOVERY_REPORT WHERE ENT_USER = '"+m_username+"') "+
						" AND    A.FINANCE_NO IN (SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOVERY_REPORT_HIS WHERE ENT_USER = '"+m_username+"' AND TRUNC(REPORT_DATE) = TO_DATE('"+m_to_date+"','DD-MM-YYYY')  UNION SELECT FINANCE_NO FROM "+m_schema_name+".AF_MIS_RECOVERY_REPORT_DAILY WHERE ENT_USER = '"+m_username+"' AND TRUNC(REPORT_DATE) = TO_DATE('"+m_to_date+"','DD-MM-YYYY') ) "+
						" AND    B.BRANCH_CODE LIKE '"+m_location+"%' "+
						" AND    A.INVOICE_NO IN "+
						      " ( SELECT INVOICE_NO "+
