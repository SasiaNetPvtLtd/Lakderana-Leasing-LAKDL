...
public class LAKDL_AF_CR_PRO_Credit_Approval_Saction_Letter extends javax.servlet.http.HttpServlet { 
	...
	*/
	...
	public void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { // synchronized
		...
		Statement stmt=null,stmt_rental=null,stmt_charges=null,stmt_price=null,stmt_ter=null;
		java.text.NumberFormat nf=null,nf1=null;
		ResultSet rs=null,rs_rental=null,rs_charges=null,rs_price=null,rs_ter=null;
		...
		try { 
			...
			double m_insurance_premium_new = 0;
			...
			*/
			...
			// added by udara 29-05-2014
			String Sql_broker = " "+
				 " SELECT NVL(D.AMOUNT,0), NVL(D.PAYEE_CODE,'-') , NVL("+m_schema_name+".AF_CO_GET_SUB_CHARG_PAYEE_NAME(D.PAYEE_CODE),'-') "+
					" ,NVL(E.PREMIUM,0) "+ // 4
					" ,(SELECT INSURANCE_DONE_BY FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS WHERE APPLICATION_NO = '"+m_applicaton_no+"' ) INSURANCE_DONE_BY   "+ // 5
				  ...
				  " WHERE E.FINANCE_NO = "+m_schema_name+".AF_CO_GET_FINANCE_NO('"+m_applicaton_no+"') "+
				...
				if(more){
					m_insurance_agent = rs.getString(3);
					
					if(rs.getString(5).equals("CLIENT"))
						m_insurance_premium_new = 0;
					else
					 	m_insurance_premium_new = rs.getDouble(4); // udara 06-06-2014
				}
				
				
				
				if(m_insurance_agent==null || m_insurance_agent.equals("")){
					
					Sql_broker=" SELECT DISTINCT B.payee_name, A.amount "+
						" FROM "+m_schema_name+".af_co_pro_app_pricing_charges a , "+m_schema_name+".af_cr_pro_sub_char_payee_ref B "+
						" WHERE A.payee_code=B.payee_code "+
						" AND   B.sub_type_code='INSURANCE' "+
						" AND   A.application_no='"+m_applicaton_no+"' ";
					...
					{
						m_insurance_agent=rs.getString(1);
						m_insurance_premium_new = rs.getDouble(2); // udara 06-06-2014
					}
					
				}
				
			// end by udara 29-05-2014
			
			// added by udara 04-06-2014
			//if(m_insurance_agent==null || m_insurance_agent.equals("")){ // commented by udara 20-12-2016
			if(m_insurance_agent==null || m_insurance_agent.equals("") || m_insurance_agent.equals("-")){ // added by udara 20-12-2016
				
				 Sql_broker = " "+
				 " SELECT  PAYEE_NAME, PREMIUM, INSURANCE_DONE_BY FROM (   "+
						 " SELECT NVL(E.SUM_INSSURED,0) SUM_INSSURED, NVL(E.INSUR_COM,'-') INSUR_COM , NVL("+m_schema_name+".AF_CO_GET_SUB_CHARG_PAYEE_NAME(E.INSUR_COM),'-') PAYEE_NAME "+
							" ,NVL(E.PREMIUM,0) PREMIUM   "+
							" ,(SELECT INSURANCE_DONE_BY FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS WHERE APPLICATION_NO = '"+m_applicaton_no+"' ) INSURANCE_DONE_BY   "+
			...
			// added by udara 21-08-2014
				...
				Sql_broker = " "+ 
					" SELECT NVL(SUM(TOTAL_AMOUNT),0) "+
						...
						" WHERE FINANCE_NO = "+m_schema_name+".AF_CO_GET_FINANCE_NO('"+m_applicaton_no+"') "+
							//" AND INVOICE_TYPE IN (SELECT TYPE_CODE  "+
							" AND INVOICE_TYPE IN (SELECT SUB_TYPE_CODE  "+ // udara 04-09-2014
			...
			String insurance_done_by = "";
			String application_comments  = "";
			...
			String Sql_client_data="SELECT DISTINCT A.APPLICATION_NO, "+//1
				...
				//" (SELECT NVL(FIRST_NAME || ' ' || LAST_NAME,'-') FROM "+m_schema_name+".AF_CO_MAS_BROKER WHERE BROKER_CODE=A.LEAD_SOURCE_NAME ) BROKER_CODE , "+ // commented by udara 21-02-2014 //4 //NVL(G.LEAD_SOURCE_NAME,'-')
				" NVL((SELECT NVL(FIRST_NAME,'-') || ' ' || NVL(LAST_NAME,'-') FROM "+m_schema_name+".AF_CO_MAS_BROKER WHERE BROKER_CODE=A.LEAD_SOURCE_NAME ),'-') BROKER_CODE , "+ // added by udara 21-02-2014
				
				//" UPPER(NVL((SELECT DISTINCT(NAME) FROM "+m_schema_name+".CO_CO_MAS_USER WHERE USER_ID=C.MK_OFFICER ),'-')) ,"+ //13
				
				//" UPPER(NVL((SELECT FIRST_NAME || ' ' || LAST_NAME FROM "+m_schema_name+".CO_CO_MAS_EMPLOYEE WHERE EMP_CODE=C.MK_OFFICER ),'-')) ,"+ //13 // commented by udara 11-02-2015
				...
				*/
				...
				" AND SUSPENSE_ENTRY_TYPE='INSURANCE' AND ROWNUM           = 1 ) INSURANCE_AGENT, "+ // 23 AND ROWNUM           = 1 added milinda 2014-02-25
				...
				*/
				...
				// recommented by udara 24-02-2014
				/*
				...
				*/
				...
				// added by udara 24-02-2013
				" ( "+
				
				" SELECT SUM_INSURED FROM(  "+
				...
				" ) SUM_INSURED, "+ // 24
				// end by udara on 24-02-2013
				
				// commented by udara 31-10-2014
				/*
				" ( SELECT NVL(REG_NO,'-') "+
				" FROM "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS "+
				" WHERE APPLICATION_NO = '"+m_applicaton_no+"' AND ACTIVE_STATUS='Y'  AND ROWNUM           = 1 ) REG_NO, "+ // 25 AND ROWNUM           = 1 added milinda 2014-02-25
				*/
				
				// added by udara 31-10-2014
				"  NVL("+m_schema_name+".AF_CO_GET_VEHICLE_NO('"+m_applicaton_no+"'),'-') REG_NO, "+
				
				// commented by udara 31-10-2014
				/*
				" ( SELECT NVL("+m_schema_name+".AF_CO_GET_VENDOR_NAME(VENDOR_CODE),'-') "+
				" FROM "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS "+
				" WHERE APPLICATION_NO = '"+m_applicaton_no+"' AND ACTIVE_STATUS='Y' AND ROWNUM           = 1) VENDOR_CODE, "+	// 26 AND ROWNUM           = 1 added milinda 2014-02-25
				...
				*/
				...
				" ( SELECT NVL("+m_schema_name+".AF_CO_GET_VENDOR_NAME(VENDOR_CODE),'-') "+
				" FROM "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS "+
				" WHERE APPLICATION_NO = '"+m_applicaton_no+"'  AND ROWNUM           = 1) VENDOR_CODE, "+
				
				// end by udara 31-10-2014
				
				" NVL("+m_schema_name+".AF_CO_GET_VEHICLE_NO_NEW(A.APPLICATION_NO),'-') "+ // added by udara on 15-07-2013 // 27
				//" , A.LEAD_SOURCE_NAME"+
				" , NVL(A.LEAD_SOURCE_NAME,'-'), "+
				
				// added by udara on 24-02-2014
				/*
				" (SELECT TO_CHAR(MAX(RENTAL_DATE),'DD')  "+
									" FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT  "+
									" WHERE APPLICATION_NO = '"+m_applicaton_no+"') DUE_DATE "+ // 29
									*/
				
				// end by udara on 24-02-2014
				
				" DECODE(A.APPLICATION_STATUS,'ENT_CON','-','ENTERED','-','VERIFY1','-','CANCEL_PO','-',   "+
				...
				" UPPER(NVL((SELECT FIRST_NAME || ' ' || LAST_NAME FROM "+m_schema_name+".CO_CO_MAS_EMPLOYEE WHERE EMP_CODE=C.CR_OFFICER ),'-')), "+//31 added milinda 2014-04-28 ck officer
				//" (SELECT NVL(E.SUM_INSSURED,0) SUM_INSSURED FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA E WHERE E.FINANCE_NO="+m_schema_name+".AF_CO_GET_FINANCE_NO('"+m_applicaton_no+"')) "+//32 ADDED MILINDA 2014-06-10
				" (SELECT NVL(E.SUM_INSSURED,0) SUM_INSSURED FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA E WHERE E.FINANCE_NO="+m_schema_name+".AF_CO_GET_FINANCE_NO('"+m_applicaton_no+"') AND ROWNUM=1) "+//mod udara 19-08-2014 32 ADDED MILINDA 2014-06-10
				//" ,INSURANCE_DONE_BY  "+ // added by udara 27-02-2015
				//" ,DECODE(INSURANCE_DONE_BY,'LICENSEE','Company','CLIENT','Client',INSURANCE_DONE_BY)  "+ // added by udara 27-02-2015 // commented by udara 23-10-2018
				" ,DECODE(TRANSACTION_TYPE,'LOANS','-',DECODE(INSURANCE_DONE_BY,'LICENSEE','Company','CLIENT','Client',INSURANCE_DONE_BY))  "+ // added by udara 23-10-2018
				" ,NVL(" + m_schema_name + ".AF_CO_GET_REMARK(A.APPLICATION_NO),'-') "+ // added by udara 15-05-2015 - 33
			...
			if(more)
			{
				...
				insurance_done_by =rs.getString(32); // added by udara 27-02-2015
				application_comments =rs.getString(33); // added by udara 15-05-2015
			}
			
			    String m_ins_company="";
				double m_premium=0.0;
				
				rs=stmt.executeQuery(
					//out.println(
					"SELECT "+m_schema_name+".AF_CO_GET_SUB_CHARG_PAYEE_NAME(INSUR_COM), " +
					"  PREMIUM " +
				...
				if(rs.next()){
					m_ins_company = rs.getString(1);
					m_premium     = rs.getDouble(2);
				}
				rs.close();
			...
			*/
			...
			String				Sql_data_asset_details="  SELECT "+												
				...
				//" AND A.ACTIVE_STATUS='Y'  "+ // commented by udara 03-11-2014
			...
			if(m_termi_type.equals("RESCHEDULE")){
				Sql_data_facility_details="  SELECT "+												
					...
					//"  AND A.ACTIVE_STATUS='Y' "+ // commented by udara 03-11-20141
			...
			String				Sql_data_Comments="  SELECT "+												
				...
				" A.APPLICATION_CODE='"+m_applicaton_no+"' AND "+
			...
			String				Sql_data_approval_prcess=" SELECT * FROM ( "+
				
				/*
				// added by udara 13-08-2014
				" SELECT "+																								 
				" APPLICATION_NO, 'Application Process - Entered','1' AS E_LEVEL ,"+m_schema_name+".AF_CO_GET_USER_NAME(A.ENT_USER),A.ENT_USER,TO_CHAR(A.ENT_DATE,'HH:MI:SS'),TO_CHAR(A.ENT_DATE,'DD-MM-YYYY'),NVL(REMARK,'-'),C.DESIGNATION_NAME,A.ENT_DATE ENT_DATE2   "+
				...
				"                   A.STATUS IN ('ENTERED') AND APPLICATION_NO='"+m_applicaton_no+"'   "+ 
				
				"  UNION  "+
				// end by udara 13-08-2014
				*/
				...
				" WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				//"                   A.STATUS IN ('ENTERED') AND APPLICATION_NO='"+m_applicaton_no+"'   "+ // commented by udara 12-05-2014
				"                   A.STATUS IN ('ENT_CON') AND APPLICATION_NO='"+m_applicaton_no+"'   "+ // added by udara 12-05-2014
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='ENTERED')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='VERIFY1' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-VERIFY1' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-VERIFY1')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='V-APP' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='V-APP')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-V-APP' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-V-APP')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='V-RECOM' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='V-RECOM')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-V-RECOM' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-V-RECOM')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='VERIFY-M' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-VERIFY-M' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-VERIFY-M')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='VERIFY2' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='VERIFY2')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-VERIFY2' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-VERIFY2')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='RET-VERY-M' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='RET-VERY-M')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='RET-EN_CON' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='RET-EN_CON')  "+
				//added by nuwan de silva 29-10-07----------------------------------------------------	
				" UNION  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='RET-VERY-1' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='RET-VERY-1')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='R-RET-VERY-1' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='R-RET-VERY-1')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='RET-VE-APP' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				//" AND A.ENT_DATE IN  "+
				//" (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_APPROVAL WHERE APPLICATION_NO='"+m_applicaton_no+"' AND STATUS='RET-VE-APP')  "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='PO_DEL' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='PO_ENT' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='PO_APP' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='PO_DISAPP' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
				...
				"                   WHERE B.USER_ID=A.ENT_USER AND "+
				"                         C.DESIGNATION_CODE=B.DESIGNATION_CODE AND "+
				"                         A.STATUS='CANCEL_PO' AND APPLICATION_NO='"+m_applicaton_no+"'   "+
			...
			String				Sql_data_score_model="  SELECT "+																								
				...
				" WHERE APPLICATION_CODE='"+m_applicaton_no+"' ";
			...
			String				Sql_data_agm_note="  SELECT "+																								
				...
				" WHERE B.USER_ID=A.ENT_USER AND  "+
				" C.DESIGNATION_CODE=B.DESIGNATION_CODE AND  "+
				" A.STATUS='AGM_APP' AND APPLICATION_NO='"+m_applicaton_no+"'   ";
     	...
     	out.println("}");
			*/
			...
			out.println("}");
				...
				*/
				...
				out.println("'<TD WIDTH=\"10%\"  align=\"left\">-</TD>'+");
				//out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+lineno+' style=\"width:250px; height:20px;\" maxlength=\"200\" size=\"200\" onblur=\"\">'+");
				out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+lineno+' style=\"width:250px; height:20px;\" maxlength=\"200\" size=\"200\" onblur=\"\" disabled >'+"); // added by udara 18-04-2016
				out.println("'<TD WIDTH=\"10%\"  align=\"left\">-</TD>'+");
				out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_TXT_STATUS'+lineno+'	VALUE=\"-\">'+");
			...
			out.println("continue;");
				...
				*/
				...
				out.println("'<TD WIDTH=\"10%\"  align=\"left\">-</TD>'+");
				//out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+j+' style=\"width:250px; height:20px;\" maxlength=\"200\" size=\"200\" onblur=\"\"></TD>'+");
				out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+j+' style=\"width:250px; height:20px;\" maxlength=\"200\" size=\"200\" onblur=\"\" disabled ></TD>'+"); // added by udara 18-04-2016
				out.println("'<TD WIDTH=\"10%\" align=\"left\">-</TD>'+");
				out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_TXT_STATUS'+j+'	      VALUE=\"-\">'+");
				...
				out.println("'<TD WIDTH=\"10%\"  align=\"left\">-</TD>'+");
				//out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+j+' style=\"width:250px; height:20px;\" value=\"'+array_condition[j]+'\" maxlength=\"200\" size=\"200\" onblur=\"\" ></TD>'+");
				out.println("'<TD WIDTH=\"30%\" align=\"left\"><input class=\"txt_input\" type=\"textarea\" name=TXT_CONDITION'+j+' style=\"width:250px; height:20px;\" value=\"'+array_condition[j]+'\" maxlength=\"200\" size=\"200\" onblur=\"\" disabled ></TD>'+"); // added by udara 18-04-2016
				out.println("'<TD WIDTH=\"10%\" align=\"left\">-</TD>'+");
				out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_TXT_STATUS'+j+'	     VALUE=\"-\" >'+");
				out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_DOCUMENT'+j+'	       VALUE=\"-\" >'+");
				out.println("'<INPUT TYPE=\"Hidden\" NAME=hid_STAGE'+j+'	           VALUE=\"-\" >'+");
				...
				if(more){
					...
					String m_address="";
					String m_mk_creditofficer=rs.getString(30);
					
					// added by udara on 10-04-2013
					//String m_insurance_agent  =rs.getString(23);
					//double m_sum_insured      =rs.getDouble(24);//COMMENTED BY MILINSDA 2014-06-10
					double m_sum_insured      =rs.getDouble(31);//ADDED MILINDA 2014-06-10
					...
					String m_due_date      =rs.getString(29); // added by udara 24-02-2014
					...
					Sql_data_exposure_prev="SELECT "+												
						...
						"		B.APPLICATION_NO <> '"+m_applicaton_no+"' AND "+
						"		A.ACTIVE_STATUS='Y' AND "+
						"		A.BALANCE_TO_BE_RECEIVED>0  AND "+
						"		A.CLIENT_CODE='"+m_client_code+"') A, "+
						...
						"		B.CLIENT_CODE='"+m_client_code+"'  AND"+
						"		A.APPLICATION_NO=B.APPLICATION_NO AND "+ 
						"		A.APPLICATION_NO <> '"+m_applicaton_no+"'  AND "+
						...
						*/
						...
						"		FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT A,  "+
						"		"+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
						"		WHERE B.CLIENT_CODE='"+m_client_code+"'  AND "+
						"			A.APPLICATION_NO=B.APPLICATION_NO AND   "+
						"		A.APPLICATION_NO <> '"+m_applicaton_no+"'  AND  "+
					...
					out.println("    <td><b>Credit Officer</td>");
					out.println("    <td>:</td>");
					out.println("    <td>"+m_mk_creditofficer+"</td>");
					out.println("   </tr>");
					...
					out.println("         <td class=div_input style={cursor:hand;} onClick=\"show_run_con_det('"+m_applicaton_no+"');\" ><u>view</u></td>"); // udara 21-10-2014
					
					
					out.println("    <td><b> Rental Date</td>"); // udara 24-02-2014 // out.println("    <td><b> </td>");
					out.println("    <td>  &nbsp; </td>");
					out.println("    <td>  "+m_due_date+" </td>");// udara 24-02-2014 //  out.println("    <td>  &nbsp; </td>");
					out.println("   </tr>");
					...
					out.println("    <td>:</td>");
					out.println("    <td>"+m_vendor_code+"</td>");
					out.println("    <td><b> Insurance Premium </td>"); // mod udara 06-06-2014 udara 24-02-2014 out.println("    <td><b> </td>");
					out.println("    <td>  &nbsp; </td>");
					out.println("    <td>  "+nf.format(m_insurance_premium_new)+" </td>"); // mod udara 06-06-2014  udara 24-02-2014 out.println("    <td>  &nbsp; </td>");
					out.println("   </tr>");
					
					// added by udara 21-08-2014
					out.println("     <tr>");
					out.println("    <td><b>Stamp Duty</td>");
					out.println("    <td>:</td>");
					out.println("    <td>"+nf.format(m_stamp_duty)+"</td>");
					out.println("    <td> Insurance Done by </td>"); // mod by udara 27-02-2015
					out.println("    <td> &nbsp; </td>");
					out.println("    <td> "+insurance_done_by+" </td>"); // mod by udara 27-02-2015
					out.println("   </tr>");
					// end by udara 21-08-2014
					
					//added by jithendra 9-04-2019
					
					out.println("     <tr>");
					out.println("    <td><b>Security Insurance Company</td>");
					out.println("    <td>:</td>");
					out.println("    <td>"+m_ins_company+"</td>");
					out.println("    <td><b> Security Insurance Premium </td>"); // mod by udara 27-02-2015
					out.println("    <td> &nbsp; </td>");
					out.println("    <td> "+nf.format(m_premium)+" </td>"); // mod by udara 27-02-2015
					out.println("   </tr>");
					
					// added by udara 15-05-2014
					/*
					out.println("     <tr>");
					out.println("    <td><b>Application Comments</td>");
					out.println("    <td>:</td>");
					out.println("    <td>"+application_comments+"</td>");
					out.println("    <td> &nbsp; </td>"); // mod by udara 27-02-2015
					out.println("    <td> &nbsp; </td>");
					out.println("    <td> &nbsp; </td>"); // mod by udara 27-02-2015
					out.println("   </tr>");
					// end by udara 15-05-2014
					*/
				...
				while(more){
					...
					*/
					...
					rs_charges =stmt_charges.executeQuery("SELECT "+
						...
						"  A.SUB_CHAGE_CODE, "+ //2
						"  B.DESCRIPTION, "+ //3
						"  NVL(SUM(C.AMOUNT),0) "+ //4
						//"  CHARGE_TYPE "+
						"  ,A.CHARGE_TYPE "+ // added by udara 09-03-2017
						"  FROM "+m_schema_name+".AF_CO_PRO_APP_PRICING_CHARGES A, "+m_schema_name+".AF_CO_MAS_SUB_CHARGES B, "+m_schema_name+".AF_MK_PRO_PRICING_CHARGES C "+
						"  WHERE A.APPLICATION_NO='"+m_applicaton_no+"'  "+
						"  AND A.PRICING_NO = '"+m_pricing_no+"'   "+
						"  AND A.PRICING_NO = C.PRICING_NO   "+
						"  AND A.SUB_CHAGE_CODE = C.SUB_CHAGE_CODE   "+
						"  AND A.SUB_CHAGE_CODE = B.SUB_TYPE_CODE  "+
						"  AND NVL(C.AMOUNT,0) <> 0 "+
						//"  GROUP BY A.PRICING_NO,A.SUB_CHAGE_CODE,B.DESCRIPTION "); // commented by udara 09-03-2017
						"  GROUP BY A.PRICING_NO,A.SUB_CHAGE_CODE,B.DESCRIPTION, A.CHARGE_TYPE "); // added by udara 09-03-2017
					...
					while(rs_charges.next()){
						...
						//total_charges=total_charges+rs_charges.getDouble(4); // commented by udara 09-03-2017
						
						// added by udara 09-03-2017
						if(!rs_charges.getString(5).equals("AMO")){
							if(!rs_charges.getString(2).equals("BROKERCOMM")){
					...
					" MAX(PERIOD) "+	
						...
						" WHERE APPLICATION_NO= '"+m_applicaton_no+"' ";
					//" AND  ENT_DATE IN(SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_CO_PRO_APP_PRICING WHERE APPLICATION_NO='"+m_applicaton_no+"')";//Added By Sandun on 24-06-2009
					*/
					...
					String sql_rent_new="   SELECT  "+
						//" TO_NUMBER(INSTALLMENT_NO) +1 , "+//1 // commented by udara 29-10-2018
						" TO_NUMBER(INSTALLMENT_NO)  , "+//1 // added by udara 29-10-2018
						" SUM(NET_RENTAL_AMOUNT),  "+//2
						" SUM(GRENTAL_AMOUNT - NET_RENTAL_AMOUNT) VAT_AMOUNT,  "+//3
						" SUM(GRENTAL_AMOUNT) "+//4
						" FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT  "+
						" WHERE  APPLICATION_NO='"+m_applicaton_no+"' AND "+
						" PRICING_NO='"+m_pricing_no+"' AND "+//m_pricing_no
					...
					if(more3){
						...
						out.println("               <td><div align=\"right\"><b> - </div></td>"); // 23-10-2018
						out.println("               <td>&nbsp;</td>");
						out.println("             </tr>");
					}				
			...
			//	}
				...
				if(_m_tmp_pricing_count > 1 ) {
					...
					rs_charges =stmt_charges.executeQuery("SELECT "+
						...
						"  A.SUB_CHAGE_CODE, "+ //2
						"  B.DESCRIPTION, "+ //3
						"  NVL(SUM(A.AMOUNT),0) "+ //4//C.AMOUNT MODIFIED BY MILINDA 2015-09-22 FOR INCORRECT CHARGE VALUE PICK ##18142
						//"  CHARGE_TYPE "+
						"  ,A.CHARGE_TYPE "+ // added by udara 09-03-2017
						"  FROM "+m_schema_name+".AF_CO_PRO_APP_PRICING_CHARGES A, "+m_schema_name+".AF_CO_MAS_SUB_CHARGES B, "+m_schema_name+".AF_MK_PRO_PRICING_CHARGES C "+
						"  WHERE A.APPLICATION_NO='"+m_applicaton_no+"'  "+
						...
						"  AND A.SUB_CHAGE_CODE = B.SUB_TYPE_CODE  "+
						"  AND NVL(C.AMOUNT,0) <> 0 "+
						//"  GROUP BY A.SUB_CHAGE_CODE,B.DESCRIPTION "); // commented by udara 09-03-2017	
						"  GROUP BY A.PRICING_NO,A.SUB_CHAGE_CODE,B.DESCRIPTION, A.CHARGE_TYPE "); // added by udara 09-03-2017
						...
						if(m_termi_type.equals("RESCHEDULE")){
							...
							*/
							...
							sql_rent_new="   SELECT  "+
								//" TO_NUMBER(INSTALLMENT_NO) +1 ,  "+ // commented by udara 29-10-2018
								" TO_NUMBER(INSTALLMENT_NO)  , "+//1 // added by udara 29-10-2018
								...
								//" AND   B.ACTIVE_STATUS='Y' "+ // commented by udara 03-11-2014
						...
						if(m_NIBSM>0){
							...
							sql_rent_new="   SELECT  "+
								...
								" AND   A.PRO_INVOICE_NO=B.INVOICE_NO "+
								" AND   A.PRICING_NO=B.PRICING_NO "+
								//" AND   B.ACTIVE_STATUS='Y' "+ // commented by udara 03-11-20141
								" GROUP BY   TO_NUMBER(INSTALLMENT_NO)   "+
								" ORDER BY TO_NUMBER(INSTALLMENT_NO)  ";
			...
			*/
					String sql_rent_tot="   SELECT  "+
						//" TO_NUMBER(INSTALLMENT_NO) +1 ,  "+ // commented by udara 29-10-2018
						" TO_NUMBER(INSTALLMENT_NO) ,  "+ // added by udara 29-10-2018
						" SUM(NET_RENTAL_AMOUNT),   "+
						" SUM(GRENTAL_AMOUNT - NET_RENTAL_AMOUNT) VAT_AMOUNT,   "+
						" SUM(GRENTAL_AMOUNT)  "+
						" FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT A, "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS B "+
				...
				out.println("      <tr>");
				out.println("        <td> <b>Application Entry Comments</b> - "+application_comments+" </td>");
				out.println("      </tr>");
				out.println("    </td>");
				out.println("  </tr>");
				out.println(" </table>");
				...
				if(!m_Hid_scr_name.equals("AF_CR_PRO_AGM_COMMENT")){
					...
					// mod by udara 19-03-2014
					if(m_app_stage.equals("VERIFY1")){
						out.println("<td width='8%' style='{text-align:center;}'> &nbsp; </td>");
					}
					else{
						out.println("<td width='8%' style='{text-align:center;}'><b>Abandoned</td>");
					}
					
					//out.println("<td width='8%' style='{text-align:center;}'><b>Abandoned</td>"); // commented by udara 17-03-2014
					out.println("<td width='8%' style='{text-align:center;}'> &nbsp; </td>"); // added by udara 17-03-2014
					out.println("<tr>");
					...
					//out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\">");  // commented by udara 17-03-2014
					//out.println("<input type='hidden' NAME='Hid_Score_Model' VALUE="+score_model+"</td>"); // commented by udara 17-03-2014
					
					
					//out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\" style='display:none' >");  // commented by udara 17-03-2014
					// mod by udara 19-03-2014
					if(m_app_stage.equals("VERIFY1")){
						out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\" style='display:none' ></td>");
					}
					else{
						out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\"  ></td>");
					}
					
					
					out.println("<td width='8%' align='center'> ");  // added by udara 17-03-2014
					out.println("<input type='hidden' NAME='Hid_Score_Model' VALUE='"+score_model+"'> </td>"); // added by udara 17-03-2014
					...
					//out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\" disabled>"); // commented by udara 17-03-2014
					
					
					//out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\" style='display:none' disabled>");
					
					// mod by udara 19-03-2014
					if(m_app_stage.equals("VERIFY1")){
						out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\" style='display:none' disabled></td>");
					}
					else{
						out.println("<td width='8%' align='center'><input type='checkbox' name='chk_rej' value=\"N\" unchecked onClick=\"change_reject()\"  disabled></td>");
					}
				...
				String m_letter=req.getParameter("letter");
				...
				if(more){
					...
					String m_mk_creditofficer=rs.getString(30);
					//String m_insurance_agent = rs.getString(23);
					//double m_sum_insured = rs.getDouble(24);//COMMENTEDD BY MILINSDA 2014-06-10
					double m_sum_insured = rs.getDouble(31);//ADDED MIILNDA 2014-03-10
					String m_veh_no = rs.getString(25); 
					String m_vendor_code = rs.getString(26);
					String m_security_details      =rs.getString(27); // added by udara on 15-07-2013
					String m_broker_code      =rs.getString(28);
					...
					out.println("    <td>:</td>");
					out.println("    <td>"+nf.format(m_stamp_duty)+"</td>");
					out.println("    <td><b>Insurance Done by</b></td>"); // mod by udara 27-02-2015
					out.println("    <td> &nbsp; </td>");
					out.println("    <td> "+insurance_done_by+" </td>"); // mod by udara 27-02-2015
					out.println("   </tr>");
					// end by udara 21-08-2014
					
					//added by jithendra 9-4-2019
					out.println("     <tr>");
					out.println("    <td><b>Security Insurance Company</td>");
					out.println("    <td>:</td>");
					out.println("    <td>"+m_ins_company+"</td>");
				...
				*/
				...
				stmt.close();
				//---------------------------------------------
				if(!Sql_data_exposure_prev.equals("")){//added milinda  2014-03-12 for handle null
				//--Create The Statement----------------------
				stmt = conn.createStatement ();
				//-------------------------------------------- 
			//	Sql_data_exposure_prev+="####";
			//out.println("CHECK  @@  "+Sql_client_data);
				rs = stmt.executeQuery(Sql_data_exposure_prev);
				...
				while(more){		
					...
					*/
					...
					//if(rs_charges.next()){
					if(m_letter.equals("first")){ //-added by ishani 2014-02-25
					out.println("  <tr> ");
					out.println("             <td width=\"40%\">Charge Type</td> ");
					out.println("             <td width=\"12%\" align=\"right\">Year1</td> ");
					// out.println("            <td width=\" 12%\"><div align=\"right\">Year2</div></td> "); // commented by udara 17-10-2013
					...
					*/
					...
					String sql_rent_new="   SELECT  "+
						//" TO_NUMBER(INSTALLMENT_NO) +1 , "+//1 // commented by udara 28-11-2017
						" TO_NUMBER(INSTALLMENT_NO)  , "+//1 // added by udara 28-11-2017
						" SUM(NET_RENTAL_AMOUNT),  "+//2
						" SUM(GRENTAL_AMOUNT - NET_RENTAL_AMOUNT) VAT_AMOUNT,  "+//3
						" SUM(GRENTAL_AMOUNT) "+//4
						" FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT  "+
						...
						" PRICING_NO='"+m_pricing_no+"' AND "+
						" TO_NUMBER(INSTALLMENT_NO) <= "+no_of_records+" "+
						//" AND INSTALLMENT_NO <> 0 "+ // coommented by udara 08-12-2017 // added by udara 23-11-2017
						" GROUP BY   TO_NUMBER(INSTALLMENT_NO),PRICING_NO "+ //,RENTAL_DATE
						" ORDER BY TO_NUMBER(INSTALLMENT_NO) ";
				...
				if(_m_tmp_pricing_count > 1 ) {
					...
					Sql_data_facility_details_sum="  SELECT "+												
						...
						"  A.INVOICE_NO=B.PRO_INVOICE_NO AND "+
						"  C.DURATION=B.PAYMENT_INTERVAL AND "+
						//"  A.ACTIVE_STATUS='Y' "; // commented by udara 03-11-2014
						" ";
					// "  GROUP BY A.PRICING_NO,B.VAT_PERCENTAGE,PERIOD,DECODE(B.PAYMENT_INTERVAL,'6','Semi Annualls','3','Quarters','1','Annualls','4','Once Every 4 Months','12','Months','365','Days','52','Weeks'),B.SUPPLIER_CREDIT,B.AMI,B.RATE ";
					...
					rs_charges =stmt_charges.executeQuery("SELECT "+
						...
						if(m_termi_type.equals("RESCHEDULE")){
							...
							*/
							...
							sql_rent_new="   SELECT  "+
								//" TO_NUMBER(INSTALLMENT_NO) +1 ,  "+  // commented by udara 28-11-2017
								" TO_NUMBER(INSTALLMENT_NO) ,  "+ // added by udara 28-11-2017
								" SUM(NET_RENTAL_AMOUNT),   "+
								" SUM(GRENTAL_AMOUNT - NET_RENTAL_AMOUNT) VAT_AMOUNT,   "+
								" SUM(GRENTAL_AMOUNT)  "+
								" FROM "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT A, "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS B "+
								...
								//" AND   B.ACTIVE_STATUS='Y' "+ // commented by udara 03-11-2014
								" AND   TO_NUMBER(INSTALLMENT_NO) <= "+no_of_records+" "+
								//" AND INSTALLMENT_NO <> 0 "+ // commented by udara 08-12-2017 // added by udara 23-11-2017
								" GROUP BY   TO_NUMBER(INSTALLMENT_NO)   "+
								" ORDER BY TO_NUMBER(INSTALLMENT_NO)  ";
