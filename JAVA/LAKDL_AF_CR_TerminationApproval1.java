...
public class LAKDL_AF_CR_TerminationApproval1 extends javax.servlet.http.HttpServlet 
{
	Connection conn;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("		document.Form1.action=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_TerminationSaveAppr1?status1="+m_st+"\";");  
				...
				out.println("window.location.href='"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_TerminationApproval1?chksql=main_page'");
				out.println(" }  ");
				out.println("}");
			...
			out.println("window.open(m_url,'displayWindow4','left=50,top=60,width=650,height=800,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
				*/
				...
				out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_TerminationApproval1?chksql=main_page&status=DELETE&hstatus=Delete';"); 
				out.println("}"); 
				...
				if(m_st.trim().equals("NEW")){
					...
					rs = stmt.executeQuery (
						// out.println(
						" SELECT A.TERMINATION_NO, A.FINANCE_NO, "+
		...
		out.println("<td  width='8%' >Value Date</td>");
				...
				out.println("<td  width='8%' align=right>Total Balance Amount</td>");
				...
				while(rs.next()){
					...
					//out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>");//+rs.getDouble(12)-rs.getDouble(23)
					
					out.println("<td align=right>"+nf.format(rs.getDouble("TOTAL_AMOUNT"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>");//+rs.getDouble(12)-rs.getDouble(23)
	...
	out.println("<td align=right>"+nf.format(rs.getDouble(6))+"<input type=hidden name=\"BAL_AM_"+j+"\" value=\""+(rs.getString(6))+"\"></td>");*/
					...
					//if(m_cnt>0){ //commented by Kanchana on 2016-06-17
					...
					out.println("<td align=center><SELECT class=\"txt_input\" style=\"width:80px\" NAME=\"drp_appro_"+j+"\"><option value=\"APPROVE\">Approve</option>");
					out.println("<option value=\"DISAPPROVE\">Disapprove</option></SELECT></td>");
					//out.println("<td><INPUT TYPE=\"checkbox\" NAME=\"chk_app_"+j+"\" value=\"N\" onClick=\"change("+j+")\"></td>");//commenetd by jithendra 04-01-2018
			...
			out.println("<td align=right>"+nf.format(rs.getDouble(6))+"<input type=hidden name=\"BAL_AM_"+j+"\" value=\""+(rs.getString(6))+"\"></td>");
						...
						*///commented by Kanchana on 2016-06-17										
						...
						out.println("<option value=\"DISAPPROVE\">Disapprove</option></SELECT></td>");
						//out.println("<td><INPUT TYPE=\"checkbox\" NAME=\"chk_app_"+j+"\" value=\"N\" onClick=\"change("+j+")\"></td>");COMMENETED by jithendra 04-01-2018
				...
				out.println("</TABLE>");
				out.println("<BR><BR>");											
				//Added by Kanchana on 2016-06-17
				rs1=stmt1.executeQuery(" SELECT DISTINCT A.TERMINATION_NO, NVL(A.ENT_USER,'-'),"+
					" NVL(TO_CHAR(A.ENT_DATE,'DD-MM-YYYY hh:mi:ss'),'-'), "+						                     
					" NVL(A.REMARKS,'-') "+	
					" FROM "+m_schema_name+".AF_CR_PRO_TERMINATION A  "+
				...
				while(more1){								
					...
					out.println("<td width='35%' class=div_input>"+rs1.getString(4)+"</td>");
					...
					rs=stmt.executeQuery(" SELECT  A.TERMINATION_NO, "+//1
						...
						" NVL(A.REMARKS,'-'), "+//3
						" INITCAP(A.STATUS), "+//4
						" TO_CHAR(A.ENT_DATE,'DD-MM-YYYY hh:mi:ss') ENT_DATE, "+ //5
						" A.ENT_USER, "+//6
						" DECODE(A.STAGE,'APPROVE1','Approve Level 1','APPROVE2','Approve Level 2'), "+ //7
						" A.FINANCE_NO "+
						" FROM "+m_schema_name+".AF_CO_PRO_TERMI_APP_REMARKS A "+
						" WHERE A.TERMINATION_NO ='"+rs1.getString(1)+"' ORDER BY TO_DATE(ENT_DATE,'DD-MM-YYYY hh:mi:ss') ");
					boolean  more =rs.next();
					
					int count_1=1;
					while(more){
						out.println("<tr >");
						out.println("<td width='5%' align='right'>"+count_1+"</td>"); 
					...
					count = count+1;
					more1 =rs1.next();
				}//more1 while loop end
				
				//Endded by Kanchana on 2016-06-17
