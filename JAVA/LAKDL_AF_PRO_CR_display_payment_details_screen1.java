...
public class LAKDL_AF_PRO_CR_display_payment_details_screen1 extends javax.servlet.http.HttpServlet { 

	Connection conn;
	Statement stmt2,stmt;
	java.text.NumberFormat nf,nf1;
	public ResultSet rs2,rs;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Credit Proces - Payment Approval 1</TITLE>"); 
			out.println("</HEAD>"); 
			...
			out.println("var foll_arry=new Array();");
      ...
      out.println("continue;");
		  ...
		  out.println("}");		
			...
			out.println("			}");
			out.println("			}");
			out.println("	else if(data_vec.length==0 && document.Form1.SCREEN_NAME.value==\"NEW\"  && document.Form1.hid_st.value==\"A4\"){");
			...
			out.println("function check_purchase_order() {");//FOR APPROVAL 1
			out.println("assig('T2')");
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_LAKDL_AF_PRO_CR_display_main_screen_payment_details&ref="+m_ref+"&sus_ref="+m_sus_ref+"&data_val="+m_val+"&ac_status=RE-APP\";");
			out.println("load_interface(m_url,'XML');");
			//VERIFY
			out.println("}");
			...
			out.println("function check_foll(){");
			out.println("for(var d=0;d<=document.Form1.hid_cnt_doc.value;d++){");
			out.println("m_chkstatus=\"chkstatus\"+d");
			out.println("	if(document.Form1.elements[\"chkstatus\"+d].checked==false && document.Form1.elements[\"CHK_NAPP_\"+d].checked==false){");
	    ...
	    out.println("else{");
		  ...
		  out.println("}");
			...
			out.println("		document.Form1.action='"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_save_payment_details_screen1?type="+m_type+"&REQ_NO="+m_requ+"&number='+document.Form1.hid_cnt_doc.value+'&number1='+document.Form1.hid_cnt_cr.value+'&number2='+document.Form1.hid_oth_doc.value+'&closest='+st_1+'&approval=VERIFY&approval1=B';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_display_payment_details_screen1?PO_NO="+m_val+"&CLSTATUS=A';");
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("help_box.innerHTML=\" Credit Process - Payment Approval 1- \"+m_val;"); 
			out.println("}"); 
			out.println(""); 

			out.println("function load_roll_out_value(){");
			out.println("help_box.innerHTML=\" Credit Process - Payment Approval 1 - \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
      ...
      out.println(" }");
			...
			out.println("     Crit = document.Form1.TXT_PURCHASE_ORDER_NO.value+\"@\"+\"VERIFY@\";"); 
			//VERIFY
			out.println("    HelpBox(Start,End,Hid_No,Crit,Sql,IfCount);"); 
			out.println("}"); 

						
			out.println("function help_updte_value_assign_99(oBj) {"); 
		  ...
		  out.println("}");	
			...
			out.println("document.Form1.elements[m_wchk].checked=false");			
			out.println("document.Form1.elements[m_wchk].value='N'");
	 	  ...
	 	  out.println("doc_det();");
			...
			out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_PRO_CR_sql_validations?chksql=m_prime_chk_LAKDL_AF_CR_PRO_Doc_details_test&m_app_no=\"+document.Form1.TXT_APPLICATION_NO.value+\"&screen=AF_CR_PRO_PAYMENT_MAIN&data_val=\"+document.Form1.TXT_PURCHASE_ORDER_NO.value+\"&data_val2=\"+array_asst[doc_row]+\"&ac_status=Y\";");
			out.println("load_interface(m_url,'XML');");
		  out.println("doc_row=doc_row+16;");
			...
			out.println("'<td width=\"14%\" align=\"center\" style=\"{font:bold;}\">Remarks </td>'+"); 
		  out.println("'<td width=\"10%\" align=\"center\" style=\"{font:bold;}\">Followup</td>'+"); 
			out.println("'<td width=\"15%\" align=\"center\" style=\"{font:bold;}\">Followup Remarks </td>'+"); 
			...
			out.println("m_remark_pre='<td width=\"13%\" align=\"center\">'+doc_vec[i+3]+'<input type=\"hidden\" class=\"txt_input\" name=TXT_REMARK_PRE'+ln_row+' value=\"'+doc_vec[i+3]+'\" maxlength=\"100\" size=\"20\" disabled></td>';");		
			out.println("}");
			out.println("m_status_f='<td width=\"10%\" align=\"center\"><input type=\"checkbox\" name=chkstatus'+row+'_'+ln_row+' value=\"N\" unchecked onclick=\"check_status('+row+','+ln_row+')\" onblur=\"check_change_fol('+row+','+ln_row+')\"></td>';");			
			...
			out.println("}");
			out.println("m_remark='<td width=\"14%\" align=\"center\"><input type=\"text\" class=\"txt_input2\" name=TXT_REMARK_NEW'+row+'_'+ln_row+' value=\"\" maxlength=\"100\" ></td>';");		
			out.println("m_follup='<td width=\"10%\" align=\"center\"><input type=\"checkbox\" name=\"CHK_FOLL'+row+'_'+ln_row+'\" value=\"N\" unchecked onclick=\"check_change_fol('+row+','+ln_row+')\" onblur=\"check_change_fol('+row+','+ln_row+')\"><input type=\"hidden\" class=\"txt_input\" name=\"TXT_INV'+row+'_'+ln_row+'\" maxlength=\"100\" size=\"20\" value=\"'+doc_vec[i]+'\"></td>';");
			out.println("m_fol_remark='<td width=\"15%\" align=\"center\"><input type=\"text\" class=\"txt_input2\" name=\"TXT_FOL_REM'+row+'_'+ln_row+'\" value=\"\" maxlength=\"100\" onblur=\"check_remark('+ln_row+')\" ></td>';");		
			...
			out.println("}");
			out.println("change1.innerHTML+=m_header+m_system+'<table align=\"center\" border\"1\" width=\"100%\" class=\"table\">'+");
	    out.println("array_docu.join(\" \")+'</table>'+'<table align=\"center\" border\"1\" width=\"100%\" class=\"table\">'+");
	   	out.println("'</table>';");
     	...
     	out.println("}");
			...
			out.println("'<td width=\"13%\" align=\"center\" style=\"{font:bold;}\">Not Applicable</td>'+"); 
			out.println("'<td width=\"14%\" align=\"center\" style=\"{font:bold;}\">Remarks </td>'+"); 
		  out.println("'<td width=\"9%\" align=\"center\" style=\"{font:bold;}\">Followup</td>'+"); 
			out.println("'<td width=\"15%\" align=\"center\" style=\"{font:bold;}\">Followup Remarks </td>'+"); 
			out.println("'</tr >'+"); 
			out.println("'<tr >'+"); 
			out.println("'</tr >'+"); 
			...
			out.println("}");
			out.println("m_remarkd='<td width=\"14%\" align=\"center\"><input  type=\"text\" class=\"txt_input2\" name=TXT_OTH_REMARK'+ln_row_othr+' value=\"\" maxlength=\"50\" onblur=\"check_remark('+ln_row_othr+')\" ></td>';");		
			out.println("m_follup_oth='<td width=\"10%\" align=\"center\"><input type=\"checkbox\" name=CHK_FOLLUP_OTH'+ln_row_othr+' value=\"N\" unchecked onclick=\"check_change_fol_oth('+ln_row_othr+')\" onblur=\"check_change_fol_oth('+ln_row_othr+')\"></td>';");
			out.println("m_follup_remark_oth='<td width=\"15%\" align=\"center\"><input  type=\"text\" class=\"txt_input2\" name=TXT_FOL_REMARK_OTH'+ln_row_othr+' value=\"\" maxlength=\"50\" onblur=\"check_remark('+ln_row_othr+')\" ></td>';");		
		  ...
		  out.println("if(doc_vec_othr[i+4]=='A'){");
			out.println("m_system_1='<br><tr class=\"tr_input1\"><td><DIV id=DIV_TXT_SYSTEM  class=div_input style=color:red><b><u>System Exception</div></td></tr>';");
...
//------------------------------------------------------------------------------------------------------------------------------
			...
			out.println("<INPUT TYPE='Hidden' NAME='Hid_scr_name' VALUE=\"AF_CR_PRO_PAYMENT_MAIN\">"); 
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Credit Process - Payment Approval 1</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<input class='but_input' type='hidden' name='BUT_TXT_APPLICATION_NO' value=\"Help\" onClick=\"help_button_3('0','10','0','m_help_TXT_APP_NO_sql_1','3')\" disabled></td>"); 
			out.println("<td width='*%'></td>"); 
			out.println("</tr>");
			...
			out.println("<input class='but_input' type='hidden' name='BUT_HELP_MAIN' value=\"Help\" onClick=\"help_update('0','10','7','m_help_TXT_PURCHASE_ORDER_NO_sql','99')\" disabled></td>"); 
			out.println("<td width='*%'></td>"); 
			out.println("</tr>"); 
			...
			out.println("</tr>"); 
			out.println("<tr >"); 
			out.println("<td width='30%' ><DIV id='TXT_APP_NAME'  class=div_input>Application Name *</DIV></td>"); 
			...
			rs=stmt.executeQuery("select requ_no,trim(nvl(TO_CHAR(SETTLE_AMT,'999,999,999,999,999,999,999,999.99'),0)) from "+m_schema_name+".AF_CR_PRO_PURCHASE_ORDER_SETT  where upper(purchase_order_no)=upper('"+m_val+"') and upper(requ_no)=upper('"+m_requ+"') ");
			int a=0;
			out.println("<table align='center' width='100%' class='table'>"); 
			...
			out.println("<tr >"); 
			out.println("<td width='30%' align=\"left\">"+rs.getString(1)+"<input class='txt_input' style=\"{text-align:right;}\" type='hidden' name=TXT_REQU_NO_"+a+" maxlength='27' size='22' value=\""+rs.getString(1)+"\"></td>"); 
			out.println("<td width='30%'  align=\"right\">"+rs.getString(2)+"<input class='txt_input' style=\"{text-align:right;}\" type='hidden' name=TXT_SET_AMOUNT_"+a+" maxlength='27' size='22' value=\""+rs.getString(2)+"\"></td>"); 
			out.println("<td width='*%'>&nbsp</td>"); 	
			out.println("</tr>");
