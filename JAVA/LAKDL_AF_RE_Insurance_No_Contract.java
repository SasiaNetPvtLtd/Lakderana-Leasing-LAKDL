...
public class LAKDL_AF_RE_Insurance_No_Contract extends javax.servlet.http.HttpServlet {
	/*
	...
	*/
		...
		try {
			...
			if(m_chksql.equals("main_page")){ 
				...
				out.println("help_box.innerHTML=\"  Insurance No Contract - \"+m_val;"); 
				out.println("}"); 
				out.println(""); 
				
				out.println("function load_roll_out_value(){");
				out.println("help_box.innerHTML=\"  Insurance No Contract - \"+document.Form1.hid_status.value;"); 
				out.println("}"); 
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_RE_Insurance_No_Contract?chksql=main_page';"); 
				out.println("		}"); 
				out.println("}"); 
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Insurance_No_Contract?chksql=\"+chkSql+\"&branch_code=\"+m_branch_code+\"&from_date=\"+m_from_date+\"&to_date=\"+m_to_date;");
				...
				out.println("</tr>");
				out.println("<tr>");
				out.println("<td align=\"left\" class=\"pdn_txtpos2\" style=\"height: 18px\" id=help_box> Contracts Without Insurance Details </td>");
				out.println("</tr>");
				out.println("<tr>");
				...
				out.println("<TR><TD align='Center' class=factoring-letter-body><B> Contracts Without Insurance Details</B></TD></TR>");
				out.println("</TABLE>");
				...
				out.println("<td width='1%'   align=center><DIV class=factoring-letter-body><b>No<DIV></td>");
				out.println("<td width='10%' align=center><DIV class=factoring-letter-body><b>Contract No</b></DIV></td>");
				out.println("<td width='10%'    align=center><DIV class=factoring-letter-body><b>Vehicle No </b></DIV></td>");
				out.println("<td width='10%'    align=center><DIV class=factoring-letter-body><b>Last Insurance End Date </b></DIV></td>");
				out.println("</tr>");
			...
			/*
				...
				" SELECT "+
					...
					" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS "+
						" WHERE TRUNC(ENT_DATE) >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
						" AND TRUNC(ENT_DATE)   <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
						//" AND APPLICATION_STATUS NOT IN ('TERMINATED','TERMI','NORM_TERMI','CANCEL')  "+
						" AND APPLICATION_STATUS = 'ACTIVATED'    "+
						" AND FINANCE_NO NOT IN ( SELECT FINANCE_NO FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA) "+
					" ");
				*/
			...
			rs= stmt.executeQuery(" "+
					...
					"  SELECT "+
					  ...
					  "    FROM ( "+
									...
									" NVL("+m_schema_name+".AF_CO_GET_VEHICLE_NO(APPLICATION_NO),'-') VEHICLE_NO "+
									" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS "+
										" WHERE TRUNC(ENT_DATE) >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
										" AND TRUNC(ENT_DATE)   <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
										" AND APPLICATION_STATUS = 'ACTIVATED'    "+
									...
									" B.CLIENT_CODE CLIENT_NO, "+ 
									" NVL("+m_schema_name+".AF_CO_GET_VEHICLE_NO(B.APPLICATION_NO),'-') VEHICLE_NO "+
		...
		/*	if(!m_branch_code.equals("")){
					m_branch_code=	" AND " + m_schema_name + ".AF_CO_GET_BRANCH_CODE(A.FINANCE_NO) LIKE '%"+m_branch_code+"' " ; // Added by Kanchana for issue no 18429
			...
			rs= stmt.executeQuery(" "+
					...
					"  SELECT "+
					  ...
					  "    FROM ( "+
									...
									" A.CLIENT_CODE CLIENT_NO, "+
									" NVL("+m_schema_name+".AF_CO_GET_VEHICLE_NO(A.APPLICATION_NO),'-') VEHICLE_NO, "+
									" NVL(TO_CHAR((SELECT MAX(END_DATE) FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA WHERE FINANCE_NO = A.FINANCE_NO),'DD-MM-YYYY'),'-') END_DATE "+
									" FROM "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS A "+
										" WHERE TRUNC(A.ENT_DATE) >= TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+
										" AND TRUNC(A.ENT_DATE)   <= TO_DATE('"+m_to_date+"','DD-MM-YYYY') "+
										" AND A.TRANSACTION_TYPE = 'HIREPURCH'  "+ // added by udara 08-10-2019
										" AND A.APPLICATION_STATUS  IN ( 'ACTIVATED','REPOSSESS')     "+ // REPOSSESS ADDED BY KANCHANA
										" AND A.FINANCE_NO NOT IN ( SELECT FINANCE_NO FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA) "+
									...
									" FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA A, "+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B  "+
										" WHERE  A.FINANCE_NO=B.FINANCE_NO  "+
										" AND B.APPLICATION_STATUS  IN ( 'ACTIVATED','REPOSSESS')  "+ // REPOSSESS ADDED BY KANCHANA
										" AND UPPER(A.BUSINESS_TYPE) IN('NEW','RENEWAL')  "+
										" AND A.END_DATE >=TO_DATE('"+m_from_date+"','DD-MM-YYYY')  "+
										" AND A.END_DATE <=TO_DATE('"+m_to_date+"','DD-MM-YYYY')   "+
										" AND B.TRANSACTION_TYPE = 'HIREPURCH'  "+ // added by udara 08-10-2019
										" AND A.ENT_DATE IN (SELECT MAX(ENT_DATE) FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA WHERE FINANCE_NO = A.FINANCE_NO) "+
										" AND B.BRANCH_CODE LIKE '"+m_branch_code+"%'  "+ // Added by Kanchana for issue no 18429
						" ) "+
						" ORDER BY FINANCE_NO "+
					" ");
				...
				try{
					...
					while(rs.next()){
						...
						out.println("<td width='1%' class=factoring-letter-body align='left'> "+count+" </td>");
						out.println("<td width='10%' class=factoring-letter-body style=cursor:hand onClick=\"show_transaction_history_new('"+rs.getString(3)+"','"+rs.getString(1)+"')\">"+rs.getString(1)+"</td>");
