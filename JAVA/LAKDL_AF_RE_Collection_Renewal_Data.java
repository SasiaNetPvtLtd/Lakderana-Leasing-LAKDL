...
public class LAKDL_AF_RE_Collection_Renewal_Data extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			if(m_screen_type.trim().equals("main_page")){	
				...
				out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Renewal_Data?chksql=main_page';"); 
				out.println("		}"); 
				out.println("}"); 
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Renewal_Data?chksql=load_details&sr_name=\"+m_scr_name+\"&finance_no=\"+finance_no+\"&devision_type=\"+m_opt+\"&from_date=\"+m_from_date+\"&to_date=\"+m_to_date+\"&done_by=\"+m_done_by+\"&insu_com=\"+m_insu_com+\"\";");
				...
				out.println("m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Renewal_Data?chksql=load_details_btt&sr_name=\"+m_scr_name+\"&invo_no=\"+invoice_no+\"&finance_no=\"+val1+\"&clint_name=\"+val2+\"&asst_deta=\"+description+\"&business_type=\"+m_business_type+\"&policy_no=\"+val4+\"&debit_note_no=\"+val5+\"&price_no=\"+m_price_no+\"&app_no=\"+m_app_no;");
				...
				if(m_scr_name.equals("New")){
					...
					while(more){
						...
						rs1=stmt1.executeQuery("SELECT B.MAKE_CODE || '-' || B.MODEL_CODE || '-' ||A.ENGINE_NO || '-'|| A.REG_NO, "+//A.REG_NO Added By SJ on 28-11-2008
							...
							// " AND D.DIVISION_CODE     = '"+m_devision_type+"'  "+ //Added By Sanudn on 08-12-2008  //Commented BY KD
				...
				/*
					...
					if(m_finance_no.equals("")){	
						...
						//out.println(" "+
							...
							" AND B.APPLICATION_STATUS NOT IN ('TERM','TERMINATED','NORM_TERMI') "+ // added by udara 20-02-2014
				...
				if(m_insu_com.equals("")){
					...
					if(m_finance_no.equals("")){	
						...
						//out.println(" "+
							...
							" and a.ENT_DATE in (select max(ent_date) from "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA where FINANCE_NO = a.FINANCE_NO ) "+ // added by udara on 20-10-2014
						...
						rs=stmt.executeQuery(" SELECT DISTINCT A.FINANCE_NO, "+//1
							...
							" AND A.END_DATE >=TO_DATE('"+m_from_date+"','DD-MM-YYYY') "+ // added by udara 22-04-2014
							" AND B.TRANSACTION_TYPE <> 'LOANS'  "+ // added by udara 10-10-2018
							" and a.ENT_DATE in (select max(ent_date) from "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA where FINANCE_NO = a.FINANCE_NO ) ");
						// " AND A.INSURED_BY ='"+m_insuarane_type+"' ");
					...
					while(more){
						...
						rs1=stmt1.executeQuery(" SELECT A.ASSET_DESCRIPTION , "+
							...
							" WHERE A.FINANCE_NO = '"+rs.getString(1)+"'  GROUP BY A.ASSET_DESCRIPTION,A.PRO_INVOICE_NO ");
						//" AND A.INSURED_BY ='"+m_insuarane_type+"'");
				...
				*/
				...
				out.println("		if(confirm(\"Are you sure you want to clear the screen?\")){ "); 
				out.println("          m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_Collection_Renewal_Data?chksql=load_details_btt&sr_name=\"+m_sr_name+\"&invo_no=\"+m_invoice_no+\"&finance_no=\"+m_finance_no+\"&clint_name=\"+m_clint_name+\"&asst_deta=\"+m_asst_deta+\"&business_type=\"+m_business_type+\"&policy_no=\"+m_policy_no+\"&debit_note_no=\"+m_debit_note_no+\"&price_no=\"+m_price_no+\"&app_no=\"+m_app_no;");
				...
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=start_date_check&hidden_policy_no=\"+document.Form2.hid_policy_no.value+\"&hidden_debit_note_no=\"+document.Form2.hid_debit_note_no.value+\"&start_date=\"+v_date+\"-\"+v_month+\"-\"+val;");
				out.println("		load_interface(m_url,'XML');");
				out.println(" function get_vector(data_vec) {");
			...
			//	out.println("			alert('CCCCCCCCC');");
				out.println("	document.Form2.hid_chk_status.value='start_date_check'; ");
				out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_RE_sql_validations?chksql=start_date_check&hidden_policy_no=\"+document.Form2.hid_policy_no.value+\"&hidden_debit_note_no=\"+document.Form2.hid_debit_note_no.value+\"&start_date=\"+document.Form2.start_dd.value+\"-\"+document.Form2.start_mm.value+\"-\"+document.Form2.start_yy.value;");
				out.println("		load_interface(m_url,'XML');");
				out.println("}");
		...
		/*		out.println("function load_c_date(val) {");
				...
				out.println("		document.Form2.action='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Save_Asset_Insurance_Detail';");   // commented by udara 21-05-2014
				out.println("		document.Form2.submit();	"); // commented by udara 21-05-2014
				...
				*/
				...
				out.println(" start_date_check(); ");  // ADD BY A/S ON 05-11-2021 FOR JB13102021-15102
				out.println("if(validate_data()){");
				...
				*/
				...
				//out.println("        if(unformat_noobject(document.Form2.txt_premium.value)>0){ "); // commented by udara 08-10-2015
					out.println("	 	if(confirm(\"Are you sure, you want to save data?\")){ ");
					...
					out.println("			document.Form2.action='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Save_Asset_Insurance_Detail_renewal';"); // added by udara 19-09-2014
					out.println("			document.Form2.submit();	");
					
					
					out.println("      }"); // are you
				//out.println("        }"); // commented by udara 08-10-2015
				
				// commented by udara 08-10-2015
				/*
				out.println("        else{");
			...
			/*	
				...
				out.println("			document.Form2.action='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Save_Asset_Insurance_Detail_renewal';"); // added by udara 09-10-2014
				out.println("			document.Form2.submit();	");
				...
				*/
				...
				// commented by udara 08-10-2015
				/*
				out.println(" else if( unformat_noobject(document.Form2.txt_payable_premium.value)<=0 ){ ");
				out.println("   alert('Payable premium should be greter than 0'); ");
				out.println(" } ");
				
				out.println(" else if( unformat_noobject(document.Form2.txt_bas_pre.value)<=0 ){ ");
				out.println("   alert('Basic premium should be greter than 0'); ");
				out.println(" } ");
				*/
				// end by udara 11-11-2014
				...
				*/
				...
				if(m_scr_name.equals("New")){
					...
					rs=stmt.executeQuery(" "+
						...
						" RCC_TC_TOTAL,  "+ // 11
						" BASIC_PREMIUM,  "+ // 12
						" BASIC_PREMIUM_COMM_RATE,  "+ // 13
						" BASIC_PREMIUM_COMMISION,  "+ // 14
						" RCC_TC_COMM_RATE,  "+ // 15
						" RCC_TC_COMMISION,  "+ // 16
						" VAT_ON_TOTAL_COMMISION,  "+ // 17
						" PAYEE_NAME,  "+ // 18
						" REMARKS,  "+ // 19
						" TAX_DUE,  "+ // 20
						" DEBIT_NOTE_NO,  "+ // 21
						...
						" SELECT A.FINANCE_NO FINANCE_NO, "+ //1
						" A.PRO_INVOICE_NO PRO_INVOICE_NO,"+ //2
						" A.POLICY_NO POLICY_NO, "+ //3
						" A.ASSET_DESCRIPTION ASSET_DESCRIPTION, "+ //4
						" TO_CHAR(A.START_DATE,'DD-MM-YYYY') START_DATE, "+ //5
						" TO_CHAR(A.END_DATE,'DD-MM-YYYY') END_DATE, "+ //6
						" NVL(A.SUM_INSSURED,0.00) SUM_INSSURED, "+ //7
						" NVL(A.PREMIUM,0.00) PREMIUM, "+ //8
						" NVL( A.RCC,0.00) RCC, "+   //9
						" NVL(A.TC,0.00) TC, "+  //10
						" NVL(A.RCC_TC_TOTAL,0.00) RCC_TC_TOTAL, "+ //11
						" NVL(A.BASIC_PREMIUM,0.00) BASIC_PREMIUM, "+ //12
						" NVL(A.BASIC_PREMIUM_COMM_RATE,0.00) BASIC_PREMIUM_COMM_RATE, "+  //13
						" NVL(A.BASIC_PREMIUM_COMMISION,0.00) BASIC_PREMIUM_COMMISION, "+  //14
						" NVL(A.RCC_TC_COMM_RATE,0.00) RCC_TC_COMM_RATE, "+ //15
						" NVL(A.RCC_TC_COMMISION,0.00) RCC_TC_COMMISION,"+ //16
						" NVL(A.VAT_ON_TOTAL_COMMISION,0.00) VAT_ON_TOTAL_COMMISION, "+  //17
						" "+m_schema_name+".AF_CO_GET_SUB_CHARG_PAYEE_NAME(A.INSUR_COM) PAYEE_NAME, "+  //18
						" NVL(A.REMARKS,'-') REMARKS, "+//19
						" NVL(A.TAX_DUE,0.00) TAX_DUE, "+ //20
						" NVL(A.DEBIT_NOTE_NO,'-') DEBIT_NOTE_NO, "+ //21
						" TAX_APPLICABILITY TAX_APPLICABILITY, "+ //22
						" NVL(A.INSUR_COM,'-') INSUR_COM, "+  //23
						//" NVL(A.VEHICLE_TYPE,'-') "+ // 24
						" A.VEHICLE_TYPE VEHICLE_TYPE,  "+ // 24
						" "+m_schema_name+".AF_CO_GET_ITEM_SUB_DESC_2(A.VEHICLE_TYPE) ITEM_SUB_DESC, "+ // 25 added by udara 22-04-2014
						" NVL(A.PAYABLE_PREMIUM,0.00) PAYABLE_PREMIUM "+ // 26 added by udara 06-05-2014
						" FROM "+m_schema_name+".AF_IS_PRO_ASET_INSUR_DETA A "+		
						" WHERE A.POLICY_NO = '"+mm_policy_no+"'"+
						" AND A.DEBIT_NOTE_NO='"+mm_debit_note_no+"' "+
					...
					*/
					...
					out.println("<tr>");
					out.println("<td width='20%' align ='left'>Start Date</td>"); 
					out.println("<td width='30%' ><input name=\"start_dd\" value=\""+m_start_date_dd+"\"  type=\"text\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=\"check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);\" onchange=\"start_date_check()\"> "); //added onchange function by Kanchana.
					out.println("    <input name=\"start_mm\" type=\"text\" value=\""+m_start_date_mm+"\" maxlength=\"2\" style=\"width: 25px\" class=\"txt_input\" onblur=\"check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);\" onchange=\"start_date_check()\"> ");
				//	out.println("    <input name=\"start_yy\"  type=\"text\" value=\""+m_start_date_yy+"\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onblur=\"set_end_date();make_request_date()\" ><a href style='{cursor:hand; }' onclick=load_calendar('2')> Calendar</a> "); // check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);
					out.println("    <input name=\"start_yy\"  type=\"text\" value=\""+m_start_date_yy+"\" maxlength=\"4\" style=\"width: 45px\" class=\"txt_input\" onchange=\"start_date_check()\" ><a href style='{cursor:hand; }' onclick=load_calendar('2')> Calendar</a> "); // check_date(document.Form2.start_dd,document.Form2.start_mm,document.Form2.start_yy);
					out.println("</td>");
