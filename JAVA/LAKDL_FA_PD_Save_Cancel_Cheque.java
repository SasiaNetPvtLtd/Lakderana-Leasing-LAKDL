...
public class LAKDL_FA_PD_Save_Cancel_Cheque extends HttpServlet {
  ...
  String reqstr;
	...
	public synchronized void  service(HttpServletRequest req, HttpServletResponse res)	throws IOException{
		...
		try {
			...
			//out.println(m_facility_no+"   "+m_client_no);
			...
			if(m_num>0)
			{
				...
				callstmt1=conn.prepareCall("BEGIN "+m_schema_name+".FA_CR_SAVE_CANCEL_POD(:1,:2,:3,:4); END;");
				callstmt2=conn.prepareCall("BEGIN "+m_schema_name+".FA_CR_SAVE_CANCEL_POD_LETTER(:1,:2,:3); END;");
				...
				count_check=0;
					for(int i=1;i<=m_num;i++)
					{
						...
						{
							callstmt1.setString(1,m_sn_methods.met_formdata(reqstr,"TXT_POD_NO_"+i).trim());
							...
							{
							callstmt2.setString(1,m_pod_ref);
							}

							callstmt2.registerOutParameter(1,java.sql.Types.CHAR);
							callstmt2.setString(2,m_sn_methods.met_formdata(reqstr,"TXT_POD_NO_"+i).trim());
							callstmt2.setInt(3,count_check);
							callstmt2.execute();
							
							if(count_check==0)
							{
			        			m_pod_ref=callstmt2.getString(1);
			        		}
							count_check+=1;
			...
			//out.println("alert('"+m_client_no+"');");
			// Commented below by Udara SOmathilake on 13-10-2010
			...
			// Added below line by Udara Somathilake on 13-10-2010
			//out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_Cancel_cheque_sent_letter?finance_no="+m_facility_no+"&client="+m_client_no+"&chksql=main_page\";"); 
			out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_Cancel_cheque_sent_letter?finance_no="+m_facility_no+"&client="+m_client_no+"&ref_no="+m_pod_ref+"&chksql=main_page\";");
			...
			out.println("window.location.href='"+m_class_url+"/"+m_client_name+"FA_OP_PD_Cancel_cheques';");
