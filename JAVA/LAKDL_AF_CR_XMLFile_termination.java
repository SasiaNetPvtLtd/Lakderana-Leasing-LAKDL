...
public class LAKDL_AF_CR_XMLFile_termination extends javax.servlet.http.HttpServlet {
	...
	public /*synchronized*/ void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			/*added by ns on 19-06-2012 */
			else if(m_chksql.trim().equals("get_future_invoices")){
				
				String m_fin_no = req.getParameter("finance_no");
				String m_date   = req.getParameter("date");
				
				rs = stmt.executeQuery(	"	SELECT COUNT(A.INVOICE_NO) "+
			...
			else if(m_chksql.trim().equals("get_Finance_no_new")){
			...
			//end 
				...
				// org SMBL
				/*
				rs = stmt.executeQuery("SELECT NVL(SUM(NET_RENTAL_AMOUNT*(1+("+m_schema_name+".AF_CO_GET_VAT_ON_RENTAL_PRO(TRANSACTION_TYPE,TO_DATE('"+m_date+"','DD-MM-YYYY'))/100))),0), "+
				...
				*/
				...
				rs = stmt.executeQuery("SELECT NVL(SUM(NET_RENTAL_AMOUNT*(1+("+m_schema_name+".AF_CO_GET_VAT_ON_RENTAL_PRO(TRANSACTION_TYPE,TO_DATE('"+m_date+"','DD-MM-YYYY'))/100))),0) - SUM(NVL("+m_schema_name+".AF_CR_FREEZED_RENTAL_AMOUNT_1(B.APPLICATION_NO,B.FINANCE_NO,TO_CHAR(A.RENTAL_DATE,'DD-MM-YYYY'),A.INSTALLMENT_NO),0)), "+ // added by udara 21-08-2017
					...
					"       NVL(SUM(NET_RENTAL_AMOUNT*"+m_schema_name+".AF_CO_GET_VAT_ON_RENTAL_PRO(TRANSACTION_TYPE,TO_DATE('"+m_date+"','DD-MM-YYYY'))/100),0) "+
					"FROM   "+m_schema_name+".AF_CO_PRO_APP_INSTALLMENT A,"+m_schema_name+".AF_CO_PRO_APPLICATION_DETAILS B "+
					"WHERE B.FINANCE_NO = '"+m_finance_no+"'  "+ // added by udara 11-03-2019
					"  AND A.APPLICATION_NO = B.APPLICATION_NO AND "+
					"       INVOICE_NO IS NULL AND    "+
					"       RENTAL_DATE < TO_DATE('"+m_date+"','DD-MM-YYYY')  AND "+
					"      (PRO_INVOICE_NO,a.APPLICATION_NO,PRICING_NO) IN (SELECT INVOICE_NO, APPLICATION_NO,PRICING_NO "+
					"                                                      FROM   "+m_schema_name+".AF_CO_PRO_APP_INVOICE_DETAILS  "+
			...
			*/
			...
			else if(m_chksql.trim().equals("get_term_rate_sn")){
				
				String m_ter_rate  = req.getParameter("ter_rate");
				
				rs = stmt.executeQuery(" SELECT '"+m_ter_rate+"' "+
					" FROM   "+m_schema_name+".AF_AD_REBATE_CONFIGURATION "+
					" WHERE  REBATE_TO >= '"+m_ter_rate+"' "+
					" AND REBATE_FROM <= '"+m_ter_rate+"' "+
			...
			else if(m_chksql.trim().equals("get_term_odi_rate_sn")){
				
				String m_ter_rate  = req.getParameter("ter_rate");
				
				rs = stmt.executeQuery(" SELECT '"+m_ter_rate+"' "+
					" FROM   "+m_schema_name+".AF_AD_REBATE_CONFIGURATION "+
					" WHERE  ODI_TO >= '"+m_ter_rate+"' "+
					" AND ODI_FROM <= '"+m_ter_rate+"' "+
					" AND USER_ID = '"+m_username+"' "+ 
					" ");
			...
			// added by udara 23-10-2018
				...
				/*rs = stmt.executeQuery(" SELECT "+m_schema_name+".AF_CO_GET_CLOSURE_IRR_N('"+m_finance_no+"','"+m_tdate+"','"+term_amt+"','"+unallo_amt+"') "+
					" FROM   DUAL");*/

			rs = stmt.executeQuery(" SELECT "+m_schema_name+".AF_CO_GET_CLOSURE_IRR_N('"+m_finance_no+"','"+m_tdate+"','"+term_amt+"') "+//kanishka on 29-01-2014
															 " FROM   DUAL");
