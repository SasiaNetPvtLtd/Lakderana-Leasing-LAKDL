...
public class LAKDL_AF_RE_save_rental_unfreez extends HttpServlet {
	...
	public synchronized void  service(HttpServletRequest req, HttpServletResponse res)	throws IOException{
		...
		try {
			...
			callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_RE_SAVE_RENTAL_UNFREEZ(:1,:2,:3,:4,:5); END;");
			...
			for(int j=0;j<m_chksql_inv;j++){
				...
				if(m_chk.equals("Y")){			
					...
					callstmt.setString(2,m_sn_methods.met_formdata(reqstr,"TXT_USER_COMMENTS_"+j));
					callstmt.setString(3,m_sn_methods.met_formdata(reqstr,"SCREEN_NAME"));
					callstmt.setString(4,m_username);
					callstmt.setString(5,"AF_RE_RENTAL_UNFREEZ");
					callstmt.execute();
			...
			out.println("window.location.href='"+m_url+"/"+m_fschema_name+"AF_RE_display_rental_unfreez?chksql=main_page';");
