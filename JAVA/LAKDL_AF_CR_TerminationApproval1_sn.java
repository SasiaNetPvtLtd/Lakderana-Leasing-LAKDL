...
public class LAKDL_AF_CR_TerminationApproval1_sn extends javax.servlet.http.HttpServlet 
{
	Connection conn;
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)
	{
		...
		try {
			...
			if (m_chksql.trim().equals("idle")) {
				...
				out.println("help_box.innerHTML=\"Finance - Termination - Approval1 - \"+m_val;"); 
				out.println("}"); 
				out.println(""); 
				
				out.println("function load_roll_out_value(){");
				out.println("help_box.innerHTML=\"Finance - Termination - Approval1 - \"+document.Form1.hid_status.value;"); 
				out.println("}"); 
				...
				out.println("		document.Form1.action=\""+m_class_url+"/"+m_fschema_name+"AF_CR_PRO_TerminationSaveAppr1_sn?status1="+m_st+"\";");  
				...
				out.println("window.location.href='"+m_servlet_client_url+":"+m_client_t3_port+"/"+m_client_name+"AF_CR_TerminationApproval1_sn?chksql=main_page'");
				out.println(" }  ");
				out.println("}");
				...
				//out.println("    document.Form1.elements[\"chk_app_\"+row].value=\"Y\";"); // commented by udara 16-10-2018
				//out.println("      if(parseFloat(unformat_noobject(document.Form1.elements['DUE_AM_2_'+row].value))!=\"0\"){ "); // commented by udara 05-11-2018
				//out.println("      if(parseFloat(unformat_noobject(document.Form1.elements['DUE_AM_2_'+row].value))!=\"0\" && document.Form1.elements['drp_appro_'+row].value!='DISAPPROVE'){ "); // commented by udara 07-03-2019 // added by udara 16-10-2018
				out.println("      if(parseFloat(unformat_noobject(document.Form1.elements['DUE_AM_2_'+row].value))>\"0\" && document.Form1.elements['drp_appro_'+row].value!='DISAPPROVE'){ "); // added by udara 07-03-2019
				out.println("          document.Form1.elements[\"chk_app_\"+row].checked = false; ");
				out.println("      }");
				out.println("      else{");
				out.println("          document.Form1.elements[\"chk_app_\"+row].checked = true; ");
				out.println("          document.Form1.elements[\"chk_app_\"+row].value=\"Y\";"); // added by udara 30-01-2019
				out.println("      }");
				...
				out.println("function change_dropdown(row) {"); 
				out.println("     document.Form1.elements[\"chk_app_\"+row].checked = false; ");
				out.println("}"); 
			...
			out.println("window.open(m_url,'displayWindow4','left=50,top=60,width=650,height=800,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
				*/
				...
				out.println("function show_termination_appr_new(finance_no,termination_type,termination_no){");
				/*out.println("m_url=\""+m_class_url+"/"+m_schema_name+"_AF_CR_TerminationApprovalReport?chksql=show_report&FINANCE_NO=\"+finance_no+\"&TERMINATION_TYPE=\"+termination_type+\"&TERMINATION_NO=\"+termination_no+\"\";");*/  
				out.println("m_url=\""+m_class_url+"/"+m_schema_name+"_AF_CR_TerminationCalculationReport_N?chksql=main_page&FINANCE_NO=\"+finance_no+\"&TERMINATION_TYPE=\"+termination_type+\"&TERMINATION_NO=\"+termination_no+\"\";");  
				out.println("window.open(m_url,'displayWindow4','left=50,top=60,width=650,height=800,toolbar=0,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1');"); 
				out.println("}");
				...
				out.println("m_url=\""+m_class_url+"/"+m_schema_name+"_AF_CR_TerminationApproval1_sn?chksql=REMARKS_HISTORY&finance_no=\"+val+\" \";");  
				...
				out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_TerminationApproval1_sn?chksql=main_page&status=DELETE&hstatus=Delete';"); 
				out.println("}"); 
				
				out.println("function load_new_page(){");
				out.println("window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_CR_TerminationApproval1_sn?chksql=main_page';"); 
				out.println("}"); 
				...
				out.println("<input type='hidden' name='Hid_scr_name' value='AF_CR_TERMINATION_APPR1' > ");
				out.println("<input type='hidden' name='TXT_SCREEN_NAME' value='AF_CR_TERMINATION_APPR1' > ");
				...
				//Add Contents Dineth
				out.println("<table class=table border='0' width='*%' >");
				...
				if(m_st.trim().equals("NEW")){
					rs = stmt.executeQuery (
						...
						"        AMOUNT - NVL(A.INVOICED_RENTALS,0), "+//"+m_schema_name+".AF_CO_GET_AMOUNT(TERMINATION_NO, FINANCE_NO,APPLY_DATE), "+
						"        A.SETTELED_AMOUNT, "+
						...
						//"        NVL("+m_schema_name+".AF_CO_GET_ODI_DUE_TER(FINANCE_NO,TERMINATION_NO),0) H, "+//,APPLY_DATE
						"        NVL(A.ODI_AMOUNT,0), "+
						...
						" ,NVL(CHARGES,0)  AS CHARGES "+
						" ,NVL("+m_schema_name+".AF_CO_GET_UNALLO_REC_CON_AMT(FINANCE_NO),0) AS UNALLO_REC "+
					...
					rs = stmt.executeQuery (" SELECT A.TERMINATION_NO, A.FINANCE_NO, "+
						...
						"        NVL("+m_schema_name+".AF_CO_GET_TER_RESIDUAL(A.TERMINATION_NO, A.FINANCE_NO,A.APPLY_DATE),0) F, "+
						//"        NVL("+m_schema_name+".AF_CO_GET_ODI_DUE_TER(FINANCE_NO,TERMINATION_NO),0) H, "+//,APPLY_DATE
						"        NVL(A.ODI_AMOUNT,0), "+//Added by Dineth on 2008-11-07
						"        "+m_schema_name+".AF_CO_GET_APP_UNIT_COUNT(A.APPLICATION_NO) , "+
						"        "+m_schema_name+".AF_CO_GET_APP_UNIT_COUNT(A.TERMINATION_NO),A.VAT_PER,NVL("+m_schema_name+".AF_CO_GET_TERMINATION_DESC(A.TERMINATION_TYPE),'-'),NVL("+m_schema_name+".AF_CO_GET_USER_NAME(ENT_USER),'-'),  "+
						//"        ,DUE_AMOUNT,INVOICE_NO,RATE,  REMARKS, CHARGES,  "+
						//"        TO_CHAR(APPLY_DATE,'DD-MM-YYYY'),TERMINATED_DATE, "+
						//"        APPLICATION_NO, REG_NO "+
						" A.RATE, "+
		...
		out.println("<tr class=pdn_txtpos2>");
				...
				out.println("<td  width='10%' >Termination No</td>");
		...
		out.println("<td  width='8%' >Value Date</td>");
				...
				//Added by Dineth on 2008-10-21
				out.println("<td  width='8%' align=right>Termination Type</td>");
				if(m_st.trim().equals("NEW")){
				out.println("<td  width='10%' align=right>Termination Approve 1</td>");
				}
				else if(m_st.trim().equals("DELETE")){
				out.println("<td  width='10%' align=right>Change to Ent</td>");
				}
				...
				out.println("<td  width='8%' >Value Date</td>");
				//out.println("<td  width='8%' >Discounted Amount</td>");
				//out.println("<td  width='8%' align=right>Due Amount</td>");
				//out.println("<td  width='8%' align=right>ODI</td>");
				...
				out.println("<td  width='8%' align=right>Net Balance Amount</td>");
				//out.println("<td  width='8%' align=right>Residual Value</td>");
				/*out.println("<td  width='8%' align=right>OD Interest</td>");
				out.println("<td  width='8%' align=right>Total Bal Amount</td>");*/
				//Added by Dineth on 2008-10-21
				...
				out.println("<td  width='8%' align=right>Action</td>");
				/*if(m_st.trim().equals("NEW")){
				out.println("<td  width='10%' align=right>Termination Approve 1</td>");
				}
				else if(m_st.trim().equals("DELETE")){
				...
				out.println("<td  width='8%' align=right>Termination Sheet</td>");
				out.println("<td  width='8%' align=right>User</td>");
				out.println("<td  width='8%' align=right>Termination No</td>");
				...
				while(rs.next()){
					...
					//out.println("<td align=right>"+nf.format((rs.getDouble(6)+rs.getDouble(7)+(rs.getDouble(12)*(rs.getDouble(16))/100)+rs.getDouble(13)))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>");
					//out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>"); // commented by udara 16-10-2018 //+rs.getDouble(12)-rs.getDouble(23)
					//out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"><input type=hidden name=\"DUE_AM_2_"+j+"\" value=\""+rs.getDouble(7)+"\"></td>"); // commented by udara 07-03-2019 // added by udara 16-10-2018
					out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"><input type=hidden name=\"DUE_AM_2_"+j+"\" value=\""+(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"\"></td>"); // added by udara 07-03-2019
                  ...
                  out.println("<td align=right>"+nf.format(rs.getDouble(6))+"<input type=hidden name=\"BAL_AM_"+j+"\" value=\""+(rs.getString(6))+"\"></td>");*/
					...
					//Modified by Dineth on 2008-10-23
					...
					//out.println("<td align=center><SELECT class=\"txt_input\" style=\"width:80px\" NAME=\"drp_appro_"+j+"\"><option value=\"APPROVE\" selected>Approve</option>"); // commented by udara 05-11-2018
					out.println("<td align=center><SELECT class=\"txt_input\" style=\"width:80px\" NAME=\"drp_appro_"+j+"\" onClick=\"change_dropdown("+j+")\" ><option value=\"APPROVE\" selected>Approve</option>"); // added by udara 05-11-2018
					if(m_st.trim().equals("NEW")){//added by kanishka dilshan on 19-05-2016
					...
					//out.println("<td><input type=\"button\" value=\"display\" class=\"but_input\" name=\"btn_dis_"+j+"\" onclick=\"show_termination_appr('"+rs.getString(2)+"','"+rs.getString(17)+"','"+rs.getString(1)+"')\"></td>");//commented by kanishka dilshan on 09-12-2013
					out.println("<td><input type=\"button\" value=\"display\" class=\"but_input\" name=\"btn_dis_"+j+"\" onclick=\"show_termination_appr_new('"+rs.getString(2)+"','"+rs.getString(17)+"','"+rs.getString(1)+"')\"></td>");
			...
			out.println("<td align=right>"+nf.format(rs.getDouble(6))+"<input type=hidden name=\"BAL_AM_"+j+"\" value=\""+(rs.getString(6))+"\"></td>");
						...
						out.println("<option value=\"disapprove\">Disapprove</option></SELECT></td>");
						out.println("<td><INPUT TYPE=\"checkbox\" NAME=\"chk_app_"+j+"\" value=\"N\" onClick=\"change("+j+")\">");
						...
						*/
						...
						out.println("<td align=right>"+nf.format(rs.getDouble(21))+"</td>");
						//out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"></td>"); // commented by udara 16-10-2018//+rs.getDouble(12)-rs.getDouble(23)
						//out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"><input type=hidden name=\"DUE_AM_2_"+j+"\" value=\""+rs.getDouble(7)+"\"></td>"); // commented by udara 06-03-2019
						out.println("<td align=right>"+nf.format(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"<input type=hidden name=\"DUE_AM_"+j+"\" value=\""+(rs.getString(6)+rs.getDouble(7)+rs.getDouble(13))+"\"><input type=hidden name=\"DUE_AM_2_"+j+"\" value=\""+(rs.getDouble(6)+rs.getDouble(7)+rs.getDouble(22)+rs.getDouble("CHARGES")- rs.getDouble("UNALLO_REC"))+"\"></td>"); // added by udara 06-03-2019
						...
						out.println("<td align=center><SELECT style=\"width:80px\" class=\"txt_input\" NAME=\"drp_appro_"+j+"\" onClick=\"change_dropdown("+j+")\" ><option value=\"APPROVE\">Approve</option>");
						if(m_st.trim().equals("NEW")){//added by kanishka dilshan on 19-05-2016
							out.println("<option value=\"DISAPPROVE\">Disapprove</option></SELECT></td>");
						}
						out.println("<td><INPUT TYPE=\"checkbox\" NAME=\"chk_app_"+j+"\" value=\"N\" onClick=\"change("+j+")\"></td>");
			...
			else if(m_chksql.equals("REMARKS_HISTORY")){
				String m_finance_no = req.getParameter("finance_no");
				
				out.println("<HTML><HEAD><TITLE>Remark History</TITLE></HEAD>");
				out.println("<link REL='STYLESHEET' HREF='"+m_html_client_url+"/css/Asset_Financing_System.css' TYPE=\"text/css\">");
				out.println("<BODY   class='body & txt-body' LEFTMARGIN='0' TOPMARGIN='0'>");
				out.println("<FORM NAME='Form1' method='post'>"); 
				out.println("<TABLE  WIDTH='100%' class='pdn_txtpos2' STYLE='{ bgcolor='#8fb382' color: black; font: 20pt arial;}'>");
				out.println("<TR><TD><CENTER><B>Remarks History</B></TD></TR>");
				out.println("</TABLE>");
				out.println("<BR><BR>");											
				...
				out.println("<td width='15%' class=div_input>Level</td>");
				out.println("<td width='10%' class=div_input>Status</td>");
				out.println("<td width='35%' class=div_input>Remarks</td>");
				out.println("<td width='15%' class=div_input>Enter Date</td>");
				out.println("<td width='15%' class=div_input>User</td>");								
				out.println("</tr>");
				...
				while(more){
					...
					out.println("<td width='12%' class=div_input>"+rs.getString(7)+"</td>");
					out.println("<td width='10%' class=div_input>"+rs.getString(4)+"</td>");
					out.println("<td width='35%' class=div_input>"+rs.getString(3)+"</td>");
