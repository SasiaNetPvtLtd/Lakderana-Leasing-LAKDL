...
//SCREEN NAME:FINAL CHEQUE RETURN REMINDER(WITHOUT REMARK)
...
public class LAKDL_FA_OP_Final_Chq_Return_Reminder_Letter_without_rem extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
		  ...
		  if (m_chksql.trim().equals("main_page")) {
			...
			out.println("<title>Final Cheque Return Reminder (Without Remark)</title></head>");
			...
			out.println("	 m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_save_final_chq_return_rem_without_rk_count?client_no="+m_client_no+"&debtor_code="+m_debtor_code+"&print="+m_print+"&receipt_no="+m_receipt_no+"&return_no="+m_return_no+"\";");  
    	...
    	out.println("m_writedata+'</table>';");
         ...
         out.println("<tr><td width='*%' class='rep-body' >"+m_city_desc+"</td></tr>");
			...
			out.println("<table border='0' width='100%' class='table'> ");	
			out.println("<tr ><td width='*%' class='rep-body' style='{text-align:center;}'><b><u>Notice of Dishonor Cheque</u></b></td></tr>");
			...
			rs2 = stmt2.executeQuery ("SELECT  NVL(TO_CHAR(ENT_DATE,'DD-MM-YYYY'),'-') "+
 					" FROM "+m_schema_name+".FA_OP_PRO_CHQ_RETURN_LETTER "+
 					" WHERE RETURN_NO='"+m_return_no+"' "+
					" AND CLIENT_CODE='"+m_client_no+"' "+
					" AND DEBTOR_CODE='"+m_debtor_code+"' "+
					" AND LETTER_NAME='INITIAL_CHEQ_RETURN_REMINDER' ");
			...
			if(more){
				out.println("We refer to our letter dated on <b>"+rs2.getString(1)+"</b>. Informing  you of the");
			}
			else {
				out.println("We refer to our letter dated on <B>N/A</B> Informing  you of the");
			}
			
			out.println("return of your cheque bearing No <b>"+m_chq_no+"</b> "+
			" drawn on <b>"+m_branch+"</b> for <b> Rs. "+nf.format(m_chq_amount)+"</b> "+
			" sent to us by you in settlement of the amount due on the Goods/Service supplied to "+
			" you by our client. ");
			
			out.println("<br><br>");						
			out.println(" Details are as follows <br>");
			...
			out.println("<table border='0' width='90%' class='table'> ");	
			out.println("<tr ><td width='*%' class='rep-body' style='{text-align:justify;}'>");
			out.println("The amount due, we regret to state remain unsettled to date. We here by demand immediate"+
			" settlement of all dues and also serve you formal notice that legal action will be instituted "+
			" against you without further notice, if you fail to do so within seven days of this letter.");
			out.println("</td></tr></table> ");						
			out.println("</font></p></blockquote>");				
