...
public class LAKDL_AF_CR_save_add_guarantor extends HttpServlet {
	...
	synchronized(this){ 
      ...
      String m_scr_name=(String)m_sn_methods.met_formdata(reqstr,"Hid_scr_name"); 
			...
			for (int j = 0; j < m_maxentries; j++) {	
							...
							callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_SAVE_APPLI_GUARAN(:1,:2,:3,:4,:5,:6,:7,:8); END;");						
	   	...
	   	out.println("<HTML><HEAD>");
			...
			out.println("window.location.href='"+m_url+"/"+m_client_name+"AF_CR_add_guarantor?chksql=main_page';");
      ...
      out.close();
     	...
     	PrintStream out = new PrintStream(res.getOutputStream());
			...
			out.println("window.location.href='"+m_url+"/LAKDL_AF_CR_add_guarantor?chksql=main_page';");
			out.println("}</SCRIPT></HEAD>");
			out.println("<body onload='displaymsg();'></body>");
			out.println("</html>");
			out.flush();
