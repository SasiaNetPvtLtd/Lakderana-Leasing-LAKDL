...
public class LAKDL_AF_MISF_Recovery_Report_gen_thread_log extends javax.servlet.http.HttpServlet {
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)	throws IOException	{
		...
		try {
			...
			String m_perform_stat   = req.getParameter("perform_stat"); 
			...
			stmt = conn.createStatement();
				...
				rs= stmt.executeQuery(" "+
						" SELECT COUNT(*) "+
						" FROM "+m_schema_name+".RECOVERY_RPT_THREAD_LOG "+
						" WHERE STATUS = 'Status - Recov Rpt End' "+
						" AND   ENT_USER = '"+m_username+"' "+ // udara 29-08-2016
						" ");
				...
				out.println("  var timeout = setTimeout(\"location.reload(true);\",2000); ");
				out.println("  function resetTimeout() { ");
				out.println("    clearTimeout(timeout); ");
				out.println("    timeout = setTimeout(\"location.reload(true);\",2000); ");
				out.println("  } ");
				//out.println(" </script> ");
				...
				out.println("    alert('Recovery report generation is completed.'); ");
				//out.println("    window.close(); ");
				out.println("    m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Recovery_Report?chksql=print_report_new&location=\"+m_location_id+\"&officer=\"+m_user_id+\"&date=\"+m_date+\"&perform_status=\"+m_perform_stat+\"&cr_officer=\"+m_cr_offic+\"&active_status=\"+m_active_yard_status+\"&region=\"+m_region;");
				...
				out.println("<TR><TD align='Center' class=factoring-letter-body><B> Recovery Report Generation - "+sys_date+" </B></TD></TR>");
				out.println("</TABLE>");
