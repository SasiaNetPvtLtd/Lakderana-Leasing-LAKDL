...
public class LAKDL_AF_CR_PRO_TerminationSaveAppr2_sn extends HttpServlet {
	Connection conn;	
	String m_msg,m_url;
	...
	public synchronized void  service(HttpServletRequest req, HttpServletResponse res)	throws IOException{
		...
		try {
			...
			String m_option_val=req.getParameter("status1");
			...
			for(int j=0;j<m_chk;j++){
				...
				String m_ter_type=m_sn_methods.met_formdata(reqstr,"TERM_TYPE_"+j);
				...
				if(m_chk_app.equals("Y")){
					callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_PRO_TERM_APPR2_NEW(:1,:2,:3,:4,:5,:6); END;");
					
					callstmt.setString(1,m_sn_methods.met_formdata(reqstr,"TER_NO_"+j).trim());
					callstmt.setString(2,m_sn_methods.met_formdata(reqstr,"TERM_TYPE_"+j).trim());
					
					callstmt.setString(3,m_screen_name.trim());
					callstmt.setString(4,m_option_val.trim());
					callstmt.setString(5,m_username.trim());
					callstmt.setString(6,m_sel_app.trim());
					
					//callstmt1=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_SAVE_TERMI_APP_REMARKS(:1,:2,:3,:4,:5,:6,:7); END;");//Added By SJ on 26-11-2008
					callstmt1=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_SAVE_TERMI_APP_REMA_NEW(:1,:2,:3,:4,:5,:6,:7); END;");
					
					
					callstmt1.setString(1,m_sn_methods.met_formdata(reqstr,"TER_NO_"+j).trim());
					callstmt1.setString(2,"APPROVE2");
			...
			out.println("window.location.href='"+m_url+"/"+m_client_name+"AF_CR_TerminationApproval2_sn?chksql=main_page';");
