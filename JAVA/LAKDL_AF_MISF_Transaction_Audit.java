...
public class LAKDL_AF_MISF_Transaction_Audit extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			//modified by madhawa function validate_date 2009-010-14 shifted upto in the source file
			...
			out.println("  }");
			out.println(" }");
			//end modified by madhawa function validate_date 2009-010-14 shifted upto in the source file
			
			
			
			//add by madhawa 2009-10-14
			
			//add by madhawa 2009-10-14
			out.println("function run_report() {");
			//modified by madhawa add if condition below 2009-10-14
			...
			out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"AF_MISF_Transaction_Audit_Date?chksql=run_report&rpt_type=\"+m_rpt_type+\"&user=\"+m_user_type+\"&from_date=\"+m_from_date+\"&to_date=\"+m_to_date+\"&rpt_name=\"+document.Form1.TXT_RPT_TYPE.options[document.Form1.TXT_RPT_TYPE.selectedIndex].text;"); 
			...
			out.println("	 }");
			out.println("	}");
			//modified by madhawa end of if 2009-10-14
			out.println("}");
			out.println("}");
			...
			*/
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"AF_MISF_Transaction_Audit?chksql=main_page';"); 
			out.println("		}"); 
			out.println("}"); 
	  	...
	  	out.println("					}"); 
			...
			rs=stmt.executeQuery(" SELECT INITCAP(DISPLAY_NAME),UPPER(SCREEN_NAME) SCREEN_NAME "+
			                     " FROM   "+m_schema_name+".CO_CO_MAS_USER_SCREEN  "+
													 ...
													 "        DISPLAY_STATUS='Y'  "+
													 " AND    (OPTION_NAME NOT IN ('MISF') "+ //added by nuwan de silva on 18-02-2010
													 " AND    SUB_OPTION1 NOT IN ('AF_FIN_INVOICING','AF_CR_PRO_CRIB','AF_RE_THANKING_LETTER','AF_FIN_SECURITY_FILE')  "+ //added by nuwan de silva on 18-02-2010
												   " OR     SUB_OPTION1 IS NULL ) "+
													 " AND    SCREEN_NAME NOT IN ( "+
													 "'AF_RE_INVOICE_BULK_PRINT', "+                      
													 "'AF_RE_RECEIPT_BULK_PRINT', "+                      
													 "'AF_RE_MONTHLY_STATE_BULK_PRINT', "+                
													 "'AF_RE_PDC_BULK_PRINT', "+                         
													 "'AF_RE_INS_BULK_PRINT', "+                         
													 "'AF_RE_RPT_COLLECTION_DUE_TODAY', "+                
													 "'AF_RE_THANKING_LETTER', "+                         
													 "'AF_RE_TERMINATION_LETTER', "+                      
													 "'AF_RE_CHEQUE_RTN_LETTER', "+                      
													 "'AF_CR_PRO_MAIN_DOC_PRINT',"+                      
													 "'AF_MAS_DELETION_LETTER', "+                       
													 "'AF_MK_APPLICATION_STATUS_REPORT',  "+             
													 "'AF_FN_PRO_LEGAL_PROVISION', "+                    
													 "'AF_CR_PRO_DISPLAY_OTHER_PAYMENTS_VOUCHER', "+     
													 "'AF_FIN_MAINTENANCE', "+                           
													 "'AF_RE_PRO_COLLECTION_MONTHLY_STATEMENT' "+
													 ")	"+
			...
			out.println("<td width='20%' ><input class=\"txt_input\" type=\"text\" name=TXT_USER maxlength=\"2\" size=\"2\" value=\"\" ></td>");
			out.println("<td width='10%'></td>"); 
			out.println("<td width='*%'></td>"); 
			out.println("</tr>"); 
			...
			out.println("<input class=\"txt_input5\" type=\"text\" name=TXT_FROM_DATE_YY maxlength=\"4\" size=\"4\" onblur=\"checkMonthLength(document.Form1.TXT_FROM_DATE_DD,document.Form1.TXT_FROM_DATE_MM,document.Form1.TXT_FROM_DATE_YY)\"  value=\"\" >");	
			out.println("</td> ");
			...
			//modified by madhawa 2009-10-14
			out.println("<input class=\"txt_input5\" type=\"text\" name=TXT_TO_DATE_YY maxlength=\"4\" size=\"4\" onblur=\"checkMonthLength(document.Form1.TXT_TO_DATE_DD,document.Form1.TXT_TO_DATE_MM,document.Form1.TXT_TO_DATE_YY)\" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input class='but_input' type='button' style='{width:150;}'  name='BUT_HELP_MAIN_1' value=\"Run Report\" onClick=\"run_report()\"><!--input class='but_input' type='button' style='{width:150;}'  name='BUT_HELP_MAIN_2' value=\"View Report\" onClick=\"view_details()\"-->");	
			out.println("</td> ");
			out.println("</table>");
