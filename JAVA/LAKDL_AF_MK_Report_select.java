...
public class LAKDL_AF_MK_Report_select  {  
	...
	public String m_help_TXT_INVOICE_NO_sql_Header= "Application Process - Invoice Number ";
  ...
  public String m_help_TXT_ACCOUNT_CODE_sql_Header="System Administration - Account Code Creation";
	...
	public String m_price_sql="";
	public String m_price_sql_Header="Price Details";
 ...
 "SELECT L.NO,L.CLIENT_CODE,L.CLIENT_TYPE,L.FULL_NAME,NVL(L.BUSINESS_SUB_SECTOR,' ') AS BUSINESS_SUB_SECTOR,L.CLIENT_CATEGORY,NVL(L.ADDRESS1,'N/L'),NVL(L.ADDRESS2,'N/L'),NVL(L.CITY_CODE,'N/L'),"+
    ...
    m_help_TXT_USER_ID_sql=
	 ...
	 m_help_TXT_ITEM_SUB_CAT_sql=
      ...
      " DESCRIPTION, "+
      " DEFAULT_VALUE "+
      " FROM LAKDL.AF_CO_MAS_ITEM_SUB_CATEGORY "+
			" WHERE ITEM_SUB_CAT LIKE UPPER('"+m_vector.elementAt(0)+"%')  AND ACTIVE_STATUS=('"+m_vector.elementAt(1)+"') "+
...
m_help_TXT_SUB_MODEL_CODE_sql=
 ...
 m_help_TXT_HOLIDAY_DATE_sql=
	...
	m_view_sql=
		 ...
		 " SELECT L.NO ,L.VALUATION_NO,L.ASSET_ID,L.DESC1 MODEL,L.DESC2 SUB_MODEL,L.VAL,L.VALUATION_DATE,L.NOTES,L.REMARKS,L.COLOUR, "+
		 " L.TYPE_OF_BODY,L.METER_READING READING,L.REG_NO,L.ENGINE_NO,L.CHASSIS_NO,L.DATE_OF_REG,L.ACTIVE_STATUS, "+
		 " L.GENERAL_INDEX,L.SEATING_CAPACITY,L.NO_OF_CYLINDERS,L.ITEM_CAT_CODE,L.DESCRIPTION "+
		 " FROM "+
		 " (SELECT ROWNUM NO,P.VALUATION_NO,P.ASSET_ID,P.DESC1,P.DESC2,P.VAL,P.VALUATION_DATE,P.NOTES,P.REMARKS, "+
		 " P.COLOUR,P.TYPE_OF_BODY,P.METER_READING,P.REG_NO,P.ENGINE_NO,P.CHASSIS_NO,P.DATE_OF_REG, "+
		 " P.ACTIVE_STATUS,P.GENERAL_INDEX,P.SEATING_CAPACITY,P.NO_OF_CYLINDERS,P.ITEM_CAT_CODE,P.DESCRIPTION "+
		 " FROM( "+
		 " SELECT DISTINCT A.VALUATION_NO,A.ASSET_ID, "+
		 " C.DESCRIPTION AS DESC1, "+ 
		 " D.DESCRIPTION AS DESC2, "+ 
		 " VALUE AS VAL, "+
		 " TO_CHAR(VALUATION_DATE,'DD-MM-YYYY') AS VALUATION_DATE, "+
		 " NVL(NOTES,'-') AS NOTES, "+
		 " NVL(REMARKS,'-') AS REMARKS, "+
		 " COLOUR, "+
		 " NVL(TYPE_OF_BODY,'-') AS TYPE_OF_BODY , "+
		 " METER_READING, "+
		 " NVL(REG_NO,'-') AS REG_NO, "+
		 " ENGINE_NO, "+
		 " CHASSIS_NO, "+
		 " NVL(TO_CHAR(DATE_OF_REG,'DD-MM-YYYY'),'-') AS DATE_OF_REG, "+
		 ...
		 " FROM "+m_schema_name+".AF_CO_PRO_APP_VALUATION A,"+m_schema_name+".AF_CO_PRO_APP_VALUATION_DET B,"+m_schema_name+".AF_CO_MAS_MODEL C, "+
		 " "+m_schema_name+".AF_CO_MAS_SUB_MODLE D "+
		 " WHERE A.VALUATION_NO=B.VALUATION_NO "+		
		 " AND A.ASSET_ID=B.ASSET_ID AND A.ACTIVE_STATUS=('"+m_vector.elementAt(2)+"') "+
		 " AND C.MODEL_CODE =A.MODEL_CODE "+
     " AND D.SUB_CODE=A.SUB_MODEL_CODE "+
		 " AND A.MODEL_CODE LIKE UPPER('"+m_vector.elementAt(1)+"%') "+
		 " AND A.SUB_MODEL_CODE LIKE UPPER('"+m_vector.elementAt(0)+"%') "+
		 " ORDER BY VALUATION_NO ASC "+
		 " )P)L "+
		 " WHERE L.NO>=  "+ Start_Val+"  AND L.NO<=  "+End_Val+" ";
...
m_price_sql=
	" SELECT L.NO,L.PRICING_NO,L.QUOTATION_NO,L.RATE,L.PERIOD "+
		" FROM  "+
		" (SELECT ROWNUM NO,P.PRICING_NO,P.QUOTATION_NO,P.RATE,P.PERIOD "+
		" FROM( "+ 
		" SELECT "+
		//" C.INQUIRY_NO, "+
		" A.PRICING_NO, "+
    ...
    " FROM "+m_schema_name+".AF_MK_PRO_QUOTATION_DET A,"+m_schema_name+".AF_MK_PRO_PRICING B,"+m_schema_name+".AF_MK_PRO_QUOTATION C "+ 
		" WHERE A.QUOTATION_NO=C.QUOTATION_NO AND A.PRICING_NO=B.PRICING_NO AND "+
		" A.QUOTATION_NO =UPPER('"+m_vector.elementAt(0)+"') "+
		"  )P)L  "+
	  " WHERE L.NO>=  "+ Start_Val+"  AND L.NO<=  "+End_Val+" ";
