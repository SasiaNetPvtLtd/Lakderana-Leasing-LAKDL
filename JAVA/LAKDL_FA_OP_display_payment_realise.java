...
public class LAKDL_FA_OP_display_payment_realise extends javax.servlet.http.HttpServlet { 
	...
	public synchronized void service(HttpServletRequest req, HttpServletResponse res)  throws IOException { 
		...
		try { 
			...
			out.println("<HTML>"); 
			out.println("<HEAD>"); 
			out.println("<TITLE>Operation Process - Payment Realise</TITLE>"); 
			out.println("</HEAD>"); 
			...
			out.println("		m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_PRO_sql_validations_normal?chksql=LOAD_PAYMENT_REALISATION\";");
			...
			*/
			...
			out.println("for(var i=0;i<parseFloat(document.Form1.NUM_CHKS.value);i++){");
			out.println("m_dd=\"TXT_ST_DD_\"+i");
			out.println("m_mm=\"TXT_ST_MM_\"+i");
			out.println("m_yy=\"TXT_ST_YY_\"+i");
			out.println("m_chk=\"TXT_CHK_\"+i");
			...
			out.println("alert('Please Enter The Realsise Date');");
      ...
      out.println("((parseFloat(FROM_MM.value))==(parseFloat(TO_MM.value)))&&");
      out.println(" ((parseFloat(FROM_YY.value))==(parseFloat(TO_YY.value)))){");
      out.println("      alert('Realise Date should be greater than Disburse Date');");
      ...
      out.println("}");
	    ...
	    //out.println("TO_DD.value=\"\";"); //COMMENT BY NS 06-01-2009 SR20100106-026
			//out.println("TO_MM.value=\"\";");
			//out.println("TO_YY.value=\"\";");
			out.println("}");
			out.println("}");
			
			// Modified by Thamali Jayatunga on 2009.10.20, Added else part
			out.println("function validate_date_realise(row) {"); 
			out.println("m_dd='TXT_ST_DD_'+row;");
			out.println("m_mm='TXT_ST_MM_'+row;");
			out.println("m_yy='TXT_ST_YY_'+row;");
			
			out.println("m_hid_dd='HID_TXT_ST_DD_'+row;");
			out.println("m_hid_mm='HID_TXT_ST_MM_'+row;");
			out.println("m_hid_yy='HID_TXT_ST_YY_'+row;");
			
			out.println(" if((document.Form1.elements[m_dd].value !=\"\")&&(document.Form1.elements[m_mm].value !=\"\")&&(document.Form1.elements[m_yy].value !=\"\")){");
			out.println("  checkMonthLength(document.Form1.elements[m_dd],document.Form1.elements[m_mm],document.Form1.elements[m_yy]);");
			out.println("  validate_date(document.Form1.elements[m_hid_dd],document.Form1.elements[m_hid_mm],document.Form1.elements[m_hid_yy],document.Form1.elements[m_dd],document.Form1.elements[m_mm],document.Form1.elements[m_yy]);");
      out.println("}");
			out.println("else {");
			out.println("alert('Please enter date')");
			out.println("}");
			out.println("}");
			...
			out.println("			}");
			out.println("			if(validate_data()){"); 
			out.println("				document.Form1.action='"+m_class_url+"/"+m_fschema_name+"FA_OP_Save_payment_realise';");  
			...
			out.println("		window.location.href='"+m_class_url+"/"+m_fschema_name+"FA_OP_display_payment_realise';"); 
			out.println("		}"); 
			out.println("}"); 
			...
			out.println("    m_help_message = \"m_help_msg_LAKDL_FA_OP_PAYMENT_CONFIRM\";"); 
			...
			out.println("	help_box.innerHTML=\"Operation Process - Payment Realise- \"+m_val;"); 
			out.println("}"); 

			out.println("function load_roll_out_value(){");
			out.println("	help_box.innerHTML=\"Operation Process - Payment Realise- \"+document.Form1.hid_status.value;"); 
			out.println("}"); 
	  	...
	  	out.println("					}"); 
			...
			*/
			...
			out.println("function show_client_availability(m_client,m_facility) {");
			out.println("	m_url=\""+m_class_url+"/"+m_fschema_name+"FA_OP_Client_availability_check?chksql=LOAD_CLIENT_AVAILABILITY&CLIENT_CODE=\"+m_client+\"&FACILITY_NO=\"+m_facility;");	
			out.println("	window.open(m_url);"); 
			out.println("}");
			...
			out.println("</tr>"); 
			out.println("<tr>"); 
			out.println("<td align='left' class='pdn_txtpos2' style='height: 18px' id='help_box'>Operation Process - Payment Realise</td>"); 
			out.println("</tr>"); 
			out.println("<tr>"); 
			...
			out.println("<td width='20%' >Client Manager</td>"); 
			out.println("<td width='25%' ><select name='TXT_PAYMENT_ORG' class='txt_input' onchange=\"makeRequest_detail()\">");
			rs= stmt.executeQuery(" SELECT DISTINCT B.EMP_CODE,B.TITLE || ' ' || B.FIRST_NAME || ' ' || B.LAST_NAME "+
					" FROM "+m_schema_name+".FA_CR_PRO_CLIENT A,"+m_schema_name+".CO_CO_MAS_EMPLOYEE B "+
					" WHERE A.CLIENT_MANAGER=B.EMP_CODE");
			while(rs.next()){
			out.println("<option value=\""+rs.getString(1)+"\">"+rs.getString(2)+"</option>");
			}
			out.println("<option value=\"ALL\" SELECTED>All Payments</option>");
