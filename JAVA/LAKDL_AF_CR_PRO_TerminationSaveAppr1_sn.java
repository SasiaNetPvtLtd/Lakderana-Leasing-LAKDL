...
public class LAKDL_AF_CR_PRO_TerminationSaveAppr1_sn extends HttpServlet {
	String m_option_val;
	Connection conn;	
	String m_msg,m_url;
		...
		try {
			...
			for(int j=0;j<m_chk;j++){
				...
				if(m_chk_app.equals("Y")){
					...
					callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_CR_PRO_TERM_APPR1_NEW(:1,:2,:3,:4,:5); END;");
			...
			out.println("window.location.href='"+m_url+"/"+m_client_name+"AF_CR_TerminationApproval1_sn?chksql=main_page';");
