...
public class LAKDL_AF_MAS_save_assign_sub_teams_to_teams extends HttpServlet {
  ...
  String reqstr;
	...
	public synchronized void  service(HttpServletRequest req, HttpServletResponse res)	throws IOException{
      ...
      String m_scr_name=(String)m_sn_methods.met_formdata(reqstr,"Hid_scr_name"); 
			...
			callstmt=conn.prepareCall("BEGIN "+m_schema_name+".AF_MAS_SAVE_SUB_TEAMS_TO_TEAMS(:1,:2,:3,:4,:5); END;");
				...
				for (int j = 0; j < m_maxentries; j++) {
				  ...
				  callstmt.setString(1,(m_sn_methods.met_formdata(reqstr,"TXT_TEAM_ID")).trim());
					...
					String m_user_id=	m_sn_methods.met_formdata(reqstr,"hid_TXT_SUB_TEAM_ID"+(Integer.toString(k)));
					if  (m_user_id.trim().equals("N")) {
						j=j-1;
					}
					else {
						callstmt.setString(2,(m_sn_methods.met_formdata(reqstr,"hid_TXT_SUB_TEAM_ID"+(Integer.toString(k)))).trim());
	   	...
	   	out.println("<HTML><HEAD>");
			...
			out.println("window.location.href='"+m_url+"/"+m_client_name+"AF_MAS_display_assign_sub_team';");
