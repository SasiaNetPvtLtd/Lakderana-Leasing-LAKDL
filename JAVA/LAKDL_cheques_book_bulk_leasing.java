...
class Print_cheques_book_bulk_leasing implements Printable {
...
public Print_cheques_book_bulk_leasing(String []  m_rec){
...
public int print (Graphics g,PageFormat pageFormat, int page) {
	...
	for(int z=0;z<=m_payment_code.length;z++){
			...
			/*
			rs=stmt.executeQuery("SELECT GROUP_PAYMENT_NO "+//1
			" UPPER(PAYEE_NAME),"+//2
			//" PAY_AMOUNT,"+//3
			" SUM(NET_AMOUNT),"+//3 //addded by nuwan de silva on 29-05-2008
			" TO_CHAR(EFF_VALDATE,'DD-MM-YYYY'), "+//4
			//" TO_CHAR(EFF_VALDATE,'DD'), "+//5
			" TO_CHAR(SYSDATE,'DD'), "+//5
			//" TO_CHAR(EFF_VALDATE,'MM'), "+//6
			" TO_CHAR(SYSDATE,'MM'), "+//6
			//" TO_CHAR(EFF_VALDATE,'YY') "+//7
			" TO_CHAR(SYSDATE,'YY') "+//7
			" FROM "+m_schema_name+".AF_RE_PRO_SETTLMENT_PAYMENT "+
			" WHERE GROUP_PAYMENT_NO='"+m_payment_code+"' "+
			"");
			...
			*/
			...
			" SELECT  UPPER(PAYEE_NAME) PAYEE_NAME, "+
			" SUM(PAY_AMOUNT) CHQ_AMOUNT,"+
			" TO_CHAR(SYSDATE,'DD') SYSDATE_DD, "+
			" TO_CHAR(SYSDATE,'MM') SYSDATE_MM, "+
			" TO_CHAR(SYSDATE,'YY') SYSDATE_YY "+
			" FROM "+m_schema_name+".AF_RE_PRO_SETTLMENT_PAYMENT "+
			" WHERE NVL(GROUP_PAYMENT_NO,PAYMENT_NO)='"+m_payment_code[z].toString()+"' "+
			" GROUP BY  UPPER(PAYEE_NAME) "+
			"" );
			
			
			if(rs.next()){
			m_pay_name=rs.getString("PAYEE_NAME");
			m_payamount=nf.format(rs.getDouble("CHQ_AMOUNT"));
			m_payamountword=m_print_method.numbersToChar(m_print_method.met_unformat_number(nf.format(rs.getDouble("CHQ_AMOUNT"))));
			m_dd1=rs.getString("SYSDATE_DD");
			m_mm1=rs.getString("SYSDATE_MM");
			m_yy1=rs.getString("SYSDATE_YY");
			}
